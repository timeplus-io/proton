...
def get_server_pid():
    ...
    cmd_pidof = "pidof -s proton-server"
...
class TestCase:
    ...
    def run_single_test(self, server_logs_level, client_options):
        ...
        #print(f"run_single_test, database = {database}, args.database = {args.database}")
        if not args.database:
            pattern = '{test} > {stdout} 2> {stderr}'
            #print(f"run_single_test, (not args.database) is true, database = {database}, pattern = {pattern}")
        else:
            pattern = '{test} > {stdout} 2> {stderr}'
            #print(f"run_single_test, (not args.database) is false, database = {database}, args.database = {args.database}, pattern = {pattern}")
...
class TestSuite:
    ...
    def read_test_suite(args, suite_dir_name: str):
        #print(f"read_test_suite, args= {args},suite_dir_name = {suite_dir_name}")
        def is_data_present():
...
def run_tests_array(all_tests_with_params):
    ...
    #print(f"run_tests_array, all_tests_with_params = {all_tests_with_params} ")
    global stop_time
    global exit_code
    global server_died
    global restarted_tests
    ...
    print(f"run_tests_array, all_tests = {all_tests}")
    while True:
        if is_concurrent:
        ...
        print(f"run_tests_array, test_case: case={case}")
...
def collect_build_flags(args):
    ...
    print(f"collect_build_flags: result = {result}")
    return result
...
def do_run_tests(jobs, test_suite: TestSuite, parallel):
    if jobs > 1 and len(test_suite.parallel_tests) > 0:
        ...
        #print(f"do_run_tests, test_suite.all_tests = {test_suite.all_tests},num_tests={num_tests},test_suite={test_suite} ")
...
def main(args):
    ...
    print(f"main,total_tests_run = {total_tests_run} ")
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--binary', default='proton',
        help='Path to proton (if monolithic build, proton-server otherwise) binary or name of binary in PATH')

    parser.add_argument('-c', '--client',
        help='Path to proton-client (if split build, useless otherwise) binary of name of binary in PATH')

    parser.add_argument('--extract_from_config', help='extract-from-config program')
    parser.add_argument('--configclient', help='Client config (if you use not default ports)')
    parser.add_argument('--configserver', default='/etc/proton-server/config.xml', help='Preprocessed server config')
    ...
    if password is not None:
        ...
        args.client += f" --password={password}"
