...
CREATE STREAM landing (n int64) engine=MergeTree order by n;
CREATE STREAM target  (n int64) engine=MergeTree order by n;
CREATE MATERIALIZED VIEW landing_to_target TO target AS
    SELECT n + throw_if(n == 3333)
    FROM landing;
...
TRUNCATE STREAM landing;
TRUNCATE STREAM target;
...
-- Currently reading from system streams inside a transaction is Not implemented:
...
SET implicit_transaction=True;
SELECT throw_if(number == 0) FROM numbers(100); -- { serverError 395 }
SELECT throw_if(number == 0) FROM numbers(100); -- { serverError 395 }
SELECT throw_if(number == 0) FROM numbers(100); -- { serverError 395 }
SELECT throw_if(number == 0) FROM numbers(100); -- { serverError 395 }
SET implicit_transaction=False;
...
SELECT throw_if(number == 0) FROM numbers(100) SETTINGS implicit_transaction=True; -- { serverError 395 }
ROLLBACK;
