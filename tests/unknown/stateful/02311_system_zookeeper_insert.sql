...
drop stream if exists test_zkinsert;

create stream test_zkinsert (
	name string,
	path string,
	value string
) ENGINE Memory;
...
insert into system.zookeeper (name, path, value) SELECT name, '/' || current_database() || path, value from test_zkinsert;

SELECT * FROM (SELECT path, name, value FROM system.zookeeper ORDER BY path, name) WHERE path LIKE '/' || current_database() || '/1-insert-test%';
...
SELECT * FROM (SELECT path, name, value FROM system.zookeeper ORDER BY path, name) WHERE path LIKE '/' || current_database() || '/2-insert-test%';
