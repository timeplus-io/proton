...
WITH
    lineWithInlines AS
    (
        SELECT DISTINCT addressToLineWithInlines(array_join(trace)) AS lineWithInlines FROM system.trace_log WHERE query_id =
        (
            SELECT query_id FROM system.query_log WHERE current_database = current_database() AND log_comment='02161_test_case' ORDER BY event_time DESC LIMIT 1
        )
    )
