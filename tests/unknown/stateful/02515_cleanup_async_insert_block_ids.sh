...
$CLICKHOUSE_CLIENT -n --query "
    DROP STREAM IF EXISTS t_async_insert_cleanup NO DELAY;
    CREATE STREAM t_async_insert_cleanup (
        KeyID uint32
...
for i in {1..300}; do
    ...
    if [ $answer == '10' ]; then
        $CLICKHOUSE_CLIENT -n --query "DROP STREAM t_async_insert_cleanup NO DELAY;"
        exit 0
    fi
    sleep 1
done
