...
for i in $(seq 1 $NUM_REPLICAS); do
    $CLICKHOUSE_CLIENT -n -q "
        DROP STREAM IF EXISTS r$i SYNC;
        CREATE STREAM r$i (x uint64) ENGINE = ReplicatedMergeTree('/clickhouse/tables/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/r', 'r$i') ORDER BY x;
    "
done
...
function thread {
    for x in {0..9}; do
        while true; do
            $CLICKHOUSE_CLIENT --query "DETACH STREAM r$1"
            $CLICKHOUSE_CLIENT --query "ATTACH STREAM r$1"
...
for i in $(seq 1 $NUM_REPLICAS); do
    # We filter out 'Removing temporary directory' on stream DROP because in this test
    # we constantly DETACH and ATTACH streams. So some replica can start fetching some part
    ...
    # left on disk until stream DROP.
    $CLICKHOUSE_CLIENT -q "DROP STREAM IF EXISTS r$i SYNC;" 2>&1 | grep -v 'Removing temporary directory' ||:
done
