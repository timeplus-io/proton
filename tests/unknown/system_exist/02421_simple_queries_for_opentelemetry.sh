...
#
${CLICKHOUSE_CLIENT} -nq "
DROP STREAM IF EXISTS ${CLICKHOUSE_DATABASE}.opentelemetry_test;
CREATE STREAM ${CLICKHOUSE_DATABASE}.opentelemetry_test (id uint64) Engine=MergeTree Order By id;
"
...
# test 3, a normal insert query on local stream
query_id=$(${CLICKHOUSE_CLIENT} -q "select generateUUIDv4()");
