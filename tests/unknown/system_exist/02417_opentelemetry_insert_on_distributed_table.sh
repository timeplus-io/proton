...
#
# Prepare streams for tests
#
${CLICKHOUSE_CLIENT} -nq "
DROP STREAM IF EXISTS ${CLICKHOUSE_DATABASE}.dist_opentelemetry;
DROP STREAM IF EXISTS ${CLICKHOUSE_DATABASE}.local_opentelemetry;

CREATE STREAM ${CLICKHOUSE_DATABASE}.dist_opentelemetry  (key uint64) Engine=Distributed('test_cluster_two_shards_localhost', ${CLICKHOUSE_DATABASE}, local_opentelemetry, key % 2);
CREATE STREAM ${CLICKHOUSE_DATABASE}.local_opentelemetry (key uint64) Engine=MergeTree ORDER BY key;
"
...
#
${CLICKHOUSE_CLIENT} -nq "
DROP STREAM ${CLICKHOUSE_DATABASE}.dist_opentelemetry;
DROP STREAM ${CLICKHOUSE_DATABASE}.local_opentelemetry;
"
