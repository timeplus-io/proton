...
$CLICKHOUSE_CLIENT --distributed_ddl_output_mode=none -q "create stream $db.t as system.query_log"   # Suppress style check: current_database=$CLICKHOUSE_DATABASE
$CLICKHOUSE_CLIENT -q "show streams from $db"

$CLICKHOUSE_CLIENT -q "system drop database replica 's1|r1' from stream t" 2>&1| grep -Fac "SYNTAX_ERROR"
...
$CLICKHOUSE_CLIENT --distributed_ddl_output_mode=none -q "create stream $db2.t2 as system.query_log" 2>&1| grep -Fac "Database is in readonly mode"   # Suppress style check: current_database=$CLICKHOUSE_DATABASE
...
$CLICKHOUSE_CLIENT --distributed_ddl_output_mode=none -q "create stream $db.t2 as system.query_log" 2>&1| grep -Fac "Database is in readonly mode"   # Suppress style check: current_database=$CLICKHOUSE_DATABASE
$CLICKHOUSE_CLIENT -q "show streams from $db"
