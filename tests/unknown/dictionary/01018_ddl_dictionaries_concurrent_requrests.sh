...
$CLICKHOUSE_CLIENT -n -q "
    ...
    DROP STREAM IF EXISTS stream_for_dict1;
    DROP STREAM IF EXISTS stream_for_dict2;

    CREATE STREAM stream_for_dict1 (key_column uint64, value_column string) ENGINE = MergeTree ORDER BY key_column;
    CREATE STREAM stream_for_dict2 (key_column uint64, value_column string) ENGINE = MergeTree ORDER BY key_column;

    INSERT INTO stream_for_dict1 SELECT number, to_string(number) from numbers(1000);
    INSERT INTO stream_for_dict2 SELECT number, to_string(number) from numbers(1000, 1000);

    CREATE DATABASE database_for_dict;

    CREATE DICTIONARY database_for_dict.dict1 (key_column uint64, value_column string) PRIMARY KEY key_column SOURCE(CLICKHOUSE(HOST 'localhost' PORT tcp_port() USER 'default' STREAM 'table_for_dict1' PASSWORD '' DB '$CLICKHOUSE_DATABASE')) LIFETIME(MIN 1 MAX 5) LAYOUT(FLAT());

    CREATE DICTIONARY database_for_dict.dict2 (key_column uint64, value_column string) PRIMARY KEY key_column SOURCE(CLICKHOUSE(HOST 'localhost' PORT tcp_port() USER 'default' STREAM 'table_for_dict2' PASSWORD '' DB '$CLICKHOUSE_DATABASE')) LIFETIME(MIN 1 MAX 5) LAYOUT(CACHE(SIZE_IN_CELLS 150));
"
...
$CLICKHOUSE_CLIENT -n -q "
    ...
    DROP STREAM stream_for_dict1;
    DROP STREAM stream_for_dict2;
"
