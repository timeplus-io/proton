...
$CLICKHOUSE_CLIENT --query "CREATE STREAM src (n uint64, type uint8) ENGINE=MergeTree ORDER BY type SETTINGS old_parts_lifetime=0";
$CLICKHOUSE_CLIENT --query "CREATE STREAM dst (n uint64, type uint8) ENGINE=MergeTree ORDER BY type SETTINGS old_parts_lifetime=0";
...
function thread_partition_src_to_dst()
{
    ...
    for i in {1..20}; do
        ...
        output=$(tx $session_id "ALTER STREAM src DROP PARTITION ID 'all'" ||:)
        ...
        output=$(tx $session_id "SELECT throwIf((SELECT (count(), sum(n)) FROM merge(current_database(), '') WHERE type=3) != ($count + 1, $sum + $i)) FORMAT Null" ||:)
...
function thread_partition_dst_to_src()
{
    ...
    while (( i <= 20 )); do
        ...
        tx $session_id "ALTER STREAM dst DROP PARTITION ID 'nonexistent';"
        ...
        output=$(tx $session_id "ALTER STREAM dst DROP PARTITION ID 'all'" ||:)
        ...
        output=$(tx $session_id "SELECT throwIf((SELECT (count(), sum(n)) FROM merge(current_database(), '') WHERE type=4) != (to_uint8($i/2 + 1), (select sum(number) from numbers(1, $i) where number % 2 or number=$i))) FORMAT Null" ||:)
        if echo "$output" | is_tx_aborted_with "FUNCTION_THROW_IF_VALUE_IS_NON_ZERO"
        then
            echo "thread_partition_dst_to_src tx_id: $tx_id session_id: $session_id" >&2
            echo "select throwIf has failed with FUNCTION_THROW_IF_VALUE_IS_NON_ZERO" >&2
...
function thread_select()
{
    ...
    while true; do
        ...
        SELECT _table, throwIf(arraySort(group_arrayIf(n, type=1)) != arraySort(group_arrayIf(n, type=2))) FROM merge(current_database(), '') GROUP BY _table FORMAT Null;

        -- all rows are inserted in insert_thread
        SELECT type, throwIf(count(n) != max(n)), throwIf(sum(n) != max(n)*(max(n)+1)/2) FROM merge(current_database(), '') WHERE type IN (1, 2) GROUP BY type ORDER BY type FORMAT Null;
        COMMIT;" 2>&1 ||:)
...
$CLICKHOUSE_CLIENT -q "SELECT type, count(n) = countDistinct(n) FROM merge(current_database(), '') GROUP BY type ORDER BY type"
$CLICKHOUSE_CLIENT -q "SELECT DISTINCT arraySort(group_arrayIf(n, type=1)) = arraySort(group_arrayIf(n, type=2)) FROM merge(current_database(), '') GROUP BY _table ORDER BY _table"
$CLICKHOUSE_CLIENT -q "SELECT count(n), sum(n) FROM merge(current_database(), '') WHERE type=4"
$CLICKHOUSE_CLIENT -q "SELECT type, count(n) == max(n), sum(n) == max(n)*(max(n)+1)/2 FROM merge(current_database(), '') WHERE type IN (1, 2) GROUP BY type ORDER BY type"
