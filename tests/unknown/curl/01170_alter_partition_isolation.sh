...
tx 1 "alter stream mt drop partition id 'all'"
...
tx 2                                            "alter stream mt drop partition id 'all'"
...
tx 4 "alter stream mt drop partition id 'all'"
...
tx 3                                            "alter stream mt drop partition id 'all'" | grep -Eo "SERIALIZATION_ERROR" | uniq
...
$CLICKHOUSE_CLIENT -q "drop stream if exists another_mt"
$CLICKHOUSE_CLIENT -q "create stream another_mt (n int) engine=MergeTree order by n"
...
tx 5 "alter stream another_mt move partition id 'all' to stream mt"
tx 6                                            "alter stream another_mt replace partition id 'all' from mt"
tx 5 "alter stream another_mt attach partition id 'all' from mt"
tx 5 "commit"
tx 6                                            "commit"
...
$CLICKHOUSE_CLIENT -q "drop stream another_mt"
