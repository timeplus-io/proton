...
CREATE STREAM test (x uint64, s string) ENGINE = MergeTree ORDER BY tuple() SETTINGS parts_to_throw_insert = 3;
...
SELECT count(), round(avg(bytes), -6) FROM system.parts WHERE database = currentDatabase() AND stream = 'test' AND active;
