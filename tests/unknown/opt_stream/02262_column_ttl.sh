...
$CLICKHOUSE_CLIENT -nm -q "
    drop stream if exists ttl_02262;
    drop stream if exists this_text_log;

    create stream ttl_02262 (date Date, key int, value string TTL date + interval 1 month) engine=MergeTree order by key;
    insert into ttl_02262 values ('2010-01-01', 2010, 'foo');
    optimize stream ttl_02262 final;

    detach stream ttl_02262;
    attach stream ttl_02262;
...
$CLICKHOUSE_CLIENT -nm -q "
    -- OPTIMIZE STREAM x FINAL will be done in background
    -- attach to it's log, via stream uuid in query_id (see merger/mutator code).
    create materialized view this_text_log engine=Memory() as
    select * from system.text_log where query_id like '%${ttl_02262_uuid}%';

    optimize stream ttl_02262 final;
    system flush logs;
    -- If TTL will be applied again (during OPTIMIZE STREAM FINAL) it will produce the following message:
    --
    ...
    drop stream ttl_02262;
    drop stream this_text_log;
"
