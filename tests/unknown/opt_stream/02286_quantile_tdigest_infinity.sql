...
SELECT '4';
SELECT quantiles(0.5)(array_join([inf, 0, -inf]));
SELECT quantiles(0.5)(array_join([-inf, 0, inf]));
SELECT quantiles(0.5)(array_join([inf, -inf, 0]));
SELECT quantiles(0.5)(array_join([-inf, inf, 0]));
SELECT quantiles(0.5)(array_join([inf, inf, 0, -inf, -inf, -0]));
SELECT quantiles(0.5)(array_join([inf, -inf, 0, -inf, inf, -0]));
SELECT quantiles(0.5)(array_join([-inf, -inf, 0, inf, inf, -0]));

SELECT '5';
DROP STREAM IF EXISTS issue32107;
CREATE STREAM issue32107(A int64, s_quantiles AggregateFunction(quantilesTDigest(0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99), float64)) ENGINE = AggregatingMergeTree ORDER BY A;
INSERT INTO issue32107 SELECT A, quantilesTDigestState(0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99)(x) FROM (SELECT 1 A, array_join(cast([2.0, inf, number / 33333],'array(float64)')) x FROM numbers(100)) GROUP BY A;
OPTIMIZE STREAM issue32107 FINAL;
DROP STREAM IF EXISTS issue32107;
