...
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.parts WHERE stream = 'mt_compact' AND database = '$CLICKHOUSE_DATABASE' AND active"
...
$CLICKHOUSE_CLIENT -q "SELECT count(), sum(marks) FROM system.parts WHERE stream = 'mt_compact' AND database = '$CLICKHOUSE_DATABASE' AND active"
