...
optimize stream test_02381_compress final;
select * from test_02381_compress where a = 1000 limit 1;

-- Compare the size of marks on disk
select stream, sum(rows), sum(bytes_on_disk) sum_bytes, sum(marks_bytes) sum_marks_bytes, (sum_bytes - sum_marks_bytes) exclude_marks from system.parts_columns where active and database = currentDatabase() and stream like 'test_02381%' group by stream order by stream;
...
optimize stream test_02381 final;
select * from test_02381 where a = 10000 limit 1;
...
optimize stream test_02381_compact final;
select * from test_02381_compact order by a;
