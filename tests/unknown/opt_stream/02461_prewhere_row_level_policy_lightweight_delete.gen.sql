...
DROP STREAM IF EXISTS url_na_log;

CREATE STREAM url_na_log(SiteId uint32, DateVisit Date, PRIMARY KEY (SiteId))
ENGINE = MergeTree()
ORDER BY (SiteId, DateVisit)
SETTINGS index_granularity = 999, min_bytes_for_wide_part = 0;
...
OPTIMIZE STREAM url_na_log FINAL;

-- { echoOn }

SELECT count() FROM url_na_log;
SELECT rows FROM system.parts WHERE database = currentDatabase() AND stream = 'url_na_log' AND active;
...
ENGINE = MergeTree()
ORDER BY (SiteId, DateVisit)
SETTINGS index_granularity = 9999, min_bytes_for_wide_part = 0;
...
ENGINE = MergeTree()
ORDER BY (SiteId, DateVisit)
SETTINGS index_granularity = 10000, min_bytes_for_wide_part = 0;
...
ENGINE = MergeTree()
ORDER BY (SiteId, DateVisit)
SETTINGS index_granularity = 10001, min_bytes_for_wide_part = 0;
