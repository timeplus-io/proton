...
drop stream if exists rmt1;
drop stream if exists rmt2;
create stream rmt1 (n int) engine=ReplicatedMergeTree('/test/02448/{database}/rmt', '1') order by tuple()
...
create stream rmt2 (n int) engine=ReplicatedMergeTree('/test/02448/{database}/rmt', '2') order by tuple()
    settings min_replicated_logs_to_keep=1, max_replicated_logs_to_keep=2, cleanup_delay_period=0, cleanup_delay_period_random_add=1, old_parts_lifetime=0, max_parts_to_merge_at_once=4;

-- insert part only on one replica
system stop replicated sends rmt1;
insert into rmt1 values (1);
detach stream rmt1;      -- make replica inactive
...
select 1, arraySort(group_array(n)) from rmt2;
...
select 2, arraySort(group_array(n)) from rmt1;
select 3, arraySort(group_array(n)) from rmt2;


truncate stream rmt1;
truncate stream rmt2;
...
select 4, arraySort(group_array(n)) from rmt1;
...
-- check that no parts are lost
select 5, arraySort(group_array(n)) from rmt1;
select 6, arraySort(group_array(n)) from rmt2;


-- insert part only on one replica
system stop replicated sends rmt1;
insert into rmt1 values (123);
alter stream rmt1 update n=10 where n=123 settings mutations_sync=1;
...
select 7, arraySort(group_array(n)) from rmt2;
...
-- check that no parts are lost
select 8, arraySort(group_array(n)) from rmt1;
select 9, arraySort(group_array(n)) from rmt2;
...
select 11, arraySort(group_array(n)) from rmt2;

system start replicated sends rmt2;
system sync replica rmt1;
select 12, arraySort(group_array(n)) from rmt1;
