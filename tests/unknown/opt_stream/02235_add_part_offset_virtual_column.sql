DROP STREAM IF EXISTS t_1;
DROP STREAM IF EXISTS t_random_1;
...
ENGINE = MergeTree
PARTITION BY to_YYYYMM(p_time)
ORDER BY order_0;

CREATE STREAM t_random_1
(
    `ordinary_1` uint32
)
...
OPTIMIZE STREAM t_1 FINAL;

ALTER STREAM t_1 ADD COLUMN foo string DEFAULT 'foo';
...
SELECT count(*) FROM t_1 WHERE order_0 IN (SELECT to_uint64(rand64()%1000) FROM system.numbers limit 100) AND _part_offset != order_0;
