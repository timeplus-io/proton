...
test_random_values() {
  ...
  $CLICKHOUSE_CLIENT -n -q "
    create stream tbl_8parts_${layers}granules_rnd (key1 uint32, sign int8) engine = CollapsingMergeTree(sign) order by (key1) partition by (key1 % 8);
    insert into tbl_8parts_${layers}granules_rnd select number, 1 from numbers_mt($((layers * 8 * 8192)));
    optimize stream tbl_8parts_${layers}granules_rnd final;
...
test_sequential_values() {
  layers=$1
  $CLICKHOUSE_CLIENT -n -q "
    create stream tbl_8parts_${layers}granules_seq (key1 uint32, sign int8) engine = CollapsingMergeTree(sign) order by (key1) partition by (key1 / $((layers * 8192)))::uint64;
    insert into tbl_8parts_${layers}granules_seq select number, 1 from numbers_mt($((layers * 8 * 8192)));
    optimize stream tbl_8parts_${layers}granules_seq final;
