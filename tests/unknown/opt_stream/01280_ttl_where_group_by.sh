...
{
    for _ in {0..20}; do
        $CLICKHOUSE_CLIENT --query "OPTIMIZE STREAM $1 FINAL SETTINGS optimize_throw_if_noop=1" 2>/dev/null && break
        sleep 0.3
    done
}
...
echo "ttl_01280_3"
$CLICKHOUSE_CLIENT -n --query "
create stream ttl_01280_3 (a int, b int, x int64, y int, d DateTime) engine = MergeTree order by (a, b) ttl d + interval 1 second group by a set b = min(b), x = argMax(x, d), y = argMax(y, d), d = max(d);
...
echo "ttl_01280_4"
$CLICKHOUSE_CLIENT -n --query "
create stream ttl_01280_4 (a int, b int, x int64, y int64, d DateTime) engine = MergeTree order by (toDate(d), -(a + b)) ttl d + interval 1 second group by toDate(d) set x = sum(x), y = max(y);
...
echo "ttl_01280_5"
$CLICKHOUSE_CLIENT -n --query "create stream ttl_01280_5 (a int, b int, x int64, y int64, d DateTime) engine = MergeTree order by (toDate(d), a, -b) ttl d + interval 1 second group by toDate(d), a set x = sum(x), b = argMax(b, -b);
...
echo "ttl_01280_6"
$CLICKHOUSE_CLIENT -n --query "
create stream ttl_01280_6 (a int, b int, x int64, y int64, d DateTime) engine = MergeTree order by (toDate(d), a, -b) ttl d + interval 1 second group by toDate(d), a;
