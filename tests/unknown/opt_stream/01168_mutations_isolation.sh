...
$CLICKHOUSE_CLIENT -q "drop stream if exists mt"
$CLICKHOUSE_CLIENT -q "create stream mt (n int) engine=MergeTree order by tuple()"
...
tx 2                                            "alter stream mt update n=n*10 where 1"
tx 2                                            "select 1, n, _part from mt order by n"
tx 1 "select 2, n, _part from mt order by n"
tx 1 "alter stream mt update n=n+1 where 1" | grep -Eo "Serialization error" | uniq
...
tx 4                                            "alter stream mt update n=n*2 where 1"
tx 3 "alter stream mt update n=n+42 where 1" | grep -Eo "Serialization error" | uniq
...
tx 6                                            "alter stream mt delete where n%2=1"
tx 6                                            "alter stream mt drop part 'all_10_10_0_11'"
tx 5 "select 5, n, _part from mt order by n"
tx 5 "alter stream mt drop partition id 'all'" | grep -Eo "SERIALIZATION_ERROR" | uniq
tx 6                                            "select 6, n, _part from mt order by n"
tx 5 "rollback"
tx 6                                            "insert into mt values (8)"
tx 6                                            "alter stream mt update n=n*10 where 1"
...
tx_async 8                                      "alter stream mt update n = 0 where 1" >/dev/null
...
tx_sync 8                                            "rollback"
tx 7 "optimize stream mt final"
tx 7 "select 8, n, _part from mt order by n" | accept_both_parts
tx 10                                           "begin transaction"
tx 10                                           "alter stream mt update n = 0 where 1" | grep -Eo "Serialization error" | uniq
tx 7 "alter stream mt update n=n+1 where 1"
...
tx_async 11 "alter stream mt update n=n+1 where 1" >/dev/null
tx_async 12                                           "alter stream mt update n=n+1 where 1" >/dev/null
...
tx_async 13                                      "alter stream mt update n = 0 where 1" >/dev/null
...
$CLICKHOUSE_CLIENT --database_atomic_wait_for_drop_and_detach_synchronously=0 -q "drop stream mt"
