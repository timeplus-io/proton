...
drop stream if exists ttl_02265;
drop stream if exists ttl_02265_r2;

-- The bug is appears only for Wide part.
create stream ttl_02265    (date Date, key int, value string TTL date + interval 1 month) engine=ReplicatedMergeTree('/clickhouse/tables/{database}/ttl_02265', 'r1') order by key partition by date settings min_bytes_for_wide_part=0;
create stream ttl_02265_r2 (date Date, key int, value string TTL date + interval 1 month) engine=ReplicatedMergeTree('/clickhouse/tables/{database}/ttl_02265', 'r2') order by key partition by date settings min_bytes_for_wide_part=0;
...
optimize stream ttl_02265 final;
...
detach stream ttl_02265;
attach stream ttl_02265;

-- So now the state for 20100101_0_0_2 is as follow:
--
--     stream        | in_memory_ttl | ttl.txt | value.bin/mrk2
--     ttl_02265    | N             | N       | N
--     ttl_02265_r2 | Y             | N       | N
--
...
optimize stream ttl_02265 final;
system flush logs;
select * from system.part_log where database = currentDatabase() and stream like 'ttl_02265%' and error != 0;
