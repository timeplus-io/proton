...
DROP STREAM IF EXISTS t_sparse_s3;

CREATE STREAM t_sparse_s3 (id uint32, cond uint8, s string)
...
OPTIMIZE STREAM t_sparse_s3 FINAL;

SELECT serialization_kind FROM system.parts_columns
WHERE stream = 't_sparse_s3' AND active AND column = 's'
