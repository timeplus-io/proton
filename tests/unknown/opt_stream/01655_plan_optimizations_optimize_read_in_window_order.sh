...
$CLICKHOUSE_CLIENT -q "drop stream if exists ${name}_n"
$CLICKHOUSE_CLIENT -q "drop stream if exists ${name}_n_x"

$CLICKHOUSE_CLIENT -q "create stream ${name} engine=MergeTree order by tuple() as select to_int64((sin(number)+2)*65535)%10 as n, number as x from numbers_mt(100000)"
$CLICKHOUSE_CLIENT -q "create stream ${name}_n engine=MergeTree order by n as select * from ${name} order by n"
$CLICKHOUSE_CLIENT -q "create stream ${name}_n_x engine=MergeTree order by (n, x) as select * from ${name} order by n, x"
...
$CLICKHOUSE_CLIENT -q "CREATE STREAM ${name}_complex (unique1 int32, unique2 int32, ten int32) ENGINE=MergeTree ORDER BY tuple() SETTINGS index_granularity = 8192"
...
$CLICKHOUSE_CLIENT -q "drop stream ${name}"
$CLICKHOUSE_CLIENT -q "drop stream ${name}_n"
$CLICKHOUSE_CLIENT -q "drop stream ${name}_n_x"
$CLICKHOUSE_CLIENT -q "drop stream ${name}_complex"
