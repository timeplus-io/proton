...
DROP STREAM IF EXISTS weird_partitions_02245;

CREATE STREAM weird_partitions_02245(d DateTime, d1 DateTime default d - to_intervalHour(8), id int64)
Engine=MergeTree
PARTITION BY (to_YYYYMM(to_datetime(d)), ignore(d1))
ORDER BY id;

INSERT INTO weird_partitions_02245(d, id)
SELECT
    to_datetime('2021-12-31 22:30:00') AS d,
    number
FROM numbers(1000);

INSERT INTO weird_partitions_02245(d, id)
SELECT
    to_datetime('2022-01-01 00:30:00') AS d,
    number
FROM numbers(1000);

INSERT INTO weird_partitions_02245(d, id)
SELECT
    to_datetime('2022-01-31 22:30:00') AS d,
    number
FROM numbers(1000);

INSERT INTO weird_partitions_02245(d, id)
SELECT
    to_datetime('2023-01-31 22:30:00') AS d,
    number
FROM numbers(1000);

OPTIMIZE STREAM weird_partitions_02245;
OPTIMIZE STREAM weird_partitions_02245;
...
DROP STREAM weird_partitions_02245;
