...
${CLICKHOUSE_CLIENT} -q "DROP STREAM IF EXISTS t_async_inserts_logs"
${CLICKHOUSE_CLIENT} -q "CREATE STREAM t_async_inserts_logs (id uint32, s string) ENGINE = MergeTree ORDER BY id"
...
${CLICKHOUSE_CLIENT} -q "OPTIMIZE STREAM t_async_inserts_logs FINAL"
${CLICKHOUSE_CLIENT} -q "ALTER STREAM t_async_inserts_logs MODIFY SETTING parts_to_throw_insert = 1"
...
${CLICKHOUSE_CLIENT} -q "
    SELECT stream, format, bytes, empty(exception), status,
    ...
    ORDER BY stream, status, format"

${CLICKHOUSE_CLIENT} -q "DROP STREAM t_async_inserts_logs"
