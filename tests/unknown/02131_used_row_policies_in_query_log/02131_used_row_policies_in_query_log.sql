DROP STREAM IF EXISTS 02131_rqtable;
CREATE STREAM 02131_rqtable (x uint8) ENGINE = MergeTree ORDER BY x;
...
DROP STREAM 02131_rqtable;

SELECT 'Check system.query_log';
SYSTEM FLUSH LOGS;
SELECT query, used_row_policies FROM system.query_log WHERE current_database == current_database() AND type == 'QueryStart' AND query_kind == 'Select' ORDER BY event_time_microseconds;
