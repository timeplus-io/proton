...
#
${CLICKHOUSE_CLIENT} -q "
DROP STREAM IF EXISTS ${CLICKHOUSE_DATABASE}.ddl_test_for_opentelemetry;
"
...
for ddl_version in 3 4; do
    ...
    execute_query $trace_id "CREATE STREAM ${CLICKHOUSE_DATABASE}.ddl_test_for_opentelemetry ON CLUSTER ${cluster_name} (id uint64) Engine=MergeTree ORDER BY id" "distributed_ddl_output_mode=none&distributed_ddl_entry_format_version=${ddl_version}"
...
#
# an exceptional case, DROP a non-exist stream
#
# Echo a separator so that the reference file is more clear for reading
echo "===exception===="

trace_id=$(${CLICKHOUSE_CLIENT} -q "select lower(hex(generateUUIDv4()))");
execute_query $trace_id "DROP STREAM ${CLICKHOUSE_DATABASE}.ddl_test_for_opentelemetry_non_exist ON CLUSTER ${cluster_name}" "distributed_ddl_output_mode=none&distributed_ddl_entry_format_version=4" 2>&1| grep -Fv "UNKNOWN_TABLE"
