...
SELECT n, source, inter FROM (
   SELECT to_float32(number % 10) AS n, 'original' AS source, number as inter FROM numbers(10) WHERE number % 3 = 1
...
SELECT source, inter FROM (
    SELECT to_float32(number % 10) AS n, 'original' AS source, number AS inter, number + 1 AS inter2 FROM numbers(10) WHERE (number % 3) = 1
...
SELECT n, source, inter AS inter_p FROM (
    SELECT to_float32(number % 10) AS n, 'original' AS source, number + NULL AS inter FROM numbers(10) WHERE (number % 3) = 1
) ORDER BY n ASC WITH FILL FROM 0 TO 11.51 STEP 0.5 INTERPOLATE ( inter_p AS inter_p + 1 );

# Test INTERPOLATE for MergeTree
DROP STREAM IF EXISTS t_inter_02233;
CREATE STREAM t_inter_02233 (n int32) ENGINE = MergeTree ORDER BY n;
