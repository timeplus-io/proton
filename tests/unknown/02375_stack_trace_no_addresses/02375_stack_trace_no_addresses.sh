...
${CLICKHOUSE_LOCAL} --query "SELECT throw_if(1)" --stacktrace --config-file "${CONFIG}" 2>&1 | grep -c -F '@ 0x'

sed -i 's/<show_addresses_in_stack_traces>false/<show_addresses_in_stack_traces>true/' "${CONFIG}"

${CLICKHOUSE_LOCAL} --query "SELECT throw_if(1)" --stacktrace --config-file "${CONFIG}" 2>&1 | grep -c -F '@ 0x' | grep -c -v '^0$'
