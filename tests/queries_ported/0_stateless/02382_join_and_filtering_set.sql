...
AS SELECT sip_hash64(number, 't1_x') % 100 AS x, sip_hash64(number, 't1_y') % 100 AS y FROM numbers(100);

CREATE STREAM t2 (x uint64, y uint64) ENGINE = MergeTree ORDER BY y
AS SELECT sip_hash64(number, 't2_x') % 100 AS x, sip_hash64(number, 't2_y') % 100 AS y FROM numbers(100);
