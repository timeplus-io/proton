...
DROP STREAM IF EXISTS with_fill_staleness;
CREATE STREAM with_fill_staleness (a datetime, b datetime, c uint64) ORDER BY a;

SELECT 'add samples';

select sleep(2) FORMAT Null;

INSERT INTO with_fill_staleness(a, b, c)
SELECT
    to_datetime('2016-06-15 23:00:00') + number AS a, a as b, number as c
FROM numbers(30)
WHERE (number % 5) == 0;
...
SELECT a, c, 'original' as original FROM with_fill_staleness ORDER BY a ASC WITH FILL TO to_datetime('2016-06-15 23:00:40') STEP 3 STALENESS INTERVAL 7 SECOND INTERPOLATE (c);
...
SELECT a, b, c, 'original' as original FROM with_fill_staleness ORDER BY a ASC WITH FILL STALENESS INTERVAL 2 SECOND, b ASC WITH FILL TO to_datetime('2016-06-15 23:01:00') STEP 2 STALENESS 5 INTERPOLATE (c);
