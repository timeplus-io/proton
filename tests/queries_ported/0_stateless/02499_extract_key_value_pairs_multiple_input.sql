...
WITH
    extract_key_value_pairs('not_important', ':', ',:', '\'') AS s_map,
    CAST(
            array_map(
                    (x) -> (x, s_map[x]), array_sort(map_keys(s_map))
                ),
...
WITH
    extract_key_value_pairs([1, 2]) AS s_map,
    CAST(
            array_map(
                    (x) -> (x, s_map[x]), array_sort(map_keys(s_map))
                ),
...
WITH
    extract_key_value_pairs('not_important', ':', '123456789', '\'') AS s_map,
    CAST(
            array_map(
                    (x) -> (x, s_map[x]), array_sort(map_keys(s_map))
                ),
...
WITH
    extract_key_value_pairs() AS s_map,
    CAST(
            array_map(
                    (x) -> (x, s_map[x]), array_sort(map_keys(s_map))
                ),
...
WITH
    extract_key_value_pairs('a', ':', ',', '"', '') AS s_map,
    CAST(
            array_map(
                    (x) -> (x, s_map[x]), array_sort(map_keys(s_map))
                ),
