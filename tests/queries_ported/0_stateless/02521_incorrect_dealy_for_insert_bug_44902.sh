...
$CLICKHOUSE_CLIENT -q "DROP STREAM IF EXISTS test_02521_insert_delay"
# Create MergeTree with settings which allow to insert maximum 5 parts, on 6th it'll throw TOO_MANY_PARTS
$CLICKHOUSE_CLIENT -q "CREATE STREAM test_02521_insert_delay (key uint32, value string) Engine=MergeTree() ORDER BY key SETTINGS parts_to_delay_insert=1, parts_to_throw_insert=5, max_delay_to_insert=1, min_delay_to_insert_ms=300"
...
do
    ...
    $CLICKHOUSE_CLIENT --query_id="$query_id" -q "INSERT INTO test_02521_insert_delay SELECT number, to_string(number) FROM numbers(${i}, 1)"
...
$CLICKHOUSE_CLIENT -q "DROP STREAM test_02521_insert_delay"
