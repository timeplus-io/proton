...
select * from (select to_low_cardinality(4) as x) s1 left join (select 4 as x) s2 using x;
select * from (select 5 as x) s1 left join (select to_low_cardinality(5) as x) s2 using x;
