drop stream if exists largestTriangleThreeBucketsTestFloat64Float64;

CREATE STREAM largestTriangleThreeBucketsTestFloat64Float64
(
    x float64,
    y float64
...
select largest_triangle_three_buckets(x, y, 0) FROM largestTriangleThreeBucketsTestFloat64Float64;

select largest_triangle_three_buckets(x, y, 1) FROM largestTriangleThreeBucketsTestFloat64Float64;

select largest_triangle_three_buckets(x, y, 2) FROM largestTriangleThreeBucketsTestFloat64Float64;

SELECT largest_triangle_three_buckets(x, y, 4) AS downsampled_data
FROM largestTriangleThreeBucketsTestFloat64Float64;

drop stream largestTriangleThreeBucketsTestFloat64Float64;

drop stream if exists largestTriangleThreeBucketsTestDecimal64Decimal64;

CREATE STREAM largestTriangleThreeBucketsTestDecimal64Decimal64
(
    x Decimal64(2),
    y Decimal64(2)
) ENGINE = MergeTree order by (y,x);
...
select largest_triangle_three_buckets(x, y, 20) from largestTriangleThreeBucketsTestDecimal64Decimal64;

drop stream largestTriangleThreeBucketsTestDecimal64Decimal64;

drop stream if exists largestTriangleThreeBucketsTestDateTime64Float64;

create stream largestTriangleThreeBucketsTestDateTime64Float64 (x DateTime64(3), y float64) engine = MergeTree order by (y,x);
...
select largest_triangle_three_buckets(x, y, 5) from largestTriangleThreeBucketsTestDateTime64Float64;

select lttb(x, y, 5) from largestTriangleThreeBucketsTestDateTime64Float64;

drop stream largestTriangleThreeBucketsTestDateTime64Float64;

CREATE STREAM largestTriangleTreeBucketsBucketSizeTest
(
    x uint32,
    y uint32
...
SELECT 
  array_join(lttb(x, y, 1000)) AS point, 
  tuple_element(point, 1) AS point_x, 
  point_x - neighbor(point_x, -1) AS point_x_diff_with_previous_row 
FROM largestTriangleTreeBucketsBucketSizeTest LIMIT 990, 10;

SELECT largest_triangle_three_buckets(0, '1900-01-01 00:00:00'::DateTime64, 1);

DROP STREAM largestTriangleTreeBucketsBucketSizeTest;
