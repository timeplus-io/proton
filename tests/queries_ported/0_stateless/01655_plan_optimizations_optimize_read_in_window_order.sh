...
$CLICKHOUSE_CLIENT -q "create STREAM ${name} engine=MergeTree order by n as select to_int64((sin(number)+2)*65535)%10 as n, number as x from numbers_mt(100000)"
...
$CLICKHOUSE_CLIENT -q "CREATE STREAM ${name}_complex (unique1 int32, unique2 int32, ten int32) ENGINE=MergeTree ORDER BY unique1 SETTINGS index_granularity = 8192"
