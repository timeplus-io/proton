...
$CLICKHOUSE_CLIENT -q "drop STREAM if exists optimize_sorting sync"
$CLICKHOUSE_CLIENT -q "create STREAM optimize_sorting (a uint64, b uint64, c uint64) engine=MergeTree() order by c"
...
$CLICKHOUSE_CLIENT -q "drop STREAM if exists optimize_sorting sync"
$CLICKHOUSE_CLIENT -q "create STREAM optimize_sorting (a uint64, b uint64, c uint64) engine=MergeTree() order by (a, b)"
...
explain_sortmode "$MAKE_OUTPUT_STABLE;EXPLAIN PLAN actions=1, header=1, sorting=1 SELECT a FROM (SELECT sip_hash64(a) AS a FROM (SELECT a FROM optimize_sorting ORDER BY a)) ORDER BY a"
...
explain_sortmode "$MAKE_OUTPUT_STABLE;EXPLAIN PLAN actions=1, header=1, sorting=1 SELECT a, z FROM (SELECT sip_hash64(a) AS a, a + 1 AS z FROM (SELECT a FROM optimize_sorting ORDER BY a + 1)) ORDER BY a + 1"
