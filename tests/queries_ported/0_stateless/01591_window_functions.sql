...
select number, count() over (partition by int_div(number, 3) order by number rows unbounded preceding) from numbers(10);

-- proper calculation across blocks
select number, max(number) over (partition by int_div(number, 3) order by number desc rows unbounded preceding) from numbers(10) settings max_block_size = 2;

-- not a window function
select number, abs(number) over (partition by to_string(int_div(number, 3)) rows unbounded preceding) from numbers(10); -- { serverError 63 }

-- no partition by
select number, avg(number) over (order by number rows unbounded preceding) from numbers(10);

-- no order by
select number, quantile_exact(number) over (partition by int_div(number, 3) AS value order by number rows unbounded preceding) from numbers(10);

-- can add an alias after window spec
select number, quantile_exact(number) over (partition by int_div(number, 3) AS value order by number rows unbounded preceding) as q from numbers(10);

-- now we should be able to compute expressions with window functions
-- proton: starts. TODO: to fix the missing column issue
-- select number, q * 10, quantile_exact(number) over (partition by int_div(number, 3) order by number rows unbounded preceding) as q, q*10 from numbers(10) order by number;
-- proton: ends

-- must work in WHERE if you wrap it in a subquery
select * from (select count(*) over (rows unbounded preceding) as c from numbers(3)) where c > 0;

-- should work in ORDER BY
select number, max(number) over (partition by int_div(number, 3) order by number desc rows unbounded preceding) as m from numbers(10) order by m desc, number;

-- also works in ORDER BY if you wrap it in a subquery
select * from (select count(*) over (rows unbounded preceding) as c from numbers(3)) order by c;
...
select number, max(number) over (partition by int_div(number, 3) order by number desc rows unbounded preceding), count(number) over (partition by int_div(number, 5) order by number rows unbounded preceding) as m from numbers(31) order by number settings max_block_size = 2;

-- two functions over the same window
-- an explain test would also be helpful, but it's too immature now and I don't
-- want to change reference all the time
select number, max(number) over (partition by int_div(number, 3) order by number desc rows unbounded preceding), count(number) over (partition by int_div(number, 3) order by number desc rows unbounded preceding) as m from numbers(7) order by number settings max_block_size = 2;

-- check that we can work with constant columns
select median(x) over (partition by x) from (select 1 as x);

-- an empty window definition is valid as well
select group_array(number) over (rows unbounded preceding) from numbers(3);
select group_array(number) over () from numbers(3);
...
-- function definition.
with number + 1 as x select int_div(number, 3) as y, sum(x + y) over (partition by y order by x rows unbounded preceding) from numbers(7);
...
window
    ...
    w2 as (partition by int_div(number, 3) as value order by number rows unbounded preceding)
;
...
select
    sum(number) over w1,
    sum(number) over (partition by int_div(number, 3) as value order by number rows unbounded preceding)
from numbers(10)
window
    w1 as (partition by int_div(number, 3) rows unbounded preceding)
;
...
select number, int_div(number, 3) as p, mod(number, 2) as o, count(number) over w as c
from numbers(31)
...
select number, int_div(number, 5) as p, mod(number, 3) as o, count(number) over w as c
from numbers(31)
window w as (partition by p order by o, number range unbounded preceding)
order by number
...
select number, int_div(number, 5) as p, mod(number, 2) as o, count(number) over w as c
from numbers(31)
window w as (partition by p order by o, number range unbounded preceding)
order by number
...
select number, int_div(number, 3) as p, mod(number, 5) as o, count(number) over w as c
from numbers(31)
window w as (partition by p order by o, number range unbounded preceding)
order by number
settings max_block_size = 2
...
select number, int_div(number, 2) as p, mod(number, 5) as o, count(number) over w as c
from numbers(31)
window w as (partition by p order by o, number range unbounded preceding)
order by number
settings max_block_size = 3
...
select number, int_div(number, 2) as p, mod(number, 3) as o, count(number) over w as c
from numbers(31)
...
select min(number) over (partition by p)  from (select number, int_div(number, 3) as p from numbers(10));
...
from
    (select number, int_div(number, 3) as p, mod(number, 5) as o
        from numbers(31))
window
...
from (select number, int_div(number, 5) as p from numbers(31))
order by p, number
settings max_block_size = 2;
...
from (select number, int_div(number, 7) as p from numbers(71))
order by p, number
settings max_block_size = 2;
...
select
    count() over (partition by int_div(number, 3)
...
SELECT number, max(number) OVER (PARTITION BY int_div(number, 7) ORDER BY number ASC NULLS LAST ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) FROM numbers(1024) SETTINGS max_block_size = 2 FORMAT Null;
...
from (select number, int_div(number, 5) as p, mod(number, 3) as o
    from numbers(31))
order by p, o, number
settings max_block_size = 2;
...
select x, min(x) over w, max(x) over w, count(x) over w from (
    select to_uint8(number) as x from numbers(11))
...
from (
    select to_uint8(if(mod(number, 2),
        to_int64(255 - int_div(number, 2)),
        to_int64(int_div(number, 2)))) as x
    from numbers(10)
)
...
select x, min(x) over w, max(x) over w, count(x) over w
from (
    select to_int8(multi_if(
        mod(number, 3) == 0, to_int64(int_div(number, 3)),
        mod(number, 3) == 1, to_int64(127 - int_div(number, 3)),
        to_int64(-128 + int_div(number, 3)))) as x
    from numbers(15)
)
window w as (order by x range between 1 preceding and 2 following)
order by x;
...
select x, min(x) over w, max(x) over w, count(x) over w from (
    select to_uint8(number) as x from numbers(11)) as t
...
-- it is disabled.
drop stream if exists window_mt;
create stream window_mt engine MergeTree order by number
    as select number, mod(number, 3) as p from numbers(100);
...
select number, p, o,
    ...
    row_number() over w
from (select number, int_div(number, 5) as p, mod(number, 3) as o
    from numbers(31) order by o, number) as t
...
select number, p, pp,
    lag_in_frame(number) over w as lag1,
    lag_in_frame(number, number - pp) over w as lag2,
    lag_in_frame(number, number - pp, number * 11) over w as lag,
    lead_in_frame(number, number - pp, number * 11) over w as lead
from (select number, int_div(number, 5) as p, p * 5 as pp from numbers(16))
...
select lag_in_frame(to_nullable(1)) over ();
select lag_in_frame_or_null(1) over (); -- { serverError 36 }
-- this is the same as `select max(Null::Nullable(Nothing))`
select int_div(1, NULL) as x, to_type_name(x), max(x) over ();
-- to make lag_in_frame return null for out-of-frame rows, cast the argument to
-- Nullable; otherwise, it returns default values.
SELECT
    number,
    lag_in_frame(to_nullable(number), 1) OVER w,
    lag_in_frame(to_nullable(number), 2) OVER w,
    lag_in_frame(number, 1) OVER w,
    lag_in_frame(number, 2) OVER w
FROM numbers(4)
...
-- proton: starts. TODO: parting 'nth_value'
...
-- SELECT
--     number,
--     nth_value(to_nullable(number), 1) OVER w as firstValue,
--     nth_value(to_nullable(number), 3) OVER w as thridValue
-- FROM numbers(5)
-- WINDOW w AS (ORDER BY number ASC)
-- ;
...
-- lag_in_frame UBsan
SELECT lag_in_frame(1, -1) OVER (); -- { serverError BAD_ARGUMENTS }
SELECT lag_in_frame(1, 0) OVER ();
SELECT lag_in_frame(1, /* INT64_MAX+1 */ 0x7fffffffffffffff+1) OVER (); -- { serverError BAD_ARGUMENTS }
SELECT lag_in_frame(1, /* INT64_MAX */ 0x7fffffffffffffff) OVER ();
SELECT lag_in_frame(1, 1) OVER ();

-- lead_in_frame UBsan
SELECT lead_in_frame(1, -1) OVER (); -- { serverError BAD_ARGUMENTS }
SELECT lead_in_frame(1, 0) OVER ();
SELECT lead_in_frame(1, /* INT64_MAX+1 */ 0x7fffffffffffffff+1) OVER (); -- { serverError BAD_ARGUMENTS }
SELECT lead_in_frame(1, /* INT64_MAX */ 0x7fffffffffffffff) OVER ();
SELECT lead_in_frame(1, 1) OVER ();
...
-- lag_in_frame Msan
SELECT lag_in_frame(1, '') OVER (); -- { serverError BAD_ARGUMENTS }

-- lead_in_frame Msan
SELECT lead_in_frame(1, '') OVER (); -- { serverError BAD_ARGUMENTS }
...
select
    count(*) over (order by to_float32(number) range 5. preceding),
    count(*) over (order by to_float64(number) range 5. preceding),
    count(*) over (order by to_float32(number) range between current row and 5. following),
    count(*) over (order by to_float64(number) range between current row and 5. following)
from numbers(7)
;

-- negative offsets should not be allowed
select count() over (order by to_int64(number) range between -1 preceding and unbounded following) from numbers(1); -- { serverError 36 }
select count() over (order by to_int64(number) range between -1 following and unbounded following) from numbers(1); -- { serverError 36 }
select count() over (order by to_int64(number) range between unbounded preceding and -1 preceding) from numbers(1); -- { serverError 36 }
select count() over (order by to_int64(number) range between unbounded preceding and -1 following) from numbers(1); -- { serverError 36 }

-- a test with aggregate function that allocates memory in arena
select sum(a[length(a)])
from (
    select group_array(number) over (partition by modulo(number, 11)
            order by modulo(number, 1111), number) as a
    from numbers_mt(10000)
) settings max_block_size = 7;

-- a test with aggregate function which is -state type
select bitmap_cardinality(bs)
from
    (
        select group_bitmap_merge_state(bm) over (order by k asc rows between unbounded preceding and current row) as bs
        from
            (
                select
                    group_bitmap_state(number) as bm, k
                from
                    (
                        select
                            number,
...
select count() over () from (select 1 as a) as l inner join (select 2 as a) as r using a;
...
window
    w0 as (partition by int_div(number, 5) as p),
    w1 as (w0 order by mod(number, 3) as o, number)
order by p, o, number
;

-- can't redefine PARTITION BY
select count() over (w partition by number) from numbers(1) window w as (partition by int_div(number, 5)); -- { serverError 36 }

-- can't redefine existing ORDER BY
select count() over (w order by number) from numbers(1) window w as (partition by int_div(number, 5) order by mod(number, 3)); -- { serverError 36 }

-- parent window can't have frame
select count() over (w range unbounded preceding) from numbers(1) window w as (partition by int_div(number, 5) order by mod(number, 3) rows unbounded preceding); -- { serverError 36 }
