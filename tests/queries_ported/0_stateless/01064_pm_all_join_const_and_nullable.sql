...
SELECT k FROM (SELECT materialize(1) AS k FROM numbers(1)) as nums
...
SELECT count(1), uniq_exact(1) FROM (
SELECT k FROM (SELECT 1 AS k FROM numbers(1)) as nums
JOIN (SELECT materialize(1) AS k, number as n FROM numbers(100000)) as j
USING k);
...
SELECT count(1), uniq_exact(1) FROM (
SELECT k FROM (SELECT materialize(to_nullable(1)) AS k FROM numbers(1)) as nums
JOIN (SELECT materialize(1) AS k, number as n FROM numbers(100000)) as j
USING k);
...
SELECT count(1), uniq_exact(1) FROM (
SELECT k FROM (SELECT to_nullable(1) as k FROM numbers(1)) as nums
JOIN (SELECT materialize(1) AS k, number as n FROM numbers(100000)) as j
USING k);
