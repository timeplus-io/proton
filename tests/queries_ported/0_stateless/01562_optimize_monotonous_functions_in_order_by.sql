...
INSERT INTO test_order_by SELECT now() + to_interval_second(number), number % 4 FROM numbers(10000);
...
CREATE STREAM test_order_by (timestamp datetime, key uint32) ENGINE=MergeTree() ORDER BY key;
INSERT INTO test_order_by SELECT now() + to_interval_second(number), number % 4 FROM numbers(10000);
OPTIMIZE STREAM test_order_by FINAL;
