...
$CLICKHOUSE_CLIENT --query="DROP STREAM IF EXISTS table_to_rename"

$CLICKHOUSE_CLIENT --query="CREATE STREAM table_to_rename(v uint64, v1 uint64)ENGINE = MergeTree ORDER BY tuple_cast() SETTINGS min_bytes_for_wide_part = 0"
...
$CLICKHOUSE_CLIENT --query="ALTER STREAM table_to_rename RENAME COLUMN v1 to v2" &
...
while [[ $counter -lt $retries ]]; do
    I=$((I + 1))
    result=$($CLICKHOUSE_CLIENT --query "show create stream table_to_rename")
    if [[ $result == *"v2"* ]]; then
        break;
    fi
    sleep 0.1
...
$CLICKHOUSE_CLIENT --query="ALTER STREAM table_to_rename UPDATE v2 = 77 WHERE 1 = 1 SETTINGS mutations_sync = 2" &
