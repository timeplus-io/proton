...
ORDER BY (key, to_start_of_interval(ts, to_interval_minute(3)), ts) 
TTL ts + INTERVAL 5 MINUTE GROUP BY key, to_start_of_interval(ts, to_interval_minute(3)) 
SET value = sum(value), min_value = min(min_value), max_value = max(max_value),  ts=min(to_start_of_interval(ts, to_interval_minute(3)));
