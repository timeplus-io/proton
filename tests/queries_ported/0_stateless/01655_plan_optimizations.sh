...
$CLICKHOUSE_CLIENT -q "explain actions = 1 select sum(x), sum(y) from (select sip_hash64(number) as x, bit_and(number, 1024) as y from numbers_mt(1000000000) limit 1000000000) where y = 0" | grep -o "FUNCTION sip_hash64\|Filter column: equals"
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0" | grep -o "Aggregating\|Filter\|COLUMN const(uint8) -> not_equals(y, 0)"
$CLICKHOUSE_CLIENT -q "
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0" |
    grep -o "Aggregating\|Filter column\|Filter column: not_equals(y, 0)\|ALIAS not_equals(s, 4) :: 1 -> and(not_equals(y, 0), not_equals(s, 4))"
$CLICKHOUSE_CLIENT -q "
    select s, y from (
        select sum(x) as s, y from (select number as x, number + 1 as y from numbers(10)) group by y
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0" |
    grep -o "Aggregating\|Filter column\|Filter column: not_equals(y, 0)\|FUNCTION _cast(minus(s, 4) :: 1, bool :: 3) -> and(not_equals(y, 0), minus(s, 4))"
$CLICKHOUSE_CLIENT -q "
    select s, y from (
        select sum(x) as s, y from (select number as x, number + 1 as y from numbers(10)) group by y
    ) where y != 0 and s - 4 order by s, y
...
$CLICKHOUSE_CLIENT --convert_query_to_cnf=0 -q "
    ...
    settings enable_optimize_predicate_expression=0" |
    grep -o "Aggregating\|Filter column\|Filter column: not_equals(y, 0)\|FUNCTION and(minus(s, 8) :: 1, minus(s, 4) :: 2) -> and(not_equals(y, 0), minus(s, 8), minus(s, 4))"
$CLICKHOUSE_CLIENT -q "
    select s, y from (
        select sum(x) as s, y from (select number as x, number + 1 as y from numbers(10)) group by y
...
$CLICKHOUSE_CLIENT --convert_query_to_cnf=0 -q "
    ...
    settings enable_optimize_predicate_expression=0" |
    grep -o "Aggregating\|Filter column\|Filter column: and(not_equals(y, 0), minus(y, 4))\|ALIAS not_equals(s, 8) :: 1 -> and(not_equals(y, 0), not_equals(s, 8), minus(y, 4))"
$CLICKHOUSE_CLIENT -q "
    select s, y from (
        select sum(x) as s, y from (select number as x, number + 1 as y from numbers(10)) group by y
...
$CLICKHOUSE_CLIENT -q "
    ...
    grep -o "Filter column: and(not_equals(y, 2), not_equals(x, 0))\|ARRAY JOIN x\|Filter column: not_equals(y, 2)"
$CLICKHOUSE_CLIENT -q "
    select x, y from (
        select range(number) as x, number + 1 as y from numbers(3)
...
#     ) where y != 0 and s != 4
#     settings enable_optimize_predicate_expression=0" |
#     grep -o "Cube\|Aggregating\|Filter column: not_equals(y, 0)"
# $CLICKHOUSE_CLIENT -q "
#     select s, x, y from (
#         select sum(x) as s, x, y from (select number as x, number + 1 as y from numbers(10)) group by x, y　with cube
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0" |
    grep -o "Distinct\|Filter column: not_equals(y, 2)"
$CLICKHOUSE_CLIENT -q "
    select x, y from (
        select distinct x, y from (select number % 2 as x, number % 3 as y from numbers(10))
...
$CLICKHOUSE_CLIENT --convert_query_to_cnf=0 -q "
    ...
    settings enable_optimize_predicate_expression = 0" |
    grep -o "Sorting\|Filter column: and(not_equals(x, 0), not_equals(y, 0))"
$CLICKHOUSE_CLIENT -q "
    select x, y from (
        select number % 2 as x, number % 3 as y from numbers(6) order by y desc
    ) where x != 0 and y != 0
...
$CLICKHOUSE_CLIENT -q "
    ...
    settings enable_optimize_predicate_expression=0" |
    grep -o "TotalsHaving\|Aggregating\|Filter column: not_equals(y, 2)"
$CLICKHOUSE_CLIENT -q "
    select * from (
        select y, sum(x) from (select number as x, number % 4 as y from numbers(10)) group by y with totals
    ) where y != 2"
...
$CLICKHOUSE_CLIENT -q "
    ...
    grep -o "Join\|Filter column: not_equals(number, 1)"
$CLICKHOUSE_CLIENT -q "
    select number as a, r.b from numbers(4) as l any left join (
        select number + 2 as b from numbers(3)
...
$CLICKHOUSE_CLIENT -q "
    ...
    grep -o "Join\|Filter column: and(not_equals(b, 2), not_equals(b, 1))"
$CLICKHOUSE_CLIENT -q "
    select number as a, r.b from numbers(4) as l any inner join (
        select number + 2 as b from numbers(3)
    ) as r on a = r.b where a != 1 and b != 2 settings enable_optimize_predicate_expression = 0"
...
$CLICKHOUSE_CLIENT -q "
    explain actions = 1 select number as n, sip_hash64(n) from numbers(100) order by number + 1 limit 5" |
...
$CLICKHOUSE_CLIENT -q "
    select throw_if(number = 5) from (select * from numbers(10)) order by number limit 1"
