...
$CLICKHOUSE_CLIENT -q "
    ...
    LAYOUT(HASHED())
    SOURCE(TIMEPLUS(HOST 'localhost' PORT tcp_port() USER 'default' STREAM 'table_for_dict1' DB '$CLICKHOUSE_DATABASE'))
...
$CLICKHOUSE_CLIENT -q "
    ...
    SOURCE(PROTON(HOST 'localhost' PORT tcp_port() USER 'default' STREAM 'table_for_dict1' DB '$CLICKHOUSE_DATABASE'))
    LIFETIME(MIN 1 MAX 10)
" 2>&1 | grep -c "Unknown key attribute 'non_existing_column'"
...
$CLICKHOUSE_CLIENT -q "
    ...
    PRIMARY KEY key1
    LAYOUT(COMPLEX_KEY_HASHED())
    SOURCE(TIMEPLUS(HOST 'localhost' PORT tcp_port() USER 'default' STERAM 'table_for_dict1' DB '$CLICKHOUSE_DATABASE'))
    LIFETIME(MIN 1 MAX 10)
" || exit 1
