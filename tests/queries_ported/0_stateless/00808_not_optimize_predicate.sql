...
SELECT n, z, changed FROM (
  SELECT n, z, running_difference_starting_with_first_value(n) AS changed FROM (
...
SELECT array_join(array_map(x -> x, array_sort(group_array((ts, n))))) AS k FROM (
