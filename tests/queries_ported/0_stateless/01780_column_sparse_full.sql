...
-- with mixed columns (sparse and full) in STREAM.
...
FROM system.parts_columns WHERE table = 't_sparse_full' AND database = current_database() AND active
...
SELECT '======';
SELECT uniq_exact(u) FROM t_sparse_full WHERE s != '';
SELECT '======';
SELECT to_uint32(s) % 5 AS k, group_uniq_array(u % 4) FROM t_sparse_full WHERE s != '' GROUP BY k ORDER BY k;
SELECT max(range(id % 10)[u]) FROM t_sparse_full;
SELECT '======';
-- proton: starts. lack of permission in timeplus to execute this query
-- SELECT id, u, s FROM remote('127.0.0.{1,2}', current_database(), t_sparse_full) ORDER BY id, u, s LIMIT 5;
-- SELECT '======';
-- proton: ends
