...
$CLICKHOUSE_CLIENT -q "create stream test_02293 (a uint32, b string, c array(tuple(array(uint32), string))) engine=Memory"
...
$CLICKHOUSE_CLIENT -q "select sum(a) as sum, avg(a) as avg from test_02293 group by a % 4 with totals order by sum, avg format JSONColumnsWithMetadata" --extremes=1 | grep -v "elapsed"
...
$CLICKHOUSE_CLIENT -q "desc file(data_02293, JSONColumns) settings input_format_max_rows_to_read_for_schema_inference=3, input_format_json_infer_incomplete_types_as_strings=0" 2>&1 | grep -F -q 'CANNOT_EXTRACT_STREAM_STRUCTURE' && echo 'OK' || echo 'FAIL'
