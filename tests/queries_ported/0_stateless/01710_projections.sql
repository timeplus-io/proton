...
create stream projection_test (`sum(block_count)` uint64, domain_alias uint64 alias length(domain), datetime DateTime, domain low_cardinality(string), x_id string, y_id string, block_count int64, retry_count int64, duration int64, kbytes int64, buffer_time int64, first_time int64, total_bytes nullable(uint64), valid_bytes nullable(uint64), completed_bytes nullable(uint64), fixed_bytes nullable(uint64), force_bytes nullable(uint64), projection p (select to_start_of_minute(datetime) dt_m, count_if(first_time = 0) / count(), avg((kbytes * 8) / duration), count(), sum(block_count) / sum(duration), avg(block_count / duration), sum(buffer_time) / sum(duration), avg(buffer_time / duration), sum(valid_bytes) / sum(total_bytes), sum(completed_bytes) / sum(total_bytes), sum(fixed_bytes) / sum(total_bytes), sum(force_bytes) / sum(total_bytes), sum(valid_bytes) / sum(total_bytes), sum(retry_count) / sum(duration), avg(retry_count / duration), count_if(block_count > 0) / count(), count_if(first_time = 0) / count(), uniq_hll12(x_id), uniq_hll12(y_id) group by dt_m, domain)) engine MergeTree partition by to_date(datetime) order by (to_start_of_ten_minutes(datetime), domain) settings index_granularity_bytes = 10000000;

insert into projection_test with row_number_in_block() as id select 1, to_datetime('2020-10-24 00:00:00') + (id / 20), to_string(id % 100), * from generate_random('x_id string, y_id string, block_count int64, retry_count int64, duration int64, kbytes int64, buffer_time int64, first_time int64, total_bytes nullable(uint64), valid_bytes nullable(uint64), completed_bytes nullable(uint64), fixed_bytes nullable(uint64), force_bytes nullable(uint64)', 10, 10, 1) limit 1000 settings max_threads = 1;

set allow_experimental_projection_optimization = 1, force_optimize_projection = 1;

select * from projection_test; -- { serverError 584 }
select to_start_of_minute(datetime) dt_m, count_if(first_time = 0) from projection_test join (select 1) x using (1) where domain = '1' group by dt_m order by dt_m; -- { serverError 584 }

select to_start_of_minute(datetime) dt_m, count_if(first_time = 0) / count(), avg((kbytes * 8) / duration) from projection_test where domain = '1' group by dt_m order by dt_m;

-- prewhere with alias
select to_start_of_minute(datetime) dt_m, count_if(first_time = 0) / count(), avg((kbytes * 8) / duration) from projection_test prewhere domain_alias = 3 where domain = '1' group by dt_m order by dt_m;
...
select to_start_of_minute(datetime) dt_m, count_if(first_time = 0) / count(), avg((kbytes * 8) / duration) from projection_test prewhere domain_alias = 1 where domain = '1' group by dt_m order by dt_m;
...
select to_start_of_minute(datetime) dt_m, count(), sum(block_count) / sum(duration), avg(block_count / duration) from projection_test group by dt_m order by dt_m;

-- TODO figure out how to deal with conflict column names
-- select to_start_of_minute(datetime) dt_m, count(), sum(block_count) / sum(duration), avg(block_count / duration) from projection_test where `sum(block_count)` = 1 group by dt_m order by dt_m;

select to_start_of_minute(datetime) dt_m, sum(buffer_time) / sum(duration), avg(buffer_time / duration), sum(valid_bytes) / sum(total_bytes), sum(completed_bytes) / sum(total_bytes), sum(fixed_bytes) / sum(total_bytes), sum(force_bytes) / sum(total_bytes), sum(valid_bytes) / sum(total_bytes) from projection_test where domain in ('12', '14') group by dt_m order by dt_m;

select to_start_of_minute(datetime) dt_m, domain, sum(retry_count) / sum(duration), avg(retry_count / duration), count_if(block_count > 0) / count(), count_if(first_time = 0) / count() from projection_test group by dt_m, domain having domain = '19' order by dt_m, domain;

select to_start_of_hour(to_start_of_minute(datetime)) dt_h, uniq_hll12(x_id), uniq_hll12(y_id) from projection_test group by dt_h order by dt_h;
