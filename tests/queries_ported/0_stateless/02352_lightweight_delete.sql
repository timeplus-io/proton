...
CREATE STREAM lwd_test (id uint64, value string) ENGINE MergeTree() ORDER BY id SETTINGS index_granularity=8192, index_granularity_bytes='10Mi';

INSERT INTO lwd_test SELECT number, random_string(10) FROM system.numbers LIMIT 1000000;

SET mutations_sync = 0;

SELECT 'Rows in parts', sum(rows) FROM system.parts WHERE database = current_database() AND table = 'lwd_test' AND active;
...
OPTIMIZE STREAM lwd_test FINAL SETTINGS mutations_sync = 2;
