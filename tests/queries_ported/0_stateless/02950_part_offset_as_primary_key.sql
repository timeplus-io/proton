...
create stream a (i int) engine MergeTree order by i settings index_granularity = 2;
...
create stream b (i int, k int) engine MergeTree order by k settings index_granularity = 2;

-- all_1_1_0
insert into b select number * 10, number from numbers(5);
-- all_2_2_0
insert into b select number * 100, number from numbers(5);
