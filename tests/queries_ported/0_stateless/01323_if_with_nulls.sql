...
FROM (SELECT 1 as k, to_int8(1) as a_num) AS x
LEFT JOIN (SELECT 2 as k, to_int8(1) as b_num) AS y
USING (k);
...
SELECT if(is_finite(to_uint64_or_zero(to_nullable('123'))), 1, 0);

SELECT if(materialize(is_finite(to_uint64_or_zero(to_nullable('123')))), 1, 0);
