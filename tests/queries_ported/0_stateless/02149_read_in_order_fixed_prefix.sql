...
DROP STREAM IF EXISTS t_read_in_order;

CREATE STREAM t_read_in_order(date Date, i uint64, v uint64)
...
SELECT to_start_of_month(date) as d, i FROM t_read_in_order ORDER BY d, i LIMIT 5;
EXPLAIN PIPELINE SELECT to_start_of_month(date) as d, i FROM t_read_in_order ORDER BY d, i LIMIT 5;

SELECT to_start_of_month(date) as d, i FROM t_read_in_order ORDER BY d DESC, -i LIMIT 5;
EXPLAIN PIPELINE SELECT to_start_of_month(date) as d, i FROM t_read_in_order ORDER BY d DESC, -i LIMIT 5;
...
CREATE STREAM t_read_in_order(a uint32, b uint32)
...
DROP STREAM t_read_in_order;

CREATE STREAM t_read_in_order(dt DateTime, d decimal64(5), v uint64)
ENGINE = MergeTree ORDER BY (to_start_of_day(dt), d);

INSERT INTO t_read_in_order SELECT to_datetime('2020-10-10 00:00:00') + number, 1 / (number % 100 + 1), number FROM numbers(1000);

EXPLAIN PIPELINE SELECT to_start_of_day(dt) as date, d FROM t_read_in_order ORDER BY date, round(d) LIMIT 5;
SELECT * from (
  SELECT to_start_of_day(dt) as date, d FROM t_read_in_order ORDER BY date, round(d) LIMIT 50000000000
  -- subquery with limit 50000000 to stabilize a test result and prevent order by d pushdown
) order by d limit 5;

EXPLAIN PIPELINE SELECT to_start_of_day(dt) as date, d FROM t_read_in_order ORDER BY date, round(d) LIMIT 5;
SELECT * from (
  SELECT to_start_of_day(dt) as date, d FROM t_read_in_order WHERE date = '2020-10-10' ORDER BY round(d) LIMIT 50000000000
  -- subquery with limit 50000000 to stabilize a test result and prevent order by d pushdown
) order by d limit 5;
