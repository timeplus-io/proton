...
FROM (select number as id, 42 as value from numbers(4)) as t1
LEFT JOIN (select number as id, 42 as value from numbers(3)) as t2 ON t1.id = t2.id
LEFT JOIN (select number as id, 42 as value from numbers(2)) as t3 ON t1.id = t3.id
