...
select number as k, repeat(to_string(number), 11) as v1, repeat(to_string(number), 12) as v2 from numbers(3e6) order by v1, v2 limit 400e3 format Null; -- { serverError 241 }
select number as k, repeat(to_string(number), 11) as v1, repeat(to_string(number), 12) as v2 from numbers(3e6) order by v1, v2 limit 400e3 settings remerge_sort_lowered_memory_bytes_ratio=2. format Null; -- { serverError 241 }
...
--     MergeSortingTransform: Re-merging intermediate ORDER BY data (20 blocks with 809600 rows) to save memory consumption
--     MergeSortingTransform: Memory usage is lowered from 188.13 MiB to 95.00 MiB
select number as k, repeat(to_string(number), 11) as v1, repeat(to_string(number), 12) as v2 from numbers(3e6) order by k limit 400e3 settings remerge_sort_lowered_memory_bytes_ratio=1.9 format Null;
