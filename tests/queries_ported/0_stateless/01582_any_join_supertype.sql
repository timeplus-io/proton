...
CREATE STREAM foo (server_date Date, server_time Datetime('Asia/Istanbul'), dimension_1 string) ENGINE = MergeTree() PARTITION BY to_YYYYMM(server_date) ORDER BY (server_date);
