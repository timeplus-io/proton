...
$CLICKHOUSE_CLIENT -q "create stream test_index (x uint32, y uint32, z uint32, t uint32, index t_minmax t % 20 TYPE minmax GRANULARITY 2, index t_set t % 19 type set(4) granularity 2) engine = MergeTree order by (x, y) partition by (y, bit_and(z, 3), int_div(t, 15)) settings index_granularity = 2, min_bytes_for_wide_part = 0, ratio_of_defaults_for_sparse_serialization = 1"
$CLICKHOUSE_CLIENT -q "select sleep(1)"
$CLICKHOUSE_CLIENT -q "insert into test_index(x, y, z, t) select number, number > 3 ? 3 : number, number = 1 ? 1 : 0, number from numbers(20)"
...
$CLICKHOUSE_CLIENT -q "CREATE stream idx (x uint32, y uint32, z uint32) engine=MergeTree ORDER BY (x, x + y) settings min_bytes_for_wide_part = 0, ratio_of_defaults_for_sparse_serialization = 1"
$CLICKHOUSE_CLIENT -q "select sleep(1)"
$CLICKHOUSE_CLIENT -q "insert into idx(x, y, z) select number, number, number from numbers(10)"
