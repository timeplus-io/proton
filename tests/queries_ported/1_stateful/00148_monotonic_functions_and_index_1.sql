...
SELECT uniq(CounterID),  count() FROM table(test.hits) WHERE to_int8(CounterID + 1 - 1) = to_int8(1731);
SELECT uniq(CounterID),  count() FROM table(test.hits) WHERE to_int8(CounterID) = to_int8(1731);

SELECT uniq(CounterID),  count() FROM table(test.hits) WHERE to_int16(CounterID + 1 - 1) = 1731;
SELECT uniq(CounterID),  count() FROM table(test.hits) WHERE to_int16(CounterID) = 1731;
...
SELECT count() FROM table(test.hits) WHERE to_day_of_month(EventDate) = 20;
SELECT count() FROM table(test.hits) WHERE to_day_of_week(EventDate) = 4;
...
SELECT count() FROM table(test.hits) WHERE to_month(EventDate) != 3;
SELECT count() FROM table(test.hits) WHERE to_year(EventDate) != 2014;
SELECT count() FROM table(test.hits) WHERE to_day_of_month(EventDate) > 23 OR to_day_of_month(EventDate) < 17;
