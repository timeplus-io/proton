SELECT PP.Key1 AS `ym:s:paramsLevel1`, sum(to_uint8(array_all(`x_1` -> `x_1`= '', ParsedParams.Key2))) AS `ym:s:visits` FROM test.hits ARRAY JOIN ParsedParams AS `PP`  WHERE CounterID = 1704509 GROUP BY `ym:s:paramsLevel1` ORDER BY PP.Key1, `ym:s:visits` LIMIT 0, 100;
