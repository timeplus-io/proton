...
INSERT INTO test.partitions SELECT EventDate + UserID % 365 AS EventDate, CounterID from table(test.hits) WHERE CounterID = 1704509;
select sleep(3);
...
INSERT INTO test.partitions SELECT EventDate + UserID % 365 AS EventDate, CounterID from table(test.hits) WHERE CounterID = 1704509 AND to_start_of_month(EventDate) = to_date('2014-03-01');
select sleep(3);
