...
def test_config_without_allowed_hosts_section(start_cluster):
    ...
    assert node4.query("CREATE TABLE table_test_4_3 (word String) Engine=URL('https://host', CSV)") == ""
    assert node4.query("CREATE TABLE table_test_4_4 (word String) Engine=URL('https://yandex.ru', CSV)") == ""
    assert node4.query("CREATE TABLE table_test_4_5 (word String) Engine=URL('ftp://something.com', CSV)") == ""
