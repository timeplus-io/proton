...
DROP STREAM IF EXISTS wikistat1 SYNC;
DROP STREAM IF EXISTS wikistat2 SYNC;
"

for i in {1..2}; do
    $CLICKHOUSE_CLIENT --query "
    CREATE STREAM wikistat$i
    (
        time DateTime,
...
$CLICKHOUSE_CLIENT --query "INSERT INTO wikistat1 SELECT to_datetime('2020-10-01 00:00:00'), 'hello', 'world', '/data/path', 10 from numbers(1000)"
...
$CLICKHOUSE_CLIENT --query "ALTER STREAM wikistat2 MODIFY SETTING max_replicated_fetches_network_bandwidth = 0"
$CLICKHOUSE_CLIENT --query "SYSTEM START FETCHES wikistat2"
wait
