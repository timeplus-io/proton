...
DROP STREAM IF EXISTS 02354_annoy_l2;

CREATE STREAM 02354_annoy_l2
(
    id int32,
...
$CLICKHOUSE_CLIENT -nm --allow_experimental_annoy_index=1 -q "
DROP STREAM IF EXISTS 02354_annoy_cosine;

CREATE STREAM 02354_annoy_cosine
(
    id int32,
    embedding array(float32),
...
$CLICKHOUSE_CLIENT -nm --allow_experimental_annoy_index=1 -q "
DROP STREAM IF EXISTS 02354_annoy;

-- Index spans >1 column

CREATE STREAM 02354_annoy
(
    id int32,
    embedding array(float32),
...
(
    id int32,
    embedding array(low_cardinality(float32)),
    INDEX annoy_index embedding TYPE annoy(100) GRANULARITY 1
)
ENGINE = MergeTree
ORDER BY id
