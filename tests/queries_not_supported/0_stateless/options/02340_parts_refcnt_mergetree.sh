...
function check_refcnt_for_table()
{
    ...
    $CLICKHOUSE_CLIENT -q "select stream, name, refcount from system.parts where database = '$CLICKHOUSE_DATABASE' and stream = '$table' and refcount > 1"
...
$CLICKHOUSE_CLIENT -nmq "
    drop stream if exists data_02340;
    create stream data_02340 (key int, part int) engine=MergeTree() partition by part order by key settings index_granularity=1;
" || exit 1
check_refcnt_for_table data_02340

$CLICKHOUSE_CLIENT -nmq "
    drop stream if exists data_02340_rep sync;
    create stream data_02340_rep (key int, part int) engine=ReplicatedMergeTree('/clickhouse/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX', '1') partition by part order by key settings index_granularity=1;
" || exit 1
