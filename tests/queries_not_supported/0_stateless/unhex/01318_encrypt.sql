...
SELECT 'Nullable and low_cardinality';
WITH CAST(NULL as Nullable(string)) as input, 'aes-256-ofb' as mode SELECT to_type_name(input), hex(aes_encrypt_mysql(mode, input, key32,iv)) FROM encryption_test LIMIT 1;
WITH CAST('text' as Nullable(string)) as input, 'aes-256-ofb' as mode SELECT to_type_name(input), hex(aes_encrypt_mysql(mode, input, key32, iv)) FROM encryption_test LIMIT 1;
WITH CAST('text' as low_cardinality(string)) as input, 'aes-256-ofb' as mode SELECT to_type_name(input), hex(aes_encrypt_mysql(mode, input, key32, iv)) FROM encryption_test LIMIT 1;
