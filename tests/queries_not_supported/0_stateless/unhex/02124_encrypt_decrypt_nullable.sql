...
-------------------------------------------------------------------------------
-- using null_if since that is the easiest way to produce `Nullable(string)` with a `null` value
...
SELECT hex(aes_encrypt_mysql(mode, to_nullable(plaintext), key));
...
SELECT hex(aes_decrypt_mysql(mode, to_nullable(ciphertext), key));
...
WITH 'aes-256-ecb' as mode, 'test_key________________________' as key
SELECT mode, hex(encrypt(mode, to_nullable('Hello World!'), key));

WITH 'aes-256-gcm' as mode, 'test_key________________________' as key, 'test_iv_____' as iv
SELECT mode, hex(encrypt(mode, to_nullable('Hello World!'), key, iv));
...
WITH 'aes-256-ecb' as mode, unhex('D1B43643E1D0E9390E39BA4EAE150851') as ciphertext, 'test_key________________________' as key
SELECT mode, decrypt(mode, to_nullable(ciphertext), key);
...
SELECT mode, decrypt(mode, to_nullable(ciphertext), key, iv);
