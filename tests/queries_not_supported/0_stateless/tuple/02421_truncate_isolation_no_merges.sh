...
function reset_table()
{
    ...
    $CLICKHOUSE_CLIENT -q "create stream $table (n int) engine=MergeTree order by tuple()"
...
function concurrent_drop_after()
{
    ...
    tx 11 "truncate stream tt"
    $CLICKHOUSE_CLIENT --database_atomic_wait_for_drop_and_detach_synchronously=0 -q "drop stream tt"
    tx 11 "commit"
}
...
function concurrent_drop_before()
{
    ...
    $CLICKHOUSE_CLIENT -q                                 "drop stream tt"
    tx 21 "truncate stream tt" | grep -Eo "UNKNOWN_TABLE" | uniq
    tx 21 "rollback"
}
...
function concurrent_insert()
{
    ...
    tx 31 "truncate stream tt"                                                       # creates all_1_4_1 all_6_6_1
    tx 31 "commit"
    tx 32                                            "commit"

    $CLICKHOUSE_CLIENT -q "select n from tt order by n"
    $CLICKHOUSE_CLIENT -q "select name, rows from system.parts
                              where stream='tt' and database=current_database() and active
                              order by name"
}
...
function concurrent_drop_part_before()
{
    ...
    tx 42                         "alter stream tt drop part 'all_2_2_0'"
    tx 41 "truncate stream tt" | grep -Eo "SERIALIZATION_ERROR" | uniq
...
function concurrent_drop_part_after()
{
    ...
    tx 61 "truncate stream drop_part_after_table"
    tx 62             "alter stream drop_part_after_table drop part 'all_2_2_0'" | grep -Eo "NO_SUCH_DATA_PART" | uniq
    tx 61 "commit"
    tx 62             "commit" | grep -Eo "INVALID_TRANSACTION" | uniq

    $CLICKHOUSE_CLIENT -q "select n from drop_part_after_table order by n"
    $CLICKHOUSE_CLIENT -q "select name, rows from system.parts
                              where stream='drop_part_after_table' and database=current_database() and active
                              order by name"
    $CLICKHOUSE_CLIENT -q "system flush logs"
    $CLICKHOUSE_CLIENT -q "select event_type, part_name from system.part_log
                              where stream='drop_part_after_table' and database=current_database()
                              order by part_name"
}
...
function concurrent_truncate_notx_after()
{
    ...
    tx 71 "alter stream tt drop part 'all_2_2_0'"
    $CLICKHOUSE_CLIENT -q                                 "truncate stream tt"
...
function concurrent_truncate_notx_before()
{
    ...
    $CLICKHOUSE_CLIENT -q                                 "truncate stream tt"
    tx 81 "alter stream tt drop part 'all_2_2_0'" | grep -Eo "NO_SUCH_DATA_PART" | uniq
...
function concurrent_rollback_truncate()
{
    ...
    tx 91       "truncate stream tt"
    tx_async 91 "rollback"
    tx 92               "truncate stream tt" | grep -vwe "PART_IS_TEMPORARILY_LOCKED" -vwe "SERIALIZATION_ERROR" ||:
    tx 92               "rollback"
    tx_wait 91
