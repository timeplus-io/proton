...
with client(name='client1>', log=log) as client1, client(name='client2>', log=log) as client2:
    ...
    client1.expect(prompt)
    client1.send('DROP STREAM IF EXISTS 01056_window_view_proc_hop_watch.mt')
    client1.expect(prompt)
    client1.send('DROP STREAM IF EXISTS 01056_window_view_proc_hop_watch.wv')
    client1.expect(prompt)
    client1.send('DROP STREAM IF EXISTS `.inner.wv`')
    client1.expect(prompt)

    client1.send('create stream 01056_window_view_proc_hop_watch.mt(a int32, timestamp datetime) ENGINE=MergeTree ORDER BY tuple()')
    client1.expect(prompt)
    ...
    if match.groups()[1]:
        ...
        client1.expect(prompt)
    client1.send('DROP STREAM 01056_window_view_proc_hop_watch.wv')
    client1.expect(prompt)
    client1.send('DROP STREAM 01056_window_view_proc_hop_watch.mt')
    client1.expect(prompt)
