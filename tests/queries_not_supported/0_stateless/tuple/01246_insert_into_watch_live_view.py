...
with client(name='client1>', log=log) as client1, client(name='client2>', log=log) as client2, client(name='client3>', log=log) as client3:
    ...
    client1.expect(prompt)
    client1.send('DROP STREAM IF EXISTS test.lv_sums')
    client1.expect(prompt)
    client1.send('DROP STREAM IF EXISTS test.mt')
    client1.expect(prompt)
    client1.send('DROP STREAM IF EXISTS test.sums')
    client1.expect(prompt)
    ...
    client1.expect(prompt)
    client1.send('create stream test.sums (s int32, version int32) Engine=MergeTree ORDER BY tuple()')
    client1.expect(prompt)
    ...
    client2.send('DROP STREAM test.lv')
    client2.expect(prompt)
    client2.send('DROP STREAM test.lv_sums')
    client2.expect(prompt)
    client2.send('DROP STREAM test.sums')
    client2.expect(prompt)
    client2.send('DROP STREAM test.mt')
    client2.expect(prompt)
