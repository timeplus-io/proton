...
with client(name="client1>", log=log) as client1, client(
    ...
    client1.expect(prompt)
    client1.send("DROP STREAM IF EXISTS 01078_window_view_alter_query_watch.mt NO DELAY")
    client1.expect(prompt)
    client1.send("DROP STREAM IF EXISTS 01078_window_view_alter_query_watch.wv NO DELAY")
    client1.expect(prompt)

    client1.send(
        "CREATE STREAM 01078_window_view_alter_query_watch.mt(a int32, timestamp DateTime) ENGINE=MergeTree ORDER BY tuple()"
    )
    client1.expect(prompt)
    client1.send(
    ...
    client2.send(
        "INSERT INTO 01078_window_view_alter_query_watch.mt VALUES (1, to_datetime('1990/01/01 12:00:00', 'US/Samoa'));"
    )
    client2.expect("Ok.")
    client2.send(
        "INSERT INTO 01078_window_view_alter_query_watch.mt VALUES (1, to_datetime('1990/01/01 12:00:05', 'US/Samoa'));"
    )
    client2.expect("Ok.")
    client1.expect("1" + end_of_block)
    client1.expect("Progress: 1.00 rows.*\)")
    client2.send(
        "ALTER STREAM 01078_window_view_alter_query_watch.wv MODIFY QUERY SELECT count(a) * 2 AS count, hopEnd(wid) AS w_end FROM 01078_window_view_alter_query_watch.mt GROUP BY hop(timestamp, INTERVAL '2' SECOND, INTERVAL '3' SECOND, 'US/Samoa') AS wid"
    )
    client2.expect("Ok.")
    client2.expect(prompt)
    ...
    client2.send(
        "INSERT INTO 01078_window_view_alter_query_watch.mt VALUES (1, to_datetime('1990/01/01 12:00:06', 'US/Samoa'));"
    )
    client2.expect("Ok.")
    client2.send(
        "INSERT INTO 01078_window_view_alter_query_watch.mt VALUES (1, to_datetime('1990/01/01 12:00:10', 'US/Samoa'));"
    )
    client2.expect("Ok.")
    ...
    if match.groups()[1]:
        ...
        client3.expect(prompt)
    client3.send("DROP STREAM 01078_window_view_alter_query_watch.wv NO DELAY;")
    client3.expect(prompt)
    client3.send("DROP STREAM 01078_window_view_alter_query_watch.mt;")
    client3.expect(prompt)
