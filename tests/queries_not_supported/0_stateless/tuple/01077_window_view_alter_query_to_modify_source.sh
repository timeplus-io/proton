...
CREATE STREAM mt2(a int32, market int64, timestamp DateTime) ENGINE=MergeTree ORDER BY tuple();
...
$CLICKHOUSE_CLIENT --multiquery <<EOF
ALTER STREAM wv MODIFY QUERY SELECT count(a) AS count, mt2.market * 2 as market, tumbleEnd(wid) AS w_end FROM mt2 GROUP BY tumble(timestamp, INTERVAL '5' SECOND, 'US/Samoa') AS wid, mt2.market;
...
$CLICKHOUSE_CLIENT --query="DROP STREAM mt2"
