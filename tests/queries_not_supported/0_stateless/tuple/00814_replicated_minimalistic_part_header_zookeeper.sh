...
DROP STREAM IF EXISTS part_header_r1;
DROP STREAM IF EXISTS part_header_r2;

SET insert_keeper_fault_injection_probability=0; -- disable fault injection; part ids are non-deterministic in case of insert retries
SET replication_alter_partitions_sync = 2;

CREATE STREAM part_header_r1(x uint32, y uint32)
...
CREATE STREAM part_header_r2(x uint32, y uint32)
...
SELECT '*** Test merges ***';
OPTIMIZE STREAM part_header_r1;
SYSTEM SYNC REPLICA part_header_r2;
SELECT '*** replica 1 ***';
...
SELECT '*** replica 1 ***';
SELECT name FROM system.parts WHERE active AND database = currentDatabase() AND stream = 'part_header_r1';
...
SELECT '*** replica 2 ***';
SELECT name FROM system.parts WHERE active AND database = currentDatabase() AND stream = 'part_header_r2';
...
SELECT '*** Test ALTER ***';
ALTER STREAM part_header_r1 MODIFY COLUMN y string;
SELECT '*** replica 1 ***';
...
SET replication_alter_partitions_sync = 2;
ALTER STREAM part_header_r1 CLEAR COLUMN y IN PARTITION tuple();
SELECT '*** replica 1 ***';
SELECT x, length(y) FROM part_header_r1 ORDER BY x;
SELECT '*** replica 2 ***';
SELECT x, length(y) FROM part_header_r2 ORDER BY x;

DROP STREAM part_header_r1;
DROP STREAM part_header_r2;
