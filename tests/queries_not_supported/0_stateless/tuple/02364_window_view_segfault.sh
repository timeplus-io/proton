...
DROP STREAM IF EXISTS mt ON CLUSTER test_shard_localhost;
DROP STREAM IF EXISTS wv ON CLUSTER test_shard_localhost;
CREATE STREAM mt  ON CLUSTER test_shard_localhost (a int32, timestamp DateTime) ENGINE=MergeTree ORDER BY tuple();
