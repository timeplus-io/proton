...
with client(
    ...
    client1.send("DROP STREAM IF EXISTS test.infile_progress")
    client1.expect(prompt)
    client1.send(f"SELECT number FROM numbers(5) INTO OUTFILE '{filename}'")
    client1.expect(prompt)
    client1.send(
        "CREATE STREAM test.infile_progress (a int32) Engine=MergeTree order by tuple()"
    )
    client1.expect(prompt)
    ...
    if match.groups()[1]:
        ...
        client1.expect(prompt)
    client1.send("DROP STREAM test.infile_progress")
    client1.expect(prompt)
