...
with client(name="client1>", log=log) as client1, client(
    ...
    client1.expect(prompt)
    client1.send("DROP STREAM IF EXISTS 01082_window_view_watch_limit.mt")
    client1.expect(prompt)
    client1.send("DROP STREAM IF EXISTS 01082_window_view_watch_limit.wv NO DELAY")
    client1.expect(prompt)

    client1.send(
        "CREATE STREAM 01082_window_view_watch_limit.mt(a int32, timestamp DateTime) ENGINE=MergeTree ORDER BY tuple()"
    )
    client1.expect(prompt)
    client1.send(
    ...
    client1.send("DROP STREAM 01082_window_view_watch_limit.wv NO DELAY")
    client1.expect(prompt)
    client1.send("DROP STREAM 01082_window_view_watch_limit.mt")
    client1.expect(prompt)
