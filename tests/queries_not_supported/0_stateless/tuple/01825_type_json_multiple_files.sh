...
${CLICKHOUSE_CLIENT} -q "DROP STREAM IF EXISTS t_json_files"
${CLICKHOUSE_CLIENT} -q "CREATE STREAM t_json_files (file string, data JSON) ENGINE = MergeTree ORDER BY tuple()" --allow_experimental_object_type 1
...
${CLICKHOUSE_CLIENT} -q "SELECT to_type_name(data) FROM t_json_files LIMIT 1"

${CLICKHOUSE_CLIENT} -q "TRUNCATE STREAM IF EXISTS t_json_files"
