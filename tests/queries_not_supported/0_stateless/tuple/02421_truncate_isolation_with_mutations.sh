...
function reset_table()
{
    ...
    $CLICKHOUSE_CLIENT -q "create stream $table (n int) engine=MergeTree order by tuple() $settings"
...
function concurrent_delete_before()
{
    ...
    tx 12                                            "alter stream tt delete where n%2=1"
    tx 11 "select 41, count() from tt"
    tx 11 "truncate stream tt" | grep -Eo "SERIALIZATION_ERROR" | uniq
...
function concurrent_delete_after()
{
    ...
    tx 21 "truncate stream tt"
    tx 22                                            "select 112, count() from tt"
    tx 22                                            "alter stream tt delete where n%2=1" | grep -Eo "UNFINISHED" | uniq
    tx 21 "commit"
    tx 22                                            "rollback"
...
function concurrent_delete_rollback()
{
    ...
    tx 32                                            "alter stream tt delete where n%2=1"
    tx 31 "select count() from tt"
    tx 32                                            "select count() from tt"
    tx 31 "select count() from tt"
    tx 32                                            "rollback"
    tx 31 "truncate stream tt"
    tx 31 "commit"
...
function concurrent_optimize_table_not_start()
{
    ...
    tx 42             "optimize stream tt final"
    tx 42             "commit"
...
function concurrent_optimize_table()
{
    ...
    tx 41 "begin transaction"
    tx 41 "optimize stream tt final"
    ...
    tx 43                                            "alter stream tt drop partition id 'all'" | grep -Eo "SERIALIZATION_ERROR" | uniq
...
function concurrent_optimize_table_before()
{
    ...
    tx 51 "optimize stream tt final" # inserts all_1_1_1
    tx 51 "rollback" # inserts all_1_1_1 is outdated
    tx 52             "alter stream tt drop partition id 'all'" | grep -vwe "PART_IS_TEMPORARILY_LOCKED" ||: # conflict with all_1_1_1
    tx 52             "rollback"
...
function drop_parts_which_already_outdated()
{
    ...
    $CLICKHOUSE_CLIENT -q "optimize stream tt final /*all_1_1_1*/"
    $CLICKHOUSE_CLIENT -q "optimize stream tt final /*all_1_1_2*/"
    $CLICKHOUSE_CLIENT -q "optimize stream tt final /*all_1_1_3*/"
    $CLICKHOUSE_CLIENT -q "optimize stream tt final /*all_1_1_4*/"
    $CLICKHOUSE_CLIENT -q "optimize stream tt final /*all_1_1_5*/"
    $CLICKHOUSE_CLIENT -q "optimize stream tt final /*all_1_1_6*/"
    ...
    tx 61 "optimize stream tt final /*all_1_2_7*/"
    tx 61 "commit"

    tx 62 "begin transaction"
    tx 62 "optimize stream tt final /*all_1_2_8*/"

    tx 69             "select 'after optimize', count(), _part from tt group by _part order by _part"
    tx 69             "alter stream tt drop partition id 'all'" | grep -Eo "SERIALIZATION_ERROR" | uniq
    tx 69             "rollback"
...
function unable_drop_one_part_which_outdated_but_visible()
{
    ...
    tx 71 "optimize stream tt final /*all_1_2_3*/"

    tx 79             "select 'after optimize', count(), _part from tt group by _part order by _part"
    tx 79             "alter stream tt drop part 'all_2_2_0'" | grep -Eo "NO_SUCH_DATA_PART" | uniq
    tx 79             "rollback"
...
function drop_one_part_which_outdated_and_reverted()
{
    ...
    tx 81 "optimize stream tt final /*all_1_2_2*/"
    ...
    tx 89             "alter stream tt drop part 'all_2_2_0'"
    tx 89             "commit"
...
function drop_one_part_which_outdated_and_reverted_no_name_intersection()
{
    ...
    tx 91 "optimize stream tt final /*all_1_2_1*/"
    ...
    tx 99             "alter stream tt drop part 'all_2_2_0'"
    tx 99             "commit"
