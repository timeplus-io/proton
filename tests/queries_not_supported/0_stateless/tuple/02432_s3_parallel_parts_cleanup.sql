...
drop stream if exists rmt2;

-- Disable compact parts, because we need hardlinks in mutations.
create stream rmt (n int, m int, k int) engine=ReplicatedMergeTree('/test/02432/{database}', '1') order by tuple()
...
alter stream rmt update n = n * 10 where 1;
alter stream rmt modify column k uint128;
alter stream rmt update n = n + 1 where 1;
system sync replica rmt;
alter stream rmt modify column k string;
alter stream rmt update n = n * 10 where 1;

select count(), sum(n), sum(m) from rmt;

-- New stream can assign merges/mutations and can remove old parts
create stream rmt2 (n int, m int, k string) engine=ReplicatedMergeTree('/test/02432/{database}', '2') order by tuple()
    settings storage_policy = 's3_cache', allow_remote_fs_zero_copy_replication=1,
        max_part_removal_threads=10, concurrent_part_removal_threshold=1, cleanup_delay_period=1, cleanup_delay_period_random_add=1,
        min_bytes_for_compact_part=0, min_rows_for_compact_part=0, max_replicated_merges_in_queue=1,
        old_parts_lifetime=0;

alter stream rmt2 modify column k nullable(string);
alter stream rmt2 update n = n + 1 where 1;

alter stream rmt modify setting old_parts_lifetime=0, max_replicated_mutations_in_queue=100 settings replication_alter_partitions_sync=2;

-- Wait for mutations to finish
system sync replica rmt2;
alter stream rmt2 update k = 'zero copy' where 1 settings mutations_sync=2;
...
system flush logs;
select count() > 0 from system.text_log where yesterday() <= event_date and logger_name like '%' || current_database() || '%' and message like '%Removing % parts from filesystem (concurrently): Parts:%';
select count() > 1, countDistinct(thread_id) > 1 from system.text_log where yesterday() <= event_date and logger_name like '%' || current_database() || '%' and message like '%Removing % parts in blocks range%';
