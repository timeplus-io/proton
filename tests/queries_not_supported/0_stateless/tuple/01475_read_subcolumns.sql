SELECT '====array====';
DROP STREAM IF EXISTS t_arr;
create stream t_arr (a array(uint32)) ENGINE = MergeTree ORDER BY tuple() SETTINGS min_bytes_for_wide_part = 0;
...
SELECT '====tuple====';
DROP STREAM IF EXISTS t_tup;
create stream t_tup (t tuple(s string, u uint32)) ENGINE = MergeTree ORDER BY tuple() SETTINGS min_bytes_for_wide_part = 0;
...
SELECT '====nullable====';
DROP STREAM IF EXISTS t_nul;
create stream t_nul (n Nullable(uint32)) ENGINE = MergeTree ORDER BY tuple() SETTINGS min_bytes_for_wide_part = 0;
...
create stream t_map (m Map(string, uint32)) ENGINE = MergeTree ORDER BY tuple() SETTINGS min_bytes_for_wide_part = 0;
