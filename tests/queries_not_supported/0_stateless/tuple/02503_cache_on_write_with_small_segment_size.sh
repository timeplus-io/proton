...
drop stream if exists ttt;
create stream ttt (id int32, value string) engine=MergeTree() order by tuple()  settings storage_policy='s3_cache_small_segment_size', min_bytes_for_wide_part=0;
insert into ttt select number, to_string(number) from numbers(100000) settings throw_on_error_from_cache_on_write_operations = 1;
"
