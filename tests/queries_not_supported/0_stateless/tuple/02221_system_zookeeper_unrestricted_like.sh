...
${CLICKHOUSE_CLIENT} --query="DROP STREAM IF EXISTS sample_table;"
${CLICKHOUSE_CLIENT} --query="DROP STREAM IF EXISTS sample_table_2;"

${CLICKHOUSE_CLIENT} -n --query="CREATE STREAM sample_table (
    key uint64
)
...
${CLICKHOUSE_CLIENT} -n --query "CREATE STREAM sample_table_2 (
    key uint64
)
