...
SELECT first, second, result FROM (
    SELECT
        ...
        tuple(round_bankers(longitude, 5), round_bankers(latitude, 5)) AS first,
        tuple(round_bankers(output_geo.1, 5), round_bankers(output_geo.2, 5)) AS second,
...
SELECT geoToS2(to_float64(to_uint64(-1)), to_float64(to_uint64(-1)));
