DROP STREAM IF EXISTS stream_map;
CREATE STREAM stream_map (id uint32, col map(string, uint64)) engine = MergeTree() ORDER BY tuple();
INSERT INTO stream_map SELECT number, map('key1', number, 'key2', number * 2) FROM numbers(1111, 3);
INSERT INTO stream_map SELECT number, map('key3', number, 'key2', number + 1, 'key4', number + 2) FROM numbers(100, 4);

SELECT mapFilter((k, v) -> k like '%3' and v > 102, col) FROM stream_map ORDER BY id;
SELECT col, mapFilter((k, v) -> ((v % 10) > 1), col) FROM stream_map ORDER BY id ASC;
SELECT mapApply((k, v) -> (k, v + 1), col) FROM stream_map ORDER BY id;
SELECT mapFilter((k, v) -> 0, col) from stream_map;
SELECT mapApply((k, v) -> tuple(v + 9223372036854775806), col) FROM stream_map; -- { serverError NUMBER_OF_ARGUMENTS_DOESNT_MATCH }
...
SELECT mapFilter((x, y) -> (to_int32(x)), map(1, 0, 2, 0)); -- { serverError ILLEGAL_TYPE_OF_ARGUMENT }
