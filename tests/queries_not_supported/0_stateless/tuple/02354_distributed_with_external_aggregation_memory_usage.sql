...
create stream t_2354_dist_with_external_aggr(a uint64, b string, c fixed_string(100)) engine = MergeTree order by tuple();

insert into t_2354_dist_with_external_aggr select number, to_string(number) as s, to_fixed_string(s, 100) from numbers_mt(5e7);
...
from remote('127.0.0.{1,2}', current_database(), t_2354_dist_with_external_aggr)
group by a, b, c
format Null
