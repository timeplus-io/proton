...
function create_db()
{
    while true; do
        ...
        # So CREATE STREAM queries will fail on all replicas except one. But it's still makes sense for a stress test.
...
function create_table()
{
    while true; do
        ...
        "create stream $database.rmt_${RANDOM}_${RANDOM}_${RANDOM} (n int) engine=ReplicatedMergeTree order by tuple() -- suppress $CLICKHOUSE_TEST_ZOOKEEPER_PREFIX" \
...
{
    while true; do
        stream=$($CLICKHOUSE_CLIENT -q "select database || '.' || name from system.tables where database like '${CLICKHOUSE_DATABASE}%' order by rand() limit 1")
        if [ -z "$table" ]; then continue; fi
        $CLICKHOUSE_CLIENT --distributed_ddl_task_timeout=0 -q \
        "alter stream $table update n = n + (select max(n) from merge(REGEXP('${CLICKHOUSE_DATABASE}.*'), '.*')) where 1 settings allow_nondeterministic_mutations=1" \
        2>&1| grep -Fa "Exception: " | grep -Fv "Cannot enqueue query" | grep -Fv "ZooKeeper session expired" | grep -Fv UNKNOWN_DATABASE | grep -Fv UNKNOWN_TABLE | grep -Fv STREAM_IS_READ_ONLY | grep -Fv STREAM_IS_DROPPED
        sleep 0.$RANDOM
    done
}
...
function insert()
{
    while true; do
        ...
        if [ -z "$table" ]; then continue; fi
        $CLICKHOUSE_CLIENT -q \
        "insert into $table values ($RANDOM)" 2>&1| grep -Fa "Exception: " | grep -Fv UNKNOWN_DATABASE | grep -Fv UNKNOWN_TABLE | grep -Fv STREAM_IS_READ_ONLY | grep -Fv STREAM_IS_DROPPED
    done
}
