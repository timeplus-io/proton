...
DROP STREAM IF EXISTS keeper_retries_r1 SYNC;
DROP STREAM IF EXISTS keeper_retries_r2 SYNC;

CREATE STREAM keeper_retries_r1(a uint8) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/test/02456_keeper_retries_during_insert', 'r1') ORDER BY tuple ();
CREATE STREAM keeper_retries_r2(a uint8) ENGINE = ReplicatedMergeTree('/clickhouse/tables/{database}/test/02456_keeper_retries_during_insert', 'r2') ORDER BY tuple();
...
DROP STREAM keeper_retries_r1 SYNC;
DROP STREAM keeper_retries_r2 SYNC;
