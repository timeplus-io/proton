...
DROP STREAM IF EXISTS t_sparse_1;

create stream t_sparse (id uint64, u uint64, s string, arr1 array(string), arr2 array(uint64))
...
INSERT INTO t_sparse SELECT
    ...
    if (number % 5 = 0, to_string(number), ''),
    if (number % 7 = 0, array_map(x -> to_string(x), range(number % 10)), []),
...
SELECT array_filter(x -> x % 2 = 1, arr2) FROM t_sparse WHERE arr2 != [] LIMIT 5;

create stream t_sparse_1 (id uint64, v int64)
ENGINE = MergeTree ORDER BY tuple()
