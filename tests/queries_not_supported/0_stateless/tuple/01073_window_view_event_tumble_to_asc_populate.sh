...
CREATE STREAM dst(count uint64, market int32, w_end DateTime) Engine=MergeTree ORDER BY tuple();
CREATE STREAM mt(a int32, market int32, timestamp DateTime) ENGINE=MergeTree ORDER BY timestamp;
