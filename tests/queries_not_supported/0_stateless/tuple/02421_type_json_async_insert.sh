...
$CLICKHOUSE_CLIENT -q "DROP STREAM IF EXISTS t_json_async_insert"
$CLICKHOUSE_CLIENT --allow_experimental_object_type=1 -q "CREATE STREAM t_json_async_insert (data JSON) ENGINE = MergeTree ORDER BY tuple()"
...
$CLICKHOUSE_CLIENT -q "SELECT count() FROM system.parts WHERE database = '$CLICKHOUSE_DATABASE' AND stream = 't_json_async_insert'"
