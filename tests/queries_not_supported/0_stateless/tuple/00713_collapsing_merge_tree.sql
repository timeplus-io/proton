DROP STREAM IF EXISTS collapsing;

create stream collapsing(key string, value string, sign int8) ENGINE CollapsingMergeTree(sign)
    ORDER BY key
...
OPTIMIZE STREAM collapsing PARTITION tuple() FINAL;

SELECT * FROM collapsing ORDER BY key;

DROP STREAM collapsing;
