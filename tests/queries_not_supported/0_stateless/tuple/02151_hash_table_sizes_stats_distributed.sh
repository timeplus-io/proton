...
# These tests don't use `current_database = current_database()` condition, because database name isn't propagated during remote queries.
...
prepare_table() {
  ...
  $CLICKHOUSE_CLIENT -q "DROP STREAM IF EXISTS ${table_name}_d;"
  $CLICKHOUSE_CLIENT -q "CREATE STREAM ${table_name}_d AS $table_name ENGINE = Distributed(test_cluster_two_shards, current_database(), $table_name);"
  stream_name="${table_name}_d"
}
