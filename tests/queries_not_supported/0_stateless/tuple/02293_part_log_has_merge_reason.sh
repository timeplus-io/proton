...
${CLICKHOUSE_CLIENT} -q 'DROP STREAM IF EXISTS t_part_log_has_merge_type_table'

${CLICKHOUSE_CLIENT} -q '
    CREATE STREAM t_part_log_has_merge_type_table
    (
        event_time DateTime,
        UserID uint64,
        Comment string
...
${CLICKHOUSE_CLIENT} -q '
  ...
  WHERE
      ...
      AND
          stream = '"'t_part_log_has_merge_type_table'"'
      AND
          database = '"'${CLICKHOUSE_DATABASE}'"'
