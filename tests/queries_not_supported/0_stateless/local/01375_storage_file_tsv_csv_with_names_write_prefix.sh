...
for format in TSVWithNames TSVWithNamesAndTypes CSVWithNames CSVWithNamesAndTypes JSONCompactEachRowWithNames JSONCompactEachRowWithNamesAndTypes JSONCompactStringsEachRow JSONCompactStringsEachRowWithNamesAndTypes; do
    ...
    ${CLICKHOUSE_LOCAL} --query="
        create stream ${format}_01375 ENGINE File($format, '01375_$format') AS SELECT * FROM numbers(1) WHERE number < 0;
        SELECT * FROM ${format}_01375;
        DROP STREAM ${format}_01375;
    "
    rm 01375_$format
done
...
for format in TSVWithNames TSVWithNamesAndTypes CSVWithNames CSVWithNamesAndTypes JSONCompactEachRowWithNames JSONCompactEachRowWithNamesAndTypes JSONCompactStringsEachRow JSONCompactStringsEachRowWithNamesAndTypes; do
    ...
    for _ in {1..2}; do
        ${CLICKHOUSE_LOCAL} --query="
            create stream ${format}_01375 ENGINE File($format, '01375_$format') AS SELECT * FROM numbers(1);
            SELECT * FROM ${format}_01375;
            DROP STREAM ${format}_01375;
        "
    done
