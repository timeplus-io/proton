...
$CLICKHOUSE_CLIENT --query "DROP STREAM IF EXISTS tmp";
$CLICKHOUSE_CLIENT --query "CREATE STREAM src (n int8, m int8, CONSTRAINT c CHECK xxHash32(n+m) % 8 != 0) ENGINE=MergeTree ORDER BY n PARTITION BY 0 < n SETTINGS old_parts_lifetime=0";
$CLICKHOUSE_CLIENT --query "CREATE STREAM dst (nm int16, CONSTRAINT c CHECK xxHash32(nm) % 8 != 0) ENGINE=MergeTree ORDER BY nm SETTINGS old_parts_lifetime=0";
$CLICKHOUSE_CLIENT --query "CREATE MATERIALIZED VIEW mv INTO dst (nm int16) AS SELECT n*m AS nm FROM src";

$CLICKHOUSE_CLIENT --query "CREATE STREAM tmp (x uint8, nm int16) ENGINE=MergeTree ORDER BY (x, nm) SETTINGS old_parts_lifetime=0"
...
function thread_optimize()
{
    set -e
    while true; do
        optimize_query="OPTIMIZE STREAM src"
        partition_id=$(( RANDOM % 2 ))
        if (( RANDOM % 2 )); then
            optimize_query="OPTIMIZE STREAM dst"
            partition_id="all"
        fi
        if (( RANDOM % 2 )); then
...
function thread_select()
{
    ...
    while true; do
        ...
        SELECT throwIf((SELECT arraySort(group_array(nm)) FROM mv) != (SELECT arraySort(group_array(nm)) FROM dst)) FORMAT Null;
        SELECT throwIf((SELECT arraySort(group_array(nm)) FROM mv) != (SELECT arraySort(group_array(n*m)) FROM src)) FORMAT Null;
        COMMIT;"
    done
}
...
function thread_select_insert()
{
    ...
    while true; do
        ...
        SELECT throwIf(1 != (SELECT countDistinct(arr) FROM (SELECT x, arraySort(group_array(nm)) AS arr FROM tmp WHERE x!=4 GROUP BY x))) FORMAT Null;
...
$CLICKHOUSE_CLIENT --query "DROP STREAM tmp";
