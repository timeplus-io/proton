...
${CLICKHOUSE_CLIENT} --query "DROP STREAM IF EXISTS test_infile;"
${CLICKHOUSE_CLIENT} --query "create stream test_infile (word string) ENGINE=Memory();"
...
${CLICKHOUSE_LOCAL} --query "create stream test_infile (word string) ENGINE=Memory(); INSERT INTO test_infile FROM INFILE '${CLICKHOUSE_TMP}/test_infile.gz' FORMAT CSV; SELECT * from test_infile;"

${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&query=DROP+TABLE" -d 'IF EXISTS test_infile_url'
${CLICKHOUSE_CURL} -sS "${CLICKHOUSE_URL}&query=CREATE" -d 'TABLE test_infile_url (x string) '
