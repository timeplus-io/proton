...
create stream ipv4_test (ipv4_ IPv4) ;

-- ipv4_ column shoud have type 'IPv4'
SHOW create stream ipv4_test;
...
create stream ipv6_test (ipv6_ IPv6) ;

-- ipv6_ column shoud have type 'IPv6'
SHOW create stream ipv6_test;
...
SELECT toIPv6('127.0.0.' || to_string(number)) FROM numbers(13);
