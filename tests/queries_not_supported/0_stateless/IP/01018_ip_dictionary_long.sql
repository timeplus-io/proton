...
create stream database_for_dict.table_ipv4_trie
(
    prefix string,
    asn uint32,
    cca2 string
...
SELECT
  '255.255.255.255/' || to_string((number + 1) * 13 % 33) AS prefix,
  to_uint32((number + 1) * 13 % 33) AS asn,
  'NA' as cca2
...
create stream database_for_dict.table_from_ipv4_trie_dict
(
  prefix string,
  asn uint32,
  cca2 string
...
DROP STREAM IF EXISTS database_for_dict.table_from_ipv4_trie_dict;
DROP DICTIONARY IF EXISTS database_for_dict.dict_ipv4_trie;
DROP STREAM IF EXISTS database_for_dict.table_ipv4_trie;
...
INSERT INTO database_for_dict.table_ipv4_trie
SELECT
  '255.255.255.255/' || to_string(number) AS prefix,
  to_uint32(number) AS val
...
create stream database_for_dict.table_ipv4_trie ( prefix string, val uint32 ) engine = TinyLog;
...
create stream database_for_dict.table_ip_trie
(
    prefix string,
    val string
)
...
SELECT
  'ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff/' || to_string((number + 1) * 13 % 129) AS prefix,
  to_string((number + 1) * 13 % 129) AS val
...
create stream database_for_dict.table_from_ip_trie_dict
(
  prefix string,
  val string
...
DROP STREAM IF EXISTS database_for_dict.table_from_ip_trie_dict;
DROP DICTIONARY IF EXISTS database_for_dict.dict_ip_trie;
DROP STREAM IF EXISTS database_for_dict.table_ip_trie;
...
INSERT INTO database_for_dict.table_ip_trie
SELECT
  'ffff:ffff:ffff:ffff:ffff:ffff:ffff:ffff/' || to_string(number) AS prefix,
  to_string(number) AS val
