...
ENGINE = Null AS
WITH (
        SELECT array_reduce('sumMapState', [(['foo'], array_map(x -> -0., ['foo']))])
    ) AS all_metrics
SELECT
    (finalize_aggregation(array_reduce('sumMapMergeState', [all_metrics])) AS metrics_tuple).1 AS metric_names,
