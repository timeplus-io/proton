...
drop stream if exists test  sync;
drop stream if exists test_mv_a sync;
drop stream if exists test_mv_b sync;
drop stream if exists test_mv_c sync;

set deduplicate_blocks_in_dependent_materialized_views=0;

CREATE STREAM test (test string, A int64, B int64) ENGINE = ReplicatedMergeTree ('/clickhouse/tables/{database}/test_02124/{table}', '1')
ORDER BY tuple();
...
drop stream test sync;
drop stream test_mv_a sync;
drop stream test_mv_b sync;
drop stream test_mv_c sync;
