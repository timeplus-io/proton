...
do
    ...
    $CLICKHOUSE_CLIENT -q "create stream t (n int) engine=$engine"
    ...
    $CLICKHOUSE_CLIENT --allow_nondeterministic_mutations=1 --mutations_sync=1 -q "alter stream t
    ...
    $CLICKHOUSE_CLIENT -q "create stream test ENGINE=$engine AS SELECT number + 100 AS n, 0 AS test FROM numbers(50)"
    $CLICKHOUSE_CLIENT -q "select count(), sum(n), sum(test) from test"
    if [[ $engine == *"ReplicatedMergeTree"* ]]; then
        $CLICKHOUSE_CLIENT -q "ALTER STREAM test
            UPDATE test = (SELECT group_array(id) FROM t1 GROUP BY 1)[n - 99] WHERE 1" 2>&1| grep -Fa "DB::Exception: " | grep -Fv "statement with subquery may be nondeterministic"
        $CLICKHOUSE_CLIENT --allow_nondeterministic_mutations=1 --mutations_sync=1 -q "ALTER STREAM test
                    UPDATE test = (SELECT group_array(id) FROM t1 GROUP BY 1)[n - 99] WHERE 1"
    elif [[ $engine == *"Join"* ]]; then
        $CLICKHOUSE_CLIENT -q "ALTER STREAM test
            UPDATE test = (SELECT group_array(id) FROM t1 GROUP BY 1)[n - 99] WHERE 1" 2>&1| grep -Fa "DB::Exception: " | grep -Fv "Table engine Join supports only DELETE mutations"
    else
        $CLICKHOUSE_CLIENT --mutations_sync=1 -q "ALTER STREAM test
            UPDATE test = (SELECT group_array(id) FROM t1 GROUP BY 1)[n - 99] WHERE 1"
    fi
    $CLICKHOUSE_CLIENT -q "select count(), sum(n), sum(test) from test"
