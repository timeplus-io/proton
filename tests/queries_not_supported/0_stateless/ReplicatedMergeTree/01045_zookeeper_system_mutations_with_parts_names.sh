...
${CLICKHOUSE_CLIENT} --query="DROP STREAM IF EXISTS table_for_mutations"

${CLICKHOUSE_CLIENT} --query="create stream table_for_mutations(k uint32, v1 uint64) ENGINE MergeTree ORDER BY k PARTITION BY modulo(k, 2)"
...
${CLICKHOUSE_CLIENT} --query="ALTER STREAM table_for_mutations UPDATE v1 = v1 + 1 WHERE 1"
...
${CLICKHOUSE_CLIENT} --query="DROP STREAM IF EXISTS replicated_table_for_mutations"

${CLICKHOUSE_CLIENT} --query="create stream replicated_table_for_mutations(k uint32, v1 uint64) ENGINE ReplicatedMergeTree('/clickhouse/tables/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/replicated_table_for_mutations', '1') ORDER BY k PARTITION BY modulo(k, 2)"
...
${CLICKHOUSE_CLIENT} --query="ALTER STREAM replicated_table_for_mutations UPDATE v1 = v1 + 1 WHERE 1"
