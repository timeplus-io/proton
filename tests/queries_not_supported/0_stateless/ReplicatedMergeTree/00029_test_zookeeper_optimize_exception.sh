...
${CLICKHOUSE_CLIENT} --query="create stream test_optimize_exception (date date) ENGINE=MergeTree() PARTITION BY to_YYYYMM(date) ORDER BY date"
${CLICKHOUSE_CLIENT} --query="create stream test_optimize_exception_replicated (date date) ENGINE=ReplicatedMergeTree('/clickhouse/tables/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/optimize', 'r1') PARTITION BY to_YYYYMM(date) ORDER BY date"
