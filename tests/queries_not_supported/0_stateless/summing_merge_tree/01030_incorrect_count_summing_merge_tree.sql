...
drop stream if exists tst;
create stream tst (timestamp DateTime, val Nullable(int8)) engine SummingMergeTree partition by toYYYYMM(timestamp) ORDER by (timestamp);
...
drop stream if exists tst;
create stream tst (timestamp DateTime, val int8) engine SummingMergeTree partition by toYYYYMM(timestamp) ORDER by (timestamp);
insert into tst values ('2018-02-01 00:00:00', 1), ('2018-02-02 00:00:00', 2);
