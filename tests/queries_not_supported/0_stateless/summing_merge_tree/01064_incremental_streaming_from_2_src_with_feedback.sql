...
DROP STREAM IF EXISTS target_table;
DROP STREAM IF EXISTS logins;
DROP STREAM IF EXISTS mv_logins2target;
DROP STREAM IF EXISTS checkouts;
DROP STREAM IF EXISTS mv_checkouts2target;

-- that is the final table, which is filled incrementally from 2 different sources

create stream target_table Engine=SummingMergeTree() ORDER BY id
SETTINGS index_granularity=128
AS
   SELECT
     number as id,
     maxState( to_datetime(0, 'UTC') ) as latest_login_time,
     maxState( to_datetime(0, 'UTC') ) as latest_checkout_time,
     minState( to_uint64(-1) ) as fastest_session,
     maxState( to_uint64(0) ) as biggest_inactivity_period
FROM numbers(50000)
GROUP BY id;
...
AS
   SELECT
     ...
     maxState( to_datetime(0, 'UTC') ) as latest_checkout_time,
     minState( to_uint64(-1) ) as fastest_session,
     if(max(current_latest_checkout_time) > 0, maxState(to_uint64(ts - current_latest_checkout_time)), maxState( to_uint64(0) ) ) as biggest_inactivity_period
   FROM logins
   LEFT JOIN (
       SELECT
          id,
...
AS
   SELECT
     ...
     if(max(current_latest_login_time) > 0, minState( to_uint64(ts - current_latest_login_time)), minState( to_uint64(-1) ) ) as fastest_session,
     maxState( to_uint64(0) ) as biggest_inactivity_period
   FROM checkouts
