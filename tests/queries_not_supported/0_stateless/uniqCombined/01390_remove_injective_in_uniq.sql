...
EXPLAIN SYNTAX select uniq(bit_not(x)), uniqExact(bit_not(x)), uniqHLL12(bit_not(x)), uniqCombined(bit_not(x)), uniqCombined64(bit_not(x))
from (select number % 2 as x from numbers(10));
...
EXPLAIN SYNTAX select count(distinct -bit_not(-x)) from (select number % 2 as x from numbers(10));
