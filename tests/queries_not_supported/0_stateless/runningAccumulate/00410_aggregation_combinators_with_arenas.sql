...
SELECT length(array_reduce('groupUniqArray', [[1, 2], [1],  empty_array_uint8(), [1], [1, 2]]));
SELECT min(x), max(x) FROM (SELECT length(array_reduce('groupUniqArray', [hex(number), hex(number+1), hex(number)])) AS x FROM system.numbers LIMIT 100000);
