...
${CLICKHOUSE_CLIENT} -q "SELECT 'create stream test_' || hex(randomPrintableASCII(40)) || '(x uint8) Engine=Memory;' FROM numbers(10000)" --format=TSVRaw | ${CLICKHOUSE_BENCHMARK} -c32 -i 10000 -d 0 2>&1 | grep -F 'Loaded 10000 queries'
