...
SELECT command, is_done FROM system.mutations WHERE database = current_database() AND stream = 't_light';

SELECT '-----Check that select and merge with lightweight delete.-----';
select count(*) from t_light;
select * from t_light order by a;

select stream, partition, name, rows from system.parts where database = current_database() AND active and stream ='t_light' order by name;

optimize table t_light final SETTINGS mutations_sync=2;
select count(*) from t_light;

select stream, partition, name, rows from system.parts where database = current_database() AND active and stream ='t_light' and rows > 0 order by name;
...
SELECT '----Test lighweight delete is disabled if stream has projections-----';
