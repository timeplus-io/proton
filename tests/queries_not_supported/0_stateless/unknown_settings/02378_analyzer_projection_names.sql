...
DROP STREAM IF EXISTS test_table_in;
CREATE STREAM test_table_in
(
    id uint64
) ENGINE=TinyLog;
...
DESCRIBE (SELECT tuple_value.* APPLY to_string FROM test_table_compound);

SELECT '--';

DESCRIBE (SELECT tuple_value.* APPLY x -> to_string(x) FROM test_table_compound);
...
DESCRIBE (SELECT cast(tuple(1, 'Value'), 'Tuple (id uint64, value string)') AS a, a.* EXCEPT value APPLY to_string);

SELECT '--';

DESCRIBE (SELECT cast(tuple(1, 'Value'), 'Tuple (id uint64, value string)') AS a, a.* EXCEPT value APPLY x -> to_string(x));
...
DESCRIBE (SELECT array_map(x -> x + 1, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT 1 AS a, array_map(x -> x + a, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT array_map(x -> x + test_table.id + test_table.id + id, [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT array_map(x -> x + (test_table.id AS first) + (test_table.id AS second) + id, [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT array_map(x -> test_table.* EXCEPT value, [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT array_map(x -> test_table.* EXCEPT value APPLY x -> x, [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT array_map(x -> test_table.* EXCEPT value APPLY to_string, [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT array_map(x -> test_table.* EXCEPT value APPLY x -> to_string(x), [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS compound_value, array_map(x -> compound_value.*, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS compound_value, array_map(x -> compound_value.* APPLY x -> x, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS compound_value, array_map(x -> compound_value.* APPLY to_string, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS compound_value, array_map(x -> compound_value.* APPLY x -> to_string(x), [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1, 'Value'), 'Tuple (id uint64, value string)') AS compound_value, array_map(x -> compound_value.* EXCEPT value, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1, 'Value'), 'Tuple (id uint64, value string)') AS compound_value, array_map(x -> compound_value.* EXCEPT value APPLY x -> x, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1, 'Value'), 'Tuple (id uint64, value string)') AS compound_value, array_map(x -> compound_value.* EXCEPT value APPLY to_string, [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1, 'Value'), 'Tuple (id uint64, value string)') AS compound_value, array_map(x -> compound_value.* EXCEPT value APPLY x -> to_string(x), [1,2,3]));

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS a, array_map(x -> untuple(a), [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS a, array_map(x -> untuple(a) AS untupled_value, [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS a, untuple(a) AS untupled_value, array_map(x -> untupled_value, [1,2,3]) FROM test_table);

SELECT '--';

DESCRIBE (SELECT cast(tuple(1), 'Tuple (id uint64)') AS a, untuple(a) AS untupled_value, array_map(x -> untupled_value AS untupled_value_in_lambda, [1,2,3]) FROM test_table);
...
DESCRIBE (SELECT array_map(x -> (SELECT 1), [1,2,3]), array_map(x -> (SELECT 2) AS a, [1, 2, 3]),  array_map(x -> (SELECT 1), [1,2,3]));
...
DESCRIBE (SELECT array_map(x -> (SELECT 1 UNION DISTINCT SELECT 1), [1,2,3]), array_map(x -> (SELECT 2 UNION DISTINCT SELECT 2) AS a, [1, 2, 3]),
array_map(x -> (SELECT 3 UNION DISTINCT SELECT 3), [1,2,3]));
...
DESCRIBE (SELECT array_map(x -> (SELECT 1 UNION DISTINCT SELECT 1), [1,2,3]), array_map(x -> (SELECT 2) AS a, [1, 2, 3]),
array_map(x -> (SELECT 3 UNION DISTINCT SELECT 3), [1,2,3]));
...
DESCRIBE (SELECT count() OVER (ORDER BY to_nullable(id) NULLS FIRST) FROM test_table);

SELECT '--';

DESCRIBE (SELECT count() OVER (ORDER BY to_nullable(id) NULLS LAST) FROM test_table);
...
DESCRIBE (SELECT * APPLY to_string FROM test_table_join_1 AS t1, test_table_join_2 AS t2);

SELECT '--';

DESCRIBE (SELECT * APPLY x -> to_string(x) FROM test_table_join_1 AS t1, test_table_join_2 AS t2);
...
DESCRIBE (SELECT test_table_join_1.* APPLY to_string, test_table_join_2.* APPLY to_string FROM test_table_join_1 AS t1 INNER JOIN test_table_join_2 AS t2 ON t1.id = t2.id);

SELECT '--';

DESCRIBE (SELECT test_table_join_1.* APPLY x -> to_string(x), test_table_join_2.* APPLY x -> to_string(x) FROM test_table_join_1 AS t1 INNER JOIN test_table_join_2 AS t2 ON t1.id = t2.id);
...
DESCRIBE (SELECT * APPLY to_string FROM test_table_join_1 AS t1, test_table_join_2 AS t2, test_table_join_3 AS t3);

SELECT '--';

DESCRIBE (SELECT * APPLY x -> to_string(x) FROM test_table_join_1 AS t1, test_table_join_2 AS t2, test_table_join_3 AS t3);
...
DESCRIBE (SELECT test_table_join_1.* APPLY to_string, test_table_join_2.* APPLY to_string, test_table_join_3.* APPLY to_string
FROM test_table_join_1 AS t1 INNER JOIN test_table_join_2 AS t2 ON t1.id = t2.id INNER JOIN test_table_join_3 AS t3 ON t2.id = t3.id);

SELECT '--';

DESCRIBE (SELECT test_table_join_1.* APPLY x -> to_string(x), test_table_join_2.* APPLY x -> to_string(x), test_table_join_3.* APPLY x -> to_string(x)
FROM test_table_join_1 AS t1 INNER JOIN test_table_join_2 AS t2 ON t1.id = t2.id INNER JOIN test_table_join_3 AS t3 ON t2.id = t3.id);
