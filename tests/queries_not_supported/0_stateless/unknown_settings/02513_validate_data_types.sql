...
select CAST(1000000, 'low_cardinality(uint64)'); -- {serverError SUSPICIOUS_TYPE_FOR_LOW_CARDINALITY}
desc file(nonexist.json, JSONEachRow, 'lc low_cardinality(uint64)'); -- {serverError SUSPICIOUS_TYPE_FOR_LOW_CARDINALITY}

set allow_suspicious_fixed_string_types=0;
select CAST('', 'fixed_string(1000)'); -- {serverError ILLEGAL_COLUMN}
desc file(nonexist.json, JSONEachRow, 'fs fixed_string(1000)'); -- {serverError ILLEGAL_COLUMN}
