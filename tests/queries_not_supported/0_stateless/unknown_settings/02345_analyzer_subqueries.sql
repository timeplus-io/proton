...
WITH subquery AS (SELECT 1 AS a) SELECT subquery.* APPLY to_string FROM subquery;
...
WITH subquery AS (SELECT 1 AS a) SELECT subquery_alias.* APPLY to_string FROM subquery AS subquery_alias;
