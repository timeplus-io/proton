...
CREATE STREAM tj (key2 uint64, key1 int64, a uint64, b uint64, x uint64, y uint64) ENGINE = Join(ALL, RIGHT, key1, key2);
...
CREATE STREAM t1 (id2 uint64, id1 int64, val uint64) ENGINE = Memory;
...
CREATE STREAM t (key2 uint64, key1 int64, b uint64, x uint64, val uint64) ENGINE = Memory;
...
SELECT * FROM t ALL RIGHT JOIN tj ON t.key1 + 1 == tj.key1 AND to_uint64(t.key2 - 1) == tj.key2 ORDER BY t.key1, tj.key2 FORMAT TSVWithNames; -- Ok: expression on the left stream
...
SELECT * FROM t ALL RIGHT JOIN tj ON t.key1 == tj.key1 AND t.key2 == tj.key2 AND t.b == 22 ORDER BY t.key1, tj.key2 FORMAT TSVWithNames; -- Ok: t.b from the left stream
