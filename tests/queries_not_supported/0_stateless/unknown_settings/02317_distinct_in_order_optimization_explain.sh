...
$CLICKHOUSE_CLIENT -q "drop stream if exists distinct_in_order_explain sync"
$CLICKHOUSE_CLIENT -q "create stream distinct_in_order_explain (a int, b int, c int) engine=MergeTree() order by (a, b)"
