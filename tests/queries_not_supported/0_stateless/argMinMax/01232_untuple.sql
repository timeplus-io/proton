...
select untuple((* except (b),)) from (select 1 as a, 2 as b, 3 as c);
select 'hello', untuple((* except (b),)), 'world' from (select 1 as a, 2 as b, 3 as c);
...
select key, untuple(arg_max((* except (key),), v1)) from kv group by key order by key format TSVWithNames;
drop stream if exists kv;