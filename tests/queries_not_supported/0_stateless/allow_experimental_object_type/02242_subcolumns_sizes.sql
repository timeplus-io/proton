...
DROP STREAM IF EXISTS t_subcolumns_sizes;

SET allow_experimental_object_type = 1;

CREATE STREAM t_subcolumns_sizes (id uint64, arr array(uint64), n nullable(string), d JSON)
...
OPTIMIZE STREAM t_subcolumns_sizes FINAL;
...
WHERE database = currentDatabase() AND stream = 't_subcolumns_sizes' AND active
...
FROM system.parts_columns ARRAY JOIN subcolumns
WHERE database = currentDatabase() AND stream = 't_subcolumns_sizes'
