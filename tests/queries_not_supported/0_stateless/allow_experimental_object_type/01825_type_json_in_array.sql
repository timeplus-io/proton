...
DROP STREAM IF EXISTS t_json_array;

CREATE STREAM t_json_array (id uint32, arr array(JSON)) ENGINE = MergeTree ORDER BY id;
...
SELECT to_type_name(arr) FROM t_json_array LIMIT 1;

TRUNCATE STREAM t_json_array;
...
SELECT array_join(array_join(arr.k1)) AS k1 FROM t_json_array ORDER BY k1 FORMAT JSONEachRow;
SELECT to_type_name(array_join(array_join(arr.k1))) AS arr FROM t_json_array LIMIT 1;

DROP STREAM t_json_array;
