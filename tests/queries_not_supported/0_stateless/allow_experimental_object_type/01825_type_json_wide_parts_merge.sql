...
DROP STREAM IF EXISTS t_json_wide_parts;

CREATE STREAM t_json_wide_parts (data JSON)
...
OPTIMIZE STREAM t_json_wide_parts FINAL;

SELECT data FROM t_json_wide_parts ORDER BY data.k1 FORMAT JSONEachRow;

SELECT type FROM system.parts_columns
WHERE stream = 't_json_wide_parts' AND database = currentDatabase() AND active;

DROP STREAM t_json_wide_parts;
