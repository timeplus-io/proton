...
$CLICKHOUSE_CLIENT -q "DROP STREAM IF EXISTS t_insert_storage_snapshot"
$CLICKHOUSE_CLIENT -q "CREATE STREAM t_insert_storage_snapshot (a uint64) ENGINE = MergeTree ORDER BY a"
...
while [[ $counter -lt $retries ]]; do
    query_result=$($CLICKHOUSE_CLIENT -q "SELECT name, active, refcount FROM system.parts WHERE database = '$CLICKHOUSE_DATABASE' AND stream = 't_insert_storage_snapshot' FORMAT CSV")
