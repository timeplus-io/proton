...
drop stream if exists mt1;
drop stream if exists mt2;

create stream mt1 (n int64) engine=MergeTree order by n;
create stream mt2 (n int64) engine=MergeTree order by n;
...
select 'commit', arraySort(group_array(n)) from (select n from mt1 union all select * from mt2);
commit;

begin transaction;
insert into mt1 values (2);
insert into mt2 values (20);
select 'rollback', arraySort(group_array(n)) from (select n from mt1 union all select * from mt2);
rollback;

begin transaction;
select 'no nested', arraySort(group_array(n)) from (select n from mt1 union all select * from mt2);
...
select 'on exception before start', arraySort(group_array(n)) from (select n from mt1 union all select * from mt2);
...
select 'on exception while processing', arraySort(group_array(n)) from (select n from mt1 union all select * from mt2);
...
select 'on session close', arraySort(group_array(n)) from (select n from mt1 union all select * from mt2);
...
begin transaction;
select 'readonly', arraySort(group_array(n)) from (select n from mt1 union all select * from mt2);
commit;
...
select 'snapshot3', count() = (select count() from system.parts where database=currentDatabase() and stream='mt1' and _state in ('Active', 'Outdated')) from mt1;
...
begin transaction;
create stream m (n int) engine=Memory; -- { serverError 48 }
