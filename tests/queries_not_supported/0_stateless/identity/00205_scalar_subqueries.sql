...
SELECT to_uint64((SELECT 9)) IN (SELECT number FROM system.numbers LIMIT 10);
SELECT (SELECT to_date('2015-01-02')) = to_date('2015-01-02'), 'Hello' = (SELECT 'Hello');
SELECT (SELECT to_date('2015-01-02'), 'Hello');
SELECT (SELECT to_date('2015-01-02'), 'Hello') AS x, x, identity((SELECT 1)), identity((SELECT 1) AS y);
