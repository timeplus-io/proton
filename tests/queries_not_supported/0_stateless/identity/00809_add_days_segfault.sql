...
SELECT sequenceCount((CAST((( SELECT NULL ) AS rg, ( SELECT ( SELECT [], '<e', caseWithExpr([NULL], -588755.149, []), retention(addWeeks((CAST((-7644612.39732) AS DateTime)), -23578040.02833), (CAST(([]) AS DateTime)), (CAST(([010977.08]) AS string))), empty_array_to_single('') ) , '\0', to_uint64([], 't3hw@'), '\0', to_start_of_quarter(-4230.1872, []) ) ) AS date))); -- { serverError 43 }
