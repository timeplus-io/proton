...
DROP STREAM IF EXISTS test_00597;
DROP STREAM IF EXISTS test_view_00597;

create stream test_00597(date date, id int8, name string, value int64) ENGINE = MergeTree(date, (id, date), 8192);
...
SELECT array_join([1,2,3]) AS id WHERE id = 1;
...
EXPLAIN SYNTAX SELECT to_string(value) AS value FROM (SELECT 1 AS value) WHERE value = '1';
SELECT to_string(value) AS value FROM (SELECT 1 AS value) WHERE value = '1';
...
EXPLAIN SYNTAX SELECT * FROM (SELECT array_join([1, 2, 3]) AS id) WHERE id = 1;
SELECT * FROM (SELECT array_join([1, 2, 3]) AS id) WHERE id = 1;

EXPLAIN SYNTAX SELECT id FROM (SELECT array_join([1, 2, 3]) AS id) WHERE id = 1;
SELECT id FROM (SELECT array_join([1, 2, 3]) AS id) WHERE id = 1;
...
EXPLAIN SYNTAX SELECT * FROM (SELECT to_uint64(b) AS a, sum(id) AS b FROM test_00597) WHERE a = 3;
SELECT * FROM (SELECT to_uint64(b) AS a, sum(id) AS b FROM test_00597) WHERE a = 3;
...
EXPLAIN SYNTAX SELECT * FROM (SELECT to_uint64(b) AS a, sum(id) AS b FROM test_00597 AS table_alias) AS outer_table_alias WHERE outer_table_alias.b = 3;
SELECT * FROM (SELECT to_uint64(b) AS a, sum(id) AS b FROM test_00597 AS table_alias) AS outer_table_alias WHERE outer_table_alias.b = 3;
...
EXPLAIN SYNTAX SELECT * FROM (SELECT to_int8(1) AS id) ANY LEFT JOIN test_00597 USING id WHERE value = 1;
SELECT * FROM (SELECT to_int8(1) AS id) ANY LEFT JOIN test_00597 USING id WHERE value = 1;

-- FIXME: no support for aliased tables for now.
EXPLAIN SYNTAX SELECT b.value FROM (SELECT to_int8(1) AS id) ANY LEFT JOIN test_00597 AS b USING id WHERE value = 1;
SELECT b.value FROM (SELECT to_int8(1) AS id) ANY LEFT JOIN test_00597 AS b USING id WHERE value = 1;
...
-- Compatibility test
EXPLAIN SYNTAX SELECT * FROM (SELECT to_int8(1) AS id, to_date('2000-01-01') AS date FROM system.numbers LIMIT 1) ANY LEFT JOIN (SELECT * FROM test_00597) AS b USING date, id WHERE b.date = to_date('2000-01-01');
SELECT * FROM (SELECT to_int8(1) AS id, to_date('2000-01-01') AS date FROM system.numbers LIMIT 1) ANY LEFT JOIN (SELECT * FROM test_00597) AS b USING date, id WHERE b.date = to_date('2000-01-01');
