...
${CLICKHOUSE_CLIENT} --query "create stream data (A uint8, B uint8) engine=MergeTree() order by A;"
...
sleep 2
${CLICKHOUSE_CLIENT} --query "select * except (time) from file('errors_server', 'CSV', 'time DateTime, database nullable(string), stream nullable(string), offset uint32, reason string, raw_data string');"
...
sleep 2
${CLICKHOUSE_CLIENT} --query "select * except (time) from file('errors_client', 'CSV', 'time DateTime, database nullable(string), stream nullable(string), offset uint32, reason string, raw_data string');"
