...
CREATE STREAM t1 (id int) ENGINE = Memory;
CREATE STREAM t2 (id int) ENGINE = Memory;
...
SELECT 70 = 10 * sum(t1.id) + sum(t2.id) AND count() == 4 FROM t1 INNER ANY JOIN t2 ON to_low_cardinality(1);
SELECT 70 = 10 * sum(t1.id) + sum(t2.id) AND count() == 4 FROM t1 INNER ANY JOIN t2 ON to_low_cardinality(to_nullable(1));
SELECT 70 = 10 * sum(t1.id) + sum(t2.id) AND count() == 4 FROM t1 INNER ANY JOIN t2 ON to_nullable(to_low_cardinality(1));
