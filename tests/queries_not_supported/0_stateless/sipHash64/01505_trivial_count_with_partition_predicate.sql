...
create stream test1(p DateTime, k int) engine MergeTree partition by toDate(p) order by k settings index_granularity = 1;
...
select count() FROM test1 where toDate(p) = '2020-09-01' and sipHash64(to_string(toDate(p))) % 2 = 1;
select count() FROM test1 where toDate(p) = '2020-09-01' and sipHash64(to_string(toDate(p))) % 2 = 0;
...
create stream test_tuple(p DateTime, i int, j int) engine MergeTree partition by (toDate(p), i) order by j settings index_granularity = 1;
