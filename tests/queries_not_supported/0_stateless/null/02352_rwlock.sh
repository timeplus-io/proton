...
while :; do
    $CLICKHOUSE_CLIENT -nm -q "
        DROP STREAM IF EXISTS ${CLICKHOUSE_DATABASE}_ordinary.data_02352;
        CREATE STREAM ${CLICKHOUSE_DATABASE}_ordinary.data_02352 (key int) Engine=Null();
    "
    ...
    # 10 second wait
    $CLICKHOUSE_CLIENT --query_id "$drop_query_id" -q "DROP STREAM ${CLICKHOUSE_DATABASE}_ordinary.data_02352 SYNC" --lock_acquire_timeout 10 > >(
