...
create stream open_events_tmp (`APIKey` uint32, `EventDate` date) ENGINE = MergeTree PARTITION BY to_monday(EventDate) ORDER BY (APIKey, EventDate);
...
(
    ...
    WHERE (EventDate >= to_date('2020-07-09')) AND (EventDate <= to_date('2020-07-11')) AND (APIKey = 2)
    GROUP BY EventDate
)
FULL OUTER JOIN 
(
    ...
    WHERE (EventDate <= to_date('2020-07-12')) AND (APIKey = 2)
    GROUP BY EventDate
        WITH TOTALS
) USING (EventDate)
...
DROP STREAM open_events_tmp;
DROP STREAM tracking_events_tmp;
