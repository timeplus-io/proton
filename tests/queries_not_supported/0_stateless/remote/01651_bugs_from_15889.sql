...
WITH concat(addressToLine(array_join(trace) AS addr), '#') AS symbol
SELECT count() > 7
FROM trace_log AS t
WHERE (query_id =
(
...
WITH addressToSymbol(array_join(trace)) AS symbol
SELECT count() > 0
FROM trace_log AS t
...
SELECT if(dateDiff('second', to_datetime(time_with_microseconds), to_datetime(t)) = -9223372036854775808, 'ok', ''); -- { serverError 43 }
...
SELECT if(dateDiff('second', to_datetime(time_with_microseconds), to_datetime(t)) = -9223372036854775808, 'ok', '');
