...
select array_map(x -> finalize_aggregation(x), state) from (select sumStateResample(0, 20, 1)(number, number % 20) as state from numbers(200));
select array_map(x -> finalize_aggregation(x), state) from
(
...
select array_map(x -> finalize_aggregation(x), state) from (select groupArrayStateResample(0, 20, 1)(number, number % 20) state from numbers(50));
