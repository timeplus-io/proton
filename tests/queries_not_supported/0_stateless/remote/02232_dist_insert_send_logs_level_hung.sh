...
$CLICKHOUSE_CLIENT "${client_opts[@]}" -nm -q "
    drop stream if exists mv_02232;
    drop stream if exists in_02232;
    drop stream if exists out_02232;

    create stream out_02232 (key int) engine=Null();
    create stream in_02232 (key int) engine=Null();
    create materialized view mv_02232 as to out_02232 as select * from in_02232;
"
...
timeout 250s $CLICKHOUSE_CLIENT "${client_opts[@]}" "${insert_client_opts[@]}" -q "insert into function remote('127.2', current_database(), in_02232) select * from numbers(1e6)"
...
$CLICKHOUSE_CLIENT "${client_opts[@]}" -nm -q "
    drop stream in_02232;
    drop stream mv_02232;
    drop stream out_02232;
"
