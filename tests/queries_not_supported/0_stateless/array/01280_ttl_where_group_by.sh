...
echo "ttl_01280_2"
$CLICKHOUSE_CLIENT -n --query "
create stream ttl_01280_2 (a int, b int, x array(int32), y Double, d DateTime) engine = MergeTree order by (a, b) ttl d + interval 1 second group by a, b set x = min_for_each(x), y = sum(y), d = max(d);
...
echo "ttl_01280_5"
$CLICKHOUSE_CLIENT -n --query "create stream ttl_01280_5 (a int, b int, x int64, y int64, d DateTime) engine = MergeTree order by (to_date(d), a, -b) ttl d + interval 1 second group by to_date(d), a set x = sum(x), b = argMax(b, -b);
