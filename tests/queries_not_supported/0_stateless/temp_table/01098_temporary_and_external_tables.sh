...
${CLICKHOUSE_CURL} -m 30 -sSk "$url" --data "DROP TEMPORARY STREAM IF EXISTS tmp_table"
${CLICKHOUSE_CURL} -m 30 -sSk "$url" --data "CREATE TEMPORARY STREAM tmp_table AS SELECT number AS n FROM numbers(42)"
...
${CLICKHOUSE_CURL} -m 30 -sSk "$url" --data "SHOW CREATE TEMPORARY STREAM tmp_table"
${CLICKHOUSE_CURL} -m 30 -sSk "$url" --data "SHOW create stream $internal_table_name"
${CLICKHOUSE_CURL} -m 30 -sSk "$url_without_session" --data "SHOW create stream $internal_table_name"
