...
function setup()
{
    $CLICKHOUSE_CLIENT -nm -q "
        drop stream if exists data_01814;
        drop stream if exists dist_01814;

        create stream data_01814 (key int) Engine=MergeTree() order by key settings index_granularity=10 as select * from numbers(100);
        create stream dist_01814 as data_01814 engine=Distributed('test_cluster_two_shards', $CLICKHOUSE_DATABASE, data_01814, key);
    "
}
...
{
    $CLICKHOUSE_CLIENT -nm -q "
        drop stream data_01814;
        drop stream dist_01814;
    "
}
    ...
    $CLICKHOUSE_CLIENT -nm -q "
        ...
        select read_rows from system.query_log
            where
                ...
                and query_kind = 'Select' /* exclude DESC STREAM */
