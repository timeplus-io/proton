...
$CLICKHOUSE_CLIENT -n --query="
    drop stream IF EXISTS numbers_100k;
...
for i in $(seq 1000000 $((20000 * $STEP_MULTIPLIER)) 10000000 && seq 10100000 $((100000 * $STEP_MULTIPLIER)) 50000000); do
    $CLICKHOUSE_CLIENT --max_memory_usage="$i" --query="
        SELECT int_div(number, 5) AS k, max(to_string(number)) FROM remote('127.0.0.{2,3}', ${CLICKHOUSE_DATABASE}.numbers_100k) GROUP BY k ORDER BY k LIMIT 1;
    " 2> /dev/null;
    CODE=$?;
    [ "$CODE" -ne "241" ] && [ "$CODE" -ne "0" ] && echo "Fail" && break;
done | uniq

$CLICKHOUSE_CLIENT --query="drop stream numbers_100k;";
