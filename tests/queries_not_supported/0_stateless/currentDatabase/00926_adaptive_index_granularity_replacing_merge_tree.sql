...
DROP STREAM IF EXISTS zero_rows_per_granule;

create stream zero_rows_per_granule (
  p date,
  k uint64,
  v1 uint64,
  v2 Int64
...
OPTIMIZE STREAM zero_rows_per_granule FINAL;
...
DROP STREAM IF EXISTS two_rows_per_granule;

create stream two_rows_per_granule (
  p date,
  k uint64,
  v1 uint64,
  v2 Int64
...
OPTIMIZE STREAM two_rows_per_granule FINAL;
...
DROP STREAM IF EXISTS four_rows_per_granule;

create stream four_rows_per_granule (
  p date,
  k uint64,
  v1 uint64,
  v2 Int64
...
OPTIMIZE STREAM four_rows_per_granule FINAL;
...
DROP STREAM IF EXISTS huge_granularity_small_blocks;

create stream huge_granularity_small_blocks (
  p date,
  k uint64,
  v1 uint64,
  v2 Int64
...
OPTIMIZE STREAM huge_granularity_small_blocks FINAL;
...
DROP STREAM IF EXISTS adaptive_granularity_alter;

create stream adaptive_granularity_alter (
  p date,
  k uint64,
  v1 uint64,
  v2 Int64
...
ALTER STREAM adaptive_granularity_alter MODIFY COLUMN v1 Int16;
...
OPTIMIZE STREAM adaptive_granularity_alter FINAL;

SELECT COUNT(*) FROM adaptive_granularity_alter;

SELECT distinct(marks) from system.parts WHERE table = 'adaptive_granularity_alter' and database=currentDatabase() and active=1;

ALTER STREAM adaptive_granularity_alter MODIFY COLUMN v2 string;
