...
create stream dist_01247 as data_01247 engine=Distributed(test_cluster_two_shards, currentDatabase(), data_01247, number);
...
select 'GROUP BY to_string(number)';
select count(), * from dist_01247 group by to_string(number);
...
drop stream if exists dist_01247;
drop stream if exists data_01247;
create stream data_01247 engine=Memory() as select number key, 0 value from numbers(2);
create stream dist_01247 as data_01247 engine=Distributed(test_cluster_two_shards, currentDatabase(), data_01247, key);
