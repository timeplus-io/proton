...
drop stream if exists merge_dist_01223;
drop stream if exists dist_01223;
drop stream if exists dist_layer_01223;
drop stream if exists data_01223;

create stream data_01223 (key int) Engine=Memory();
create stream dist_layer_01223 as data_01223 Engine=Distributed(test_cluster_two_shards, currentDatabase(), data_01223);
create stream dist_01223 as data_01223 Engine=Distributed(test_cluster_two_shards, currentDatabase(), dist_layer_01223);
...
select 'LEFT JOIN';
select to_int32(number) key, b.key from numbers(2) a left join (select distinct * from dist_01223) b using key order by b.key;
select 'RIGHT JOIN';
select to_int32(number) key, b.key from numbers(2) a right join (select distinct * from dist_01223) b using key order by b.key;
...
drop stream dist_01223;
drop stream dist_layer_01223;

-- only one shard in nested
select 'only one shard in nested';
create stream dist_layer_01223 as data_01223 Engine=Distributed(test_shard_localhost, currentDatabase(), data_01223);
create stream dist_01223 as data_01223 Engine=Distributed(test_cluster_two_shards, currentDatabase(), dist_layer_01223);
select count() from dist_01223;
...
-- wrap with merge()
select 'merge()';
create stream merge_dist_01223 as dist_01223 engine=Merge(currentDatabase(), 'dist_01223');
...
-- global in
select 'GLOBAL IN';
select distinct * from dist_01223 where key global in (select to_int32(1));

drop stream merge_dist_01223;
drop stream dist_01223;
drop stream dist_layer_01223;
drop stream data_01223;
