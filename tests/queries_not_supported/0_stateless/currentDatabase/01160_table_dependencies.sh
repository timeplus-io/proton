...
$CLICKHOUSE_CLIENT -q "drop stream if exists dict_src;"
$CLICKHOUSE_CLIENT -q "drop dictionary if exists dict1;"
$CLICKHOUSE_CLIENT -q "drop dictionary if exists dict2;"
$CLICKHOUSE_CLIENT -q "drop stream if exists join;"
$CLICKHOUSE_CLIENT -q "drop stream if exists t;"

$CLICKHOUSE_CLIENT -q "create stream dict_src (n int, m int, s string) engine=MergeTree order by n;"
...
$CLICKHOUSE_CLIENT -q "create stream join(n int, m int default dictGet('$CLICKHOUSE_DATABASE.dict1', 'm', 42::uint64)) engine=Join(any, left, n);"
...
$CLICKHOUSE_CLIENT -q "create stream s (x default joinGet($CLICKHOUSE_DATABASE.join, 'm', 42::int)) engine=Set"

$CLICKHOUSE_CLIENT -q "create stream t (n int, m int default joinGet($CLICKHOUSE_DATABASE.join, 'm', 42::int),
...
$CLICKHOUSE_CLIENT -q "alter stream t add column x int default in(1, $CLICKHOUSE_DATABASE.s), drop column y"
...
$CLICKHOUSE_CLIENT -q "drop stream join" 2>&1| grep -Fa "some tables depend on it" >/dev/null && echo "OK"
...
$CLICKHOUSE_CLIENT -q "drop stream mv"
...
$CLICKHOUSE_CLIENT -q "drop stream ${t_database}.t;"
$CLICKHOUSE_CLIENT -q "drop stream s;"
...
$CLICKHOUSE_CLIENT -q "drop stream join;"
$CLICKHOUSE_CLIENT -q "drop dictionary dict1;"
$CLICKHOUSE_CLIENT -q "drop stream dict_src;"
...
$CLICKHOUSE_CLIENT -q "drop database if exists ${CLICKHOUSE_DATABASE}_1"
$CLICKHOUSE_CLIENT -q "create database ${CLICKHOUSE_DATABASE}_1"
$CLICKHOUSE_CLIENT -q "create stream ${CLICKHOUSE_DATABASE}_1.xdict_src (n int, m int, s string) engine=MergeTree order by n;"
...
$CLICKHOUSE_CLIENT -q "create stream ${CLICKHOUSE_DATABASE}_1.zjoin(n int, m int default dictGet('${CLICKHOUSE_DATABASE}_1.ydict1', 'm', 42::uint64)) engine=Join(any, left, n);"
