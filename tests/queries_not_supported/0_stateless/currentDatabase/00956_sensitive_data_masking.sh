...
$CLICKHOUSE_CLIENT \
  --query="SELECT 'find_me_TOPSECRET=TOPSECRET', int_div( 100, number - 10) FROM numbers(11) FORMAT Null" \
...
echo 8
$CLICKHOUSE_CLIENT \
   --query="drop stream if exists sensetive; create stream sensitive ( id uint64, date date, value1 string, value2 uint64) Engine=MergeTree ORDER BY id PARTITION BY date;
insert into sensitive select number as id, to_date('2019-01-01') as date, 'abcd' as value1, rand() as valuer from numbers(10000);
insert into sensitive select number as id, to_date('2019-01-01') as date, 'find_me_TOPSECRET=TOPSECRET' as value1, rand() as valuer from numbers(10);
insert into sensitive select number as id, to_date('2019-01-01') as date, 'abcd' as value1, rand() as valuer from numbers(10000);
select * from sensitive WHERE value1 = 'find_me_TOPSECRET=TOPSECRET' FORMAT Null;
drop stream sensitive;" --log_queries=1 --ignore-error --multiquery >"$tmp_file" 2>&1
