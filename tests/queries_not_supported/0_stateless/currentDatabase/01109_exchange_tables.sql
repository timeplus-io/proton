...
create stream t0 ENGINE=MergeTree() ORDER BY tuple() AS SELECT rowNumberInAllBlocks(), * FROM (SELECT toLowCardinality(array_join(['exchange', 'tables'])));
-- NOTE: database = currentDatabase() is not mandatory
create stream t1 ENGINE=Log() AS SELECT * FROM system.tables AS t JOIN system.databases AS d ON t.database=d.name;
create stream t2 ENGINE=MergeTree() ORDER BY tuple() AS SELECT rowNumberInAllBlocks() + (SELECT count() FROM t0), * FROM (SELECT array_join(['hello', 'world']));
...
create stream test_01109_other_atomic.t3 ENGINE=MergeTree() ORDER BY tuple()
    AS SELECT rowNumberInAllBlocks() + (SELECT max((*,*).1.1) + 1 FROM (SELECT (*,) FROM t1 UNION ALL SELECT (*,) FROM t2)), *
    FROM (SELECT array_join(['another', 'db']));

create stream test_01109_ordinary.t4 AS t1;
...
create stream t0 ENGINE=Log() AS SELECT * FROM system.numbers limit 2;
