...
create stream enums (e Enum8('a' = 0, 'b' = 1, 'c' = 2, 'd' = 3)) engine = TinyLog;
...
-- Conversion
select toInt8(e), toint16(e), to_uint64(e), to_string(e), e from enums;

drop stream if exists enums_copy;
create stream enums_copy engine = TinyLog as select * from enums;
select * from enums_copy;

drop stream enums_copy;
create stream enums_copy engine = TinyLog as select * from remote('127.0.0.2', currentDatabase(), enums);
