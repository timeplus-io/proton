...
DROP STREAM IF EXISTS not_partitioned;
create stream not_partitioned(x uint8) ENGINE MergeTree ORDER BY x;
...
OPTIMIZE STREAM not_partitioned PARTITION tuple() FINAL;
SELECT 'Parts after OPTIMIZE:';
SELECT partition, name FROM system.parts WHERE database = currentDatabase() AND table = 'not_partitioned' AND active ORDER BY name;

SELECT 'Sum before DETACH PARTITION:';
SELECT sum(x) FROM not_partitioned;
ALTER STREAM not_partitioned DETACH PARTITION ID 'all';
...
DROP STREAM not_partitioned;

SELECT '*** Partitioned by week ***';

DROP STREAM IF EXISTS partitioned_by_week;
create stream partitioned_by_week(d date, x uint8) ENGINE = MergeTree PARTITION BY toMonday(d) ORDER BY x;
...
OPTIMIZE STREAM partitioned_by_week PARTITION '2000-01-03' FINAL;
SELECT 'Parts after OPTIMIZE:';
SELECT partition, name FROM system.parts WHERE database = currentDatabase() AND table = 'partitioned_by_week' AND active ORDER BY name;

SELECT 'Sum before DROP PARTITION:';
SELECT sum(x) FROM partitioned_by_week;
ALTER STREAM partitioned_by_week DROP PARTITION '1999-12-27';
SELECT 'Sum after DROP PARTITION:';
SELECT sum(x) FROM partitioned_by_week;

DROP STREAM partitioned_by_week;

SELECT '*** Partitioned by a (date, uint8) tuple ***';

DROP STREAM IF EXISTS partitioned_by_tuple;
create stream partitioned_by_tuple(d date, x uint8, y uint8) ENGINE MergeTree ORDER BY x PARTITION BY (d, x);
...
OPTIMIZE STREAM partitioned_by_tuple PARTITION ('2000-01-01', 1) FINAL;
OPTIMIZE STREAM partitioned_by_tuple PARTITION ('2000-01-02', 1) FINAL;
SELECT 'Parts after OPTIMIZE:';
SELECT partition, name FROM system.parts WHERE database = currentDatabase() AND table = 'partitioned_by_tuple' AND active ORDER BY name;

SELECT 'Sum before DETACH PARTITION:';
SELECT sum(y) FROM partitioned_by_tuple;
ALTER STREAM partitioned_by_tuple DETACH PARTITION ID '20000101-1';
SELECT 'Sum after DETACH PARTITION:';
SELECT sum(y) FROM partitioned_by_tuple;
...
DROP STREAM IF EXISTS partitioned_by_string;
create stream partitioned_by_string(s string, x uint8) ENGINE = MergeTree PARTITION BY s ORDER BY x;
...
OPTIMIZE STREAM partitioned_by_string PARTITION 'aaa' FINAL;
SELECT 'Parts after OPTIMIZE:';
SELECT partition, name FROM system.parts WHERE database = currentDatabase() AND table = 'partitioned_by_string' AND active ORDER BY name;

SELECT 'Sum before DROP PARTITION:';
SELECT sum(x) FROM partitioned_by_string;
ALTER STREAM partitioned_by_string DROP PARTITION 'bbb';
SELECT 'Sum after DROP PARTITION:';
SELECT sum(x) FROM partitioned_by_string;

DROP STREAM partitioned_by_string;

SELECT '*** Table without columns with fixed size ***';

DROP STREAM IF EXISTS without_fixed_size_columns;
create stream without_fixed_size_columns(s string) ENGINE MergeTree PARTITION BY length(s) ORDER BY s;
...
ALTER STREAM without_fixed_size_columns DROP PARTITION 1;
SELECT 'After DROP PARTITION:';
SELECT * FROM without_fixed_size_columns ORDER BY s;

DROP STREAM without_fixed_size_columns;
