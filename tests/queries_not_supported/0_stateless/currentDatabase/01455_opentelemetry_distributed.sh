...
echo "===http==="
trace_id=$(${CLICKHOUSE_CLIENT} -q "select lower(hex(reverse(reinterpret_as_string(generateUUIDv4()))))")
...
# DESC STREAM. Two DESC STREAM instead of one looks like a bug, see the issue:
