...
create stream in_memory (a uint32, b uint32)
...
ALTER STREAM in_memory DELETE WHERE b = 0;

SELECT count() FROM in_memory;
SELECT * FROM in_memory ORDER BY a LIMIT 5;

ALTER STREAM in_memory ADD COLUMN arr array(uint64);
ALTER STREAM in_memory UPDATE arr = [a, a * a] WHERE b = 1;

SELECT arr FROM in_memory ORDER BY a LIMIT 5;

ALTER STREAM in_memory MODIFY COLUMN b string;
ALTER STREAM in_memory RENAME COLUMN b to str;
SELECT DISTINCT str, length(str) FROM in_memory ORDER BY str;
ALTER STREAM in_memory DROP COLUMN str;

SELECT * FROM in_memory ORDER BY a LIMIT 5;

-- in-memory parts works if they're empty.
ALTER STREAM in_memory DELETE WHERE 1;
SELECT count() FROM in_memory;

DROP STREAM in_memory;
