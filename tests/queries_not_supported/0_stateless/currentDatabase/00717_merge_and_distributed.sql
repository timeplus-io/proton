...
DROP STREAM IF EXISTS test_local_1;
DROP STREAM IF EXISTS test_local_2;
DROP STREAM IF EXISTS test_distributed_1;
DROP STREAM IF EXISTS test_distributed_2;

create stream test_local_1 (date date, value uint32) ENGINE = MergeTree(date, date, 8192);
create stream test_local_2 (date date, value uint32) ENGINE = MergeTree(date, date, 8192);
create stream test_distributed_1 AS test_local_1 ENGINE = Distributed('test_shard_localhost', currentDatabase(), test_local_1, rand());
create stream test_distributed_2 AS test_local_2 ENGINE = Distributed('test_shard_localhost', currentDatabase(), test_local_2, rand());
...
DROP STREAM IF EXISTS test_u64_local;
DROP STREAM IF EXISTS test_s64_local;
DROP STREAM IF EXISTS test_u64_distributed;
DROP STREAM IF EXISTS test_s64_distributed;

create stream test_s64_local (date date, value Int64) ENGINE = MergeTree(date, date, 8192);
create stream test_u64_local (date date, value uint64) ENGINE = MergeTree(date, date, 8192);
create stream test_s64_distributed AS test_s64_local ENGINE = Distributed('test_shard_localhost', currentDatabase(), test_s64_local, rand());
create stream test_u64_distributed AS test_u64_local ENGINE = Distributed('test_shard_localhost', currentDatabase(), test_u64_local, rand());
