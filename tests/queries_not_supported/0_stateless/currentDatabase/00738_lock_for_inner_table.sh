...
echo "DROP STREAM IF EXISTS tab_00738 SYNC;
DROP STREAM IF EXISTS mv SYNC;
create stream tab_00738(a int) ENGINE = MergeTree() ORDER BY a;
-- The matview will take at least 2 seconds to be finished (10000000 * 0.0000002)
CREATE MATERIALIZED VIEW mv UUID '$uuid'   AS SELECT sleepEachRow(0.0000002) FROM tab_00738;" | ${CLICKHOUSE_CLIENT} -n

${CLICKHOUSE_CLIENT} --query_id insert_$CLICKHOUSE_DATABASE --query "INSERT INTO tab_00738 SELECT number FROM numbers(10000000)" &

function drop()
{
    ${CLICKHOUSE_CLIENT} --query "DROP STREAM \`.inner_id.$uuid\`" -n
}
...
echo "DROP STREAM IF EXISTS tab_00738;
DROP STREAM IF EXISTS mv;" | ${CLICKHOUSE_CLIENT} -n
