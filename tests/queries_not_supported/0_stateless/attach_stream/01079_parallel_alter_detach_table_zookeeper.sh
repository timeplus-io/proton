...
function detach_attach_thread()
{
    while true; do
        REPLICA=$(($RANDOM % 3 + 1))
        $CLICKHOUSE_CLIENT --query "DETACH STREAM concurrent_alter_detach_$REPLICA"
        sleep 0.$RANDOM
        $CLICKHOUSE_CLIENT --query "ATTACH STREAM concurrent_alter_detach_$REPLICA"
    done
}
...
for i in $(seq $REPLICAS); do
    $CLICKHOUSE_CLIENT --query "ATTACH STREAM concurrent_alter_detach_$i" 2> /dev/null
done
