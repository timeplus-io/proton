...
ATTACH STREAM local.test (id uint64, d date, s string) Engine=MergeTree ORDER BY id PARTITION BY toYYYYMM(d);
EOF
...
ATTACH STREAM local.data_csv (id uint64, d date, s string) Engine=File(CSV, '${WORKING_FOLDER_01528}/data.csv');
EOF
...
ATTACH STREAM local.stdin (id uint64, d date, s string) Engine=File(CSV, stdin);
EOF
...
${CLICKHOUSE_LOCAL} --query "OPTIMIZE STREAM local.test FINAL;" --path="${WORKING_FOLDER_01528}"
