...
function detach_attach_thread()
{
    while true; do
        REPLICA=$(($RANDOM % 5 + 1))
        $CLICKHOUSE_CLIENT --query "DETACH STREAM concurrent_mutate_mt_$REPLICA"
        sleep 0.$RANDOM
        sleep 0.$RANDOM
        sleep 0.$RANDOM
        $CLICKHOUSE_CLIENT --query "ATTACH STREAM concurrent_mutate_mt_$REPLICA"
    done
}
...
for i in $(seq $REPLICAS); do
    $CLICKHOUSE_CLIENT --query "ATTACH STREAM concurrent_mutate_mt_$i" 2> /dev/null
done
