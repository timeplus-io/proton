...
$CLICKHOUSE_CLIENT --query "DROP STREAM IF EXISTS mt";
$CLICKHOUSE_CLIENT --query "CREATE STREAM mt (n int8, m int8) ENGINE=MergeTree ORDER BY n PARTITION BY 0 < n SETTINGS old_parts_lifetime=0";
...
function thread_select()
{
    while true; do
        ...
        CREATE TEMPORARY STREAM tmp AS SELECT arraySort(group_array(n)), arraySort(group_array(m)), arraySort(group_array(_part)) FROM mt FORMAT Null;
        SELECT throwIf((SELECT sum(n) FROM mt) != 0) FORMAT Null;
        SELECT throwIf((SELECT count() FROM mt) % 2 != 0) FORMAT Null;
        select throwIf((SELECT * FROM tmp) != (SELECT arraySort(group_array(n)), arraySort(group_array(m)), arraySort(group_array(_part)) FROM mt)) FORMAT Null;
        COMMIT;"
    done
}
