...
CREATE STREAM sum_map(date Date, timeslot DateTime, statusMap nested(status uint16, requests uint64)) ENGINE = Log;
...
CREATE STREAM sum_map_decimal_nullable (`statusMap` Array(Tuple(goal_id UInt16, revenue Nullable(Decimal(9, 5))))) engine=Log;
...
DROP STREAM sum_map_decimal_nullable;
