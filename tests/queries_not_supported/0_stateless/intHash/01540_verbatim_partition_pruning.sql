...
drop stream if exists xy;

create stream xy(x int, y int) engine MergeTree partition by intHash64(x) % 2 order by y settings index_granularity = 1;
...
drop stream if exists xyz;
create stream xyz(x int, y int, z int) engine MergeTree partition by if(to_uint8(x), y, z) order by x settings index_granularity = 1;
...
create stream test(d date, k int64, s string) Engine=MergeTree partition by (toYYYYMM(d),k) order by (d, k);
...
drop stream if exists myTable;
create stream myTable (myDay date, myOrder int32, someData string) ENGINE = ReplacingMergeTree PARTITION BY floor(toYYYYMMDD(myDay), -1) ORDER BY (myOrder);
