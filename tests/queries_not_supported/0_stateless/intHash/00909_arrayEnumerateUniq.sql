...
SELECT array_enumerate_uniq(         [1,1,2,2,1,1],    [1,2,1,2,2,2]);
...
SELECT array_enumerate_uniq(         [[[1,2,3],[1,2,3],[1,2,3]],[[1,2,3],[1,2,3],[1,2,3]],[[1,2]]]);
...
-- дублируем логику array_map( aEU), ответ [[,,],[,,],[]]
SELECT '[[1,1,1],[1,2,1],[1]] =';
...
-- подсчитываем вхождения в отдельных массивах первого уровня, смотрим в глубину на два уровня,
-- "одномерные" массивы мысленно растягиваем до "двухмерных", дублируем логику array_map( aEU), ответ [[,,],[,,],[]]
SELECT '[[1,1,1],[1,2,1],[1]] =';
...
create stream arrays_test (a1 array(UInt16), a2 array(UInt16), a3 array(array(UInt16)), a4 array(array(UInt16)) ) ;
...
select 'array_enumerate_uniq(a1, a2) =';
SELECT array_enumerate_uniq(a1, a2) FROM arrays_test ORDER BY a1, a2;
select '---------GO2';
...
create stream arrays_test (a3 array(array(uint8)), a4 array(array(uint32)) ) ;
...
DROP STREAM IF EXISTS arrays_test;
create stream arrays_test (a1 array(uint8), a2 array(uint32) ) ;
...
DROP STREAM IF EXISTS arr_tests_visits;

create stream arr_tests_visits
(
    CounterID        uint32,
    StartDate        date,
    Sign             int8,
...
values (1, to_date('2019-06-06'), 1, 1, 1, 1, [1], [1], [[]], [[]]),       (1, to_date('2019-06-06'), -1, 1, 1, 1, [1], [1], [[]], [[]]),       (1, to_date('2019-06-06'), 1, 1, 1, 2, [1,2], [1,1], [[],[1,2,3,4]], [[],[1001, 1002, 1003, 1004]]),       (1, to_date('2019-06-06'), 1, 2, 1, 1, [3], [1], [[3,4,5]], [[2001, 2002, 2003]]),       (1, to_date('2019-06-06'), 1, 3, 2, 1, [4, 5], [1, 0], [[5,6],[]], [[3001, 3002],[]]),       (1, to_date('2019-06-06'), 1, 4, 2, 1, [5, 5, 6], [1, 0, 0], [[1,2], [1, 2], [3]], [[1001, 1002],[1002, 1003], [2001]]);
