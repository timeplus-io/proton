...
create stream test_1602.tbl (`EventDate` DateTime, `CounterID` uint32, `UserID` uint32) ENGINE = MergeTree() PARTITION BY toYYYYMM(EventDate) ORDER BY (CounterID, EventDate, intHash32(UserID)) SETTINGS index_granularity = 8192;
...
SHOW create stream; -- { clientError 62 }
