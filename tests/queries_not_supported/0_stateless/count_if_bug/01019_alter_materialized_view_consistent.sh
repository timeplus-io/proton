...
DROP STREAM IF EXISTS src_a;
DROP STREAM IF EXISTS src_b;

DROP STREAM IF EXISTS mv;

create stream src_a (v uint64) ENGINE = Null;
create stream src_b (v uint64) ENGINE = Null;
...
function insert_thread() {
    ...
    while true; do
        ...
        is_done=$($CLICKHOUSE_CLIENT -q "SELECT count_if(case = 1) > 0 AND count_if(case = 2) > 0 FROM mv;")
...
function alter_thread() {
    ...
    ALTER[0]="ALTER STREAM mv MODIFY QUERY SELECT v == 1 as test, v as case FROM src_a;"
    ALTER[1]="ALTER STREAM mv MODIFY QUERY SELECT v == 2 as test, v as case FROM src_b;"
...
$CLICKHOUSE_CLIENT -q "SELECT count_if(case = 1) > 0 AND count_if(case = 2) > 0 FROM mv LIMIT 1;"
$CLICKHOUSE_CLIENT -q "SELECT 'inconsistencies', count() FROM mv WHERE test == 0;"

$CLICKHOUSE_CLIENT -q "DROP VIEW mv"
$CLICKHOUSE_CLIENT -q "DROP STREAM src_a"
$CLICKHOUSE_CLIENT -q "DROP STREAM src_b"
