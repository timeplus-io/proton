-- Tags: no-parallel, not_supported, blocked_by_currentDatabse
...
SELECT uniqArray([1, 1, 2]),
       ...
       week(to_date('2000-12-05')),
       CAST(array_join([NULL, NULL]) AS Nullable(TEXT)),
       avgOrDefaultIf(number, number % 2),
       sumOrNull(number),
       to_type_name(sumOrNull(number)),
       count_if(to_date('2000-12-05') + number as d,
       toDayOfYear(d) % 2)
FROM numbers(100);
...
FROM system.query_log WHERE current_database = currentDatabase() AND type = 'QueryFinish' AND (query LIKE '%to_date(\'2000-12-05\')%')
...
drop stream test_query_log_factories_info1.memory_table;
