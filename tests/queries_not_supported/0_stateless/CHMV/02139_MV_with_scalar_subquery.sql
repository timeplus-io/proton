...
create stream source (a int32) ENGINE=MergeTree() ORDER BY tuple();
create stream source_null AS source ENGINE=Null;
create stream dest_a (count uint32, min int32, max int32, count_subquery int32, min_subquery int32, max_subquery int32) ENGINE=MergeTree() ORDER BY tuple();
