...
create stream src (v uint64) ENGINE = Null;
...
function alter_thread()
{
    ...
    ALTERS[0]="ALTER STREAM mv MODIFY QUERY SELECT v FROM src;"
    ALTERS[1]="ALTER STREAM mv MODIFY QUERY SELECT v * 2 as v FROM src;"
...
$CLICKHOUSE_CLIENT -q "DROP STREAM src"
