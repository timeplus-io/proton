...
create stream src (s string, x string DEFAULT 'a') ENGINE=MergeTree() PARTITION BY tuple() ORDER BY s;
...
create stream dst (s string, x string DEFAULT 'c') ENGINE=MergeTree() PARTITION BY tuple() ORDER BY s;
CREATE MATERIALIZED VIEW mv2 TO dst (s string, x string DEFAULT 'd') AS SELECT (*,).1 || 'mv2' as s FROM src;
create stream dist (s string, x string DEFAULT 'asdf') ENGINE=Distributed(test_shard_localhost, test_01155_ordinary, src);
