...
DROP STREAM IF EXISTS truncate_test_log;
DROP STREAM IF EXISTS truncate_test_memory;
DROP STREAM IF EXISTS truncate_test_tiny_log;
DROP STREAM IF EXISTS truncate_test_stripe_log;
DROP STREAM IF EXISTS truncate_test_merge_tree;
DROP STREAM IF EXISTS truncate_test_materialized_view;
DROP STREAM IF EXISTS truncate_test_materialized_depend;

CREATE DATABASE truncate_test;
create stream truncate_test_set(id uint64) ENGINE = Set;
create stream truncate_test_log(id uint64)  ;
create stream truncate_test_memory(id uint64) ;
create stream truncate_test_tiny_log(id uint64) ;
create stream truncate_test_stripe_log(id uint64) ENGINE = StripeLog;
create stream truncate_test_merge_tree(p date, k uint64) ENGINE = MergeTree(p, k, 1);
create stream truncate_test_materialized_depend(p date, k uint64) ENGINE = Null;
...
DROP STREAM IF EXISTS truncate_test_set;
DROP STREAM IF EXISTS truncate_test_log;
DROP STREAM IF EXISTS truncate_test_memory;
DROP STREAM IF EXISTS truncate_test_tiny_log;
DROP STREAM IF EXISTS truncate_test_stripe_log;
