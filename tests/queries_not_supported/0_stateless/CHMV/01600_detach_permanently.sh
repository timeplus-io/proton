...
clickhouse_local "create stream db_ordinary.log_table Engine=Log AS SELECT * FROM numbers(10)"
clickhouse_local "create stream db_ordinary.mt_table Engine=MergeTree ORDER BY tuple() AS SELECT * FROM numbers(10)"
clickhouse_local "create stream db_ordinary.null_table Engine=Null AS SELECT * FROM numbers(10)"

clickhouse_local "create stream db_atomic.log_table Engine=Log AS SELECT * FROM numbers(10)"
clickhouse_local "create stream db_atomic.mt_table Engine=MergeTree ORDER BY tuple() AS SELECT * FROM numbers(10)"
clickhouse_local "create stream db_atomic.null_table Engine=Null AS SELECT * FROM numbers(10)"
...
echo "##################"
echo "test for MV"
clickhouse_local "create stream db_ordinary.src Engine=Null AS system.numbers"
clickhouse_local "create stream db_ordinary.dst Engine=Log AS system.numbers"
...
# clickhouse_local "DROP DICTIONARY db_ordinary.dummy; CREATE DICTIONARY db_ordinary.dummy (id uint64) PRIMARY KEY id LAYOUT(FLAT()) SOURCE(FILE(path 'dummy_dict.tsv' format 'TabSeparated')) LIFETIME(MIN 1 MAX 10); DETACH DICTIONARY db_ordinary.dummy PERMANENTLY; SELECT dictGet('db_ordinary.dummy','val',to_uint64(1));"
