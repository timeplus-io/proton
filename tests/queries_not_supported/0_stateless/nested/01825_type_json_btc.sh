...
${CLICKHOUSE_CLIENT} -q "DROP STREAM IF EXISTS btc"

${CLICKHOUSE_CLIENT} -q "create stream btc (data JSON) ENGINE = MergeTree ORDER BY tuple()" --allow_experimental_object_type 1
...
${CLICKHOUSE_CLIENT} -q "SELECT data.out.spending_outpoints AS outpoints FROM btc WHERE array_exists(x -> not_empty(x), outpoints)"
