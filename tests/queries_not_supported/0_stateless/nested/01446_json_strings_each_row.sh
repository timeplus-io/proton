...
echo "DROP STREAM IF EXISTS test_table;" | ${CLICKHOUSE_CLIENT}
echo "DROP STREAM IF EXISTS test_table_2;" | ${CLICKHOUSE_CLIENT}
echo "SELECT 1;" | ${CLICKHOUSE_CLIENT}
# Check JSONStringsEachRow Output
echo "create stream test_table (value uint8, name string) ENGINE = MergeTree() ORDER BY value;" | ${CLICKHOUSE_CLIENT}
...
echo "create stream test_table (v1 string, v2 uint8, v3 DEFAULT v2 * 16, v4 uint8 DEFAULT 8) ENGINE = MergeTree() ORDER BY v2;" | ${CLICKHOUSE_CLIENT}
...
# Check nested
echo "create stream test_table_2 (v1 uint8, n nested(id uint8, name string)) ENGINE = MergeTree() ORDER BY v1;" | ${CLICKHOUSE_CLIENT}
