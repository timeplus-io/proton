...
${CLICKHOUSE_CLIENT} -q "DROP STREAM IF EXISTS nbagames"

${CLICKHOUSE_CLIENT} -q "create stream nbagames (data JSON) ENGINE = MergeTree ORDER BY tuple()" --allow_experimental_object_type 1
...
( \
    ...
    FROM \
    ( \
        SELECT array_join(array_join(data.teams.players)) as players from nbagames \
    ) \
) \
