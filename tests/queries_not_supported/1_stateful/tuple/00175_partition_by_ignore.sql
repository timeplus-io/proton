...
DROP STREAM IF EXISTS partition_by_ignore SYNC;

CREATE STREAM partition_by_ignore (ts DateTime, ts_2 DateTime) ENGINE=MergeTree PARTITION BY (to_YYYYMM(ts), ignore(ts_2)) ORDER BY tuple();
