...
$CLICKHOUSE_CLIENT --max_threads 1 --query="SELECT URL, Title, SearchPhrase from table(test.hits) LIMIT 1000" > "${CLICKHOUSE_TMP}"/data.tsv

echo $CLICKHOUSE_OBFUSCATOR
