...
--query_workers: how many query processes to be created to run query to the streams, and one query_worker will get one query from query_list to run (i.e. on query_worker only keep running one query), query_list could be checked in the script.

now the query_list is: 
query_list = [
