...
def workspace_clean():
    ...
    for tp_operate_obj in tp_operate_objs:
        ...
        for target_workspace in target_workspaces:
            logger.debug(f"workspace_clean: target_workspace.name = {target_workspace.name}, target_workspace.access_privilege = {target_workspace.access_privilege}")
...
def result_check(tp_operate):
    operate_tracking = tp_operate.tracking.get(tpexporter.OperateTrackingType.SCHEMA_EXPORT.value)
    source_workspace = tp_operate.source_workspace
    assert operate_tracking != None
    logger.debug(f"tp_operate.name = {tp_operate.name}, operate_tracking = {operate_tracking}")
    ...
    if source_workspace.name != "tp-duckbill": #tp-duckbill has a view with issue (Code: 80.\nDB::Exception: MaterializedView default.mv_lpn_list_new cannot select from itself), which is not exported
        for operate_tracking_item in operate_tracking:
            for key in operate_tracking_item["summary"]["total_source_obj_2_export_number"].keys():
                assert operate_tracking_item["summary"]["total_source_obj_2_export_number"][key] == (operate_tracking_item["summary"]["total_schema_exported_obj_number"][key] + operate_tracking_item["summary"]["total_schema_export_skip_number"][key])
    else:
        for operate_tracking_item in operate_tracking:
            for key in operate_tracking_item["summary"]["total_source_obj_2_export_number"].keys():
                if key == "view":
