SELECT count() from table(test.hits) WHERE EventDate = '2014-03-18';
SELECT count() from table(test.hits) WHERE EventDate < '2014-03-18';
SELECT count() from table(test.hits) WHERE EventDate > '2014-03-18';
SELECT count() from table(test.hits) WHERE EventDate <= '2014-03-18';
SELECT count() from table(test.hits) WHERE EventDate >= '2014-03-18';
SELECT count() from table(test.hits) WHERE EventDate IN ('2014-03-18', '2014-03-19');

SELECT count() from table(test.hits) WHERE EventDate = to_date('2014-03-18');
SELECT count() from table(test.hits) WHERE EventDate < to_date('2014-03-18');
SELECT count() from table(test.hits) WHERE EventDate > to_date('2014-03-18');
SELECT count() from table(test.hits) WHERE EventDate <= to_date('2014-03-18');
SELECT count() from table(test.hits) WHERE EventDate >= to_date('2014-03-18');
SELECT count() from table(test.hits) WHERE EventDate IN (to_date('2014-03-18'), to_date('2014-03-19'));

SELECT count() from table(test.hits) WHERE EventDate = concat('2014-0', '3-18');

DROP STREAM IF EXISTS test.hits_indexed_by_time;
CREATE STREAM test.hits_indexed_by_time (EventDate date, EventTime DateTime('Europe/Moscow')) ENGINE = MergeTree ORDER BY (EventDate, EventTime);
INSERT INTO test.hits_indexed_by_time SELECT EventDate, EventTime from table(test.hits);
