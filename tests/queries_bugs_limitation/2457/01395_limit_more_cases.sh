...
for OFFSET in {0..15}; do
    for LIMIT in {0..15}; do
        echo "SELECT
            ...
            throw_if(first != ($OFFSET < $SIZE AND $LIMIT > 0 ? $OFFSET : 0)),
            throw_if(last != ($OFFSET < $SIZE AND $LIMIT > 0 ? least($SIZE - 1, $OFFSET + $LIMIT - 1) : 0)),
            throw_if((c != 0 OR first != 0 OR last != 0) AND (c != last - first + 1))
