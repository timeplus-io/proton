...
$CLICKHOUSE_CLIENT -q "select multisearchallposicionutf7casesensitive('abc');" 2>&1 | grep "Maybe you meant: \['multi_search_all_positions_case_insensitive','multi_search_all_positions_case_insensitiveUTF8'\]." &>/dev/null;
...
$CLICKHOUSE_CLIENT -q "select multiSearchAllPosicionSensitiveUTF8('abc');" 2>&1 | grep "Maybe you meant: \['multi_search_any_case_insensitive_utf8','multi_search_all_positions_case_insensitiveUTF8'\]." &>/dev/null;
...
$CLICKHOUSE_CLIENT --allow_deprecated_syntax_for_merge_tree=1 -q "CREATE STREAM stored_aggregates (d Date, Uniq aggregate_function(uniq, uint64)) ENGINE = MergeTre(d, d, 8192);" 2>&1 | grep "Maybe you meant: \['MergeTree'\]." &>/dev/null
$CLICKHOUSE_CLIENT --allow_deprecated_syntax_for_merge_tree=1 -q "CREATE STREAM stored_aggregates (d Date, Uniq agregate_function(uniq, uint64)) ENGINE = MergeTree(d, d, 8192);" 2>&1 | grep "Maybe you meant: \['aggregate_function'\]." &>/dev/null