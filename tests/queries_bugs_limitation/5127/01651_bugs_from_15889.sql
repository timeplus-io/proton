...
CREATE STREAM xp (`A` date, `B` int64, `S` string) PARTITION BY to_YYYYMM(A) ORDER BY B;
SELECT sleep(1);
INSERT INTO xp(A, B, S) SELECT '2020-01-01', number, '' FROM numbers(100000);

CREATE STREAM xp_d AS xp;

SELECT count(7 = (SELECT number FROM numbers(0) ORDER BY number ASC NULLS FIRST LIMIT 7)) FROM table(xp_d) PREWHERE to_YYYYMM(A) GLOBAL IN (SELECT NULL = (SELECT number FROM numbers(1) ORDER BY number DESC NULLS LAST LIMIT 1), to_YYYYMM(min(A)) FROM table(xp_d)) WHERE B > NULL; -- { serverError 8 }
...
DROP STREAM IF EXISTS trace_log;
CREATE STREAM trace_log
(
   `event_date` Date,
   `event_time` DateTime,
