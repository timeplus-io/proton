...
${CLICKHOUSE_CLIENT} --query="DROP STREAM IF EXISTS kill_mutation"

${CLICKHOUSE_CLIENT} --query="create stream kill_mutation(d date, x uint32, s string) ENGINE MergeTree ORDER BY x PARTITION BY d"
...
${CLICKHOUSE_CLIENT} --query="ALTER STREAM kill_mutation DELETE WHERE to_uint32(s) = 1 SETTINGS mutations_sync = 1" 2>/dev/null
...
${CLICKHOUSE_CLIENT} --query="ALTER STREAM kill_mutation DELETE WHERE to_uint32(s) = 1"
${CLICKHOUSE_CLIENT} --query="ALTER STREAM kill_mutation DELETE WHERE x = 1 SETTINGS mutations_sync = 1" 2>&1 | grep -o "happened during execution of mutations 'mutation_4.txt, mutation_5.txt'" | head -n 1
...
# waiting	test	kill_mutation	mutation_4.txt	DELETE WHERE to_uint32(s) = 1
...
${CLICKHOUSE_CLIENT} --query="ALTER STREAM kill_mutation DELETE WHERE x = 1 SETTINGS mutations_sync = 1"
...
${CLICKHOUSE_CLIENT} --query="DROP STREAM kill_mutation"
