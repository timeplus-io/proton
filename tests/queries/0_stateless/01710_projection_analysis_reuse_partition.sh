...
${CLICKHOUSE_CLIENT} -q "create stream t(s low_cardinality(string), e DateTime64(3), projection p1 (select * order by s, e)) engine MergeTree partition by to_YYYYMM(e) order by s settings index_granularity = 8192, index_granularity_bytes = '100M'"
${CLICKHOUSE_CLIENT} -q "insert into t select 'AAP', to_DateTime('2023-07-01') + 360 * number from numbers(50000)"
${CLICKHOUSE_CLIENT} -q "insert into t select 'AAPL', to_DateTime('2023-07-01') + 360 * number from numbers(50000)"
