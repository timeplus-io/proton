...
create stream bad_date_time (time Datetime('Europe/Moscow'), count uint16) Engine = MergeTree() ORDER BY (time);
...
select to_date(time, 'UTC') dt, min(to_datetime(time, 'UTC')), max(to_datetime(time, 'UTC')), sum(count) from bad_date_time where to_date(time, 'UTC') = '2020-12-19' group by dt;
