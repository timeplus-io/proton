...
create stream dist_01758 as system.one engine=Distributed(test_cluster_two_shards, system, one, dummy);
select * from dist_01758 where dummy = 0 format Null;
" |& grep -o "StorageDistributed (dist_01758).*"

$CLICKHOUSE_CLIENT -q "drop stream dist_01758" 2>/dev/null
