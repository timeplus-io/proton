...
INSERT INTO test_extract (str) WITH range(8) as range_arr, array_map(x-> concat(to_string(x),'Id'), range_arr) as key, array_map(x -> rand() % 8, range_arr) as val, arrayStringConcat(array_map((x,y) -> concat(x,'=',to_string(y)), key, val),',') as str SELECT str FROM numbers(500000);

ALTER STREAM test_extract ADD COLUMN `15Id` Nullable(uint16) DEFAULT toUInt16OrNull(arrayFirst((v, k) -> (k = '4Id'), arr[2], arr[1]));
