...
$CLICKHOUSE_CLIENT -q "create stream test_lc (a LowCardinality(string)) Engine = MergeTree order by tuple()"
...
$CLICKHOUSE_CLIENT -q "create stream test_lc (a LowCardinality(Nullable(string))) Engine = MergeTree order by tuple()"
