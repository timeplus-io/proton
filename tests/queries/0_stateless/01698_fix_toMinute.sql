/* toDateTime or to_string or other functions which should call the to_minute() function will all meet this bug. tests below will verify the toDateTime and to_string. */
SELECT 'Check the bug causing situation: the special Australia/Lord_Howe time zone. toDateTime and to_string functions are all tested at once';
...
SELECT to_unix_timestamp(x) as tt, (to_datetime('2020-10-04 01:40:00', 'Australia/Lord_Howe') + INTERVAL number * 600 SECOND) AS x, timezoneOffset(x) as res,(to_datetime(to_string(x), 'UTC') - x ) AS calc FROM numbers(576) where res != calc;
SELECT to_unix_timestamp(x) as tt, (to_datetime('2019-04-07 01:00:00', 'Australia/Lord_Howe') + INTERVAL number * 600 SECOND) AS x, timezoneOffset(x) as res,(to_datetime(to_string(x), 'UTC') - x ) AS calc FROM numbers(576) where res != calc;
