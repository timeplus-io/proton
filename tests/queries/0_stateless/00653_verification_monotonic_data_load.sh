...
${CLICKHOUSE_CLIENT} --query="create stream fixed_string_test_table (val FixedString(1)) ENGINE = MergeTree ORDER BY val SETTINGS index_granularity = 1, index_granularity_bytes = 0, min_bytes_for_wide_part = 0;"
...
${CLICKHOUSE_CLIENT} --query="SELECT count() FROM fixed_string_test_table WHERE to_uint64(val) == 0;" 2>&1 |grep -q "3 marks to read from 1 ranges" && echo "no monotonic int case: FixedString -> uint64"
