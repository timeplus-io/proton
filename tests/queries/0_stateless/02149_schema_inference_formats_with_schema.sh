...
do
    ...
    $CLICKHOUSE_CLIENT -q "select to_float32(number * 1.2) as float32, toFloat64(number / 1.3) as float64, to_decimal32(number / 0.3, 5) as decimal32, to_decimal64(number / 0.003, 5) as decimal64 from numbers(2) format $format" > $DATA_FILE
    $CLICKHOUSE_CLIENT -q "desc file('$FILE_NAME', '$format')"
    $CLICKHOUSE_CLIENT -q "select * from file('$FILE_NAME', '$format')"
...
$CLICKHOUSE_CLIENT -q "select to_float32(number * 1.2) as float32, toFloat64(number / 1.3) as float64 from numbers(2) format Avro" > $DATA_FILE
$CLICKHOUSE_CLIENT -q "desc file('$FILE_NAME', 'Avro')"
$CLICKHOUSE_CLIENT -q "select * from file('$FILE_NAME', 'Avro')"
