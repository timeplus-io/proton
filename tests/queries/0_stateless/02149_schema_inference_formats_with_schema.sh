...
USER_FILES_PATH=$(clickhouse-client --query "select _path,_file from file('nonexist.txt', 'CSV', 'val1 string')" 2>&1 | grep Exception | awk '{gsub("/nonexist.txt","",$9); print $9}')
...
do
    ...
    $CLICKHOUSE_CLIENT -q "select to_date(number) as date, to_date32(number) as date32 from numbers(2) format $format" > $DATA_FILE
    $CLICKHOUSE_CLIENT -q "desc file('$FILE_NAME', '$format')"
    $CLICKHOUSE_CLIENT -q "select * from file('$FILE_NAME', '$format')"
    ...
    $CLICKHOUSE_CLIENT -q "select [number, number + 1] as array, (number, to_string(number)) as tuple, map_cast(to_string(number), number) as map from numbers(2) format $format" > $DATA_FILE
    $CLICKHOUSE_CLIENT -q "desc file('$FILE_NAME', '$format')"
    $CLICKHOUSE_CLIENT -q "select * from file('$FILE_NAME', '$format')"

    $CLICKHOUSE_CLIENT -q "select [([number, number + 1], map_cast('42', number)), ([], map_cast()), ([42], map_cast('42', 42))] as nested1, (([[number], [number + 1], []], map_cast(number, [(number, '42'), (number + 1, '42')])), 42) as nested2 from numbers(2) format $format" > $DATA_FILE
    $CLICKHOUSE_CLIENT -q "desc file('$FILE_NAME', '$format')"
    $CLICKHOUSE_CLIENT -q "select * from file('$FILE_NAME', '$format')"
done
