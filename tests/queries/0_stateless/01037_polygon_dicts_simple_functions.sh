...
create stream test_01037.polygons_array (key array(array(array(array(float64)))), name string, value uint64) ;
...
DROP STREAM IF EXISTS test_01037.polygons_tuple;

create stream test_01037.polygons_tuple (key array(array(array(tuple(float64, float64)))), name string, value uint64) ;
...
create stream test_01037.points (x float64, y float64, def_i uint64, def_s string) ;
...
do
    ...
    select 'dictGetOrDefault', 'test_01037.dict_array' as dict_name, tuple(x, y) as key,
        ...
        dictGetOrDefault(dict_name, 'value', key, to_uint64(1234)) from test_01037.points order by x, y;
    select 'dictGetOrDefault', 'test_01037.dict_array' as dict_name, tuple(x, y) as key,
...
DROP STREAM test_01037.polygons_array;
DROP STREAM test_01037.polygons_tuple;
