...
drop stream if exists dist_01756;
drop stream if exists dist_01756_str;
drop stream if exists dist_01756_column;
drop stream if exists data_01756_str;
...
-- │                       0 │                       1 │
-- └─────────────────────────┴─────────────────────────┘
create stream dist_01756 as system.one engine=Distributed(test_cluster_two_shards, system, one, intHash64(dummy));
...
select * from dist_01756 where dummy in (to_uint8(0)); -- { serverError 507 }
...
-- Literal is NULL
select (dummy IN (to_uint8(2),)), * from dist_01756 where dummy in (0, 2) format Null;

-- different type
select 'different types -- prohibited';
create stream data_01756_str (key string) engine=Memory();
create stream dist_01756_str as data_01756_str engine=Distributed(test_cluster_two_shards, currentDatabase(), data_01756_str, cityHash64(key));
...
create stream dist_01756_column as system.one engine=Distributed(test_cluster_two_shards, system, one, dummy);
...
drop stream dist_01756;
drop stream dist_01756_str;
drop stream dist_01756_column;
drop stream data_01756_str;
