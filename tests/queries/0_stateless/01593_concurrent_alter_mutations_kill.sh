...
function alter_thread
{
    while true; do
        ...
        if [ "$TYPE" == "string" ]; then
            $CLICKHOUSE_CLIENT --query "ALTER STREAM concurrent_mutate_kill MODIFY COLUMN value uint64 SETTINGS replication_alter_partitions_sync=2"
        else
            $CLICKHOUSE_CLIENT --query "ALTER STREAM concurrent_mutate_kill MODIFY COLUMN value string SETTINGS replication_alter_partitions_sync=2"
        fi
    done
}
...
counter=0
while true; do
    if $CLICKHOUSE_CLIENT --query "ALTER STREAM concurrent_mutate_kill MODIFY COLUMN value int64 SETTINGS replication_alter_partitions_sync=2" 2> /dev/null ; then
        break
    fi
...
$CLICKHOUSE_CLIENT --query "SHOW create stream concurrent_mutate_kill"
$CLICKHOUSE_CLIENT --query "OPTIMIZE STREAM concurrent_mutate_kill FINAL"
