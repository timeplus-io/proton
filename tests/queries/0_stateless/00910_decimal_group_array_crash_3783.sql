...
select group_array(s) from (select sum(n) s from (select to_decimal128(1, 10) as n));
...
select group_array(s) from (select sum(n) s from (select to_decimal128(number, 10) as n from numbers(1000)));
...
select s.a, s.b, max(s.dt1) dt1, s.c, s.d, s.f, s.i, max(s.dt2) dt2 from (
    select to_uint64(4360430)                   a
        ...
        , 'txt'                                c
        , to_decimal128('274.350000000000', 12) d
        , to_decimal128(268.970000000000, 12)   f
        , to_decimal128(0.000000000000, 12)     i
    ...
    select to_uint64(4341757)                   a
        ...
        , 'txt'                                c
        , to_decimal128('321.380000000000', 12) d
        , to_decimal128(315.080000000000, 12)   f
        , to_decimal128(0.000000000000, 12)     i
        , to_datetime('2018-11-02 00:00:00', 'Europe/Moscow')    dt2
    union all
    select to_uint64(4360430)                   a
