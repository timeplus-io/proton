...
$CLICKHOUSE_CLIENT --query "DROP STREAM IF EXISTS database_for_dict.table_for_dict"
$CLICKHOUSE_CLIENT --query "create stream database_for_dict.table_for_dict (key_column uint64, second_column uint64, third_column string) ENGINE = MergeTree() ORDER BY key_column"
