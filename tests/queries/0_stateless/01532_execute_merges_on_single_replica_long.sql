...
DROP STREAM IF EXISTS execute_on_single_replica_r1 NO DELAY;
DROP STREAM IF EXISTS execute_on_single_replica_r2 NO DELAY;

/* that test requires fixed zookeeper path, so we cannot use ReplicatedMergeTree({database}) */
create stream execute_on_single_replica_r1 (x uint64) ENGINE=ReplicatedMergeTree('/clickhouse/tables/test_01532/execute_on_single_replica', 'r1') ORDER BY tuple() SETTINGS execute_merges_on_single_replica_time_threshold=10;
create stream execute_on_single_replica_r2 (x uint64) ENGINE=ReplicatedMergeTree('/clickhouse/tables/test_01532/execute_on_single_replica', 'r2') ORDER BY tuple() SETTINGS execute_merges_on_single_replica_time_threshold=10;
...
OPTIMIZE STREAM execute_on_single_replica_r1 FINAL;
SYSTEM SYNC REPLICA execute_on_single_replica_r2;

/* all_0_0_2 - will be merged by r1, and downloaded by r2 */
OPTIMIZE STREAM execute_on_single_replica_r2 FINAL;
...
OPTIMIZE STREAM execute_on_single_replica_r1 FINAL SETTINGS replication_alter_partitions_sync=0;
...
ALTER STREAM execute_on_single_replica_r1 MODIFY SETTING execute_merges_on_single_replica_time_threshold=0;
ALTER STREAM execute_on_single_replica_r2 MODIFY SETTING execute_merges_on_single_replica_time_threshold=0;
...
DROP STREAM execute_on_single_replica_r1 NO DELAY;
DROP STREAM execute_on_single_replica_r2 NO DELAY;
