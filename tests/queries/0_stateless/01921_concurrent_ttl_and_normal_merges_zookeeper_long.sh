...
for i in $(seq 1 $NUM_REPLICAS); do
    $CLICKHOUSE_CLIENT --query "DROP STREAM IF EXISTS ttl_table$i" &
done

wait

for i in $(seq 1 $NUM_REPLICAS); do
    $CLICKHOUSE_CLIENT -n --query "create stream ttl_table$i(
        key DateTime
    )
...
function optimize_thread
{
    while true; do
        REPLICA=$(($RANDOM % 5 + 1))
        $CLICKHOUSE_CLIENT --query "OPTIMIZE STREAM ttl_table$REPLICA FINAl"
    done
}
...
for i in $(seq 1 $NUM_REPLICAS); do
    ...
    $CLICKHOUSE_CLIENT --query "ALTER STREAM ttl_table$i MODIFY SETTING max_replicated_merges_with_ttl_in_queue=0"
done
check_replication_consistency "ttl_table" "count(), sum(to_uint64(key))"
