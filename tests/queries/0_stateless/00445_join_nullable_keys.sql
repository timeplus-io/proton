...
FROM
(
    SELECT nullIf(number, 7) AS k, to_string(number) AS a FROM system.numbers LIMIT 10
) js1
ANY INNER JOIN
(
...
ANY LEFT JOIN
(
    SELECT nullIf(number, 8) AS k, to_string(number) AS b FROM system.numbers LIMIT 5, 10
) js2 USING (k) ORDER BY k;
