...
declare -A engines
engines[0]="MergeTree"
engines[1]="ReplicatedMergeTree('/test/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/{shard}/src', '{replica}_' || to_string(randConstant()))"
engines[2]="ReplicatedMergeTree('/test/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/{shard}/src_' || to_string(randConstant()), '{replica}')"

for ((i=0; i<16; i++)) do
    $CLICKHOUSE_CLIENT -q "create stream dst_$i (p uint64, k uint64, v uint64)
    ...
    $CLICKHOUSE_CLIENT -q "create stream src_$i (p uint64, k uint64, v uint64) ENGINE=$engine
          PARTITION BY p % 10 ORDER BY k" 2>&1| grep -Pv "Retrying createReplica|created by another server at the same moment, will retry|is already started to be removing" 2>&1 &
done
wait
...
#    while true; do
#        REPLICA=$(($RANDOM % 16))
#        $CLICKHOUSE_CLIENT -q "DROP STREAM src_$REPLICA;"
#        arr=("$@")
#        engine=${arr[$RANDOM % ${#arr[@]}]}
#        $CLICKHOUSE_CLIENT -q "create stream src_$REPLICA (p uint64, k uint64, v uint64) ENGINE=$engine PARTITION BY p % 10 ORDER BY k"
#        sleep 0.$RANDOM;
#    done
#}
...
function move_partition_src_dst_thread()
{
    while true; do
        ...
        PARTITION=$(($RANDOM % 10))
        $CLICKHOUSE_CLIENT -q "ALTER STREAM src_$FROM_REPLICA MOVE PARTITION $PARTITION TO TABLE dst_$TO_REPLICA" 2>/dev/null
        sleep 0.$RANDOM;
    done
}
...
function replace_partition_src_src_thread()
{
    while true; do
        ...
        TO_REPLICA=$(($RANDOM % 16))
        PARTITION=$(($RANDOM % 10))
        $CLICKHOUSE_CLIENT -q "ALTER STREAM src_$TO_REPLICA REPLACE PARTITION $PARTITION FROM src_$FROM_REPLICA" 2>/dev/null
        sleep 0.$RANDOM;
    done
}
...
function drop_partition_thread()
{
    while true; do
        REPLICA=$(($RANDOM % 16))
        PARTITION=$(($RANDOM % 10))
        $CLICKHOUSE_CLIENT -q "ALTER STREAM dst_$REPLICA DROP PARTITION $PARTITION" 2>/dev/null
        sleep 0.$RANDOM;
    done
}
...
function optimize_thread()
{
  while true; do
        ...
        $CLICKHOUSE_CLIENT -q "OPTIMIZE STREAM ${TABLE}_$REPLICA" 2>/dev/null
        sleep 0.$RANDOM;
    done
}
...
function drop_part_thread()
{
    while true; do
        ...
        $CLICKHOUSE_CLIENT -q "ALTER STREAM dst_$REPLICA DROP PART '$part'" 2>/dev/null
        sleep 0.$RANDOM;
    done
}
...
for ((i=0; i<16; i++)) do
    $CLICKHOUSE_CLIENT -q "DROP STREAM dst_$i" 2>&1| grep -Fv "is already started to be removing" &
    $CLICKHOUSE_CLIENT -q "DROP STREAM IF EXISTS src_$i" 2>&1| grep -Fv "is already started to be removing" &
done
wait
