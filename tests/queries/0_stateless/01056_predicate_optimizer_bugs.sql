...
EXPLAIN SYNTAX SELECT k, v, d, i FROM (SELECT t.1 AS k, t.2 AS v, runningDifference(v) AS d, runningDifference(cityHash64(t.1)) AS i FROM (   SELECT array_join([('a', 1), ('a', 2), ('a', 3), ('b', 11), ('b', 13), ('b', 15)]) AS t)) WHERE i = 0;
SELECT k, v, d, i FROM (SELECT t.1 AS k, t.2 AS v, runningDifference(v) AS d, runningDifference(cityHash64(t.1)) AS i FROM (   SELECT array_join([('a', 1), ('a', 2), ('a', 3), ('b', 11), ('b', 13), ('b', 15)]) AS t)) WHERE i = 0;
...
create stream t1 (id uint32, value1 string ) ENGINE ReplacingMergeTree() ORDER BY id;
create stream t2 (id uint32, value2 string ) ENGINE ReplacingMergeTree() ORDER BY id;
create stream t3 (id uint32, value3 string ) ENGINE ReplacingMergeTree() ORDER BY id;
...
create stream A (ts DateTime, id string, id_b string) ENGINE = MergeTree PARTITION BY to_start_of_hour(ts) ORDER BY (ts,id);
create stream B (ts DateTime, id string, id_c string) ENGINE = MergeTree PARTITION BY to_start_of_hour(ts) ORDER BY (ts,id);

EXPLAIN SYNTAX SELECT ts, id, id_b, b.ts, b.id, id_c FROM (SELECT ts, id, id_b FROM A) AS a ALL LEFT JOIN B AS b ON b.id = a.id_b WHERE a.ts <= to_datetime('1970-01-01 03:00:00');
EXPLAIN SYNTAX SELECT ts AS `--a.ts`, id AS `--a.id`, id_b AS `--a.id_b`, b.ts AS `--b.ts`, b.id AS `--b.id`, id_c AS `--b.id_c` FROM (SELECT ts, id, id_b FROM A) AS a ALL LEFT JOIN B AS b ON `--b.id` = `--a.id_b` WHERE `--a.ts` <= to_datetime('1970-01-01 03:00:00');
...
create stream test ( A int32, B int32 ) ();
...
EXPLAIN SYNTAX SELECT * FROM (SELECT * FROM system.one) WHERE array_map(x -> x + 1, [dummy]) = [1];
SELECT * FROM (SELECT * FROM system.one) WHERE array_map(x -> x + 1, [dummy]) = [1];

EXPLAIN SYNTAX SELECT *  FROM (SELECT 1 AS id, 2 AS value) INNER JOIN (SELECT 1 AS id, 3 AS value_1) USING id WHERE array_map(x -> x + value + value_1, [1]) = [6];
SELECT *  FROM (SELECT 1 AS id, 2 AS value) INNER JOIN (SELECT 1 AS id, 3 AS value_1) USING id WHERE array_map(x -> x + value + value_1, [1]) = [6];
