...
with client(name='client1>', log=log) as client1, client(name='client2>', log=log) as client2:
    ...
    client1.expect(prompt)
    client1.send('create stream test.mt (time DateTime, location string, temperature uint32) Engine=MergeTree order by tuple()')
    client1.expect(prompt)
    client1.send('CREATE LIVE VIEW test.lv AS SELECT to_start_of_day(time) AS day, location, avg(temperature) FROM test.mt GROUP BY day, location ORDER BY day, location')
    client1.expect(prompt)
