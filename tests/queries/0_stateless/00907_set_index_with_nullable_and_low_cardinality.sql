...
drop stream if exists nullable_set_index;
create stream nullable_set_index (a uint64, b LowCardinality(Nullable(string)), INDEX b_index b TYPE set(0) GRANULARITY 8192) engine = MergeTree order by a;
insert into nullable_set_index values (1, 'a');
insert into nullable_set_index values (2, 'b');
select * from nullable_set_index where b = 'a';
select '-';
...
drop stream if exists nullable_set_index;
create stream nullable_set_index (a uint64, b LowCardinality(Nullable(string)), INDEX b_index b TYPE set(1) GRANULARITY 8192) engine = MergeTree order by a;
insert into nullable_set_index values (1, 'a');
insert into nullable_set_index values (2, 'b');
select * from nullable_set_index where b = 'a';
select '-';
