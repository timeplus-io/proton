...
{
    while true; do
        $CLICKHOUSE_CLIENT -n -q "DROP STREAM IF EXISTS test_table_$1 SYNC;
            create stream test_table_$1 (a uint8) ENGINE = ReplicatedMergeTree('/clickhouse/tables/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/alter_table', 'r_$1') ORDER BY tuple();" 2>&1 |
...
for i in {1,2}; do $CLICKHOUSE_CLIENT -q "DROP STREAM IF EXISTS test_table_$i"; done
