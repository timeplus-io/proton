...
$CLICKHOUSE_CLIENT --query "DROP STREAM IF EXISTS replicated_mutation_table"

$CLICKHOUSE_CLIENT --query "
    create stream replicated_mutation_table(
        date date,
        key uint64,
        value string
    )
...
$CLICKHOUSE_CLIENT --query "INSERT INTO replicated_mutation_table SELECT to_date('2019-10-02'), number, '42' FROM numbers(4)"

$CLICKHOUSE_CLIENT --query "INSERT INTO replicated_mutation_table SELECT to_date('2019-10-02'), number, 'Hello' FROM numbers(4)"

$CLICKHOUSE_CLIENT --query "ALTER STREAM replicated_mutation_table UPDATE key = key + 1 WHERE sleepEachRow(1) == 0 SETTINGS mutations_sync = 2" 2>&1 | grep -o 'Mutation 0000000000 was killed' | head -n 1 &
...
$CLICKHOUSE_CLIENT --query "ALTER STREAM replicated_mutation_table MODIFY COLUMN value uint64 SETTINGS replication_alter_partitions_sync = 2" 2>&1 | grep -o "Cannot parse string 'Hello' as uint64" | head -n 1 &
...
$CLICKHOUSE_CLIENT --query "ALTER STREAM replicated_mutation_table MODIFY COLUMN value string SETTINGS replication_alter_partitions_sync = 2"
