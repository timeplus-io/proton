...
create_query="create stream subcolumns(n Nullable(uint32), a1 array(uint32),\
...
for engine in "${ENGINES[@]}"; do
    echo $engine
    $CLICKHOUSE_CLIENT --query "DROP STREAM IF EXISTS subcolumns"
...
$CLICKHOUSE_CLIENT -q "DROP STREAM subcolumns"
