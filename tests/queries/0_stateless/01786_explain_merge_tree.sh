...
$CLICKHOUSE_CLIENT -q "create stream test_index (x uint32, y uint32, z uint32, t uint32, index t_minmax t % 20 TYPE minmax GRANULARITY 2, index t_set t % 19 type set(4) granularity 2) engine = MergeTree order by (x, y) partition by (y, bitAnd(z, 3), int_div(t, 15)) settings index_granularity = 2, min_bytes_for_wide_part = 0"
