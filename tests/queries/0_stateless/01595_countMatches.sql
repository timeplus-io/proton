...
select countMatches(concat(to_string(number), 'foofoo'), 'foo') from numbers(2);
...
select countMatchesCaseInsensitive(concat(to_string(number), 'Foofoo'), 'foo') from numbers(2);
...
select countMatches('foobarfoo', to_string(number)) from numbers(1); -- { serverError 44; }
