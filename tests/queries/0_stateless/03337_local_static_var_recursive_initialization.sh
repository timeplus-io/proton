...
out=$($CLICKHOUSE_LOCAL --log-level=test 'select throwIf(1) -- { serverError FUNCTION_THROW_IF_VALUE_IS_NON_ZERO }' 2>&1 || true)
echo "$out" | grep -F -q "Exception" || { echo "$out"; exit 1; }
echo "$out" | grep -F -q "__cxa_guard_acquire detected recursive initialization" && { echo "$out"; exit 1; }
