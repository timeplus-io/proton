...
select count() FROM test1 where to_date(p) = '2020-09-01' and sipHash64(to_string(to_date(p))) % 2 = 0;
-- non-optimized (some predicate depends on non-partition_expr columns)
select count() FROM test1 where to_date(p) = '2020-09-01' and k = 2; -- { serverError 158; }
-- optimized
select count() from test1 where to_date(p) > '2020-09-01';
-- non-optimized
...
-- optimized
select count() from test_tuple where to_date(p) > '2020-09-01';
-- optimized
select count() from test_tuple where to_date(p) > '2020-09-01' and i = 1;
-- optimized
select count() from test_tuple where i > 1;
-- optimized
select count() from test_tuple where i < 1;
-- non-optimized
select count() from test_tuple array join [p,p] as c where to_date(p) = '2020-09-01'; -- { serverError 158; }
select count() from test_tuple array join [1,2] as c where to_date(p) = '2020-09-01' settings max_rows_to_read = 4;
-- non-optimized
select count() from test_tuple array join [1,2,3] as c where to_date(p) = '2020-09-01'; -- { serverError 158; }
select count() from test_tuple array join [1,2,3] as c where to_date(p) = '2020-09-01' settings max_rows_to_read = 6;

create stream test_two_args(i int, j int, k int) engine MergeTree partition by i + j order by k settings index_granularity = 1;
