...
sqlite3 "${DB_PATH}" 'DROP STREAM IF EXISTS table1'
sqlite3 "${DB_PATH}" 'DROP STREAM IF EXISTS table2'
sqlite3 "${DB_PATH}" 'DROP STREAM IF EXISTS table3'
sqlite3 "${DB_PATH}" 'DROP STREAM IF EXISTS table4'
sqlite3 "${DB_PATH}" 'DROP STREAM IF EXISTS table5'

sqlite3 "${DB_PATH}" 'create stream table1 (col1 text, col2 smallint);'
sqlite3 "${DB_PATH}" 'create stream table2 (col1 int, col2 text);'
...
sqlite3 "${DB_PATH}" 'create stream table3 (col1 text, col2 int);'
...
sqlite3 "${DB_PATH}" 'create stream table4 (a int, b integer, c tinyint, d smallint, e mediumint, bigint, int2, int8)'
sqlite3 "${DB_PATH}" 'create stream table5 (a character(20), b varchar(10), c real, d double, e double precision, f float)'
...
${CLICKHOUSE_CLIENT} --query="SHOW create stream ${CURR_DATABASE}.table1;" | sed -r 's/(.*SQLite)(.*)/\1/'
...
${CLICKHOUSE_CLIENT} --query="SHOW create stream ${CURR_DATABASE}.table4;" | sed -r 's/(.*SQLite)(.*)/\1/'
${CLICKHOUSE_CLIENT} --query="SHOW create stream ${CURR_DATABASE}.table5;" | sed -r 's/(.*SQLite)(.*)/\1/'

${CLICKHOUSE_CLIENT} --query="DROP DATABASE IF EXISTS ${CURR_DATABASE}"


${CLICKHOUSE_CLIENT} --query="select 'create stream engine with table3'";
${CLICKHOUSE_CLIENT} --query='DROP STREAM IF EXISTS sqlite_table3'
${CLICKHOUSE_CLIENT} --query="create stream sqlite_table3 (col1 string, col2 int32) ENGINE = SQLite('${DB_PATH}', 'table3')"

${CLICKHOUSE_CLIENT} --query='SHOW create stream sqlite_table3;' | sed -r 's/(.*SQLite)(.*)/\1/'
...
sqlite3 "${DB_PATH2}" 'DROP STREAM IF EXISTS table1'
sqlite3 "${DB_PATH2}" 'create stream table1 (col1 text, col2 smallint);'
