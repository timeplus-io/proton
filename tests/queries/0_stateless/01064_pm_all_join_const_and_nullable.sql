...
SELECT count(1), uniq_exact(1) FROM (
SELECT materialize(to_nullable(1)) as k FROM numbers(1) nums
JOIN (SELECT materialize(1) AS k, number n FROM numbers(100000)) j
USING k);
...
SELECT count(1), uniq_exact(1) FROM (
SELECT to_nullable(1) as k FROM numbers(1) nums
JOIN (SELECT materialize(1) AS k, number n FROM numbers(100000)) j
USING k);
...
SELECT count(1), uniq_exact(1) FROM (
SELECT materialize(1) as k FROM numbers(1) nums
JOIN (SELECT materialize(to_nullable(1)) AS k, number n FROM numbers(100000)) j
USING k);

SELECT count(1), uniq_exact(1) FROM (
SELECT materialize(1) as k FROM numbers(1) nums
JOIN (SELECT to_nullable(1) AS k, number n FROM numbers(100000)) j
USING k);
