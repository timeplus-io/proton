...
$CLICKHOUSE_CLIENT -q "select to_int32(number % 2 ? number : NULL) as int, to_uint64(number % 2 ? NULL : number) as uint, to_float32(number) as float, concat('Str: ', to_string(number)) as str, [[number, number + 1], [number]] as arr, map(number, [number, number + 1]) as map from numbers(3) format MsgPack" > $DATA_FILE
