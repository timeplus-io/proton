...
function thread1()
{
    while true; do $CLICKHOUSE_CLIENT -n --query "create stream test_01320.r (x uint64) ENGINE = ReplicatedMergeTree('/test/$CLICKHOUSE_TEST_ZOOKEEPER_PREFIX/table', 'r') ORDER BY x; DROP STREAM test_01320.r;"; done
}
