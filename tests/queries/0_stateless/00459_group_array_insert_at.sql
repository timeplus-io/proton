SELECT groupArrayInsertAt(to_string(number), number * 2) FROM (SELECT * FROM system.numbers LIMIT 10);
SELECT groupArrayInsertAt('-')(to_string(number), number * 2) FROM (SELECT * FROM system.numbers LIMIT 10);
...
SELECT k, groupArrayInsertAt('-', 10)(to_string(x), x) FROM (SELECT number AS k, number AS x FROM system.numbers LIMIT 11) GROUP BY k ORDER BY k;
