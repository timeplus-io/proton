...
create stream bloom_filter_not_has (ary array(LowCardinality(Nullable(string))), d date, INDEX idx_ary ary TYPE bloom_filter(0.01) GRANULARITY 1024) ENGINE = MergeTree() PARTITION BY d ORDER BY d;
