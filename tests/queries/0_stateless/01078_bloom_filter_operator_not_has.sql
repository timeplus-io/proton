DROP STREAM IF EXISTS bloom_filter_not_has;

create stream bloom_filter_not_has (ary array(low_cardinality(nullable(string))), d date, INDEX idx_ary ary TYPE bloom_filter(0.01) GRANULARITY 1024) ENGINE = MergeTree() PARTITION BY d ORDER BY d;
