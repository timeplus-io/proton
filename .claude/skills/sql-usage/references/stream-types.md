...
| `replication_factor` | 1 (single) / 3 (cluster) | HA replicas |
...
Row-based with PRIMARY KEY (required). Similar to MySQL table. Supports UPDATE, UPSERT, DELETE.
...
| Setting | Default | Purpose |
|---------|---------|---------|
| `shards` | 1 | Parallel distribution |
| `replication_factor` | 1 / 3 | HA replicas |
| `version_column` | "" | Store only latest version by column |
| `coalesced` | false | Coalesced mode |
| `ttl_seconds` | -1 (no TTL) | Row time-to-live |
| `flush_rows` | 100000 | Row threshold for RocksDB flush |
| `flush_ms` | 30000 | Time threshold for RocksDB flush |
| `kvstore_codec` | — | RocksDB compression: `snappy`, `lz4`, `zstd` |
| `logstore_codec` | none | WAL compression |
| `ack` | — | Acknowledgment: `quorum`, `local`, `none` |
| `ingest_mode` | — | Ingestion method: `sync`, `async` |

### Operations

```sql
-- Insert / upsert (same key = upsert)
INSERT INTO user_profiles VALUES (1, 'Alice', now());

-- Delete (fast with primary/secondary index predicates)
DELETE FROM user_profiles WHERE user_id = 1;

-- Auto-increment column
CREATE MUTABLE STREAM auto_incr(
    id uint64 AUTO_INCREMENT, p string
) PRIMARY KEY (p);
```
...
Note: Do not set TTL on historical storage for versioned streams. Enterprise users should prefer mutable stream for better performance.
...
Types: `kafka`, `pulsar`, `timeplus` (Enterprise), `log` (experimental).
...
| Updates/upserts, point queries | mutable |
| Version history, ASOF JOINs | versioned_kv |
| CDC, track deletes | changelog_kv |
| General KV workload | mutable (first choice) |
