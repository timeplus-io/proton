...
Right side is a mutable dimension; latest version auto-selected.
...
## 5. Bidirectional JOIN (mutable + mutable)
...
FROM left_mutable JOIN right_mutable
    ON left_mutable.k = right_mutable.kk
GROUP BY k;
```
...
| mutable | mutable | INNER, LEFT, FULL | ALL | Both sides build hash tables |
| stream | stream (self) | INNER | ALL | `date_diff_within()` required |
| any | mutable | LEFT, INNER | ALL | `join_algorithm='direct'` optional |
