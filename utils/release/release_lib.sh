...
function get_version {
    ...
    VERSION_POSTFIX_TAG="${VERSION_POSTFIX}"

    gen_version_string
}

# Generate rc number.
function gen_rc_version {
    if [ -z "$VERSION_RC" ]; then
        BASEDIR=$(dirname "${BASH_SOURCE[0]}")/../../
        VERSION_RC=`grep "SET(VERSION_DESCRIBE" ${BASEDIR}/cmake/autogenerated_versions.txt | sed 's/^.*VERSION_DESCRIBE \(.*\)/\1/' | sed 's/[) ].*//' | sed -n 's/.*-rc\.\([0-9]\+\)$/\1/p'`

        # no rc suffix in autogenerated_versions.txt
        if [ -z "$VERSION_RC" ]; then
            VERSION_RC=-1
        fi
    fi

    # current version is a release candidate, no need to bump it
    ...
    if [[ $STANDALONE != 'yes' ]]; then
        ...
        while [ $succeeded -eq 0 ] && [ $attempts -le $max_attempts ]; do
            ...
            if [ "$TYPE" == "major" ]; then
                ...
                # Enable VERSION_REVISION incrementation for checkpointing
                    VERSION_REVISION=$(($VERSION_REVISION + 1))
                # fi
            ...
            git_tag_grep=`git tag | grep -x "$VERSION_PREFIX$VERSION_STRING$VERSION_POSTFIX_TAG"`
        ...
        git_log_grep=`git log --oneline --max-count=1 | grep "$auto_message"` | grep -v "Revert"
        if [ "$git_log_grep" == "" ] || [ "$VERSION_RC" != "-1" ]; then
...
function gen_changelog {
    ...
    sed \
        ...
        -e "s/[@]DATE[@]/$CHDATE/g" \
        -e "s/[@]AUTHOR[@]/$AUTHOR/g" \
        -e "s/[@]EMAIL[@]/$(whoami)@timeplus.io/g" \
        < $CHLOG.in > $CHLOG
    # proton: ends.
}
