...
{
    static constexpr auto name = "extract_key_value_pairs";
};

struct NameExtractKeyValuePairsWithEscaping
{
    static constexpr auto name = "extract_key_value_pairs_with_escaping";
};
...
REGISTER_FUNCTION(ExtractKeyValuePairs)
{
    factory.registerFunction<ExtractKeyValuePairs<NameExtractKeyValuePairs, false>>(
        FunctionDocumentation{
            ...
            **Syntax**
            ``` sql
            extract_key_value_pairs(data, [key_value_delimiter], [pair_delimiter], [quoting_character])
            ```
            ...
            **Simple case**
            ``` sql
            arthur :) select extract_key_value_pairs('name:neymar, age:31 team:psg,nationality:brazil') as kv

            SELECT extract_key_value_pairs('name:neymar, age:31 team:psg,nationality:brazil') as kv
            ...
            **Single quote as quoting character**
            ``` sql
            arthur :) select extract_key_value_pairs('name:\'neymar\';\'age\':31;team:psg;nationality:brazil,last_key:last_value', ':', ';,', '\'') as kv

            SELECT extract_key_value_pairs('name:\'neymar\';\'age\':31;team:psg;nationality:brazil,last_key:last_value', ':', ';,', '\'') as kv
            ...
            **Escape sequences without escape sequences support**
            ``` sql
            arthur :) select extract_key_value_pairs('age:a\\x0A\\n\\0') as kv

            SELECT extract_key_value_pairs('age:a\\x0A\\n\\0') AS kv
    ...
    factory.registerFunction<ExtractKeyValuePairs<NameExtractKeyValuePairsWithEscaping, true>>(
        FunctionDocumentation{
            .description=R"(Same as `extract_key_value_pairs` but with escaping support.
            ...
            - Pre-escaping: Pre-escaping it will output: `a: "aaaa"bbb"` and `extract_key_value_pairs` will then output: `a: aaaa`
            - Post-escaping: `extract_key_value_pairs` will output `a: aaaa\` and post-escaping will keep it as it is.

            Leading escape sequences will be skipped in keys and will be considered invalid for values.

            **Escape sequences with escape sequence support turned on**
            ``` sql
            arthur :) select extract_key_value_pairs_with_escaping('age:a\\x0A\\n\\0') as kv

            SELECT extract_key_value_pairs_with_escaping('age:a\\x0A\\n\\0') AS kv
    ...
    factory.registerAlias("map_from_string", NameExtractKeyValuePairs::name, FunctionFactory::CaseInsensitive);
}
