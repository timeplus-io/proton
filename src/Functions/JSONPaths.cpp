...
{
    static constexpr auto name = "json_all_paths";
...
{
    static constexpr auto name = "json_all_paths_with_types";
    static constexpr auto paths_mode = PathsMode::ALL_PATHS;
...
{
    static constexpr auto name = "json_dynamic_paths";
...
{
    static constexpr auto name = "json_dynamic_paths_with_types";
    static constexpr auto paths_mode = PathsMode::DYNAMIC_PATHS;
    static constexpr auto with_types = true;
};
...
{
    static constexpr auto name = "json_shared_data_paths";
...
{
    static constexpr auto name = "json_shared_data_paths_with_types";
    static constexpr auto paths_mode = PathsMode::SHARED_DATA_PATHS;
    static constexpr auto with_types = true;
};
...
SELECT json, json_all_paths(json) FROM test;
)",
            R"(
┌─json─────────────────────────────────┬─json_all_paths(json)─┐
│ {"a":"42"}                           │ ['a']                │
│ {"b":"Hello"}                        │ ['b']                │
...
Returns the list of all paths and their data types stored in each row in JSON column.
        ...
        .syntax = {"json_all_paths_with_types(json)"},
        .arguments = {{"json", "JSON column"}},
        .examples = {{{
            "Example",
            R"(
CREATE TABLE test (json JSON(max_dynamic_paths=1)) ENGINE = Memory;
INSERT INTO test FORMAT JSONEachRow {"json" : {"a" : 42}}, {"json" : {"b" : "Hello"}}, {"json" : {"a" : [1, 2, 3], "c" : "2020-01-01"}}
SELECT json, json_all_paths_with_types(json) FROM test;
)",
            R"(
┌─json─────────────────────────────────┬─json_all_paths_with_types(json)───────────────┐
│ {"a":"42"}                           │ {'a':'int64'}                                 │
│ {"b":"Hello"}                        │ {'b':'string'}                                │
...
Returns the list of dynamic paths that are stored as separate subcolumns in JSON column.
        ...
        .syntax = {"json_dynamic_paths(json)"},
        .arguments = {{"json", "JSON column"}},
        .examples = {{{
            "Example",
            R"(
CREATE TABLE test (json JSON(max_dynamic_paths=1)) ENGINE = Memory;
INSERT INTO test FORMAT JSONEachRow {"json" : {"a" : 42}}, {"json" : {"b" : "Hello"}}, {"json" : {"a" : [1, 2, 3], "c" : "2020-01-01"}}
SELECT json, json_dynamic_paths(json) FROM test;
)",
            R"(
┌─json─────────────────────────────────┬─json_dynamic_paths(json)─┐
│ {"a":"42"}                           │ ['a']                    │
│ {"b":"Hello"}                        │ []                       │
...
Returns the list of dynamic paths that are stored as separate subcolumns and their types in each row in JSON column.
        ...
        .syntax = {"json_dynamic_paths_with_types(json)"},
        .arguments = {{"json", "JSON column"}},
        .examples = {{{
            "Example",
            R"(
CREATE TABLE test (json JSON(max_dynamic_paths=1)) ENGINE = Memory;
INSERT INTO test FORMAT JSONEachRow {"json" : {"a" : 42}}, {"json" : {"b" : "Hello"}}, {"json" : {"a" : [1, 2, 3], "c" : "2020-01-01"}}
SELECT json, json_dynamic_paths_with_types(json) FROM test;
)",
            R"(
┌─json─────────────────────────────────┬─json_dynamic_paths_with_types(json)─┐
│ {"a":"42"}                           │ {'a':'int64'}                       │
│ {"b":"Hello"}                        │ {}                                  │
...
Returns the list of paths that are stored in shared data structure in JSON column.
        ...
        .syntax = {"json_shared_data_paths(json)"},
        .arguments = {{"json", "JSON column"}},
        .examples = {{{
            "Example",
            R"(
CREATE TABLE test (json JSON(max_dynamic_paths=1)) ENGINE = Memory;
INSERT INTO test FORMAT JSONEachRow {"json" : {"a" : 42}}, {"json" : {"b" : "Hello"}}, {"json" : {"a" : [1, 2, 3], "c" : "2020-01-01"}}
SELECT json, json_shared_data_paths(json) FROM test;
)",
            R"(
┌─json─────────────────────────────────┬─json_shared_data_paths(json)─┐
│ {"a":"42"}                           │ []                          │
│ {"b":"Hello"}                        │ ['b']                       │
...
Returns the list of paths that are stored in shared data structure and their types in each row in JSON column.
        ...
        .syntax = {"json_shared_data_paths_with_types(json)"},
        .arguments = {{"json", "JSON column"}},
        .examples = {{{
            "Example",
            R"(
CREATE TABLE test (json json(max_dynamic_paths=1)) ENGINE = Memory;
INSERT INTO test FORMAT JSONEachRow {"json" : {"a" : 42}}, {"json" : {"b" : "Hello"}}, {"json" : {"a" : [1, 2, 3], "c" : "2020-01-01"}}
SELECT json, json_shared_data_paths_with_types(json) FROM test;
)",
            R"(
┌─json─────────────────────────────────┬─json_shared_data_paths_with_types(json)─┐
│ {"a":"42"}                           │ {'a':'int64'}                           │
│ {"b":"Hello"}                        │ {}                                      │
│ {"a":["1","2","3"],"c":"2020-01-01"} │ {'a':'array(nullable(int64))'}          │
└──────────────────────────────────────┴─────────────────────────────────────────┘
