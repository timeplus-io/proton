...
namespace
{
    ...
    public:
        ...
        ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr &, size_t /*input_rows_count*/) const override
        {
            ...
            {
                vectorConstant(col->getChars(), col->getOffsets(), col_pattern->getValue<String>(), 1, vec_res, offsets_res);
            }
            else if (isColumnConst(*column_index))
            {
                const auto * col_const_index = typeid_cast<const ColumnConst *>(&*column_index);
                ssize_t index = col_const_index ? col_const_index->getInt(0) : 1;
