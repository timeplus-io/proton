...
struct BitBoolMaskOrImpl
{
    ...
    static inline Result apply([[maybe_unused]] A left, [[maybe_unused]] B right)
    {
        if constexpr (!std::is_same_v<A, ResultType> || !std::is_same_v<B, ResultType>)
            ...
            throw DB::Exception("It's a bug! Only uint8 type is supported by __bit_bool_mask_or.", ErrorCodes::BAD_ARGUMENTS);
...
struct NameBitBoolMaskOr { static constexpr auto name = "__bit_bool_mask_or"; };
