...
public:
    ...
    static constexpr auto name = "ulid_string_to_datetime";
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        if (!arg_string && !(arg_fixed_string && arg_fixed_string->getN() == ULID_LENGTH))
            throw Exception(
                ...
                "Illegal type {} of argument of function {}. Must be string or fixed_string(26).",
        ...
        if (column_fixed_string)
        {
            if (column_fixed_string->getN() != ULID_LENGTH)
                throw Exception(
                    ...
                    "Illegal column {} of argument of function {}, expected string or fixed_string({})",
...
REGISTER_FUNCTION(ulid_string_to_datetime)
{
...
The function expects the ULID to be provided as the first argument, which can be either a string or a fixed_string(26) data type.
An optional second argument can be passed to specify a timezone for the timestamp.
)",
            .examples{
                {"ulid", "SELECT ulid_string_to_datetime(generate_ulid())", ""},
                {"timezone", "SELECT ulid_string_to_datetime(generate_ulid(), 'Asia/Istanbul')", ""}
            },
            .category = "ULID"
        });
}
