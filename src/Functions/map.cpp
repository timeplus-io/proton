...
public:
    ...
    DataTypePtr getReturnTypeImpl(const DataTypes & arguments) const override
    {
        ...
        /// proton : starts
        if (!arguments.empty()
            && (checkAndGetDataType<DataTypeArray>(arguments[0].get()) || checkAndGetDataType<DataTypeMap>(arguments[0].get())))
    ...
    ColumnPtr executeImpl(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const override
    {
        /// proton : starts
        if (!arguments.empty()
            && (checkAndGetDataType<DataTypeArray>(arguments[0].type.get()) || checkAndGetDataType<DataTypeMap>(arguments[0].type.get())))
            /// Proxy to map_from_arrays
