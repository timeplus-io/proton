...
ArraysDepths getArraysDepths(const ColumnsWithTypeAndName & arguments)
{
            ...
            if (depth_column && isColumnConst(*depth_column))
            {
                ...
                if (!value)
                    throw Exception(ErrorCodes::BAD_ARGUMENTS,
                                    "Incorrect arguments for function array_enumerate_uniq_ranked "
                                    "or array_enumerate_dense_ranked: depth ({}) cannot be less or equal 0.",
                                    std::to_string(value));
                ...
                else
                {
                    if (depths.size() >= array_num)
                        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                                        "Incorrect arguments for function array_enumerate_uniq_ranked "
                                        "or array_enumerate_dense_ranked: depth ({}) for missing array.",
                                        std::to_string(value));
    ...
    if (depths.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS,
                        "Incorrect arguments for function array_enumerate_uniq_ranked or array_enumerate_dense_ranked: "
