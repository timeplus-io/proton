...
{
public:
    static constexpr auto name = "dynamic_type";
...
{
public:
    static constexpr auto name = "is_dynamic_element_in_shared_data";
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        return DataTypeFactory::instance().get("bool");    /// proton: updates
    }
...
Returns true for rows in dynamic column that are not separated into subcolumns and stored inside shared variant in binary form.
        ...
        .syntax = {"is_dynamic_element_in_shared_data(dynamic)"},
        .arguments = {{"dynamic", "dynamic column"}},
        .examples = {{{
            "Example",
            R"(
...
INSERT INTO test VALUES (NULL), (42), ('Hello, World!'), ([1, 2, 3]);
SELECT d, is_dynamic_element_in_shared_data(d) FROM test;
)",
            R"(
┌─d─────────────┬─is_dynamic_element_in_shared_data(d)─┐
│ ᴺᵁᴸᴸ          │ false                                │
│ 42            │ false                                │
│ Hello, World! │ true                                 │
│ [1,2,3]       │ true                                 │
