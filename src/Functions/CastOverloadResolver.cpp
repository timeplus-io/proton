...
public:
    ...
    explicit CastOverloadResolverImpl(ContextPtr context_, std::optional<CastDiagnostic> diagnostic_, bool keep_nullable_)
        : context(context_)
    ...
    static FunctionOverloadResolverPtr createImpl(ContextPtr context, std::optional<CastDiagnostic> diagnostic = {}, bool keep_nullable = false)
    {
        assert(!internal || !keep_nullable);
        return std::make_unique<CastOverloadResolverImpl>(context, std::move(diagnostic), keep_nullable);
    }

    static FunctionOverloadResolverPtr createImpl(std::optional<CastDiagnostic> diagnostic = {}, bool keep_nullable = false)
    {
        assert(!internal || !keep_nullable);
