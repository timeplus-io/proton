...
namespace DB
{
    ...
    public:
        ...
        DataTypePtr getReturnTypeImpl(const DataTypes & arguments) const override
        {
            ...
            if (!(sketch_type0 && sketch_type0->getFunctionName() == "uniq_theta"))
                throw Exception(
                        "First argument for function " + getName() + " must be a uniq_theta but it has type " + arguments[0]->getName(),
                        ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT);

            const auto * sketch_type1 = typeid_cast<const DataTypeAggregateFunction *>(arguments[1].get());
            if (!(sketch_type1 && sketch_type1->getFunctionName() == "uniq_theta"))
                throw Exception(
                        "Second argument for function " + getName() + " must be a uniq_theta but it has type " + arguments[1]->getName(),
                        ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT);
            ...
            if (arg_data_types0.size() != arg_data_types1.size())
                throw Exception(
                        "The nested type in uniq_thetas must be the same length, but one is " + std::to_string(arg_data_types0.size())
            ...
            for (size_t i = 0; i < types_size; ++i)
            {
                if (!arg_data_types0[i]->equals(*arg_data_types1[i]))
                    throw Exception(
                            "The " + std::to_string(i) + "th nested type in uniq_thetas must be the same, but one is " + arg_data_types0[i]->getName()
    ...
    {
        static constexpr auto name = "uniq_theta_intersect";
    };

    struct NameUniqThetaUnion
    {
        static constexpr auto name = "uniq_theta_union";
    };

    struct NameUniqThetaNot
    {
        static constexpr auto name = "uniq_theta_not";
    };
