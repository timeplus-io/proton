...
If the argument can't be parsed as URL, the function returns an empty string.
        ...
        Documentation::Examples{{"domainWithoutWWW", "SELECT domain_without_www('https://www.clickhouse.com')"}},
        Documentation::Categories{"URL"}
    });
    factory.registerFunction<FunctionDomainWithoutWWWRFC>(
    {
        R"(Similar to `domain_without_www` but follows stricter rules to be compatible with RFC 3986 and less performant.)",
