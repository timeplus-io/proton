...
struct NameTopLevelDomainRFC { static constexpr auto name = "top_level_domain_rfc"; };
...
Returns an empty string if the argument cannot be parsed as a URL or does not contain a top-level domain.
        ...
        Documentation::Examples{{"topLevelDomain", "SELECT top_level_domain('svn+ssh://www.some.svn-hosting.com:80/repo/trunk')"}},
        Documentation::Categories{"URL"}
    });

    factory.registerFunction<FunctionTopLevelDomainRFC>(
    {
        R"(Similar to top_level_domain, but conforms to RFC 3986.)",
