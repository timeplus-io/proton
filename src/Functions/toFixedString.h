...
public:
    ...
    DataTypePtr getReturnTypeImpl(const ColumnsWithTypeAndName & arguments) const override
    {
        ...
        if (!isStringOrFixedString(arguments[0].type))
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "{} is only implemented for types string and fixed_string", getName());
        ...
        if (const auto * column_string = checkAndGetColumn<ColumnString>(column.get()))
        {
            ...
            for (size_t i = 0; i < in_offsets.size(); ++i)
            {
                ...
                if (len > n)
                {
                    ...
                    {
                        throw Exception(ErrorCodes::TOO_LARGE_STRING_SIZE, "String too long for type fixed_string({})", toString(n));
                    }
                    else
                    {
