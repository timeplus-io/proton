...
public:
    ...
    String getName() const override { return "function_expression"; }
...
public:
    ...
    String getName() const override { return "function_capture"; }
        ...
        : expression_actions(std::move(expression_actions_))
    {
        ...
        for (const auto & action : expression_actions->getActions())
            if (action.node->type == ActionsDAG::ActionType::ARRAY_JOIN)
                throw Exception("Expression with array_join or other unusual action cannot be captured", ErrorCodes::BAD_ARGUMENTS);
