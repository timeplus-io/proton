...
namespace ErrorCodes
{
    ...
    extern const int CANNOT_DELETE_STORAGE_POLICY; /// proton: updated
...
{
    std::vector<String> disk_names; /// proton: updated
    for (size_t index = 0; index < volumes.size(); ++index)
    {
        ...
        for (const auto & disk : volume->getDisks())
        {
            ...
            disk_names.emplace_back(disk_name); /// proton: updated
    ...
    DatabaseCatalog::instance().addDisksInUse(name, disk_names);    /// proton: updated
}
    ...
    DiskSelectorPtr disks)
{
    ...
    /// proton: starts. load storage policy from metastore
    loadStoragePoliciesFromMetaStore(disks);
    /// proton: ends
...
///proton:starts
void StoragePolicySelector::remove(const DB::String & name)
{
    auto it = policies.find(name);
    if (it == policies.end())
        throw Exception(ErrorCodes::UNKNOWN_POLICY, "Unknown storage policy {}", name);

    auto used_by = DatabaseCatalog::instance().getStoragesByStoragePolicy(name);
    if (!used_by.empty())
        throw Exception(ErrorCodes::CANNOT_DELETE_STORAGE_POLICY, "storage policy {} is used by {}, cannot be removed", name, fmt::join(used_by, ", "));

    /// remove from DiskInUse map
    std::vector<String> disk_names;
    auto disks = it->second->getDisks();
    disk_names.reserve(disks.size());
    for(const auto & disk: disks)
        disk_names.push_back(disk->getName());

    DatabaseCatalog::instance().removeDisksInUse(name, disk_names);
