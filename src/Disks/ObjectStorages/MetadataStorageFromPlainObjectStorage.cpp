...
{
    if (metadata_storage.isDirectory(path))
    {
        auto paths = metadata_storage.listDirectory(path);
...
{
    /// FIXME: implement hard link based on object storage
    auto object_from = StoredObject(std::filesystem::path(metadata_storage.getPath()) / path_from);
    auto object_to = StoredObject(std::filesystem::path(metadata_storage.getPath()) / path_to);
    metadata_storage.object_storage->copyObject(object_from, object_to, /*read_settings=*/{}, /*write_settings=*/{});
}
/// proton: ends.
