    ...
    const S3ObjectStorageSettings & settings)
{
    ...
    client_configuration.http_keep_alive_timeout = config.getUInt(config_prefix + ".http_keep_alive_timeout", 5);
    client_configuration.http_keep_alive_max_requests = config.getUInt(config_prefix + ".http_keep_alive_max_requests", 100);
    ...
    if (proxy_config)
    {
        client_configuration.per_request_configuration = [proxy_config]() { return proxy_config->resolve(); };
    ...
    return S3::ClientFactory::instance().create(
        ...
        /* session_token */ "",
        /// SSE-S3 (e.g. "AES256"): set on PUT/Copy/CreateMultipartUpload so buckets that mandate
        /// server-side encryption (x-amz-server-side-encryption header) accept uploads from this disk.
        config.getString(config_prefix + ".server_side_encryption", config.getString("s3.server_side_encryption", "")));
}
