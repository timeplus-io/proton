...
std::pair<int64_t, bool> tryParseSequenceBasedSeek(const String & seek_to)
{
    try
    {
        ...
        if (sn < 0)
            return {-1, false}; /// Sequence number shall be greater than or equal to 0
...
std::pair<SeekToType, int64_t> doParse(const String & seek_to, bool utc)
{
    ...
    /// -1 latest, -2 earliest
    if (seek_to == "latest")
        return {SeekToType::SEQUENCE_NUMBER, cluster::Constants::LatestSN};
    else if (seek_to == "earliest")
        return {SeekToType::SEQUENCE_NUMBER, cluster::Constants::EarliestSN};
    ...
    if (auto [relative_seek_point, relative_valid] = tryParseRelativeTimeSeek(seek_to, utc); relative_valid)
        ...
        throw Exception(
            ...
            "-6m or "
            "-2h or -1d), sequence number based supports to 'earliest' or 'latest' or integer greater than or equal to {}",
            seek_to,
...
void SeekToInfo::replicateForShards(UInt32 shards)
{
...
std::pair<SeekToType, std::vector<int64_t>> SeekToInfo::parse(const String & seek_to_str, bool utc)
{
    if (seek_to_str.empty())
        return {SeekToType::SEQUENCE_NUMBER, std::vector<int64_t>(1, cluster::Constants::LatestSN)};
...
bool SeekToInfo::isAbosolute() const
{
    switch (type)
    {
        case SeekToType::ABSOLUTE_TIME:
        ...
        case SeekToType::SEQUENCE_NUMBER:
        {
            return std::ranges::all_of(seek_points, [](const auto & seek_point) {
                return seek_point != cluster::Constants::LatestSN; /// earliest is treated like absolute
            });
        }
    }
}
