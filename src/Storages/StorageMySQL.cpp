    ...
    const ContextPtr & context_)
{
    ...
    if (columns == tables_and_columns.end())
        throw Exception(ErrorCodes::UNKNOWN_STREAM, "MySQL table {} doesn't exist.",
...
public:
    explicit StorageMySQLSink(
        ...
        std::optional<Block> sample_block) /// proton: updates. Add sample_block argument
        : SinkToStorage(sample_block.has_value() ? sample_block.value() : metadata_snapshot_->getSampleBlock(), ProcessorID::MySQLSinkID)
        , storage{storage_}
    ...
    void writeBlockData(const Block & block)
    {
        ...
        auto writer = FormatFactory::instance().getOutputFormat("Values", sqlbuf, getHeader(), storage.getContext()); /// proton: updates
        writer->write(block);
