...
void registerStorageStream(StorageFactory & factory)
{
    ...
    factory.registerStorage(
        ...
        [](const StorageFactory::Arguments & args) {
            /** * Stream engine arguments : Stream(shards, shard_by_expr)
            * - shards
            * - shard_by_expr
            ...
            */
            ...
            if (!args.attach && (arg_cnt < min_num_params || arg_cnt > max_num_params))
            {
                String msg;
                ...
                throw Exception::createRuntime(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, msg);
            }
            ...
            if (!args.attach && !args.storage_def->order_by)
                throw Exception(
            ...
            if (is_regular_storage())
                metadata.sorting_key
                    = KeyDescription::getSortingKeyFromAST(args.storage_def->order_by->ptr(), metadata.columns, args.getContext(), {});
            else
                /// Add version column to sorting key
                metadata.sorting_key = KeyDescription::getSortingKeyFromAST(
                    args.storage_def->order_by->ptr(),
                    metadata.columns,
                    args.getContext(),
                    {properties->storage_settings->version_column.value});
            ...
            metadata.setVersion(args.schema_version);
