...
namespace DB
{
StreamCallbackData::StreamCallbackData(StreamShardStore * stream_shard_store_, const SequenceRanges & missing_sequence_ranges_)
    : stream_shard_store(stream_shard_store_), missing_sequence_ranges(missing_sequence_ranges_)
{
    assert(stream_shard_store);
}
...
void StreamCallbackData::commit(cluster::SchemaRecordPtrs records)
{
    ...
    else
    {
        ...
        /// to avoid data lost and duplicate data. Duplicate data is enforced later
        ...
        for (auto & records_seqs_pair : range_buckets)
        {
            ...
            LOG_INFO(
                ...
                "Recovery phase. Committing missing_ranges={}",
                sequenceRangesToString(records_seqs_pair.second));
...
inline void StreamCallbackData::doCommit(cluster::SchemaRecordPtrs records, SequenceRanges sequence_ranges)
{
    try
    {
        stream_shard_store->commit(std::move(records), std::move(sequence_ranges));
    }
    catch (...)
    {
        LOG_ERROR(
