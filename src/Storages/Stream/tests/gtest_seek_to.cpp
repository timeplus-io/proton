...
{
    {
        ...
        ASSERT_FALSE(seek_to_info.isTimeBased());
        ASSERT_EQ(timestamps.size(), 1);
        ASSERT_EQ(timestamps[0], cluster::Constants::LatestSN);
    }

    {
        ...
        ASSERT_EQ(timestamps.size(), 2);
        for (auto timestamp : timestamps)
            ASSERT_EQ(timestamp, cluster::Constants::LatestSN);
    }
...

    {
        ...
        ASSERT_FALSE(seek_to_info.isTimeBased());
        ASSERT_EQ(timestamps.size(), 1);
        ASSERT_EQ(timestamps[0], cluster::Constants::EarliestSN);
    }

    {
        ...
        ASSERT_EQ(timestamps[2], cluster::Constants::LatestSN);
    }
}
