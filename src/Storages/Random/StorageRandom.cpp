...
ColumnPtr fillColumnWithRandomData(const DataTypePtr type, UInt64 limit, pcg64 & rng, ContextPtr context)
{
    ...
    switch (idx)
    {
        ...
        default:
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "The 'Generating' is not implemented for type {}", type->getName());
    }
}
...
class GenerateRandomSource final : public Streaming::ISource
{
public:
    GenerateRandomSource(
        ...
        size_t shard_num_)
        : Streaming::ISource(
              Nested::flatten(prepareBlockToFill(block_header_)),
              true,
              getLogger("GenerateRandSource"),
              ProcessorID::GenerateRandomSourceID)
        , max_block_size(max_block_size_)
        , our_columns(our_columns_)
        , context(context_)
        // , events_per_second(events_per_second_)
        , header_chunk(getPort().getHeader().getColumns(), 0)
        ...
        , total_events(total_events_)
    {
        ...
        for (const auto & elem : block_header_)
        {
            if (our_columns.hasDefault(elem.name))
                continue;

            block_to_fill.insert(elem);
        }

        auto dag = evaluateMissingDefaults(block_to_fill, block_header_.getNamesAndTypesList(), our_columns, context, true, false, true);
        if (dag)
        {
    ...
    UInt64 normalMode()
    {
        ...
        if (now_time > boundary_time)
        {
            size_t is_special = index - index / interval_count * interval_count;
        ...
        {
            sleepForMilliseconds(boundary_time - now_time);
        }
...
protected:
    Chunk generate() override
    {
        ...
        {
            LOG_INFO(logger, "Finish generating total_events={}  generated_events={}", total_events, generated_events);
            return {};
        }
    ...
    Chunk doGenerate(UInt64 block_size_)
    {
        ...
        auto block_to_fill_as_result = block_to_fill.cloneWithColumns(columns);
...
using GenerateRandomSourcePtr = std::shared_ptr<GenerateRandomSource>;

GenerateRandomSourcePtr makeGenerateRandomSource(
    UInt64 max_block_size_,
    UInt64 random_seed_,
    Block block_header_,
    const ColumnsDescription & our_columns_,
...
{
    auto source = std::make_shared<GenerateRandomSource>(
        max_block_size_,
        random_seed_,
        block_header_,
        our_columns_,
        ...
        total_events_,
        shard_num_);
    source->setProgressCallback(std::move(callback));
    return source;
}
}
...
StorageRandom::StorageRandom(
    ...
    , metric_time(MonotonicMilliseconds::now())
{
    ...
    auto creator_fn = [](const StorageFactory::Arguments & args) {
        ...
        if (engine_args.size() > 3)
            throw Exception(
                ...
                "Storage Random requires at most three arguments: random_seed, max_string_length, max_array_length.");
        ...
        if (storage_random_settings->shards.value == 0)
            throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "Invalid shards, shall not be 0");
    ...
    size_t /*num_streams*/)
{
    ...
    auto read_callback = [this](const Progress & progress) {
        const auto & value = progress.getValues();
        this->read_bytes += value.read_bytes;
        this->read_rows += value.read_rows;
    };
    ...
    if (eps < shards)
    {
        if (eps == 0)
        {
            ...
            {
                pipes.emplace_back(makeGenerateRandomSource(
                    max_block_size,
                    generate(),
                    block_header,
                    our_columns,
...
Int64 StorageRandom::getMetricTime(bool reset)
{
    if (reset)
...
UInt64 StorageRandom::readBytes(bool reset, [[maybe_unused]] bool external_ingress)
{
    if (reset)
        return read_bytes.exchange(0, std::memory_order_relaxed);
    else
        return read_bytes.load(std::memory_order_relaxed);
}

UInt64 StorageRandom::readRows(bool reset, [[maybe_unused]] bool external_ingress)
{
    if (reset)
