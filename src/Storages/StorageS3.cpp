...
StorageS3Source::StorageS3Source(
    ...
    : ISource(info.source_header, false, ProcessorID::StorageS3SourceID)
    , WithContext(context_)
    , name(std::move(name_))
    , bucket(bucket_)
    , version_id(version_id_)
...
public:
    ...
    void onCancel() noexcept override
    {
        ...
        {
            tryLogCurrentException(getLogger("StorageS3Sink"), "Failed to finalize");
        }
        cancelled = true;
    }
...
bool StorageS3::parallelizeOutputAfterReading(ContextPtr context_) const
{
    return FormatFactory::instance().checkParallelizeOutputAfterReading(configuration.format, context_);
}
            ...
            if (key_with_info.key.empty())
            {
                if (first)
                    throw Exception(
                        ...
                        "in S3 or all files are empty. You must specify stream structure manually",
...
/// proton: starts
/// Disabled S3 storage engine in favor of S3 external stream.
#if 0
