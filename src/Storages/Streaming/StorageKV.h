...
/** A StorageKV is an table engine that uses merge tree and replicated via
  ...
  */
class StorageKV final : public shared_ptr_helper<StorageKV>, public MergeTreeData
{
    friend struct shared_ptr_helper<StorageKV>;
...
public:
    ...
    using StreamKVShardPtrs = std::vector<std::shared_ptr<StreamKVShard>>;
    struct ShardsToRead
    {
        QueryMode mode;
        StreamKVShardPtrs local_shards; /// Also as streaming shards when mode is STREAMING || STREAMING_CONCAT
        StreamKVShardPtrs remote_shards;
...
public:
    ...
    const StreamKVShardPtrs & getStreamShards() const { return stream_shards; }

    /// Used for validity check before background ingestion of materialized view
    /// @native_log or @kafka_log become not nullptr only if StorageKV::startup completes.
    ...
    friend class StreamKVSink;
    friend class MergeTreeData;
    friend class StorageEmbeddedRocksDB;
...
private:
    ...
    StreamKVShardPtrs stream_shards; /// All shards
    StreamKVShardPtrs local_shards; /// Only local shards
    StreamKVShardPtrs remote_shards; /// Only remote virtual shards
