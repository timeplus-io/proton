...
#include <Storages/Streaming/StreamKVSink.h>
...
String makeFormattedNameOfStreamingShards(const StorageKV::StreamKVShardPtrs & streaming_shards)
{
    WriteBufferFromOwnString buf;
...
String makeFormattedNameOfLocalShards(const StorageKV::StreamKVShardPtrs & local_shards)
{
    WriteBufferFromOwnString buf;
...
String makeFormattedNameOfConcatShards(const StorageKV::StreamKVShardPtrs & concat_shards)
{
    WriteBufferFromOwnString buf;
    ...
    , max_outstanding_blocks(context_->getSettingsRef().aysnc_ingest_max_outstanding_blocks)
{
    /// Init StreamShard
    {
        ...
        for (auto shard : host_shards)
            stream_shards.push_back(std::make_shared<StreamKVShard>(
                replication_factor,
                shards,
                shard,
                table_id_,
    ...
    for (Int32 shard_id = 0; shard_id < shards; ++shard_id)
    {
        ...
        {
            stream_shards.emplace_back(std::make_shared<StreamKVShard>(
                replication_factor,
                shards,
                shard_id,
                table_id_,
...
NamesAndTypesList StorageKV::getVirtuals() const
{
    return virtual_column_names_and_types;
}

NamesAndTypesList StorageKV::getVirtualsHistory() const
{
...
void StorageKV::readConcat(
    const StreamKVShardPtrs & shards_to_read,
    QueryPlan & query_plan,
    SelectQueryInfo & query_info,
    Names column_names,
    const StorageSnapshotPtr & storage_snapshot,
    ...
    size_t num_streams)
{
    ...
    for (const auto & stream_shard : shards_to_read)
    {
        auto plan = std::make_unique<QueryPlan>();

        assert(stream_shard->storage_rocksdb);
        stream_shard->storage_rocksdb->read(
            column_names,
            stream_shard->storage_rocksdb->getStorageSnapshot(storage_snapshot->metadata, context_),
            query_info,
            context_,
            processed_stage,
            max_block_size,
...
String StorageKV::getName() const
{
    return "Stream";
}

bool StorageKV::isRemote() const
{
...
StorageKV::ShardsToRead StorageKV::getRequiredShardsToRead(ContextPtr context_, const SelectQueryInfo & query_info) const
{
    ShardsToRead result;
...
bool StorageKV::supportsParallelInsert() const
{
    return true;
}

bool StorageKV::supportsIndexForIn() const
{
    return true;
}

bool StorageKV::supportsSubcolumns() const
{
    return true;
}

std::optional<UInt64> StorageKV::totalRows(const Settings & settings) const
{
...
std::optional<UInt64> StorageKV::totalRowsByPartitionPredicate(const SelectQueryInfo & query_info, ContextPtr context_) const
{
    std::optional<UInt64> rows;
    for (const auto & stream_shard : stream_shards)
    {
...
std::optional<UInt64> StorageKV::totalBytes(const Settings & settings) const
{
...
SinkToStoragePtr StorageKV::write(const ASTPtr & /*query*/, const StorageMetadataPtr & metadata_snapshot, ContextPtr context_)
{
    checkReady();
    ...
    return std::make_shared<StreamKVSink>(*this, metadata_snapshot, context_);
}

void StorageKV::checkTableCanBeDropped(ContextPtr context_) const
{
...
void StorageKV::preRename(const StorageID & new_table_id)
{
    if (native_log)
    {
        const auto & storage_id = getStorageID();
...
void StorageKV::alter(const AlterCommands & commands, ContextPtr context_, AlterLockHolder & alter_lock_holder)
{
    for (auto & stream_shard : stream_shards)
    {
        if (stream_shard->storage)
...
void StorageKV::mutate(const MutationCommands & commands, ContextPtr context_)
{
    for (auto & stream_shard : stream_shards)
        if (stream_shard->storage)
...
std::vector<MergeTreeMutationStatus> StorageKV::getMutationsStatus() const
{
...
CancellationCode StorageKV::killMutation(const String & mutation_id)
{
    for (auto & stream_shard : stream_shards)
    {
        if (stream_shard->storage)
...
ActionLock StorageKV::getActionLock(StorageActionBlockType action_type)
{
...
void StorageKV::onActionLockRemove(StorageActionBlockType action_type)
{
    if (action_type != ActionLocks::StreamConsume)
    {
        for (auto & stream_shard : stream_shards)
...
CheckResults StorageKV::checkData(const ASTPtr & query, ContextPtr context_)
{
...
bool StorageKV::scheduleDataProcessingJob(BackgroundJobsAssignee & assignee)
{
    for (auto & stream_shard : stream_shards)
    {
        if (stream_shard->storage)
...
QueryProcessingStage::Enum StorageKV::getQueryProcessingStage(
    ContextPtr context_,
...
StorageSnapshotPtr StorageKV::getStorageSnapshot(const StorageMetadataPtr & metadata_snapshot, ContextPtr query_context) const
{
...
void StorageKV::dropPartNoWaitNoThrow(const String & part_name)
{
    for (auto & stream_shard : stream_shards)
        if (stream_shard->storage)
...
void StorageKV::dropPart(const String & part_name, bool detach, ContextPtr context_)
{
    for (auto & stream_shard : stream_shards)
        if (stream_shard->storage)
...
void StorageKV::dropPartition(const ASTPtr & partition, bool detach, ContextPtr context_)
{
    for (auto & stream_shard : stream_shards)
        if (stream_shard->storage)
...
StorageKV::attachPartition(const ASTPtr & partition, const StorageMetadataPtr & metadata_snapshot, bool part, ContextPtr context_)
{
...
void StorageKV::replacePartitionFrom(const StoragePtr & source_table, const ASTPtr & partition, bool replace, ContextPtr context_)
{
    for (auto & stream_shard : stream_shards)
        if (stream_shard->storage)
...
void StorageKV::movePartitionToTable(const StoragePtr & dest_table, const ASTPtr & partition, ContextPtr context_)
{
    for (auto & stream_shard : stream_shards)
        if (stream_shard->storage)
...
bool StorageKV::partIsAssignedToBackgroundOperation(const DataPartPtr & part) const
{
    for (const auto & stream_shard : stream_shards)
    {
        if (stream_shard->storage)
...
MutationCommands StorageKV::getFirstAlterMutationCommandsForPart(const DataPartPtr & part) const
{
...
void StorageKV::startBackgroundMovesIfNeeded()
{
    for (const auto & stream_shard : stream_shards)
        if (stream_shard->storage)
...
std::unique_ptr<StreamSettings> StorageKV::getDefaultSettings() const
{
    return std::make_unique<StreamSettings>(getContext()->getStreamSettings());
}

/// Distributed query related functions
ClusterPtr StorageKV::getCluster() const
{
...
ClusterPtr StorageKV::skipUnusedShards(
...
ClusterPtr
StorageKV::getOptimizedCluster(ContextPtr context_, const StorageSnapshotPtr & storage_snapshot, const ASTPtr & query_ptr) const
{
...
QueryProcessingStage::Enum StorageKV::getQueryProcessingStageRemote(
    ContextPtr context_,
    QueryProcessingStage::Enum to_stage,
    const StorageSnapshotPtr & storage_snapshot,
    SelectQueryInfo & query_info) const
...
/// New functions
IColumn::Selector StorageKV::createSelector(const ColumnWithTypeAndName & result) const
{
...
IColumn::Selector StorageKV::createSelector(const Block & block) const
{
...
const ExpressionActionsPtr & StorageKV::getShardingKeyExpr() const
{
    return sharding_key_expr;
}

size_t StorageKV::getRandomShardIndex() const
{
    std::lock_guard lock(rng_mutex);
    return std::uniform_int_distribution<size_t>(0, shards - 1)(rng);
}

Int32 StorageKV::getNextShardIndex() const
{
    return next_shard++ % shards;
}

void StorageKV::append(
...
inline void StorageKV::appendToNativeLog(
...
inline void StorageKV::appendToKafka(
    nlog::RecordPtr & record,
    IngestMode ingest_mode,
    klog::AppendCallback callback,
    klog::CallbackData data,
    ...
    try
    {
        ...
        switch (ingest_mode)
        {
            case IngestMode::ASYNC: {
                ...
                auto data_wrapper = std::make_shared<AppendAsyncCallbackData>(
                    AppendAsyncCallbackData{shared_from_this(), std::move(callback), std::move(data), base_block_id, sub_block_id});
                static auto callback_wrapper = [](const auto & result, const auto & data_wrapper_) {
                    ...
                    auto & storage = pdata.storage->as<StorageKV &>();
...
void StorageKV::getIngestionStatuses(const std::vector<UInt64> & block_ids, std::vector<IngestingBlocks::IngestStatus> & statuses) const
{
    if (kafka_log)
        kafka_log->ingesting_blocks.getStatuses(block_ids, statuses);
}

UInt64 StorageKV::nextBlockId() const
{
...
void StorageKV::poll(Int32 timeout_ms)
{
    if (kafka_log)
        kafka_log->log->poll(timeout_ms, kafka_log->append_ctx);
}

std::vector<std::pair<Int32, Int64>> StorageKV::lastCommittedSequences() const
{
...
void StorageKV::cacheVirtualColumnNamesAndTypes()
{
...
void StorageKV::updateLogStoreCodec(const String & settings_codec)
{
