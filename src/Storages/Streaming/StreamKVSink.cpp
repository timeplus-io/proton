...
StreamKVSink::StreamKVSink(StorageKV & storage_, const StorageMetadataPtr metadata_snapshot_, ContextPtr query_context_)
    : SinkToStorage(
...
BlocksWithShard StreamKVSink::doShardBlock(Block block) const
{
...
BlocksWithShard StreamKVSink::shardBlock(Block block) const
{
    Int32 shard = 0;
    if (storage.shards > 1)
    {
...
inline IngestMode StreamKVSink::getIngestMode() const
{
...
void StreamKVSink::consume(Chunk chunk)
{
...
void StreamKVSink::checkpoint(CheckpointContextPtr ckpt_ctx)
{
