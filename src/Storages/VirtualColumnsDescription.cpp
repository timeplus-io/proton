...
namespace ErrorCodes
{
    ...
    extern const int NO_SUCH_COLUMN_IN_STREAM;    /// proton: updates
}
...
NameAndTypePair VirtualColumnsDescription::get(const String & name, VirtualsKind kind) const
{
    ...
    if (!column)
        throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_STREAM, "There is no virtual column {}", name);    /// proton: updates
    return *column;
}
