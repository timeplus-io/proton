...
* Definitions:
    ...
    - `data_version` - optional value, incremented when a part is mutated (again, mutated data is always only written to a new part, since parts are immutable)
        ...
        {"last_removal_attemp_time",                    std::make_shared<DataTypeDateTime>(), "The time when the last removal attempt was made."},
        {"removal_state",                               std::make_shared<DataTypeString>(), "The state of the part removal."},
    }
    )
{
}
