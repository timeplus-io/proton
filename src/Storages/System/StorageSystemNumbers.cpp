...
class NumbersSource final : public ISource
{
public:
    NumbersSource(UInt64 block_size_, UInt64 offset_, UInt64 step_)
        : ISource(createHeader(), true, ProcessorID::NumbersSourceID), block_size(block_size_), next(offset_), step(step_) {}
...
class NumbersMultiThreadedSource final : public ISource
{
public:
    NumbersMultiThreadedSource(NumbersMultiThreadedStatePtr state_, UInt64 block_size_, UInt64 max_counter_)
        : ISource(createHeader(), true, ProcessorID::NumbersMultiThreadedSourceID)
