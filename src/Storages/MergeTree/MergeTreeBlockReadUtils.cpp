    ...
    Names & columns)
{
    ...
    for (size_t i = 0; i < columns.size(); ++i)
    {
        ...
        if (!storage_snapshot->tryGetColumn(options, columns[i]))
            throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_STREAM, "There is no physical column or subcolumn {} in stream", columns[i]);
    ...
    bool with_subcolumns)
{
    ...
    if (prewhere_info)
    {
        ...
        if (prewhere_info->row_level_filter)
        {
            ...
            injectRequiredColumns(data_part_info_for_reader, storage_snapshot, with_subcolumns, row_filter_column_names);
