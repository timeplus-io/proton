...
namespace
{
    struct VirtualColumnsInserter
    {
        ...
        /// proton: starts.
        virtual void insertInt32Column(const ColumnPtr & column, const String & name)
        {
    ...
    if (!virtual_columns.empty())
    {
        ...
        for (const auto & virtual_column_name : virtual_columns)
        {
            ...
            else if (virtual_column_name == ProtonConsts::RESERVED_SHARD)
            {
                ColumnPtr column;
                if (rows)
                    column = DataTypeInt32().createColumnConst(rows, part->storage.shardID())->convertToFullColumnIfConst();
                else
