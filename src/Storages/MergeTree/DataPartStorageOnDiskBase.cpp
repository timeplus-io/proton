    ...
    LoggerPtr log) const
{
    ...
    {
        disk->createDirectories(to);
        src_disk->copyDirectoryContent(getRelativePath(), disk, path_to_clone, read_settings, write_settings);
        src_disk->removeFileIfExists(fs::path(path_to_clone) / "delete-on-destroy.txt");
    }
    catch (...)
    {
