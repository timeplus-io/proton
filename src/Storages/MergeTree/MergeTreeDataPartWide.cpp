                ...
                getSerialization(name_type.name)->enumerateStreams([&](const ISerialization::SubstreamPath & substream_path)
                {
                    ...
                    if (!checksums.files.contains(mrk_file_name))
                        throw Exception(
                            ...
                            mrk_file_name, name_type.name, fullPath(volume->getDisk(), path));

                    if (!checksums.files.contains(bin_file_name))
                        throw Exception(
                            ErrorCodes::NO_FILE_IN_DATA_PART,
                            "No {} file checksum for column {} in part ",
                            bin_file_name, name_type.name, fullPath(volume->getDisk(), path));
                });
            }
        }
    }
                ...
                if (volume->getDisk()->exists(file_path))
                {
                    ...
                    if (!file_size)
                        throw Exception(
                            ...
                            path,
                            fullPath(volume->getDisk(), file_path));
                    ...
                    if (!marks_size)
                        ...
                        throw Exception(
                            ErrorCodes::BAD_SIZE_OF_FILE_IN_DATA_PART,
                            "Part {} is broken: marks have different sizes.", path);
                }
            });
        }
    }
