...
/// proton: starts
MergeTreeDataPartWriterPtr createMergeTreeDataPartInMemoryWriter(
    ...
    const MergeTreeIndexGranularity & computed_index_granularity)
{
    if (part_type == MergeTreeDataPartType::Compact)
        return createMergeTreeDataPartCompactWriter(data_part_name_, logger_name_, serializations_, data_part_storage_,
            index_granularity_info_, storage_settings_, columns_list, column_positions, metadata_snapshot, indices_to_recalc,
        ...
        return createMergeTreeDataPartWideWriter(data_part_name_, logger_name_, serializations_, data_part_storage_,
            index_granularity_info_, storage_settings_, columns_list, metadata_snapshot, indices_to_recalc,
            marks_file_extension_, default_codec_, writer_settings, computed_index_granularity);
    /// proton: starts
    else if (part_type == MergeTreeDataPartType::InMemory)
        return createMergeTreeDataPartInMemoryWriter(
            data_part,
            data_part_name_,
            serializations_,
            data_part_storage_,
