    ...
    , mode(mode_)
{
    ...
    extension.all_callback(
        InitialAllRangesAnnouncement{.description = parts_ranges.getDescriptions(), .replica_num = extension.number_of_current_replica});
}
...
MergeTreeReadTaskPtr MergeTreeReadPoolParallelReplicasInOrder::getTask(size_t task_idx, MergeTreeReadTask * previous_task)
{
    ...
    auto response = extension.callback(ParallelReadRequest{
        ...
        .min_number_of_marks = pool_settings.min_marks_for_concurrent_read * request.size(),
        .description = request});
