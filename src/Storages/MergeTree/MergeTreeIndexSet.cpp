...
void MergeTreeIndexBulkGranulesSet::deserializeBinary(size_t granule_num, ReadBuffer & istr, MergeTreeIndexVersion version)
{
    ...
    for (size_t i = 0; i < num_columns; ++i)
    {
        ...
        serializations[i]->deserializeBinaryBulkStatePrefix(settings, state, nullptr);
        serializations[i]->deserializeBinaryBulkWithMultipleStreams(column, rows_to_read, settings, state, nullptr);
    ...
    std::unordered_map<const ActionsDAG::Node *, const ActionsDAG::Node *> & node_to_result_node) const
{
    ...
    else if (const auto * atom_node_ptr = atomFromDAG(node, result_dag, context))
    {
        ...
        {
            auto bit_wrapper_function = FunctionFactory::instance().get("__bit_wrapper_func", context);
    ...
    std::unordered_map<const ActionsDAG::Node *, const ActionsDAG::Node *> & node_to_result_node) const
{
    ...
    if (function_name == "not")
    {
        ...
        auto bit_swap_last_two_function = FunctionFactory::instance().get("__bit_swap_last_two", context);
        return &result_dag->addFunction(bit_swap_last_two_function, {argument}, {});
    }
    else if (function_name == "and" || function_name == "index_hint" || function_name == "or")
    {
        if (arguments_size < 1)
            return nullptr;
        ...
        if (function_name == "and" || function_name == "index_hint")
            function = FunctionFactory::instance().get("__bit_bool_mask_and", context);
        else
            function = FunctionFactory::instance().get("__bit_bool_mask_or", context);
