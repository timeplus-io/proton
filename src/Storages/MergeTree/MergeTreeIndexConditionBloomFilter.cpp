        ...
        if (functionIsInOrGlobalInOperator(function_name))
        {
            if (auto future_set = rhs_argument.tryGetPreparedSet(); future_set)
            {
                if (auto prepared_set = future_set->buildOrderedSetInplace(rhs_argument.getTreeContext().getQueryContext()); prepared_set)
                {
                 ...
                 function_name == "not_equals" ||
                 function_name == "has" ||
                 function_name == "map_contains" ||
                 function_name == "index_of" ||
                 function_name == "has_any" ||
                 function_name == "has_all")
        {
            Field const_value;
            DataTypePtr const_type;
    ...
    RPNElement & out)
{
    ...
    if (key_node.isFunction())
    {
        ...
        if (which.isTuple() && key_node_function_name == "tuple_cast")
        {
        ...
        if (key_node_function_name == "array_element")
        {
            ...
            auto map_keys_index_column_name = fmt::format("map_keys({})", column_name);
            auto map_values_index_column_name = fmt::format("map_values({})", column_name);
            ...
            {
                /// For map_values we serialize set with bloom filter
...
static bool indexOfCanUseBloomFilter(const RPNBuilderTreeNode * parent)
{
    ...
    if (function_name == "and")
    {
        ...
        || function_name == "greater" || function_name == "greater_or_equals"
        || function_name == "less" || function_name == "less_or_equals")
    {
        ...
        else if (function_name == "not_equals" && constant_equal_zero)
        {
            /// indexOf(...) != c, c = 0
            return true;
        }
        ...
        else if (function_name == (reversed ? "less_or_equals" : "greater_or_equals") && applyVisitor(FieldVisitorAccurateLess(), zero, constant_value))
        {
            /// indexOf(...) >= c, c > 0
            return true;
        }
    ...
    const RPNBuilderTreeNode * parent)
{
    ...
    {
        auto map_keys_index_column_name = fmt::format("map_keys({})", key_column_name);
        if (!header.has(map_keys_index_column_name))
            return false;
    ...
    if (key_node.isFunction())
    {
        ...
        if (key_node_function_name == "array_element" && (function_name == "equals" || function_name == "not_equals"))
        {
            /** Try to parse arrayElement for map_keys index.
