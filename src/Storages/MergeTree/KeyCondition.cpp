...
static const std::set<std::string_view> no_relaxed_atom_functions
    = {"not_like", "not_in", "global_not_in", "not_null_in", "global_not_null_in", "not_equals", "not_empty"};
...
void KeyCondition::getAllSpaceFillingCurves()
{
    ...
    for (const auto & action : key_expr->getActions())
    {
        if (action.node->type == ActionsDAG::ActionType::FUNCTION
            ...
            && action.node->function_base->getName() == "morton_encode")
        {
...
KeyCondition::KeyCondition(
    ...
    const ExpressionActionsPtr & key_expr_,
    bool single_point_)
    : key_expr(key_expr_), key_subexpr_names(getAllSubexpressionNames(*key_expr)), single_point(single_point_)
{
    size_t key_index = 0;
    for (const auto & name : key_column_names)
    {
...
static std::set<std::string_view> date_time_parsing_functions = {
    ...
    "parse_datetime_best_effort_us",
    "parse_datetime32_best_effort",
    "parse_datetime64_best_effort",
    "parse_datetime",
        ...
        if (it != key_columns.end())
        {
            ...
            if (is_valid_chain)
            {
                ...
                while (!chain.empty())
                {
                    ...
                    {
                        auto func_or_null = FunctionFactory::instance().get(func_name + "_or_null", context);
                        ColumnsWithTypeAndName arguments;
                        int i = 0;
    ...
    size_t & out_key_column_num)
{
    ...
    if (left_arg.isFunction())
    {
        ...
        if (left_arg_tuple.getFunctionName() == "tuple_cast" && left_arg_tuple.getArgumentsSize() > 1)
        {
            ...
            if (key_arg_pos == 1)
            {
                if (func_name == "less")
                    ...
                    func_name = "greater_or_equals";
                         ...
                         func_name == "starts_with" || func_name == "match")
                {
