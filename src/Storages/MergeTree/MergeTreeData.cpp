...
BackupEntries MergeTreeData::backupDataParts(const DataPartsVector & data_parts)
{
    ...
    for (const auto & part : data_parts)
        part->getDataPartStorage().backup(temp_dirs, part->checksums, part->getFileNamesWithoutChecksums(), backup_entries);
                                                           ...
                                                           SimpleIncrement * increment)
{
    ...
    for (const String & part_name : part_names)
    {
        ...
        auto restore_task = [this,
                             ...
                             data_path_in_backup,
                             part_name,
                             c_part_info = std::move(part_info),
                             c_filenames = std::move(filenames),
                             reservation,
                             increment]()
        {
            ...
            for (const String & filename : c_filenames)
            {
            ...
            auto part = createPart(part_name, *c_part_info, data_part_storage);
