...
void MergeTreeDataPartCompact::loadIndexGranularityImpl(
    ...
    size_t column_count, const IDataPartStorage & data_part_storage_)
{
    ...
    while (!buffer->eof())
    {
        ...
        buffer->seek(column_count * sizeof(MarkInCompressedFile), SEEK_CUR);
        size_t granularity;
        readIntBinary(granularity, *buffer);
        index_granularity_.appendMark(granularity);
    }

    if (index_granularity_.getMarksCount() * index_granularity_info_.getMarkSizeInBytes(column_count) != marks_file_size)
