...
IcebergSink::IcebergSink(
    ...
    , context(context_)
    , logger(&Poco::Logger::get("IcebergSink"))
{
    /// The timer should only start when data start flowing.
    upload_idle_timer.stop();
    next_callback = [this](size_t new_data_size, size_t /*parts_count*/) { current_total_size += new_data_size; };
}
...
void IcebergSink::consume(Chunk chunk)
{
    ...
    if (!writer)
    {
        ...
        } while (DB::S3::objectExists(*s3_configuration.client, s3_configuration.url.bucket, object_key, s3_configuration.url.version_id));
