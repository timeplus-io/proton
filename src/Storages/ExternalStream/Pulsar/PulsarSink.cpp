...
PulsarSink::PulsarSink(
    ...
    , one_message_per_row(pulsar->produceOneMessagePerRow())
    , logger(logger_)
    , external_stream_counter(external_stream_counter_)
{
    ...
    for (const auto & pair : pulsar->properties())
    {
    ...
    rd_kafka_conf_set_dr_msg_cb(conf, &PulsarSink::onMessageDelivery);
    ...
    wb = std::make_unique<WriteBufferFromPulsarSink>([this](char * pos, size_t len) { addMessageToBatch(pos, len); });
    ...
    {
        /// The callback allows `IRowOutputFormat` based formats produce one Pulsar message per row.
    ...
    if (kafka->hasCustomShardingExpr())
    {
        ...
        partitioner = std::make_unique<PulsarStream::ChunkSharder>(buildExpression(header, ast, context), ast->getColumnName());
    }
    else
        partitioner = std::make_unique<PulsarStream::ChunkSharder>();
...
void PulsarSink::addMessageToBatch(char * pos, size_t len)
{
...
void PulsarSink::consume(Chunk chunk)
{
    ...
    /// When one_message_per_row is set to true, one Pulsar message will be generated for each row.
...
void PulsarSink::onMessageDelivery(rd_kafka_t * /* producer */, const rd_kafka_message_t * msg, void * opaque)
{
    static_cast<PulsarSink *>(opaque)->onMessageDelivery(msg);
}

void PulsarSink::onMessageDelivery(const rd_kafka_message_t * msg)
{
    if (msg->err)
    {
