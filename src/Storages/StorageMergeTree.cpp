        ...
        shard_)
{
    ...
    is_virtual_storage = false;

    reader = std::make_unique<MergeTreeDataSelectExecutor>(*this);
    writer = std::make_unique<MergeTreeDataWriter>(*this);
    /// Merge concurrency is a server-level setting; read it from ServerSettings (top-level config),
    /// not the deprecated query-level duplicate, so it matches the actual merge thread pool size.
    merger_mutator = std::make_unique<MergeTreeDataMergerMutator>(*this,
                       static_cast<size_t>(
    ...
    if (!attach && !getDataPartsForInternalUsage().empty())
        throw Exception(ErrorCodes::INCORRECT_DATA,
                        "Data directory for stream already containing data parts - probably "
                        "it was unclean DROP stream or manual intervention. "
                        "You must either clear directory by hand or use ATTACH STREAM instead "
                        "of CREATE STREAM if you need to use that parts.");
...
void StorageMergeTree::flush(bool dropping)
{
    if (flush_called.exchange(true) || dropping)
        return;
    ...
    size_t num_streams)
{
    if (local_context->canUseParallelReplicasOnInitiator())
    {
        ...
        /// Single-instance: create a simple local cluster with default node_id=1
        auto cluster = DB::getCluster(local_context);
        ...
        ClusterProxy::executeQueryWithParallelReplicas(
            ...
            local_context, query_info, cluster, /*is_streaming=*/false, /*parallelized_to_all_replicas=*/false);
    }
    else
    {
    ...
    AlterLockHolder & table_lock_holder)
{
    ...
    if (commands.isSettingsAlter())
    {
        ...
        /// proton: starts. Only alter pure MergeTree table, if it's a inner storage of Stream, the Stream will be altered by itself on the outside.
        if (auto table = DatabaseCatalog::instance().tryGetTable(table_id, local_context); table && table->getName() == getName())
...

    {
        ...
        if (old_storage_settings->non_replicated_deduplication_window != new_storage_settings->non_replicated_deduplication_window)
        {
            ...
            if (new_storage_settings->non_replicated_deduplication_window != 0 && format_version < MERGE_TREE_DATA_MIN_FORMAT_VERSION_WITH_CUSTOM_PARTITIONING)
                throw Exception(ErrorCodes::BAD_ARGUMENTS, "Deduplication for the current engine in old syntax is not supported");
...
void StorageMergeTree::movePartitionToTable(const StoragePtr & dest_table, const ASTPtr & partition, ContextPtr local_context)
{
    ...
    if (!dest_table_storage)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
                        "Stream {} supports movePartitionToTable only for MergeTree family of table engines. Got {}",
                        getStorageID().getNameForLogs(), dest_table->getName());
    if (dest_table_storage->getStoragePolicy() != this->getStoragePolicy())
        throw Exception(ErrorCodes::UNKNOWN_POLICY,
                        "Destination stream {} should have the same storage policy of source table {}. {}: {}, {}: {}",
...
{
    if (isStaticStorage())
        throw Exception(ErrorCodes::STREAM_IS_READ_ONLY, "Table is in readonly mode due to static storage");
}
...
void StorageMergeTree::locateSNFile()
{
    ...
    for (const auto & disk : disks)
    {
        ...
        if (disk->exists(sn_file_path))
        {
            ...
            {
                LOG_ERROR(log, "Duplication of {} file {} and {}", MergeTreeData::COMMITTED_SN_FILE_NAME, fullPath(sn_file.second, sn_file.first), sn_file_path);
                throw Exception(ErrorCodes::CORRUPTED_DATA, "Multiple committed_sn.txt file");
            }
...
Int64 StorageMergeTree::loadSN() const
{
    ...
    auto buf = sn_file.second->readFile(sn_file.first, getReadSettings());
    assertString("1\n", *buf);
...
void StorageMergeTree::populateCommittedSNFromParts()
{
    ...
    std::map<Int64, String> idempotent_keys;
        ...
        if (part->seq_info->idempotent_keys)
        {
            for (const auto & key : *part->seq_info->idempotent_keys)
                if (!idempotent_keys.contains(key.first))
                    idempotent_keys[key.first] = key.second;
        }
    }

    auto [missing_ranges, next_expecting_sn, max_expecting_sn] = DB::missingSequenceRanges(sequence_ranges, committed, log.load());
    ...
    for (; keys_iter != idempotent_keys.end(); ++keys_iter)
    {
        ...
        last_idempotent_keys.push_back(std::move(*keys_iter));
    }
}
/// proton: ends
}
