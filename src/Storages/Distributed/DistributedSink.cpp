...
DistributedSink::DistributedSink(
    ...
    : SinkToStorage(metadata_snapshot_->getSampleBlock(), ProcessorID::DistributedSinkID)
...
DistributedSink::runWritingJob(JobReplica & job, const Block & current_block, size_t num_shards)
{
    ...
    return [this, thread_group, &job, &current_block, num_shards]()
    {
        ...
        span.addAttribute("proton.shard_num", shard_info.shard_num);
        span.addAttribute("proton.cluster", this->storage.cluster_name);
        span.addAttribute("proton.distributed", this->storage.getStorageID().getFullNameNotQuoted());
        span.addAttribute("proton.remote", [this]() { return storage.remote_database + "." + storage.remote_table; });
        span.addAttribute("proton.rows", rows);
        span.addAttribute("proton.bytes", [&shard_block]() { return toString(shard_block.bytes()); });
...
void DistributedSink::writeToLocal(const Cluster::ShardInfo & shard_info, const Block & block, size_t repeats)
{
    ...
    span.addAttribute("proton.distributed", this->storage.getStorageID().getFullNameNotQuoted());
    span.addAttribute("proton.remote", [this]() { return storage.remote_database + "." + storage.remote_table; });
    span.addAttribute("proton.rows", [&block]() { return toString(block.rows()); });
    span.addAttribute("proton.bytes", [&block]() { return toString(block.bytes()); });
