...
public:
    explicit PostgreSQLSink(
        ...
        const String & remote_table_schema_,
        const String & on_conflict_)
        : SinkToStorage(metadata_snapshot_->getSampleBlock(), ProcessorID::PostgresSQLSinkID)
    ...
    void consume(Chunk chunk) override
    {
        ...
        /// proton: starts
        /// Commit the transaction on every consume() call.
        /// Otherwise insert from streaming source will not be visible.
        if (isStreaming())
        {
            inserter->complete();
            inserter.reset();
