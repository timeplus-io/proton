...
void TableMetadata::setLocation(const std::string & location_)
{
    ...
    LOG_TEST(&Poco::Logger::get("TableMetadata"), "Parsed location without path: {}, path: {}", location_without_path, path);
}
...
void TableMetadata::setSchemaJSON(const std::string & schema_json_)
{
    if (!with_schema)
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Data schema was not requested");

    iceberg_schema = schema_json_;
}
...
const std::string & TableMetadata::getSchemaJSON() const
{
    if (!with_schema)
        throw DB::Exception(DB::ErrorCodes::LOGICAL_ERROR, "Data schema was not requested");
