...
{
    /// Since we also generate timeplusd stream schemas based on Iceberg table schemas,
    /// we need to make sure that timeplusd types and Iceberg types are 1:1 mapped.
    /// We could consider using structs for timeplusd types which are not supported by Iceberg table format.
