...
void IcebergSchemaProcessor::addIcebergTableSchema(Poco::JSON::Object::Ptr schema_ptr)
{
    ...
    else
    {
        ...
        auto timeplus_schema = std::make_shared<NamesAndTypesList>();
        ...
        for (size_t i = 0; i != fields->size(); ++i)
        {
            ...
            timeplus_schema->push_back(NameAndTypePair{name, type});
            timeplus_types_by_source_ids[{schema_id, field->getValue<Int32>("id")}] = NameAndTypePair{current_full_name, type};
        }
        timeplus_table_schemas_by_ids[schema_id] = timeplus_schema;
    }
...
{
    auto it = timeplus_types_by_source_ids.find({schema_version, source_id});
    if (it == timeplus_types_by_source_ids.end())
...
NamesAndTypesList IcebergSchemaProcessor::tryGetFieldsCharacteristics(Int32 schema_id, const std::vector<Int32> & source_ids) const
{
    ...
    {
        auto it = timeplus_types_by_source_ids.find({schema_id, source_id});
        if (it != timeplus_types_by_source_ids.end())
    ...
    if (type_name == "struct")
    {
        ...
        for (size_t i = 0; i != fields->size(); ++i)
        {
            ...
            if (is_subfield_of_root)
            {
                ...
                timeplus_types_by_source_ids[{current_schema_id.value(), field->getValue<Int32>("id")}]
    ...
    const Poco::JSON::Object::Ptr & old_schema, const Poco::JSON::Object::Ptr & new_schema, Int32 old_id, Int32 new_id)
{
    if (old_id > 0 || new_id > 0)
        throw Exception(ErrorCodes::UNSUPPORTED_METHOD, "IcebergSchemaProcessor::getSchemaTransformationDag is not ready to be called.");
...
std::shared_ptr<NamesAndTypesList> IcebergSchemaProcessor::getTimeplusTableSchemaById(Int32 id)
{
    auto it = timeplus_table_schemas_by_ids.find(id);
    if (it == timeplus_table_schemas_by_ids.end())
