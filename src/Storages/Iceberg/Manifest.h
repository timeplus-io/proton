...
/* proton: this code was modiffied for fixing all the compiler warnings and matching our code style. */
...
enum class DataFileContent : int32_t
{
    Data = 0,
    PositionDeletes = 1,
    EqualityDeletes = 2
...
struct DataFile
{
    ...
    DataFile() : content(DataFileContent::Data), record_count(int64_t()), file_size_in_bytes(int64_t()) { }
};
...
struct codec_traits<Apache::Iceberg::DataFile>
{
    ...
    {
        avro::encode(e, static_cast<int32_t>(v.content));
        ...
        if (avro::ResolvingDecoder * rd = dynamic_cast<avro::ResolvingDecoder *>(&d))
        {
            ...
            for (auto it : fo)
            {
                switch (it)
                {
                    case 0:
                        int32_t content;
                        avro::decode(d, content);
                        v.content = static_cast<Apache::Iceberg::DataFileContent>(content);
                        break;
                    case 1:
