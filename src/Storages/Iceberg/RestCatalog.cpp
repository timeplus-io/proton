...
RestCatalog::RestCatalog(const std::string & warehouse_, const std::string & base_url_, const Options & options, DB::ContextPtr context_)
    ...
    , log(&Poco::Logger::get("RestCatalog(" + warehouse_ + ")"))
    ...
    , pool(CurrentMetrics::LocalThread, CurrentMetrics::LocalThreadActive, 1)
{
    ...
    else if (!options.auth_header.empty())
        auth_header = parseAuthHeader(options.auth_header);

    if (options.enable_sigv4)
    {
        aws_credentials = pickCredentialsProvider(options.signing_region);
        aws_auth_signer
            = std::make_shared<const Aws::Client::AWSAuthV4Signer>(aws_credentials, options.signing_name.data(), options.signing_region);
    }
    ...
    const DB::HTTPHeaderEntries & headers) const
{
    ...
    auto create_buffer = [&](bool update_token) {
        ...
        return std::make_unique<DB::ReadWriteBufferFromHTTP>(
            ...
            DB::ConnectionTimeouts::getHTTPTimeouts(context),
            credentials,
            /*max_redirects=*/0,
...
{
    const std::string endpoint = base_url / config.prefix / NAMESPACES_ENDPOINT / encodeNamespaceName(name);

    auto auth_headers = getAuthHeaders(endpoint, Poco::Net::HTTPRequest::HTTP_HEAD);
    ...
    auto resp = DB::sendRequest(
        ...
        /*payload=*/"",
        headers,
        {/*connection_timeout_=*/{10, 0}, /*send_timeout_=*/{10, 0}, /*receive_timeout_=*/{10, 0}},
        &Poco::Logger::get("IcebergRestCatalog::existsNamespace"));

    if (resp.second < 300)
        return true;

    if (resp.second == Poco::Net::HTTPResponse::HTTP_NOT_FOUND)
        return false;

    throw DB::Exception(
        DB::ErrorCodes::ICEBERG_CATALOG_ERROR,
        "Failed to check namespace existence, HTTP response code={}, body={}",
        resp.second,
        resp.first);
}
    ...
    std::unordered_map<std::string, std::string> properties)
{
    ...
    const std::string endpoint = base_url / config.prefix / NAMESPACES_ENDPOINT / encodeNamespaceName(namespace_name) / "tables";

    auto auth_headers = getAuthHeaders(endpoint, Poco::Net::HTTPRequest::HTTP_POST, oss);
    std::vector<std::pair<String, String>> headers;
    headers.reserve(auth_headers.size());
    for (const auto & header : auth_headers)
        headers.emplace_back(header.name, header.value);
    ...
    auto resp = DB::sendRequest(
        ...
        headers,
        {/*connection_timeout_=*/{10, 0}, /*send_timeout_=*/{10, 0}, /*receive_timeout_=*/{10, 0}},
        &Poco::Logger::get("IcebergRestCatalog::createTable"));
...
{
    const std::string endpoint
        = base_url / config.prefix / NAMESPACES_ENDPOINT / encodeNamespaceName(namespace_name) / "tables" / table_name;

    auto auth_headers = getAuthHeaders(endpoint, Poco::Net::HTTPRequest::HTTP_DELETE);
    std::vector<std::pair<String, String>> headers;
    headers.reserve(auth_headers.size());
    for (const auto & header : auth_headers)
        headers.emplace_back(header.name, header.value);
    ...
    auto resp = DB::sendRequest(
        ...
        headers,
        {/*connection_timeout_=*/{10, 0}, /*send_timeout_=*/{10, 0}, /*receive_timeout_=*/{10, 0}},
        &Poco::Logger::get("IcebergRestCatalog::deleteTable"));
...
namespace
{
void extractTableMetadata(const std::string & json_str, TableMetadata & result, Poco::Logger * log)
{
    Poco::JSON::Parser parser;
    Poco::Dynamic::Var json = parser.parse(json_str);
    const Poco::JSON::Object::Ptr & object = json.extract<Poco::JSON::Object::Ptr>();
    ...
    TableMetadata & table_metadata)
{
    ...
    const std::string endpoint = base_url / config.prefix / NAMESPACES_ENDPOINT / encodeNamespaceName(namespace_name) / "tables" / name;

    auto auth_headers = getAuthHeaders(endpoint, Poco::Net::HTTPRequest::HTTP_POST, payload_json);
    std::vector<std::pair<String, String>> headers;
    headers.reserve(auth_headers.size());
    for (const auto & header : auth_headers)
        headers.emplace_back(header.name, header.value);
    ...
    auto resp = DB::sendRequest(
        ...
        headers,
        {/*connection_timeout_=*/{10, 0}, /*send_timeout_=*/{10, 0}, /*receive_timeout_=*/{10, 0}},
        &Poco::Logger::get("IcebergRestCatalog::commitTable"));
