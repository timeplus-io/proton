...
IAggregateFunction * createWithExtraTypes(const DataTypePtr & argument_type, bool use_arena, TArgs && ... args)
{
    ...
    else
    {
        ...
        if (argument_type->isValueUnambiguouslyRepresentedInContiguousMemoryRegion())
        {
            if (use_arena)
                ...
                return new AggregateFunctionGroupUniqArrayGenericWithoutArena<true, HasLimit>(argument_type, std::forward<TArgs>(args)...);
        }
        else
        {
            if (use_arena)
                ...
                return new AggregateFunctionGroupUniqArrayGenericWithoutArena<false, HasLimit>(argument_type, std::forward<TArgs>(args)...);
        }
    }
}

template <typename HasLimit, typename ... TArgs>
inline AggregateFunctionPtr createAggregateFunctionGroupUniqArrayImpl(const std::string & name, const DataTypePtr & argument_type, bool use_arena, TArgs ... args)
{
    ...
    if (!res)
        res = AggregateFunctionPtr(createWithExtraTypes<HasLimit>(argument_type, use_arena, std::forward<TArgs>(args)...));
    ...
    const std::string & name, const DataTypes & argument_types, const Array & parameters, const Settings * settings)
{
    ...
    if (!limit_size)
        return createAggregateFunctionGroupUniqArrayImpl<std::false_type>(name, argument_types[0], use_arena, parameters);
    else
        return createAggregateFunctionGroupUniqArrayImpl<std::true_type>(name, argument_types[0], use_arena, parameters, max_elems);
}
...
void registerAggregateFunctionGroupUniqArray(AggregateFunctionFactory & factory)
{
    ...
    factory.registerFunction("group_uniq_array", { createAggregateFunctionGroupUniqArray, properties });
}
