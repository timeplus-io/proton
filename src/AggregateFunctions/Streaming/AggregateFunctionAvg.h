...
public:
    ...
    void negate(AggregateDataPtr __restrict place, const IColumn ** columns, size_t row_num, Arena *) const final
    {
        decrement(place, static_cast<const ColVecType &>(*columns[0]).getData()[row_num]);
        --this->data(place).denominator;
    }
        ...
        const IColumn * delta_col) const final
    {
        ...
        auto & denominator = this->data(place).denominator;
    ...
    String getName() const override { return "__avg_retract"; }
...
private:
    ...
    void NO_SANITIZE_UNDEFINED decrement(AggregateDataPtr __restrict place, Numerator inc) const
    {
        this->data(place).numerator -= inc;
    }
};
