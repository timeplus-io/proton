...
struct AggregateFunctionSumAddOverflowImpl
{
    ...
    static void NO_SANITIZE_UNDEFINED ALWAYS_INLINE negate(T & lhs, const T & rhs) { lhs -= rhs; }
};
...
struct AggregateFunctionSumAddOverflowImpl<Decimal<DecimalNativeType>>
{
    ...
    static void NO_SANITIZE_UNDEFINED ALWAYS_INLINE negate(Decimal<DecimalNativeType> & lhs, const Decimal<DecimalNativeType> & rhs)
    {
        lhs -= rhs;
    }
};
...
struct AggregateFunctionSumData
{
    ...
    void NO_SANITIZE_UNDEFINED ALWAYS_INLINE add(T value) { Impl::add(sum, value); }
    void NO_SANITIZE_UNDEFINED ALWAYS_INLINE negate(T value) { Impl::negate(sum, value); }

    /// Vectorized version
    template <typename Value>
    void NO_SANITIZE_UNDEFINED NO_INLINE addMany(const Value * __restrict ptr, size_t start, size_t end, const IColumn * delta_col)
    {
        ptr += start;
        ...
        if constexpr (std::is_floating_point_v<T> || (is_integer<T> && !is_big_int_v<T>) || (is_decimal<T> && !std::is_same_v<T, Decimal256> && !std::is_same_v<T, Decimal128>))
        {
            ...
            while (ptr < unrolled_end)
            {
                for (size_t i = 0; i < unroll_count; ++i)
                    Impl::add(partial_sums[i], ptr[i] * (*delta_flags++));
                ptr += unroll_count;
            }
    ...
    addManyConditionalInternal(const Value * __restrict ptr, const UInt8 * __restrict condition_map, size_t start, size_t end, const IColumn * delta_col)
    {
        ptr += start;
        size_t count = end - start;
        const auto * end_ptr = ptr + count;
        ...
        if constexpr (
            (is_integer<T> && !is_big_int_v<T>) || (is_decimal<T> && !std::is_same_v<T, Decimal256> && !std::is_same_v<T, Decimal128>))
        {
            ...
            while (ptr < end_ptr)
            {
                ...
                Impl::add(local_sum, *ptr * multiplier * (*delta_flags));
                ++ptr;
                ++condition_map;
                ++delta_flags;
            }
        ...
        if constexpr (std::is_floating_point_v<T>)
        {
            ...
            while (ptr < unrolled_end)
            {
                for (size_t i = 0; i < unroll_count; ++i)
                {
                    ...
                    Impl::add(partial_sums[i], d * (*delta_flags++));
                }
                ptr += unroll_count;
    ...
    template <typename Value>
    void ALWAYS_INLINE addManyNotNull(const Value * __restrict ptr, const UInt8 * __restrict null_map, size_t start, size_t end, const IColumn * delta_col)
    {
        return addManyConditionalInternal<Value, true>(ptr, null_map, start, end, delta_col);
    }

    template <typename Value>
    void ALWAYS_INLINE addManyConditional(const Value * __restrict ptr, const UInt8 * __restrict cond_map, size_t start, size_t end, const IColumn * delta_col)
    {
        return addManyConditionalInternal<Value, false>(ptr, cond_map, start, end, delta_col);
    }
...
{
    static_assert(std::is_floating_point_v<T>, "It doesn't make sense to use Kahan Summation algorithm for non floating point types");
    ...
    template <typename Value>
    void ALWAYS_INLINE negateImpl(Value value, T & out_sum, T & out_compensation)
    {
        auto compensated_value = static_cast<T>(value) - out_compensation;
        auto new_sum = out_sum - compensated_value;
        out_compensation = (out_sum - new_sum) - compensated_value;
        out_sum = new_sum;
    }

    void ALWAYS_INLINE add(T value) { addImpl(value, sum, compensation); }
    void ALWAYS_INLINE negate(T value) { negateImpl(value, sum, compensation); }

    /// Vectorized version
    template <typename Value>
    void NO_INLINE addMany(const Value * __restrict ptr, size_t start, size_t end, const IColumn * delta_col)
    {
        ...
        {
            for (size_t i = 0; i < unroll_count; ++i)
                addImpl(ptr[i] * (*delta_flags++), partial_sums[i], partial_compensations[i]);
            ptr += unroll_count;
        }
        ...
        while (ptr < end_ptr)
        {
            addImpl(*ptr * (*delta_flags++), sum, compensation);
            ++ptr;
        }
    }

    template <typename Value, bool add_if_zero>
    void NO_INLINE addManyConditionalInternal(const Value * __restrict ptr, const UInt8 * __restrict condition_map, size_t start, size_t end, const IColumn * delta_col)
    {
        constexpr size_t unroll_count = 4;
        T partial_sums[unroll_count]{};
        T partial_compensations[unroll_count]{};
...
public:
    ...
    String getName() const override
    {
        if constexpr (Type == AggregateFunctionTypeSum)
            return "__sum_retract";
        else if constexpr (Type == AggregateFunctionTypeSumWithOverflow)
            return "__sum_with_overflow_retract";
        else if constexpr (Type == AggregateFunctionTypeSumKahan)
            return "__sum_kahan_retract";
        __builtin_unreachable();
    }

    AggregateFunctionSum(const DataTypes & argument_types_)
        : IAggregateFunctionDataHelper<Data, AggregateFunctionSum<T, TResult, Data, Type>>(argument_types_, {}), scale(0)
    {
    }
    ...
    void negate(AggregateDataPtr __restrict place, const IColumn ** columns, size_t row_num, Arena *) const override
    {
        const auto & column = assert_cast<const ColVecType &>(*columns[0]);
        if constexpr (is_big_int_v<T>)
            this->data(place).negate(static_cast<TResult>(column.getData()[row_num]));
        else
            this->data(place).negate(column.getData()[row_num]);
    }
        ...
        const IColumn * delta_col) const override
    {
        ...
        if (if_argument_pos >= 0)
        {
            ...
            this->data(place).addManyConditional(column.getData().data(), flags.data(), row_begin, row_end, delta_col);
        }
        else
        {
            this->data(place).addMany(column.getData().data(), row_begin, row_end, delta_col);
        }
    }
        ...
        const IColumn * delta_col) const override
    {
        ...
        if (if_argument_pos >= 0)
        {
            ...
            this->data(place).addManyConditional(column.getData().data(), final_flags.get(), row_begin, row_end, delta_col);
        }
        else
        {
            this->data(place).addManyNotNull(column.getData().data(), null_map, row_begin, row_end, delta_col);
        }
    }
        ...
        const std::vector<llvm::Value *> & argument_values) const override
    {
        ...
        auto * sum_result_value = sum_value->getType()->isIntegerTy() ? b.CreateAdd(sum_value, value_cast_to_result)
                                                                      : b.CreateFAdd(sum_value, value_cast_to_result);
    ...
    compileMerge(llvm::IRBuilderBase & builder, llvm::Value * aggregate_data_dst_ptr, llvm::Value * aggregate_data_src_ptr) const override
    {
        ...
        auto * sum_return_value = sum_value_dst->getType()->isIntegerTy() ? b.CreateAdd(sum_value_dst, sum_value_src)
                                                                          : b.CreateFAdd(sum_value_dst, sum_value_src);
