...
/// When serialize_counter is set to true, the counter about presence of values is serialized
///  regardless to the "result_is_nullable" even if it's unneeded - for protocol compatibility.

template <bool result_is_nullable, bool serialize_counter, typename Derived>
...
protected:
    ...
    /** In addition to data for nested aggregate function, we keep a non-NULL counter
      *  indicating - was non-NULL value accumulated count.
      * In case of no not-NULL values, the function will return NULL.
      *
      * We use prefix_size bytes for counter to satisfy the alignment requirement of nested state.
      */

    static uint32_t & getCounterRef(AggregateDataPtr __restrict place) noexcept
    {
        if constexpr (result_is_nullable)
            return *reinterpret_cast<uint32_t *>(place);

        UNREACHABLE();
    }

    static uint32_t getCounter(ConstAggregateDataPtr __restrict place) noexcept
    {
        if constexpr (result_is_nullable)
            return *reinterpret_cast<const uint32_t *>(place);
        else
            return 1; /// return any non-zero value
    }

    AggregateDataPtr nestedPlace(AggregateDataPtr __restrict place) const noexcept { return place + prefix_size; }

    ConstAggregateDataPtr nestedPlace(ConstAggregateDataPtr __restrict place) const noexcept { return place + prefix_size; }

    static void initCounter(AggregateDataPtr __restrict place, uint32_t counter) noexcept
    {
        if constexpr (result_is_nullable)
            getCounterRef(place) = counter;
    }

    static void addCounter(AggregateDataPtr __restrict place) noexcept
    {
        if constexpr (result_is_nullable)
            getCounterRef(place)++;
    }

    static void negateCounter(AggregateDataPtr __restrict place) noexcept
    {
        if constexpr (result_is_nullable)
            getCounterRef(place)--;
    }
...
public:
    AggregateFunctionNullBase(AggregateFunctionPtr nested_function_, const DataTypes & arguments, const Array & params)
        : IAggregateFunctionHelper<Derived>(arguments, params, createResultType(nested_function_)), nested_function{nested_function_}
    {
        if constexpr (result_is_nullable)
            prefix_size = std::max(sizeof(uint32_t), nested_function->alignOfData());
        else
            prefix_size = 0;
    }
    ...
    {
        if constexpr (result_is_nullable)
            initCounter(place, getCounter(place) + getCounter(rhs));
    ...
    {
        uint32_t counter = getCounter(place);
        if constexpr (serialize_counter)
...
/** There are two cases: for single argument and variadic.
  ...
  */
template <bool result_is_nullable, bool serialize_counter>
class AggregateFunctionNullUnary final : public AggregateFunctionNullBase<
                                             result_is_nullable,
                                             serialize_counter,
                                             AggregateFunctionNullUnary<result_is_nullable, serialize_counter>>
{
public:
    AggregateFunctionNullUnary(AggregateFunctionPtr nested_function_, const DataTypes & arguments, const Array & params)
        : AggregateFunctionNullBase<
              result_is_nullable,
              serialize_counter,
              AggregateFunctionNullUnary<result_is_nullable, serialize_counter>>(std::move(nested_function_), arguments, params)
    {
    }
    ...
    void negate(AggregateDataPtr __restrict place, const IColumn ** columns, size_t row_num, Arena * arena) const override
    {
        ...
        if (!column->isNullAt(row_num))
        {
            this->negateCounter(place);
            this->nested_function->negate(this->nestedPlace(place), &nested_column, row_num, arena);
        }
    }
...
class AggregateFunctionNullVariadic final : public AggregateFunctionNullBase<
                                                result_is_nullable,
                                                serialize_counter,
                                                AggregateFunctionNullVariadic<result_is_nullable, serialize_counter>>
{
public:
    AggregateFunctionNullVariadic(AggregateFunctionPtr nested_function_, const DataTypes & arguments, const Array & params)
        : AggregateFunctionNullBase<
              result_is_nullable,
              serialize_counter,
              AggregateFunctionNullVariadic<result_is_nullable, serialize_counter>>(std::move(nested_function_), arguments, params)
        , number_of_arguments(arguments.size())
    {
        if (number_of_arguments < 3)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error: 1 or 2 arguments are passed to AggregateFunctionNullVariadic");
    ...
    void negate(AggregateDataPtr __restrict place, const IColumn ** columns, size_t row_num, Arena * arena) const override
    {
        ...
        this->negateCounter(place);
        this->nested_function->negate(this->nestedPlace(place), nested_columns.data(), row_num, arena);
    }
