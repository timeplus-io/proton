...
inline void assertArityAtMost(const std::string & name, const DataTypes & argument_types)
{
    ...
    if constexpr (maximal_arity == 0)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Aggregate function {} cannot have any arguments", name);
    ...
    if constexpr (maximal_arity == 2)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH, "Aggregate function {} requires zero, one, or two arguments", name);
