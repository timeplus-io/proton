...
class AggregateFunctionTopKExact
    : public IAggregateFunctionDataHelper<AggregateFunctionTopKData<T>, AggregateFunctionTopKExact<T, is_weighted>>
{
protected:
    using State = AggregateFunctionTopKData<T>;
    UInt64 threshold;
    UInt64 reserved=100*1024*1024; //100MB;

public:
    AggregateFunctionTopKExact(UInt64 threshold_, const DataTypes & argument_types_, const Array & params)
        : IAggregateFunctionDataHelper<AggregateFunctionTopKData<T>, AggregateFunctionTopKExact<T, is_weighted>>(argument_types_, params)
    ...
    void add(AggregateDataPtr __restrict place, const IColumn ** columns, size_t row_num, Arena *) const override
    {
        ...
        if constexpr (is_weighted)
            {
                set.insert(assert_cast<const ColumnVector<T> &>(*columns[0]).getData()[row_num], columns[1]->getUInt(row_num));
                size=sizeof(assert_cast<const ColumnVector<T> &>(*columns[0]).getData()[row_num])*columns[1]->getUInt(row_num);
            }
        else
           {
                set.insert(assert_cast<const ColumnVector<T> &>(*columns[0]).getData()[row_num]);
                size=sizeof(assert_cast<const ColumnVector<T> &>(*columns[0]).getData()[row_num]);
           }

        if (set.size()*size >= reserved)
...
class AggregateFunctionTopKExactGeneric
    : public IAggregateFunctionDataHelper<AggregateFunctionTopKGenericData, AggregateFunctionTopKExactGeneric<is_plain_column, is_weighted>>
{
protected:
    using State = AggregateFunctionTopKGenericData;
...
public:
    AggregateFunctionTopKExactGeneric(
        UInt64 threshold_, const DataTypes & argument_types_, const Array & params)
        : IAggregateFunctionDataHelper<AggregateFunctionTopKGenericData, AggregateFunctionTopKExactGeneric<is_plain_column, is_weighted>>(argument_types_, params)
    ...
    void add(AggregateDataPtr __restrict place, const IColumn ** columns, size_t row_num, Arena * arena) const override
    {
        ...
        if constexpr (is_plain_column)
        {
            if constexpr (is_weighted)
                {
                    set.insert(columns[0]->getDataAt(row_num), columns[1]->getUInt(row_num));
                    size=sizeof(columns[0]->getDataAt(row_num))*columns[1]->getUInt(row_num);
                }
            else
                {
                    set.insert(columns[0]->getDataAt(row_num));
                    size=sizeof(columns[0]->getDataAt(row_num));
                }
        }
        else
        {
            ...
            if constexpr (is_weighted)
                {
                    set.insert(str_serialized, columns[1]->getUInt(row_num));
                    size=str_serialized.size*columns[1]->getUInt(row_num);
                }
            else
                {
                    set.insert(str_serialized);
                    size=str_serialized.size;
                }
            arena->rollback(str_serialized.size);
        }
        if (set.size()*size >= reserved)
