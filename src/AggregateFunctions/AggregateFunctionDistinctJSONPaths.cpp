...
public:
    ...
    void ALWAYS_INLINE addBatchSinglePlace(
        ...
        const IColumn * delta_col [[maybe_unused]]) const override    /// proton: updates
    {
        if (if_argument_pos >= 0 || row_begin != 0 || row_end != columns[0]->size())
            IAggregateFunctionDataHelper<Data, AggregateFunctionDistinctJSONPathsAndTypes<Data>>::addBatchSinglePlace(
                row_begin, row_end, place, columns, arena, if_argument_pos, delta_col);    /// proton: updates
...
{
    factory.registerFunction("distinct_json_paths", createAggregateFunctionDistinctJSONPathsAndTypes<AggregateFunctionDistinctJSONPathsData>);
    factory.registerFunction("distinct_json_paths_and_types", createAggregateFunctionDistinctJSONPathsAndTypes<AggregateFunctionDistinctJSONPathsAndTypesData>);
}
