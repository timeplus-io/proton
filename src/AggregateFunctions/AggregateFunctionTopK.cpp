...
public:
    ...
    {
        if (is_approx_top_k)
            return is_weighted ? "approx_top_k_sum" : "approx_top_k";
        else
            return is_weighted ? "top_k_weighted" : "top_k";
    }
    ...
    void insertResultInto(AggregateDataPtr __restrict place, IColumn & to, Arena *) const override
    {
        ...
        if (include_counts)
        {
            ...
            size_t i = 0;
            for (auto it = result_vec.begin(), it_end = result_vec.end(); it != it_end; ++it, ++i)
            {
...
public:
    ...
    void deserialize(AggregateDataPtr __restrict place, ReadBuffer & buf, std::optional<size_t> /* version */, Arena *) const override
    {
        ...
        {
            readStringBinary(buf_str, buf);
            UInt64 count;
            UInt64 error;
            readVarUInt(count, buf);
            readVarUInt(error, buf);
            set.insert(buf_str, count, error);
            buf_str.clear();
        }
...
AggregateFunctionPtr createAggregateFunctionTopK(const std::string & name, const DataTypes & argument_types, const Array & params, const Settings *)
{
    ...
    else
    {
        ...
        if (!isInteger(argument_types[1]))
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "The second argument for aggregate function 'top_k_weighted' must have integer type");
    }

    /// Syntax: top_k(key, k [, with_count = true] [, load_factor = 1000)
    /// Syntax: top_k_weighted(key, weighted, k, [, with_count = true] [, load_factor = 1000)
    ...
    if (!params.empty())
    {
        ...
        if (params.size() >= 2)
        {
            if constexpr (is_approx_top_k)
            {
                /// approx_top_k(column, k, reserved) => approx_top_k(k, reserved)(column)
                /// approx_top_k_sum(column, weight, k, reserved) => approx_top_k_sum(k, reserved)(column, weight)
            ...
            {
                include_counts = applyVisitor(FieldVisitorConvertToNumber<bool>(), params[1]);
            }
        }
        ...
        if (reserved > TOP_K_MAX_SIZE || load_factor > TOP_K_MAX_SIZE || threshold * load_factor > TOP_K_MAX_SIZE)
            throw Exception(ErrorCodes::ARGUMENT_OUT_OF_BOUND,
...
void registerAggregateFunctionTopK(AggregateFunctionFactory & factory)
{
    ...
    factory.registerFunction("top_k", { createAggregateFunctionTopK<false, false>, properties });
    factory.registerFunction("top_k_weighted", { createAggregateFunctionTopK<true, false>, properties });
    ...
    factory.registerFunction("approx_top_k_sum", { createAggregateFunctionTopK<true, true>, properties }, AggregateFunctionFactory::CaseInsensitive);
    factory.registerAlias("approx_top_k_count", "approx_top_k", AggregateFunctionFactory::CaseInsensitive);
}
