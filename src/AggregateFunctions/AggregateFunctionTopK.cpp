...
createAggregateFunctionTopK(const std::string & name, const DataTypes & argument_types, const Array & params, const Settings *)
{
    ...
    if (with_count)
    {
        ...
        if (!res)
            res = AggregateFunctionPtr(createWithExtraTypes<is_weighted, true>(argument_types, threshold, load_factor, params));
    }
    else
    {
        res.reset(
        ...
        if (!res)
            res = AggregateFunctionPtr(createWithExtraTypes<is_weighted, false>(argument_types, threshold, load_factor, params));
    }
