...
public:
    ...
    virtual int32_t append(const Record & record, AppendCallback callback, void * data, std::any & ctx) = 0;

    /// Poll status for appended record for async `append`
    virtual void poll(int32_t timeout_ms, std::any & ctx) = 0;
    ...
    /// and fast
    virtual int32_t consume(ConsumeCallback callback, void * data, std::any & ctx) = 0;
    ...
    /// caller
    virtual ConsumeResult consume(uint32_t count, int32_t timeout_ms, std::any & ctx) = 0;

    /// return nonzero if failed, otherwise return 0
    virtual int32_t stopConsume(std::any & ctx) = 0;

    /// Move the consuming sequence numbers forward
    /// return non-zero if failed, otherwise return zero
    virtual int32_t commit(RecordSN sn, std::any & ctx) = 0;

    /// Admin APIs
    /// `create` creates a WAL log named `name`. Returns 0 if success; otherwise non-zero
    virtual int32_t create(const std::string & name, std::any & ctx) = 0;

    /// `remove` deletes a WAL log named `name`. Returns 0 if success; otherwise non-zero
    virtual int32_t remove(const std::string & name, std::any & ctx) = 0;

    /// `describe` return 0 if success; otherwise return non-zero
    virtual DescribeResult describe(const std::string & name, std::any & ctx) const = 0;
