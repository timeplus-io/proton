...
{
    assert (request.min_requested_nodes > 0 && request.max_requested_nodes > 0 && request.max_requested_nodes >= request.min_requested_nodes);
    ...
    for (const auto & [node, metrics] : nodes_metrics)
    {
        ...
        if (metrics->staled || metrics->node.roles.find("search") != String::npos || metrics->node.roles.find("ingest") != String::npos)
            continue;
    ...
    if (qualified_nodes.size() < request.min_requested_nodes)
        return {};

    if (qualified_nodes.size() == request.min_requested_nodes || qualified_nodes.size() == request.max_requested_nodes)
    ...
    if (qualified_nodes.size() > request.max_requested_nodes)
        return {qualified_nodes.begin(), qualified_nodes.begin() + request.max_requested_nodes};
    else
        return {qualified_nodes.begin(), qualified_nodes.begin() + request.min_requested_nodes};
}
