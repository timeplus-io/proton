        ...
        if (!settings.try_infer_datetimes_only_datetime64)
        {
            ...
            switch (settings.date_time_input_format)
            {
                ...
                case FormatSettings::DateTimeInputFormat::BestEffort:
                    if (tryParseDateTimeBestEffort(tmp, buf, DateLUT::instance(), DateLUT::instance("UTC")) && buf.eof())
                        return std::make_shared<DataTypeDateTime>();
                    break;
                case FormatSettings::DateTimeInputFormat::BestEffortUS:
                    if (tryParseDateTimeBestEffortUS(tmp, buf, DateLUT::instance(), DateLUT::instance("UTC")) && buf.eof())
                        return std::make_shared<DataTypeDateTime>();
                    break;
            }
        }
