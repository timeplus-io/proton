...
static DataTypePtr getEnumDataTypeFromEnumSchema(const capnp::EnumSchema & enum_schema)
{
    ...
    throw Exception(ErrorCodes::CANNOT_EXTRACT_STREAM_STRUCTURE, "proton supports only 8 and 16-but Enums");
}
...
static DataTypePtr getDataTypeFromCapnProtoType(const capnp::Type & capnp_type)
{
    switch (capnp_type.which())
    {
        ...
        case capnp::schema::Type::STRUCT:
        {
            ...
            if (checkIfStructContainsUnnamedUnion(struct_schema))
                throw Exception(ErrorCodes::CANNOT_EXTRACT_STREAM_STRUCTURE, "Unnamed union is not supported");
        ...
        default:
            throw Exception(ErrorCodes::CANNOT_EXTRACT_STREAM_STRUCTURE, "Unsupported CapnProtoType: {}", getCapnProtoFullTypeName(capnp_type));
    }
}
