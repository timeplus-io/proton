...
MetaStateManager::MetaStateManager(int server_id_, const std::string & host, int port, const std::string & logs_path)
...
MetaStateManager::MetaStateManager(
    ...
    , log_store(nuraft::cs_new<KeeperLogStore>(
                    ...
                    coordination_settings->rotate_log_storage_interval, coordination_settings->force_sync, false))
    , cluster_config(nuraft::cs_new<nuraft::cluster_config>())
{
...
void MetaStateManager::loadLogStore(uint64_t last_commited_index, uint64_t logs_to_keep)
{
...
void MetaStateManager::save_config(const nuraft::cluster_config & config)
{
...
void MetaStateManager::save_state(const nuraft::srv_state & state)
{
     // Just keep in memory in this example.
     // Need to write to disk here, if want to make it durable.
