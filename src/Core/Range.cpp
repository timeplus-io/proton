...
/// proton : starts
bool Range::sameCompareDirection(const Range & rhs) const noexcept
{
    assert(!point());
    assert(!rhs.point());
    ...
    /// {xxx, yyy}, {-Inf, aaa} => no
    /// {xxx, yyy}, {aaa, bbb} => no
    return (equals(left, NEGATIVE_INFINITY) && equals(rhs.left, NEGATIVE_INFINITY))
        || (equals(right, POSITIVE_INFINITY) && equals(rhs.right, POSITIVE_INFINITY));
}

void Range::merge(const Range & rhs)
{
    /// range{-INF, dev03}, range{dev01, +INF} => range{dev01, dev03}
    /// range{dev01, +INF}, range{-INF, dev03} => range{dev01, dev03}
    if (less(left, rhs.left))
    {
        left = rhs.left;
        left_included = rhs.left_included;
    }
    else if (equals(left, rhs.left))
    {
        left_included = std::min(left_included, rhs.left_included);
    }

    if (less(rhs.right, right))
    {
        right = rhs.right;
        right_included = rhs.right_included;
    }
    else if (equals(right, rhs.right))
    {
        right_included = std::min(right_included, rhs.right_included);
    }
}
/// proton : ends
}
