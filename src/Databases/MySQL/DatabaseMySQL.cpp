...
ASTPtr DatabaseMySQL::getCreateTableQueryImpl(const String & table_name, ContextPtr local_context, bool throw_on_error) const
{
    ...
    {
        if (throw_on_error)
            throw Exception(ErrorCodes::UNKNOWN_STREAM, "MySQL table {}.{} doesn't exist.", database_name_in_mysql, table_name);
        return nullptr;
    }

    auto storage = local_tables_cache[table_name].second;
    /// proton: starts.
    /// we extract a helper function to generate the create table query
    return generateCreateTableQuery(table_name, storage, throw_on_error);
    /// proton: ends.
}

/// proton: starts.
ASTPtr DatabaseMySQL::generateCreateTableQuery(const String & table_name, const StoragePtr & storage, bool throw_on_error) const
{
    ...
    return DB::getCreateQueryFromStorage(
        storage, table_storage_define, true, static_cast<unsigned>(settings.max_parser_depth), throw_on_error);
}
/// proton: ends.
    ...
    const std::map<String, UInt64> & tables_modification_time, ContextPtr local_context) const
{
    ...
    for (const auto & table_and_columns : tables_and_columns)
    {
        ...
        /// proton: starts. 
        auto storage = std::make_shared<StorageMySQL>(
        ...
        ASTPtr origin_create_query = generateCreateTableQuery(table_name, storage, true);
        const auto & new_create_query = parseCreateQueryFromAST(origin_create_query, database_name, table_name);
        storage->setInMemoryCreateQuery(new_create_query);

        local_tables_cache[table_name] = std::make_pair(table_modification_time, std::move(storage));
        /// proton: ends.
    }
}
...
void DatabaseMySQL::shutdown()
{
    ...
    for (const auto & [table_name, modify_time_and_storage] : tables_snapshot)
        modify_time_and_storage.second->flushAndShutdown(/*dropping=*/false);
    ...
    while (!quit.load(std::memory_order_relaxed))
    {
        for (auto iterator = outdated_tables.begin(); iterator != outdated_tables.end();)
        {
            ...
            else
            {
                ...
                (*iterator)->flushAndShutdown(/*dropping=*/true);
...
void DatabaseMySQL::attachTable(ContextPtr /* context_ */, const String & table_name, const StoragePtr & storage, const String &)
{
    ...
    if (!local_tables_cache.contains(table_name))
        throw Exception(ErrorCodes::UNKNOWN_STREAM, "Cannot attach table {}.{} because it does not exist.",
            backQuoteIfNeed(database_name), backQuoteIfNeed(table_name));

    if (!remove_or_detach_tables.contains(table_name))
        throw Exception(ErrorCodes::STREAM_ALREADY_EXISTS, "Cannot attach table {}.{} because it already exists.",
            backQuoteIfNeed(database_name), backQuoteIfNeed(table_name));
...
StoragePtr DatabaseMySQL::detachTable(ContextPtr /* context */, const String & table_name)
{
    ...
    if (remove_or_detach_tables.contains(table_name))
        throw Exception(ErrorCodes::STREAM_IS_DROPPED, "Table {}.{} is dropped",
            backQuoteIfNeed(database_name), backQuoteIfNeed(table_name));

    if (!local_tables_cache.contains(table_name))
        throw Exception(ErrorCodes::UNKNOWN_STREAM, "Table {}.{} doesn't exist.",
            backQuoteIfNeed(database_name), backQuoteIfNeed(table_name));
...
void DatabaseMySQL::loadStoredObjects(ContextMutablePtr, bool /*force_restore*/, bool /*force_attach*/, bool /*skip_startup_tables*/)
{
    std::lock_guard lock{mutex};
    for (const auto it = db_disk->iterateDirectory(metadata_path); it->isValid(); it->next())
    {
        ...
        if (db_disk->exists(path) && endsWith(path.filename(), suffix))
        {
...
void DatabaseMySQL::detachTablePermanently(ContextPtr, const String & table_name)
{
    ...
    if (remove_or_detach_tables.contains(table_name))
        throw Exception(ErrorCodes::STREAM_IS_DROPPED, "Table {}.{} is dropped", backQuoteIfNeed(database_name), backQuoteIfNeed(table_name));

    if (db_disk->exists(remove_flag))
    ...
    if (table_iter == local_tables_cache.end())
        throw Exception(ErrorCodes::UNKNOWN_STREAM, "Table {}.{} doesn't exist", backQuoteIfNeed(database_name), backQuoteIfNeed(table_name));
...
void DatabaseMySQL::dropTable(ContextPtr local_context, const String & table_name, bool /*sync*/, const ASTPtr & /*query)*/)
{
