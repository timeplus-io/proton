...
namespace ErrorCodes
{
    ...
    extern const int STREAM_IS_DROPPED;    /// proton: updates
    extern const int STREAM_ALREADY_EXISTS;    /// proton: updates
}
...
DatabaseTablesIteratorPtr DatabasePostgreSQL::getTablesIterator(ContextPtr local_context, const FilterByNameFunction & /* filter_by_table_name */) const
{
    ...
    /// proton: updates: unless get_tables_ignore_error is false (SHOW STREAMS)
    try
    {
        auto connection_holder = pool->get();
    ...
    {
        /// proton: starts
        if (!local_context->getSettingsRef().get_tables_ignore_error)
            throw;
        /// proton: ends
...
void DatabasePostgreSQL::attachTable(ContextPtr /* context_ */, const String & table_name, const StoragePtr & storage, const String &)
{
    ...
    if (!detached_or_dropped.contains(table_name))
        throw Exception(ErrorCodes::STREAM_ALREADY_EXISTS,    /// proton: updates
...
StoragePtr DatabasePostgreSQL::detachTable(ContextPtr /* context_ */, const String & table_name)
{
    ...
    if (detached_or_dropped.contains(table_name))
        throw Exception(ErrorCodes::STREAM_IS_DROPPED, "Cannot detach table {}. It is already dropped/detached", getTableNameForLogs(table_name));  /// proton: updates
...
void DatabasePostgreSQL::createTable(ContextPtr local_context, const String & table_name, const StoragePtr & storage, const ASTPtr & create_query)
{
    ...
    /// Set in-memory create query to ensure system.tables queries always use the in-memory metadata
    /// rather than falling back to ClickHouse's legacy code path, which could lead to inconsistencies
    ASTPtr origin_create_query = create_query->clone();
    const auto & new_create_query = parseCreateQueryFromAST(origin_create_query, database_name, table_name);
    storage->setInMemoryCreateQuery(new_create_query);
    /// proton: ends.
...
void DatabasePostgreSQL::dropTable(ContextPtr, const String & table_name, bool /* sync */, const ASTPtr & /*query*/)
{
    std::lock_guard lock{mutex};
    ...
    if (detached_or_dropped.contains(table_name))
        throw Exception(ErrorCodes::STREAM_IS_DROPPED, "Table {} is already dropped/detached", getTableNameForLogs(table_name));  /// proton: updates
...
void DatabasePostgreSQL::loadStoredObjects(ContextMutablePtr, bool /*force_restore*/, bool /*force_attach*/, bool /*skip_startup_tables*/)
{
    {
        std::lock_guard lock{mutex};
        ...
        for (const auto it = db_disk->iterateDirectory(getMetadataPath()); it->isValid(); it->next())
        {
            ...
            if (db_disk->exists(path) && db_disk->isFile(path) && endsWith(path.filename(), suffix))
            {
