...
namespace ErrorCodes
{
    ...
    extern const int CANNOT_GET_CREATE_STREAM_QUERY;    /// proton: updates
}
...
ASTPtr DatabaseMaterializedPostgreSQL::getCreateTableQueryImpl(const String & table_name, ContextPtr local_context, bool throw_on_error) const
{
    ...
    try
    {
        ...
        /// proton: starts.
        ast_storage = generateCreateTableQuery(storage.get(), table_name, throw_on_error);
        /// proton: ends.
    }
    catch (...)
    {
        if (throw_on_error)
        {
            throw Exception(ErrorCodes::CANNOT_GET_CREATE_STREAM_QUERY,    /// proton: updates
...
ASTPtr DatabaseMaterializedPostgreSQL::createAlterSettingsQuery(const SettingChange & new_setting)
{
    ...
    command->settings_changes = command->children.emplace_back(std::move(set));
...
void DatabaseMaterializedPostgreSQL::createTable(ContextPtr local_context, const String & table_name, const StoragePtr & table, const ASTPtr & query)
{
    ...
    auto new_create_query = parseCreateQueryFromAST(query_copy, table->getStorageID().database_name, table_name);
...
void DatabaseMaterializedPostgreSQL::attachTable(ContextPtr context_, const String & table_name, const StoragePtr & table, const String & relative_table_path)
{
    ...
    if (CurrentThread::isInitialized() && CurrentThread::get().getQueryContext())
    {
        ...
        try
        {
            ...
            /// Set in-memory create query to ensure system.tables queries always use the in-memory metadata
            /// rather than falling back to ClickHouse's legacy code path, which could lead to inconsistencies
            ASTPtr origin_create_query = generateCreateTableQuery(storage.get(), table_name, true);
            if (origin_create_query)
                storage->setInMemoryCreateQuery(parseCreateQueryFromAST(origin_create_query, database_name, table_name));
            /// proton: ends.
        ...
        catch (...)
        {
            ...
            DatabaseAtomic::dropTable(current_context, table_name, true, nullptr);
            throw;
        }
    }
    else
...
void DatabaseMaterializedPostgreSQL::dropTable(ContextPtr local_context, const String & table_name, bool sync, const ASTPtr & query)
{
    /// Modify context into nested_context and pass query to Atomic database.
    DatabaseAtomic::dropTable(StorageMaterializedPostgreSQL::makeNestedTableContext(local_context), table_name, sync, query);
}
...
DatabaseTablesIteratorPtr DatabaseMaterializedPostgreSQL::getTablesIterator(ContextPtr local_context, const DatabaseOnDisk::FilterByNameFunction & filter_by_table_name) const
{
    /// Modify context into nested_context and pass query to Atomic database.
