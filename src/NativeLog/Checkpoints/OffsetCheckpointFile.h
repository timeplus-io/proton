...
struct TopicShardOffset
{
    ...
    {
        DB::writeText(topic_shard.topic, out);
        DB::writeText(SEP, out);
        DB::writeText(topic_shard.shard, out);
        DB::writeText(SEP, out);
        DB::writeText(offset, out);
        DB::writeText('\n', out);
    }

    static TopicShardOffset read(const std::string & data)
    {
        TopicShardOffset ts_offset;

        /// Topic
        std::string::size_type pos = 0;
        auto rpos = data.find(SEP, pos);
        if (rpos == std::string::npos)
            throw DB::Exception("Failed to parse topic", DB::ErrorCodes::INVALID_DATA);

        ts_offset.topic_shard.topic = std::string(data, pos, rpos - pos);
        ...
        ts_offset.topic_shard.shard = DB::parseIntStrict<int32_t>(data, pos, rpos);
...
/// tp1 shard1 1   <- the format is: TOPIC SHARD OFFSET
/// tp2 shard2 2
/// ------checkpoint file end------
...
public:
    ...
    void write(const std::vector<TopicShardOffset> & offsets) { checkpoint.write(offsets); }

    std::vector<TopicShardOffset> read() { return checkpoint.read(); }

private:
    static const int32_t CURRENT_VERSION = 1;

private:
    CheckpointFileWithFailureHandler<TopicShardOffset> checkpoint;
};
