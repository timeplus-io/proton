...
TEST(ParserCreateRemoteFunctionQuery, UDFHeaderMethodIsNone)
{
    ...
    /// AUTH METHOD
    EXPECT_EQ(remote_func_settings->children.back()->as<ASTPair>()->second->as<ASTLiteral>()->value.safeGet<String>(), "none");
}
...
TEST(ParserCreateRemoteFunctionQuery, UDFHeaderMethodIsAuthHeader)
{
    ...
    EXPECT_EQ(
        ...
        "https://hn6wip76uexaeusz5s7bh3e4u40lrrrz.lambda-url.us-west-2.on.aws/");
    /// AUTH METHOD
    EXPECT_EQ(remote_func_settings->children[4]->as<ASTPair>()->second->as<ASTLiteral>()->value.safeGet<String>(), "auth_header");
    /// AUTH HEADER
    EXPECT_EQ(remote_func_settings->children[1]->as<ASTPair>()->second->as<ASTLiteral>()->value.safeGet<String>(), "auth");
    /// AUTH KEY
    EXPECT_EQ(remote_func_settings->children[2]->as<ASTPair>()->second->as<ASTLiteral>()->value.safeGet<String>(), "proton");
    /// EXECUTION TIMEOUT
