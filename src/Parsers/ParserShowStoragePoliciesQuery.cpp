...
namespace DB
{
bool ParserShowStoragePoliciesQuery::parseImpl(Pos & pos, ASTPtr & node, Expected & expected, [[maybe_unused]] bool hint)
{
    ParserKeyword s_show("SHOW");
    ParserKeyword s_storage_policies("STORAGE POLICIES");
    ...
    auto query = std::make_shared<ASTShowStoragePoliciesQuery>();

    if (!s_show.ignore(pos, expected))
        return false;

    if (!s_storage_policies.ignore(pos, expected))
        return false;
