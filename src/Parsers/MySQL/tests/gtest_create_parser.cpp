...
TEST(CreateTableParser, LikeCreate)
{
    ...
    String like_create_01 = "CREATE STREAM IF NOT EXISTS table_name LIKE table_name_01";
    parseQuery(p_create_query, like_create_01.data(), like_create_01.data() + like_create_01.size(), "", 0, 0);
    String like_create_02 = "CREATE STREAM IF NOT EXISTS table_name (LIKE table_name_01)";
...
{
    ParserCreateQuery p_create_query;
    String input = "CREATE STREAM IF NOT EXISTS table_name(col_01 VARCHAR(100), INDEX (col_01), CHECK 1) ENGINE INNODB PARTITION BY HASH(col_01)";
...
{
    ParserCreateQuery p_create_query;
    String input = "CREATE STREAM `test_table_1` (`a` int DEFAULT NULL, `b` int DEFAULT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci";
    ASTPtr ast = parseQuery(p_create_query, input.data(), input.data() + input.size(), "", 0, 0);
