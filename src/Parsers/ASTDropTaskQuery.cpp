...
ASTPtr ASTDropTaskQuery::clone() const
{
    auto cloned = std::make_shared<ASTDropTaskQuery>(*this);
...
void ASTDropTaskQuery::formatImpl(const IAST::FormatSettings & settings, IAST::FormatState &, IAST::FormatStateStacked) const
{
    settings.ostr << (settings.hilite ? hilite_keyword : "") << "DROP TASK ";
...
String ASTDropTaskQuery::getDatabase() const
{
    String result;
...
String ASTDropTaskQuery::getName() const
{
    String result;
