...
void ASTColumnsExceptTransformer::transform(ASTs & nodes) const
{
    ...
    if (is_strict && !expected_columns.empty())
    {
        ...
        throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_STREAM, "Columns transformer EXCEPT expects following column(s) :{}",
...
void ASTColumnsReplaceTransformer::transform(ASTs & nodes) const
{
    ...
    if (is_strict && !replace_map.empty())
    {
        ...
        throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_STREAM, "Columns transformer REPLACE expects following column(s) : {}",
            expected_columns);
    }
