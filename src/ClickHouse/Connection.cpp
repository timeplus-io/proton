...
#include <ClickHouse/NativeReader.h>
...
void Connection::connect(const ConnectionTimeouts & timeouts)
{
    try
    {
        ...
        {
#if USE_SSL
            /// Create SSL context based on settings (like Kafka does, default is "relaxed").
            ...
            else if (ssl_verify_mode == "strict")
                verification_mode = Poco::Net::Context::VERIFY_STRICT;
            else if (ssl_verify_mode == "once")
                verification_mode = Poco::Net::Context::VERIFY_ONCE;
            else if (ssl_verify_mode != "relaxed")
                throw Exception(
                    ErrorCodes::BAD_ARGUMENTS, "Invalid ssl_verify_mode '{}'. Valid values: none, relaxed, strict, once", ssl_verify_mode);

            if (!ssl_cert_file.empty() && !std::filesystem::exists(ssl_cert_file))
                throw Exception(ErrorCodes::BAD_ARGUMENTS, "SSL certificate file '{}' does not exist", ssl_cert_file);
            if (!ssl_key_file.empty() && !std::filesystem::exists(ssl_key_file))
                throw Exception(ErrorCodes::BAD_ARGUMENTS, "SSL private key file '{}' does not exist", ssl_key_file);
            if (!ssl_ca_cert_file.empty() && !std::filesystem::exists(ssl_ca_cert_file))
                throw Exception(ErrorCodes::BAD_ARGUMENTS, "SSL CA certificate file '{}' does not exist", ssl_ca_cert_file);

            Poco::Net::Context::Ptr ssl_context;
            try
            {
                ssl_context = new Poco::Net::Context(
                    Poco::Net::Context::CLIENT_USE,
                    ssl_cert_file, /// client certificate file
                    ssl_key_file, /// client private key file
            ...
            {
                throw Exception(ErrorCodes::BAD_ARGUMENTS, "Failed to create SSL context: {}", e.displayText());
            }

            socket = std::make_unique<Poco::Net::SecureStreamSocket>(ssl_context);
...
#else
            throw Exception(
                ErrorCodes::SUPPORT_IS_DISABLED, "tcp_secure protocol is disabled because poco library was built without NetSSL support.");
#endif
        }
        else
        {
        ...
        if (timeouts.tcp_keep_alive_timeout.totalSeconds() != 0)
        {
...
void Connection::sendHello()
{
    ...
    if (has_control_character(default_database) || has_control_character(user) || has_control_character(password))
        throw Exception(
            ErrorCodes::BAD_ARGUMENTS, "Parameters 'default_database', 'user' and 'password' must not contain ASCII control characters");
    ...
    /// ClickHouse/Connection.cpp is not sync with Client/Connection.cpp.
    /// So fixed the DBMS_TCP_PROTOCOL_VERSION to 54460
    /// writeVarUInt(DBMS_TCP_PROTOCOL_VERSION, *out);
    writeVarUInt(54460, *out);
...
#else
        throw Exception(
            ErrorCodes::SUPPORT_IS_DISABLED, "Inter-server secret support is disabled, because timeplus was built without SSL library");
#endif
    }
    else
    {
...
void Connection::getServerVersion(
    ...
    std::function<void(const Progress &)>)
{
    ...
    span.addAttribute("proton.query_id", query_id_);
    span.addAttribute("proton.query", query);
    ...
    if ((client_info != nullptr) && current_trace_context.isTraceEnabled())
    {
    ...
    if (settings != nullptr)
    {
        ...
        CompressionCodecFactory::instance().validateCodec(
            method, level, !settings->allow_suspicious_codecs, settings->allow_experimental_codecs);
    ...
    {
        if ((client_info != nullptr) && !client_info->empty())
    ...
    /// if (server_revision >= DBMS_MIN_REVISION_WITH_INTERSERVER_SECRET)
    {
        ...
        if ((client_info != nullptr) && !cluster_secret.empty() && client_info->query_kind != ClientInfo::QueryKind::INITIAL_QUERY)
        {
#if USE_SSL
    ...
    writeVarUInt(static_cast<UInt64>(compression), *out);
...
void Connection::sendData(
    const Block & block, const std::unordered_map<String, String> & column_type_names, const String & name, bool scalar)
{
    if (!block_out)
    {
        if (compression == Protocol::Compression::Enable)
    ...
    block_out->write(block, &column_type_names);
    maybe_compressed_out->next();
    out->next();
...
public:
    ...
    ExternalTableDataSink(Block header, Connection & connection_, ExternalTableData & table_data_, OnCancell callback)
        : ISink(std::move(header), ProcessorID::ExternalTableDataSinkID)
