...
{
public:
    Client(TypeNameProviderPtr, ConnectionPool::Entry, ConnectionTimeouts, LoggerPtr);

    /// ConnectionPool is provided so that client can manage connection create and release.
    Client(TypeNameProviderPtr, ConnectionPoolPtr connection_pool_, UInt64 connection_pool_max_wait_ms, ConnectionTimeouts, LoggerPtr);
    ...
    /// \param query - the `INSERT INTO ... VALUES` part of the SQL statement.
    /// \param block - data to be inserted.
    void executeInsertQuery(const String & query, const Block & block, const String & query_id = "");
    ...
    /// Allow user to explicitly put connection back to the pool. Client should be constructed with ConnectionPool.
    void releaseConnection();

private:
    bool receiveAndProcessPacket();
    void receiveInsertQueryResponse();
    ...
    TypeNameProviderPtr type_name_provider;

    ConnectionPoolPtr connection_pool;
    std::optional<Settings> get_connection_settings;
    std::optional<ConnectionPool::Entry> connection;
