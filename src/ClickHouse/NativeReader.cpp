...
NativeReader::NativeReader(ReadBuffer & istr_, UInt64 server_revision_) : istr(istr_), server_revision(server_revision_)
{
}
    ...
    : istr(istr_), server_revision(server_revision_), use_index(true), index_block_it(index_block_it_), index_block_end(index_block_end_)
{
    ...
    if (istr_concrete == nullptr)
        throw Exception(
            ErrorCodes::LOGICAL_ERROR, "When need to use index for NativeBlockInputStream, istr must be CompressedReadBufferFromFile.");
...
void NativeReader::readData(
    const ISerialization & serialization, ColumnPtr & column, ReadBuffer & istr, size_t rows, double avg_value_size_hint)
{
...
Block NativeReader::read()
{
    ...
    for (size_t i = 0; i < columns; ++i)
    {
        if (use_index)
        {
            ...
            istr_concrete->seek(
                index_column_it->location.offset_in_compressed_file, index_column_it->location.offset_in_decompressed_block);
        }
        ...
        column.type = data_type_factory.get(type_name, /*compatible_with_clickhouse=*/true);
        ...
        if (rows != 0u) /// If no rows, nothing to read.
        ...
        if (header)
        {
            if (header.has(column.name))
            {
                ...
                {
                    column.column = recursiveLowCardinalityTypeConversion(column.column, column.type, header.safeGetByPosition(i).type);
            ...
            else
            {
                if (!skip_unknown_columns)
                    throw Exception(
                        ErrorCodes::INCORRECT_DATA, "Unknown column with name {} found while reading data in Native format", column.name);
                use_in_result = false;
            }
        }
    ...
    if ((rows != 0u) && header)
    {
        ...
        for (size_t column_i = 0; column_i != header.columns(); ++column_i)
        {
            ...
            else
            {
                ...
                if (block_missing_values != nullptr)
