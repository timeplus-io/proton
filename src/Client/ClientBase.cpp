...
void ClientBase::onData(Block & block, ASTPtr parsed_query)
{
    ...
    if (need_render_progress && tty_buf && !select_into_file)
...
try
{
    if (!output_format)
    {
        ...
        auto format_settings = getFormatSettings(global_context);
...
{
    /// proton: starts. Need to port ClickHouse #27928 first
...
void ClientBase::runInteractive()
{
    ...
    /// proton: starts. add a fallback path for history file.
    ...
    else
    {
        ...
        else if (!home_path.empty())
        {
            ...
            try
            {
                if (!FS::canWrite(fs::path(history_file).parent_path().string()))
                {
                    /// Try /var/lib/proton path first
                    history_file = "/var/lib/proton/.proton-client-history";
                    if (!FS::canWrite(fs::path(history_file).parent_path().string()))
                    {
            ...
            {
                /// If any filesystem operation fails, fallback to /tmp
                history_file = "/tmp/.proton-client-history";
            }
        }
        else
...
void ClientBase::init(int argc, char ** argv)
{
    ...
    options_description.main_description->add_options()
        ...
        ("disable_suggestion,A", "Disable loading suggestion data. Note that suggestion data is loaded asynchronously through a second connection to proton server. Also it is reasonable to disable suggestion if you want to paste a query with TAB characters. Shorthand option -A is for those who get used to mysql client.")
