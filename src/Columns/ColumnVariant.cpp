...
ColumnVariant::ColumnVariant(DB::MutableColumnPtr local_discriminators_, DB::MutableColumnPtr offsets_, DB::MutableColumns && variants_, const std::vector<Discriminator> & local_to_global_discriminators_)
{
    if (variants_.size() > MAX_NESTED_COLUMNS)
        throw Exception(
            ErrorCodes::BAD_ARGUMENTS, "Type variant with more than {} nested types is not allowed", ColumnVariant::MAX_NESTED_COLUMNS);
...
/// proton: starts
size_t ColumnVariant::allocatedMetadataBytes() const
{
    size_t res = local_discriminators->allocatedMetadataBytes() + offsets->allocatedMetadataBytes();
    for (const auto & variant : variants)
        res += variant->allocatedMetadataBytes();
    return res;
}
/// proton: ends
