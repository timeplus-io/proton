...
size_t ColumnObject::allocatedMetadataBytes() const
{
    ...
    for (const auto & [_, column] : typed_paths)
        size += column->allocatedMetadataBytes();
    for (const auto & [_, column] : dynamic_paths_ptrs)
        size += column->allocatedMetadataBytes();
    size += shared_data->allocatedMetadataBytes();
    return size;
}
/// proton : ends
...
/// proton: starts.
void ColumnObject::serializeValueIntoBuffer(size_t n, WriteBuffer & wb) const
{
    /// First serialize values from typed paths in sorted order. They are the same for all instances of this column.
    for (auto path : sorted_typed_paths)
        typed_paths.find(path)->second->serializeValueIntoBuffer(n, wb);
    ...
    wb.write(reinterpret_cast<const char *>(&num_paths), sizeof(num_paths));
    ...
    for (size_t i = offset; i != end; ++i)
    {
        ...
        while (dynamic_paths_it != sorted_dynamic_paths.end() && *dynamic_paths_it < path)
        {
            ...
            if (!dynamic_column->isNullAt(n))
            {
                WriteBufferFromOwnString buf;
                getDynamicSerialization()->serializeBinary(*dynamic_column, n, buf, getFormatSettings());
                serializePathAndValueIntoBuffer(StringRef(*dynamic_paths_it), buf.str(), wb);
            }
            ++dynamic_paths_it;
        }
        serializePathAndValueIntoBuffer(StringRef(path), shared_data_values->getDataAt(i), wb);
    }
...
void ColumnObject::serializePathAndValueIntoBuffer(StringRef path, StringRef value, WriteBuffer & wb) const
{
    size_t value_size = value.size;
    size_t path_size = path.size;
    wb.write(reinterpret_cast<const char *>(&path_size), sizeof(size_t));
    wb.write(path.data, path_size);
    wb.write(reinterpret_cast<const char *>(&value_size), sizeof(size_t));
    wb.write(value.data, value_size);
}
/// proton: ends.
