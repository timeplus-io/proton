...
public:
    ...
    /// proton: starts
    bool equal(size_t, size_t, const IColumn &, int) const override
    {
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "equal is not implemented for {}", getName());
    }

    bool equal(size_t, const Field &, int) const override
    {
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "equal is not implemented for {}", getName());
    }
    /// proton: ends
...
private:
    ...
    /// proton: starts.
    void serializePathAndValueIntoBuffer(StringRef path, StringRef value, WriteBuffer & wb) const;
    /// proton: ends.
