...
/// proton: starts.
void ColumnTuple::serializeValueIntoBuffer(size_t n, WriteBuffer & wb) const
{
    for (const auto & column : columns)
        column->serializeValueIntoBuffer(n, wb);
}
/// proton: ends.
...
bool ColumnTuple::equal(size_t n, size_t m, const IColumn & rhs, int nan_direction_hint) const
{
    const size_t tuple_size = columns.size();
    for (size_t i = 0; i < tuple_size; ++i)
    {
        auto is_equal = columns[i]->equal(n, m, *assert_cast<const ColumnTuple &>(rhs).columns[i], nan_direction_hint);
        if (!is_equal)
            return false;
    }
    return true;
...
bool ColumnTuple::equal(size_t n, const Field & rhs, int nan_direction_hint) const
{
    assert(rhs.getType() == Field::Types::Which::Tuple);
    const auto & tuple_field = rhs.get<Tuple>();

    const size_t lhs_tuple_size = columns.size();
    const size_t rhs_tuple_size = tuple_field.size();
    if (lhs_tuple_size != rhs_tuple_size)
        return false;

    for (size_t i = 0; i < lhs_tuple_size; ++i)
    {
        if (!columns[i]->equal(n, tuple_field[i], nan_direction_hint))
            return false;
    }
    return true;
}
