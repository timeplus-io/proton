...
/// proton: starts.
template <typename Derived, typename Parent>
void IColumnHelper<Derived, Parent>::serializeValueIntoBuffer(size_t n, WriteBuffer & wb) const
{
    if constexpr (!std::is_base_of_v<ColumnFixedSizeHelper, Derived>)
        return IColumn::serializeValueIntoBuffer(n, wb);

    const auto & self = static_cast<const Derived &>(*this);
    auto raw_data = self.getDataAt(n);
    writeString(raw_data.data, raw_data.size, wb);
}
/// proton: ends.
