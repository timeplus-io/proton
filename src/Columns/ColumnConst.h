...
public:
    ...
    /// proton: starts.
    void serializeValueIntoBuffer(size_t, WriteBuffer & wb) const override
    {
        data->serializeValueIntoBuffer(0, wb);
    }
    /// proton: ends.
    ...
    bool equal(size_t, size_t, const IColumn & rhs, int nan_direction_hint) const override
    {
        return data->equal(0, 0, *assert_cast<const ColumnConst &>(rhs).data, nan_direction_hint);
    }

    bool equal(size_t, const Field & rhs, int nan_direction_hint) const override
    {
        return data->equal(0, rhs, nan_direction_hint);
    }
    /// proton : ends
