...
public:
    ...
    {
        return CompareHelper<T>::equals(data[n], assert_cast<const Self &>(rhs_).data[m], nan_direction_hint);
    }

    bool equal(size_t n, const Field & rhs_, int nan_direction_hint) const override
    {
        auto rhs_v = static_cast<T>(rhs_.get<T>());
        return CompareHelper<T>::equals(data[n], rhs_v, nan_direction_hint);
    }
    /// proton: ends
