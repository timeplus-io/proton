...
public:
    ...
    /// proton: starts.
    /// Same as above but serialize into WriteBuffer.
    virtual void serializeValueIntoBuffer(size_t /* n */, WriteBuffer & /* wb */) const
    {
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Method serializeValueIntoBuffer is not supported for {}", getName());
    }
    /// proton: ends.
    ...
    /// proton : starts
    /// Faster implementation than compareAt and compare to one field
    /// \return true if compared equal otherwise return false
    [[nodiscard]] virtual bool equal(size_t n, size_t m, const IColumn & rhs, int nan_direction_hint) const = 0;

    /// Faster implementation than compareAt and compare to one field
    /// \return true if compared equal otherwise return false
    [[nodiscard]] virtual bool equal(size_t n, const Field & rhs, int nan_direction_hint) const = 0;
    /// proton : ends
    ...
    [[nodiscard]] virtual int compareAtWithCollation(size_t, size_t, const IColumn &, int, const Collator &) const
    {
        throw Exception(ErrorCodes::BAD_COLLATION,
                        "Collations could be specified only for string, low_cardinality(string), nullable(string) "
