...
AsynchronousInsertQueue::InsertData::Entry::Entry(String && bytes_, String && query_id_, MemoryTracker * user_memory_tracker_)
    : bytes(std::move(bytes_)), query_id(std::move(query_id_)), user_memory_tracker(user_memory_tracker_)
{
}
...
void AsynchronousInsertQueue::scheduleDataProcessingJob(const InsertQuery & key, InsertDataPtr data, ContextPtr global_context)
{
    ...
    pool.scheduleOrThrowOnError([key, global_context, c_data = std::make_shared<InsertDataPtr>(std::move(data))]() mutable
    {
        processData(key, std::move(*c_data), std::move(global_context));
    });
}
