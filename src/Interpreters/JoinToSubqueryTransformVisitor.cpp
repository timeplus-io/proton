...
public:
    struct Data
    {
        ...
        Data(const std::vector<TableWithColumnNamesAndTypes> & tables, bool join_result_emit_changelog_) : join_result_emit_changelog(join_result_emit_changelog_)
        /// proton: ends.
        {
    ...
    static void visit(const ASTExpressionList & node, const ASTPtr &, Data & data)
    {
        ...
        for (const auto & child : node.children)
        {
            ...
            if (const auto * asterisk = child->as<ASTAsterisk>())
            {
                ...
                if (data.join_result_emit_changelog)
...
bool needRewrite(ASTSelectQuery & select, std::vector<const ASTTableExpression *> & table_expressions)
{
    ...
    for (size_t i = 0; i < num_tables; ++i)
    {
        ...
        if (!table)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Stream expected");
    ...
    if (num_using && version <= 1)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Multiple JOIN does not support USING");
    if (num_array_join)
        ...
        if (auto table_pos = IdentifierSemantic::chooseTableColumnMatch(*ident, tables, allow_ambiguous))
        {
            if (!ident->isShort())
            {
                if (got_alias)
                {
                    ...
                    if (alias_ident->as<ASTIdentifier>())
                    {
...
void JoinToSubqueryTransformMatcher::visit(ASTSelectQuery & select, ASTPtr & ast, Data & data)
{
    ...
    {
        /// proton: starts.
        ExtractAsterisksVisitor::Data asterisks_data(data.tables, data.join_result_emit_changelog);
        /// proton: ends.
...
ASTPtr JoinToSubqueryTransformMatcher::replaceJoin(ASTPtr ast_left, ASTPtr ast_right, ASTPtr subquery_template)
{
    ...
    if (!right->table_join)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Stream join expected");
