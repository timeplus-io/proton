...
void InterpreterSetQuery::applySettingsFromQuery(const ASTPtr & ast, ContextMutablePtr context_)
{
    ...
    if (const auto * query_with_output = dynamic_cast<const ASTQueryWithOutput *>(ast.get()))
    {
        ...
        if (const auto * create_function_query = ast->as<ASTCreateFunctionQuery>())
        {
            if (auto new_sttings = create_function_query->settings())
                InterpreterSetQuery(new_sttings, context_).executeForCurrentContext(/* ignore_setting_constraints= */ false);
        }
        else if (const auto * create_query = ast->as<ASTCreateQuery>(); create_query && create_query->select)
