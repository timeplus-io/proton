    ...
    void appendFromBlock(const Block & block, size_t row_num)
    {
        ...
        if (is_join_get)
        {
            ...
            for (size_t j = 0, size = right_indexes.size(); j < size; ++j)
            {
                ...
                if (auto * nullable_col = typeid_cast<ColumnNullable *>(columns[j].get()); nullable_col && !column_from_block.column->isNullable())
...
DataTypePtr HashJoin::joinGetCheckAndGetReturnType(const DataTypes & data_types, const String & column_name, bool or_null) const
{
    ...
    if (!sample_block_with_columns_to_add.has(column_name))
        throw Exception("StorageJoin doesn't contain column " + column_name, ErrorCodes::NO_SUCH_COLUMN_IN_STREAM);
