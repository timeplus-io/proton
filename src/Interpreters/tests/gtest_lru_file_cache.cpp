...
const fs::path & cachesDir()
{
    static fs::path caches_dir = fs::current_path() / "lru_cache_test";
    return caches_dir;
}

const std::string & cacheBasePath()
{
    static std::string cache_base_path = cachesDir() / "cache1" / "";
...
void download(FileSegment & file_segment)
{
    ...
    download(cacheBasePath(), file_segment);
    ASSERT_EQ(file_segment.state(), State::DOWNLOADING);
...
public:
    ...
    void SetUp() override
    {
        ...
        if (fs::exists(cacheBasePath()))
            fs::remove_all(cacheBasePath());
        fs::create_directories(cacheBasePath());
    }
...
TEST_F(FileCacheTest, get)
{
    ...
    settings.base_path = cacheBasePath();
    ...
    std::cerr << "Step 12\n";
    {
        ...
        settings2.base_path = cachesDir() / "cache2";
