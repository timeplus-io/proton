...
TEST(EliminateSubquery, OptimizedQuery)
{
    ...
    EXPECT_EQ(
        optimizeSubquery("SELECT count(b) FROM (SELECT a AS b FROM product) GROUP BY b"), "SELECT count(b) FROM product GROUP BY a AS b");
    EXPECT_EQ(
    ...
    EXPECT_EQ(
        optimizeSubquery(
            "SELECT count(b.code) FROM (SELECT a.code, b.code FROM product_info as a, product_info as b WHERE (a.productid = b.productid))"),
        "SELECT count(b.code) FROM product_info AS a, product_info AS b WHERE a.productid = b.productid");
    EXPECT_EQ(
        optimizeSubquery(
            "SELECT count(a.code) FROM (SELECT a.code, b.code FROM product_info as a, product_info as b WHERE (a.productid = b.productid))"),
