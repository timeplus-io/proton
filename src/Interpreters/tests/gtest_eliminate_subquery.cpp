...
TEST(EliminateSubquery, FailedOptimizedQuery)
{
    ...
    EXPECT_EQ(
        ...
        "SELECT   count(*) FROM (SELECT     runningDifference(i) AS diff FROM (SELECT       * FROM test_query ORDER BY       i DESC) WHERE "
        "   diff > 0)");
    ...
    EXPECT_EQ(
        optimizeSubquery("SELECT count(t.code) FROM (SELECT a.code, b.code FROM product_info as a, product_info as b WHERE a.productid = "
                         "b.productid) AS t"),
        "SELECT   count(t.code) FROM (SELECT     a.code, b.code FROM product_info AS a, product_info AS b WHERE    a.productid = "
        "b.productid) AS "
        "t");
    EXPECT_EQ(
        optimizeSubquery("SELECT count(t.b.code) FROM (SELECT a.code, b.code FROM product_info as a, product_info as b WHERE a.productid = "
                         "b.productid) AS t"),
        "SELECT   count(t.b.code) FROM (SELECT     a.code, b.code FROM product_info AS a, product_info AS b WHERE    a.productid = "
        "b.productid) AS "
        "t");
    EXPECT_EQ(
        optimizeSubquery(
            "SELECT count(t.code) FROM (SELECT code, code FROM product_info as a, product_info as b WHERE a.productid = b.productid) AS t"),
        "SELECT   count(t.code) FROM (SELECT     code, code FROM product_info AS a, product_info AS b WHERE    a.productid = b.productid) "
        "AS t");
    ...
    EXPECT_EQ(
        ...
        "SELECT   count(code) FROM (SELECT     a.code, b.code FROM product_info AS a, product_info AS b WHERE    a.productid = "
        "b.productid)");
