...
bool TranslateQualifiedNamesMatcher::Data::unknownColumn(size_t table_pos, const ASTIdentifier & identifier) const
{
    ...
    if (source_columns.contains(full_name))
        return false;
    /// proton: ends.
    ...
    if (IdentifierSemantic::getColumnName(identifier))
    {
        ...
        if (auto best_pos = IdentifierSemantic::chooseTable(identifier, data.tables, allow_ambiguous))
        {
            ...
            if (data.unknownColumn(table_pos, identifier))
            {
                ...
                throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "There's no column '{}' in stream '{}'", identifier.name(), table_name);
            }
...
void TranslateQualifiedNamesMatcher::visit(ASTFunction & node, const ASTPtr &, Data & data)
{
    ...
    if ((func_name_lowercase == "count" || func_name_lowercase == "countstate" || func_name_lowercase == "__count_retract") &&
