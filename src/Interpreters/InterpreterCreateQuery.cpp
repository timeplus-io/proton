...
InterpreterCreateQuery::InterpreterCreateQuery(const ASTPtr & query_ptr_, ContextMutablePtr context_)
    : InterpreterCreateQuery(query_ptr_, /*schema_version=*/1, std::move(context_))
{
}

InterpreterCreateQuery::InterpreterCreateQuery(
    const ASTPtr & query_ptr_, UInt32 schema_version_, ContextMutablePtr context_)
    : WithMutableContext(context_)
    , query_ptr(query_ptr_)
    , schema_version(schema_version_)
    , log(getLogger("InterpreterCreateQuery"))
{
}
...
BlockIO InterpreterCreateQuery::createDatabase(ASTCreateQuery & create)
{
    ...
    /// proton: starts
    createDatabaseEngineFromSettings(create.storage);
    /// proton: ends
    ...
    if (!create.storage && create.attach)
    {
        ...
        if (create.table || !create.storage)
            throw Exception(
                ErrorCodes::INCORRECT_QUERY, "Metadata file {} contains incorrect CREATE DATABASE query", metadata_file_path.string());
        create.attach = true;
    ...
    if (create.storage->engine->name == "MaterializedPostgreSQL"
        && !getContext()->getSettingsRef().allow_experimental_database_materialized_postgresql && !internal)
    {
        throw Exception(
            ErrorCodes::UNKNOWN_DATABASE_ENGINE,
...
ASTPtr InterpreterCreateQuery::formatColumns(const NamesAndTypesList & columns, const NamesAndAliases & alias_columns)
{
    ...
    for (const auto & alias_column : alias_columns)
    {
        ...
        column_declaration->default_expression
            = parseQuery(expression_parser, alias_pos, alias_end, "expression", 0, DBMS_DEFAULT_MAX_PARSER_DEPTH);
...
ASTPtr InterpreterCreateQuery::formatColumns(const ColumnsDescription & columns)
{
    ...
    for (const auto & column : columns)
    {
        ...
        /// proton : starts
        if (column.default_desc.kind == ColumnDefaultKind::AutoIncrement)
        {
            column_declaration->default_specifier = toString(column.default_desc.kind);
        }
        else if (column.default_desc.expression) /// proton : ends
        {
            column_declaration->default_specifier = toString(column.default_desc.kind);
...
ColumnsDescription InterpreterCreateQuery::getColumnsDescription(const ASTExpressionList & columns_ast, ContextPtr context_, bool attach)
{
    ...
    for (auto ast_it = columns_ast.children.begin(); ast_it != columns_ast.children.end(); ++ast_it, ++name_type_it)
    {
        ...
        if (col_decl.default_specifier == "AUTO_INCREMENT")
            column.default_desc.kind = ColumnDefaultKind::AutoIncrement;
        /// proton : ends
        ...
        if (column.default_desc.kind == ColumnDefaultKind::AutoIncrement)
        {
            WhichDataType which_data_type(column.type->getTypeId());
            auto supported = which_data_type.isUInt64();
            if (!supported)
                throw Exception(ErrorCodes::UNSUPPORTED, "Only uint64 column type supports auto-increment");
        }
        /// proton : ends
    ...
    if (res.getAllPhysical().empty())
        throw Exception(ErrorCodes::EMPTY_LIST_OF_COLUMNS_PASSED, "Cannot CREATE stream without physical columns");
...
InterpreterCreateQuery::TableProperties InterpreterCreateQuery::getTablePropertiesAndNormalizeCreateQuery(ASTCreateQuery & create) const
{
    ...
    if (create.columns_list)
    {
        if (create.as_table_function && (create.columns_list->indices || create.columns_list->constraints))
            throw Exception(ErrorCodes::INCORRECT_QUERY, "Indexes and constraints are not supported for the current engine");
        ...
        if (create.columns_list->indices)
            for (const auto & index : create.columns_list->indices->children)
                properties.indices.push_back(IndexDescription::getIndexFromAST(index->clone(), properties.columns, getContext()));
    ...
    else if (create.select)
    {
        ...
        Block as_select_sample = InterpreterSelectWithUnionQuery::getSampleBlock(
            ...
            getContext(),
            false /* is_subquery */,
            nullptr /*output_data_stream_semantic*/,
...
void InterpreterCreateQuery::validateTableStructure(
    const ASTCreateQuery & create, const InterpreterCreateQuery::TableProperties & properties) const
{
    ...
    if (create.storage && create.storage->engine
        && (create.storage->engine->name == "Stream" || endsWith(create.storage->engine->name, "MergeTree")))
    {
        auto search = all_columns.find(LightweightDeleteDescription::FILTER_COLUMN.name);
        if (search != all_columns.end())
            throw Exception(
                ErrorCodes::ILLEGAL_COLUMN,
                "Cannot create table with column '{}' for *{} engines because it "
    ...
    if (!create.attach && !create.isMaterializedView())
    {
...
void InterpreterCreateQuery::setEngine(ASTCreateQuery & create) const
{
    ...
    if (create.storage || create.isDictionary() || create.isView())
    {
        if (create.temporary && create.storage && create.storage->engine && create.storage->engine->name != "Memory")
            throw Exception(
                ErrorCodes::INCORRECT_QUERY,
                "Temporary tables can only be created with ENGINE = Memory, not {}",
                create.storage->engine->name);
    ...
    else if (!create.as_table.empty())
    {
        ...
        if (as_create.isOrdinaryView())
            throw Exception(ErrorCodes::INCORRECT_QUERY, "Cannot CREATE a stream AS {}, it is a View", qualified_name);

        if (as_create.isDictionary())
            throw Exception(ErrorCodes::INCORRECT_QUERY, "Cannot CREATE a stream AS {}, it is a Dictionary", qualified_name);
...
static void generateUUIDForTable(ASTCreateQuery & create)
{
    ...
    bool need_uuid_for_inner_table = !create.attach && create.isMaterializedView() && create.to_table_id.empty();
    /// proton: ends.
...
{
    const auto * kind = create.isDictionary() ? "Dictionary" : "Table";
    const auto * kind_upper = create.isDictionary() ? "DICTIONARY" : "TABLE";

    if (database->getEngineName() == "Replicated" && getContext()->getClientInfo().is_replicated_database_internal && !internal)
    {
        if (create.uuid == UUIDHelpers::Nil)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Stream UUID is not specified in DDL log");
    }
    ...
    else
    {
        ...
        if (has_uuid && !is_on_cluster)
            throw Exception(
                ErrorCodes::INCORRECT_QUERY, "{} UUID specified, but engine of database {} is not Atomic", kind, create.getDatabase());
...
void InterpreterCreateQuery::handleExternalStreamCreation(ASTCreateQuery & create)
{
    ...
    if ((create.storage != nullptr) && (create.storage->engine != nullptr) && create.storage->engine->name == "ExternalTable")
        return;
    ...
    else if (create.storage->settings != nullptr)
...
/// Prepare engine and settings for create query before creating stream
void InterpreterCreateQuery::completeTableCreationAST(ASTCreateQuery & create, ContextMutablePtr local_context)
{
    if (create.isView() || create.isDictionary() || create.isExternal() || create.isRandomStream() || create.isNullStream())
        return;

    if ((create.storage == nullptr) || (create.storage->engine == nullptr))
    {
        String current_database = local_context->getCurrentDatabase();
        auto database_name = create.database ? create.getDatabase() : current_database;

        /// TODO move the logic to DataBaseIceberg
        auto database = DatabaseCatalog::instance().getDatabase(database_name);
        if (database->configureTableEngine(create))
            return;

        Streaming::prepareStorageEngine(create, local_context);
    }
    else if (create.storage->engine->name != "Stream")
    {
        /// We only need complete `Stream` table engine for now
        return;
    }

    /// Fill in some reserved columns etc if absent
    Streaming::checkAndPrepareCreateQueryForStream(create, local_context);
    /// Fill in default storage engine settings if absent
    Streaming::prepareStorageEngineSettings(create, local_context);
}
/// proton: ends
...
BlockIO InterpreterCreateQuery::createTable(ASTCreateQuery & create)
{
    ...
    if (create.temporary && create.database)
        throw Exception(
            ...
            "Temporary streams cannot be inside a database. "
    ...
    /// proton: start
    /// Fixup the storage engine / settings etc
    if (!create.attach)
        completeTableCreationAST(create, getContext());
    handleExternalStreamCreation(create);

    /// Prepare reserved columns for MaterializedView and RandomStream
    if (create.isMaterializedView() || create.isRandomStream())
        Streaming::checkAndPrepareColumnsAndIndexes(create, getContext());
    /// proton: end
    ...
    if (create.attach && !create.storage && !create.columns_list)
    {
        ...
        if (!create.isDictionary() && create_query.isDictionary())
            throw Exception(
                ErrorCodes::INCORRECT_QUERY,
    ...
    if (create.attach_from_path)
    {
        ...
        if (getContext()->getClientInfo().query_kind == ClientInfo::QueryKind::INITIAL_QUERY)
        {
            ...
            if (!startsWith(data_path, user_files))
                throw Exception(
                    ...
                    "Data directory {} must be inside {} to attach it",
                    String(data_path),
                    String(user_files));
        ...
        create.attach && !create.attach_short_syntax && getContext()->getClientInfo().query_kind != ClientInfo::QueryKind::SECONDARY_QUERY)
    {
        LOG_WARNING(
            log,
            "ATTACH STREAM query with full stream definition is not recommended: "
...
bool InterpreterCreateQuery::doCreateTable(ASTCreateQuery & create, const InterpreterCreateQuery::TableProperties & properties)
{
    ...
    /// assert(!create.attach || create.attach && schema_version > 0);

    if (create.temporary)
    {
        if (create.if_not_exists && local_context->tryResolveStorageID({"", create.getTable()}, Context::ResolveExternal))
            return false;

        String temporary_table_name = create.getTable();
        auto temporary_table = TemporaryTableHolder(local_context, properties.columns, properties.constraints, query_ptr);
        local_context->getSessionContext()->addExternalTable(temporary_table_name, std::move(temporary_table));
        return true;
    }
    ...
    auto storage_already_exists_error_code
        = create.isDictionary() ? ErrorCodes::DICTIONARY_ALREADY_EXISTS : ErrorCodes::STREAM_ALREADY_EXISTS;

    /// Table can be created before or it can be created concurrently in another thread, while we were waiting in DDLGuard.
    if (database->isTableExist(create.getTable(), local_context))
    {
        ...
        else if (create.replace_view)
        {
            ...
            throw Exception(
                storage_already_exists_error_code,
                "{} {}.{} already exists",
                storage_name,
    ...
    if (!create.attach && !data_path.empty() && fs::exists(fs::path{local_context->getPath()} / data_path))
        throw Exception(
            storage_already_exists_error_code, "Directory for {} data {} already exists", Poco::toLower(storage_name), String(data_path));
    ...
    if (from_path)
    {
        if (data_path.empty())
            throw Exception(
                ...
                "ATTACH ... FROM ... query is not supported for {} database engine",
    ...
    if (create.attach)
    {
        ...
        bool throw_if_table_in_use = local_context->getSettingsRef().database_atomic_wait_for_drop_and_detach_synchronously;
    ...
    if (create.as_table_function)
    {
        ...
        auto table_func = factory.get(create.as_table_function, local_context);
        ...
        res = table_func->execute(
            create.as_table_function, local_context, create.getTable(), properties.columns, /*use_global_context=*/true);
    ...
    if (from_path && !res->storesDataOnDisk())
        throw Exception(
            ...
            "because such streams do not store any data on disk. Use CREATE instead.",
            res->getName());
    ...
    /// proton: starts.
    auto auto_increment_columns = res->getInMemoryMetadataPtr()->getColumns().getAutoIncrement();
    if (!auto_increment_columns.empty())
    {
        if (!res->supportsAutoIncrementColumn())
            throw Exception(ErrorCodes::UNSUPPORTED, "Current stream type doesn't support auto increment column");

        if (auto_increment_columns.size() != 1)
            throw Exception(ErrorCodes::UNSUPPORTED, "Only one auto increment column is supported");
    }

    try
    {
        database->createTable(local_context, create.getTable(), res, query_ptr);
        ...
        /// SystemLog tables are not Stream table, so we need to start them up here.
        if (res->isLocal())
            res->startup();
    }
    catch (...)
    {
        /// It's important to unlock UUID before dropping table.
        /// If drop table first, release uuid_lock next, here will throw an LOGICAL_ERROR
        /// because called `removeUUIDMappingFinally` twice:
        /// Once when releasing `uuid_lock` and once in the asynchronous task `dropTableDataTask()`.
        uuid_lock = {};
        ...
        auto drop_context = Context::createCopy(local_context);
        InterpreterDropQuery interpreter(drop_ast, drop_context);
        interpreter.execute();
...
BlockIO InterpreterCreateQuery::fillTableIfNeeded(const ASTCreateQuery & create)
{
    ...
    if (create.select && !create.attach && !create.isView())
    {
...
BlockIO InterpreterCreateQuery::execute()
{
    ...
    {
        /// proton: starts. For system database, it always has `local` setting (hence local); for other databases, it is always distributed
        /// There are several code path which will reach here
        /// 1) CREATE DATABASE ...; to create database in a cluster
        /// 2) MetadataUpdater to apply 1) on each node locally
        /// 3) During reboot, loadMetadata to load database from local file system or metastore
        /// For 2) & 3), we explicitly set SECONDARY_QUERY
        /// For 1), all non-system database, non-explicitly `local` database are distributed.
        const bool is_secondary_query = getContext()->getClientInfo().query_kind == ClientInfo::QueryKind::SECONDARY_QUERY;
        const bool execute_on_local = is_secondary_query || create.isLocal() || create.attach;
        return execute_on_local ? createDatabase(create) : createDatabaseOnCluster(create);
    }
    else
    {
...
AccessRightsElements InterpreterCreateQuery::getRequiredAccess() const
{
    ...
    else
    {
        ...
        {
            /// proton: starts. we don't support replace table for now
            // if (create.replace_table)
    ...
    if (create.to_table_id)
        required_access.emplace_back(
            AccessType::SELECT | AccessType::INSERT, create.to_table_id.database_name, create.to_table_id.table_name);
...
/// proton: starts
void InterpreterCreateQuery::createDatabaseEngineFromSettings(ASTStorage * storage)
{
    /// Create storage engine ast when 'ENGINE' is not specified in query but settings have the engine type and other required arguments.
    /// For example, CREATE DATABASE abc SETTINGS type='XYZ', ...

    if (storage == nullptr || storage->engine != nullptr || storage->settings == nullptr)
        return;

    if (storage->settings->changes.tryGet("type") == nullptr)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Database 'type' missing in SETTINGS");

    auto & changes = storage->settings->changes;

    auto check_setting = [&changes](std::string_view name, bool required) -> bool {
        if (changes.tryGet(name) == nullptr)
        {
            if (required)
                throw Exception(ErrorCodes::BAD_ARGUMENTS, "Setting {} is not set.", name);
            else
                return false;
        }
        return true;
    ...
    /// Get string field value and remove it from storage settings
    auto extract_setting_str = [&changes](std::string_view name) {
        const auto * field = changes.tryGet(name);
        String value = field->safeGet<String>();
        /// Remove engine settings which are not part of database creator settings
        changes.removeSetting(name);
        return value;
    };

    /// Get int field value and remove it from storage settings
    auto extract_setting_int = [&changes](std::string_view name) {
        const auto * field = changes.tryGet(name);
        auto value = field->safeGet<UInt8>();
        /// Remove engine settings which are not part of database creator settings
        changes.removeSetting(name);
        return value;
    };
    ...
    check_setting("type", true);
    auto type = extract_setting_str("type");

    if (type == "mysql")
    {
        check_setting("address", true);
        check_setting("database", true);
        check_setting("user", true);
        check_setting("password", true);

        engine_str = fmt::format(
            "MySQL('{}', '{}', '{}', '{}')",
            extract_setting_str("address"),
            extract_setting_str("database"),
            extract_setting_str("user"),
            extract_setting_str("password"));
    }
    else if (type == "postgresql")
    {
        ...
        if (check_setting("schema", false))
        {
            parameters = fmt::format("{}, '{}'", parameters, extract_setting_str("schema"));
            if (check_setting("use_table_cache", false))
                parameters = fmt::format("{}, {}", parameters, extract_setting_int("use_table_cache"));
        }

        engine_str = fmt::format("PostgreSQL({})", parameters);
    }
    else if (type == "iceberg")
    {
        check_setting("catalog_uri", true);
        engine_str = fmt::format("Iceberg('{}')", extract_setting_str("catalog_uri"));
    }

    if (engine_str.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Unknown database engine type: type={}", type);

    ParserIdentifierWithOptionalParameters parser;
    ASTPtr engine_ast = parseQuery(parser, engine_str, 0, DBMS_DEFAULT_MAX_PARSER_DEPTH);
    if (engine_ast == nullptr)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid database engine arguments: str={}", engine_str);
...
{
    if (create.isDictionary() || (create.columns_list && create.columns_list->columns && !create.columns_list->columns->children.empty()))
        return;

    auto ast_storage = std::make_shared<ASTStorage>();
    unsigned max_parser_depth_v = static_cast<unsigned>(getContext()->getSettingsRef().max_parser_depth);
    // unsigned max_parser_backtracks_v = static_cast<unsigned>(getContext()->getSettingsRef().max_parser_backtracks);
    auto query_from_storage
        = DB::getCreateQueryFromStorage(storage, ast_storage, false, max_parser_depth_v, /*max_parser_backtracks_v, */ true);
