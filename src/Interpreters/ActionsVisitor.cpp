...
void ScopeStack::addArrayJoin(const std::string & source_name, std::string result_name)
{
    ...
    if (!source_node)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Expression with array_join cannot depend on lambda argument: {}",
                        source_name);
...
void ActionsMatcher::visit(const ASTFunction & node, const ASTPtr & ast, Data & data)
{
    ...
    if (node.name == "array_join")
    {
        if (node.arguments->children.size() != 1)
            throw Exception(ErrorCodes::TYPE_MISMATCH, "array_join requires exactly 1 argument");
    ...
    if (node.name == "emit_version")
    {
        if (!node.arguments->children.empty())
            throw Exception(ErrorCodes::TYPE_MISMATCH, "emit_version requires can't have argument");
    ...
    FunctionOverloadResolverPtr function_builder = UserDefinedFunctionFactory::instance().tryGet(node.name, data.getContext());
...
FutureSetPtr ActionsMatcher::makeSet(const ASTFunction & node, Data & data, bool no_subqueries)
{
    ...
    if (right_in_operand->as<ASTSubquery>() || identifier)
    {
        ...
        PreparedSets::Hash set_key = right_in_operand->getTreeHash();
