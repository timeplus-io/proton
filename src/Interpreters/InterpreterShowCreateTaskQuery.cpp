...
BlockIO InterpreterShowCreateTaskQuery::execute()
{
    BlockIO res;
...
Block InterpreterShowCreateTaskQuery::getSampleBlock() const
{
...
QueryPipeline InterpreterShowCreateTaskQuery::executeImpl()
{
    const auto * show_query = query_ptr->as<ASTShowCreateTaskQuery>();
    ...
    if (resp->hasError())
        throw Exception(
            ...
            "Failed to load task {} in database {}, error={{{}}}",
            show_query->getName(),
            database,
            resp->error().string());
    ...
    {
        ASTCreateTaskQuery create_query;
        ...
        create_query.cron = desc->cron;
        create_query.interval = desc->interval;
        create_query.interval_unit = desc->interval_unit;

        create_query.timeout = desc->timeout;
        create_query.timeout_unit = desc->timeout_unit;

        create_query.checkpoint = desc->checkpoint_init_values;

        if (!desc->target_table_name.empty())
            create_query.to_table_id = StorageID{desc->target_database_name, desc->target_table_name};

        create_query.sql = desc->sql;
