...
ColumnPtr extractNestedColumnsAndNullMap(ColumnRawPtrs & key_columns, ConstNullMapPtr & null_map)
{
    ...
    else
    {
        for (auto & column : key_columns)
        {
            if (const auto * column_nullable = checkAndGetColumn<ColumnNullable>(*column))
            {
                ...
                else
                {
                    ...
                    PaddedPODArray<UInt8> & mutable_null_map = assert_cast<ColumnBool &>(*mutable_null_map_holder).getData();
        ...
        null_map = null_map_holder ? &assert_cast<const ColumnBool &>(*null_map_holder).getData() : nullptr;
    }
