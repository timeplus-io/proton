...
HashMapSizes HashBlocks::hashMapSizes(const DB::Streaming::MemoryHashJoin * hash_join) const
{
...
BufferedStreamData::BufferedStreamData(MemoryHashJoin * join_)
...
BufferedStreamData::BufferedStreamData(
    MemoryHashJoin * join_, const RangeAsofJoinContext & range_asof_join_ctx_, const String & asof_column_name_)
    : join(join_)
...
void BufferedStreamData::deserialize(
    ReadBuffer & rb,
    VersionType version,
    DeserializedIndicesToRowRefListMultiple<JoinDataBlock> * deserialized_indices_to_row_ref_list_multiple)
{
    std::scoped_lock lock(mutex);
