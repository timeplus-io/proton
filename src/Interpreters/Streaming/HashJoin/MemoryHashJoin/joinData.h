...
#include <Interpreters/Streaming/HashJoin/MemoryHashJoin/joinSerder_fwd.h>
#include <Interpreters/Streaming/HashJoin/MemoryHashJoin/joinTuple.h>
...
using HashBlocksPtrs = std::vector<HashBlocksPtr>;

SERDE struct BufferedStreamData
{
    explicit BufferedStreamData(MemoryHashJoin * join_);

    /// For asof join
    BufferedStreamData(MemoryHashJoin * join_, const RangeAsofJoinContext & range_asof_join_ctx_, const String & asof_column_name_);
    ...
    void deserialize(
        ReadBuffer & rb,
        VersionType version,
        DeserializedIndicesToRowRefListMultiple<JoinDataBlock> * deserialized_indices_to_row_ref_list_multiple = nullptr);

    NO_SERDE MemoryHashJoin * join;
