    ...
    , num_used_hash_joins(slots_)
{
    for (size_t i = 0; i < slots; ++i)
    {
        ...
        inner_hash_join->data = std::make_unique<MemoryHashJoin>(table_join, left_join_stream_desc, right_join_stream_desc);
...
void ConcurrentHashJoin::rescale(size_t slots_)
{
    ...
    {
        /// scale up
        hash_joins.reserve(new_slots);
        for (; slots < new_slots; ++slots)
        {
            auto inner_hash_join = std::make_shared<InternalHashJoin>();
            switch (hash_joins.back()->data->type())
            {
...
BlocksWithShard ConcurrentHashJoin::dispatchBlock(const std::vector<size_t> & key_column_positions, Block && from_block)
{
    ...
    if (selector.size() == 1)
        return BlocksWithShard{{std::move(from_block), static_cast<UInt32>(selector[0])}};
...
void ConcurrentHashJoin::serialize(WriteBuffer & wb, VersionType version) const
{
    ...
    else
    {
        ...
        std::unique_lock<std::mutex> lk(serialize_mutex);
        serialized.wait(lk, [this] { return is_cancelled || serialize_requested.load(std::memory_order_relaxed) == 0; });
    }
}
...
void ConcurrentHashJoin::write(VersionType version)
{
    /// Only last join thread to do serialization
    if (serialize_requested.fetch_add(1) + 1 == num_used_hash_joins)
    {
        for (const auto & hash_join : hash_joins)
        {
            ...
            chassert(hash_join->data->type() == HashJoinType::Hybrid);
            std::lock_guard lock(hash_join->mutex);
            auto hybrid_hash_join = static_cast<HybridHashJoin *>(hash_join->data.get());
            hybrid_hash_join->write(version);
        }
