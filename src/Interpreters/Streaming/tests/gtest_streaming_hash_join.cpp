    ...
    std::shared_ptr<TableJoin> table_join, const Block & left_header, const Block & right_header, UInt64 keep_versions, ContextPtr context)
{
    ...
    auto left_join_stream_desc
        = std::make_shared<Streaming::JoinStreamDescription>(tables[0], Block{}, tables[0].output_data_stream_semantic, keep_versions, 0);
    auto right_join_stream_desc
        = std::make_shared<Streaming::JoinStreamDescription>(tables[1], right_header, tables[1].output_data_stream_semantic, keep_versions, 0);
