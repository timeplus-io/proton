...
SERDE struct TrackingUpdatesT
{
    static ALWAYS_INLINE Derive & data(AggregateDataPtr __restrict place) { return *reinterpret_cast<Derive *>(place); }
    static ALWAYS_INLINE const Derive & data(ConstAggregateDataPtr __restrict place) { return *reinterpret_cast<const Derive *>(place); }
    ...
    static void merge(AggregateDataPtr __restrict place, ConstAggregateDataPtr __restrict rhs)
    {
        data(place).updates += data(rhs).updates;
        data(place).updated_since_last_finalization |= data(rhs).updated_since_last_finalization;
    }
...
SERDE struct TrackingUpdates : public TrackingUpdatesT<TrackingUpdates>
{
};

SERDE struct TrackingUpdatesWithRetract : public TrackingUpdatesT<TrackingUpdatesWithRetract>
{
    static ALWAYS_INLINE AggregateDataPtr & getRetract(AggregateDataPtr place)
    {
        return reinterpret_cast<TrackingUpdatesWithRetract *>(place)->retract_data;
    }
    static ALWAYS_INLINE bool hasRetract(ConstAggregateDataPtr __restrict place)
    {
        return reinterpret_cast<const TrackingUpdatesWithRetract *>(place)->retract_data;
    }
