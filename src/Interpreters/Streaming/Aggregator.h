...
//    std::unique_ptr<AggregationMethodSingleLowCardinalityColumn<AggregationMethodFixedString<StreamingAggregatedDataWithNullableStringKeyTwoLevel>>> streaming_low_cardinality_key_fixed_string_two_level;
    ...
    ///   STATE V3 - REVISION 2 (Add expanded data)
    static constexpr UInt64 STATE_V2_MIN_REVISION = 1;
    static constexpr UInt64 STATE_V3_MIN_REVISION = 2;
    static constexpr VersionType ENABLE_DATA_EX_MIN_VERSION = 3;
...
public:
    ...
    /// should_abort: if the processing should be aborted (with group_by_overflow_mode = 'break') return true, otherwise false.
    /// need_finalization : only for UDA aggregation. If there is no UDA, always false
    std::pair<bool, bool> executeAndRetractOnBlockLegacy(
        Columns columns,
        size_t row_begin,
        size_t row_end,
        AggregatedDataVariants & result,
    ...
    /// [Legacy] Used for merge changed groups and return the <retracted_state, aggregated_state> of changed groups
    std::pair<AggregatedDataVariantsPtr, AggregatedDataVariantsPtr>
    mergeRetractedGroupsLegacy(ManyAggregatedDataVariants & aggregated_data, ManyAggregatedDataVariants & retracted_data) const;
...
private:
    ...
    /** Create states of aggregate functions for one key.
      */
    template <bool skip_expanded_data = false, bool skip_compiled_aggregate_functions = false>
    ...
    /// [Legacy]
    template <typename Method>
    bool executeAndRetractImplLegacy(
        Method & method,
        Arena * aggregates_pool,
    ...
    /// [Legacy]
    template <typename Method>
    void mergeRetractedGroupsImplLegacy(ManyAggregatedDataVariants & aggregated_data, ManyAggregatedDataVariants & retracted_data) const;
    ...
    void mergeRetractedState(AggregateDataPtr & dst, AggregateDataPtr & src, Arena * arena) const;
    /// proton: ends.
...
private:
    void checkpoint(const AggregatedDataVariants & data_variants, WriteBuffer & wb, VersionType version) const;
    void recover(AggregatedDataVariants & data_variants, ReadBuffer & rb, VersionType version) const;
