...
VersionType AggregatedDataVariants::getVersionFromRevision(UInt64 revision) const
{
...
void AggregatedDataVariants::serialize(WriteBuffer & wb, VersionType version) const
{
    assert(aggregator);
    return aggregator->checkpoint(*this, wb, version);
}

void AggregatedDataVariants::deserialize(ReadBuffer & rb, VersionType version)
{
    assert(aggregator);
    return aggregator->recover(*this, rb, version);
}
...
template <bool skip_expanded_data, bool skip_compiled_aggregate_functions>
            ...
            for (AggregateFunctionInstruction * inst = aggregate_instructions; inst->that; ++inst)
            {
                inst->batch_that->addBatchLookupTable8(
                    ...
                    [&](AggregateDataPtr & aggregate_data)
                    {
                        ...
                        createAggregateStates</*skip_expanded_data*/ true>(aggregate_data);
                    },
                    state.getKeyData(),
...
{
    size_t reserved_rows = 10;
    if constexpr (type == AggregateStateType::Normal)
        reserved_rows = rows;

    constexpr bool final = true;
    auto out_cols = prepareOutputBlockColumns(params, aggregate_functions, getHeader(final), aggregates_pools, final, reserved_rows);
    ...
    ManyAggregatedDataVariants & data, Int64 bucket, Arena * arena, bool clear_states, std::atomic<bool> * is_cancelled) const
{
    ...
    for (size_t result_num = 1, size = data.size(); result_num < size; ++result_num)
    {
        ...
        getDataVariant<Method>(current).data.buckets_updated[bucket] = false;
    }
}
...
void Aggregator::checkpoint(const AggregatedDataVariants & data_variants, WriteBuffer & wb, VersionType version) const
{
...
void Aggregator::recover(AggregatedDataVariants & data_variants, ReadBuffer & rb, VersionType version) const
{
    /// Serialization layout
    /// [version] + [states layout]
...
#define M(NAME) \
    ...
    { \
        ...
        { \
            ...
            return convertOneBucketToBlockImpl(result_variants, *result_variants.NAME, result_variants.aggregates_pool, final, clear_states, 0); \
        } \
        else \
...
#define M(NAME) \
    ...
    { \
        ...
        for (auto bucket : gcd_buckets) \
            mergeBucketImpl<Method>(*prepared_data, bucket, first.aggregates_pool, clear_states); \
        if (gcd_buckets.size() > 1) \
        { \
            AggregatedDataVariants result_variants; \
            result_variants.aggregator = this; \
            initDataVariants(result_variants, method_chosen, key_sizes, params); \
            spliceBucketsImpl<Method>(result_variants, first, gcd_buckets, result_variants.aggregates_pool, clear_states); \
            ...
            return convertOneBucketToBlockImpl(first, *first.NAME, first.aggregates_pool, final, clear_states, gcd_buckets[0]); \
    }
...
/// [Legacy]
std::pair<bool, bool> Aggregator::executeAndRetractOnBlockLegacy(
    Columns columns,
    size_t row_begin,
    size_t row_end,
    AggregatedDataVariants & result,
    ...
    bool & no_more_keys) const
{
    ...
    else
    {
        ...
        #define M(NAME, IS_TWO_LEVEL) \
                ...
                need_finalization = executeAndRetractImplLegacy(*result.NAME, result.aggregates_pool, *retracted_result.NAME, retracted_result.aggregates_pool, row_begin, row_end, key_columns, aggregate_functions_instructions.data());
...
Aggregator::mergeRetractedGroupsLegacy(ManyAggregatedDataVariants & aggregated_data, ManyAggregatedDataVariants & retracted_data) const
{
...
#define M(NAME, ...) \
        ...
        mergeRetractedGroupsImplLegacy<decltype(first->NAME)::element_type>(*prepared_data, *prepared_retracted_data);
...
/// [Legacy]
template <typename Method>
void Aggregator::mergeRetractedGroupsImplLegacy(
...
void Aggregator::destroyAggregateStates(AggregateDataPtr & place) const
{
    if (place)
    {
        assert(expanded_data_type != ExpandedDataType::UpdatedWithRetracted || !RetractedDataEx::hasRetracted(place));
...
void Aggregator::doRecoverV3(AggregatedDataVariants & data_variants, ReadBuffer & rb) const
{
    ...
    if (data_variants.type != recovered_data_variants_type)
        throw Exception(
            ErrorCodes::RECOVER_CHECKPOINT_FAILED,
            "Failed to recover aggregation function checkpoint. Aggregated data variant type is not compatible, checkpointed={}, current={}",
    ...
    if (recovered_expanded_data_type != expanded_data_type)
        throw Exception(
            ErrorCodes::RECOVER_CHECKPOINT_FAILED,
            "Failed to recover aggregation function checkpoint. Expanded data type is not the same, checkpointed={}, current={}",
    ...
    auto state_deserializer = [this](auto & place, auto & rb_, Arena * arena) {
        place = arena->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
        createAggregateStates(place);
        ...
        if (expanded_data_type == ExpandedDataType::UpdatedWithRetracted)
        {
            ...
            if (has_retracted)
            {
                retracted = arena->alignedAlloc(total_size_of_aggregate_states, align_aggregate_states);
                createAggregateStates</*skip_expanded_data*/ true>(retracted);
                for (size_t i = 0; i < params.aggregates_size; ++i)
...
#define M(NAME, IS_TWO_LEVEL) \
        ...
        if (use_string_hash_map) \
            DB::deserializeHashMap<true>(data_variants.NAME->data, [&](auto & mapped, Arena & pool, ReadBuffer & rb_) { state_deserializer(mapped, rb_, &pool); }, *data_variants.aggregates_pool, rb); \
        else \
            DB::deserializeHashMap<false>(data_variants.NAME->data, [&](auto & mapped, Arena & pool, ReadBuffer & rb_) { state_deserializer(mapped, rb_, &pool); }, *data_variants.aggregates_pool, rb); \
    }
    ...
    AggregateFunctionInstruction * aggregate_instructions) const
{
    ...
    for (size_t i = row_begin; i < row_end; ++i)
    {
        ...
        else
        {
            ...
            /// If there are aggregate data and no retracted data, copy aggregate data to retracted data before changeds
            if (!UpdatedDataEx::isEmpty(aggregate_data) && !RetractedDataEx::hasRetracted(aggregate_data))
                mergeAggregateStates(RetractedDataEx::getRetracted(aggregate_data), aggregate_data, retracted_pool, /*clear_states*/ false);
        }
    ...
    bool & no_more_keys) const
{
    ...
    if (result.type == AggregatedDataVariants::Type::without_key)
    {
        /// Save last finalization state into `retracted_result` before processing new data.
        /// We shall clear and reset it after finalization
        if (result.without_key && !UpdatedDataEx::isEmpty(result.without_key))
            mergeAggregateStates(RetractedDataEx::getRetracted(result.without_key), result.without_key, result.retracted_pool.get(), /*clear_states*/ false);
...
{
    LOG_TRACE(log, "Converting updated aggregated data to blocks");
...
void NO_INLINE Aggregator::mergeUpdatedGroupsImpl(ManyAggregatedDataVariants & non_empty_data, Arena * arena) const
{
    ...
    for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num)
    {
        ...
        auto merge_updated_func = [&](const auto & key, auto & mapped) {
            ...
            dst_table.emplace(key, dst_it, inserted);
            if (inserted)
                dst_it->getMapped() = nullptr; /// To be merged in next step
        };

        if constexpr (is_two_level)
            src_table.forEachValueOfUpdatedBuckets(std::move(merge_updated_func));
        else
...
{
    LOG_TRACE(log, "Converting retracted aggregated data to blocks");
...
void Aggregator::mergeRetractedState(AggregateDataPtr & dst, AggregateDataPtr & src, Arena * arena) const
{
    assert(src);
    if (!dst)
    {
    ...
    if (RetractedDataEx::hasRetracted(src))
        mergeAggregateStates(dst, RetractedDataEx::getRetracted(src), arena, /*clear_states*/ true);
    else
        /// If retracted data not exist, assume it does't be changed, we should use original data
        mergeAggregateStates(dst, src, arena, /*clear_states*/ false);
}
...
void Aggregator::mergeRetractedGroupsImpl(ManyAggregatedDataVariants & non_empty_data, Arena * arena) const
{
    ...
    for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num)
    {
        ...
        dst_table.forEachValue([&](const auto & key, auto & mapped) {
            assert(!mapped);
            if (auto find_it = src_table.find(key))
                /// NOTE: We always clear the retracted data after merged
                mergeRetractedState(mapped, find_it->getMapped(), arena);
        });
...
AggregatedDataVariantsPtr Aggregator::mergeRetractedGroups(ManyAggregatedDataVariants & data_variants) const
{
    ...
    if (first.type == AggregatedDataVariants::Type::without_key)
    {
        ...
        for (size_t result_num = 1, size = prepared_data_ptr->size(); result_num < size; ++result_num)
            mergeRetractedState(first.without_key, (*prepared_data_ptr)[result_num]->without_key, first.aggregates_pool);
    }
    else
    {
        if (false) { } // NOLINT
