...
void FutureSetFromSubquery::buildSetInplace(const ContextPtr & context)
{
    ...
    auto builder = plan->buildQueryPipeline(QueryPlanOptimizationSettings::fromContext(context), BuildQueryPipelineSettings::fromContext(context), context);
...
SetPtr FutureSetFromSubquery::buildOrderedSetInplace(const ContextPtr & context)
{
    ...
    /// proton: starts
    if (plan->isStreaming())
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Building ordered set for subquery in streaming mode is not supported");
    /// proton: ends
