    ...
    , is_unaligned(context->getSettingsRef().enable_unaligned_array_join)
{
    ...
    if (is_unaligned)
    {
        ...
        function_array_resize = FunctionFactory::instance().get("array_resize", context);
    }
    else if (is_left)
...
void ArrayJoinAction::execute(Block & block)
{
    ...
    auto any_array = getArrayJoinColumnRawPtr(any_array_map_ptr);
    if (!any_array)
        throw Exception(ErrorCodes::TYPE_MISMATCH, "ARRAY JOIN of not array or map: {}", *columns.begin());
    ...
    for (size_t i = 0; i < num_columns; ++i)
    {
        ...
        if (columns.contains(current.name))
        {
            if (!typeid_cast<const DataTypeArray *>(&*current.type))
                throw Exception(ErrorCodes::TYPE_MISMATCH, "ARRAY JOIN of not array: {}", current.name);
