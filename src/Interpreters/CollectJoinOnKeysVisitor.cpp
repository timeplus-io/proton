...
void CollectJoinOnKeysMatcher::visit(const ASTFunction & func, const ASTPtr & ast, Data & data)
{
    ...
    /// `lag_behind` is not actually a join condition but interpreted as a stream modifier`
    ...
    if (func.name == "equals")
    {
        ...
        if (table_numbers.first == table_numbers.second)
        {
            if (table_numbers.first == JoinIdentifierPos::Unknown)
                throw Exception(ErrorCodes::AMBIGUOUS_COLUMN_NAME, "Ambiguous column in expression '{}' in JOIN ON section", queryToString(ast));
