    ...
    const ContextPtr & context)
{
    ...
    for (const String & column_name : updated_columns)
    {
        ...
        if (!found)
        {
            ...
            throw Exception(ErrorCodes::NO_SUCH_COLUMN_IN_STREAM, "There is no column {} in stream", backQuote(column_name));
        }
...
void MutationsInterpreter::prepare(bool dry_run)
{
    ...
    for (auto & command : commands)
    {
        ...
        if (command.type == MutationCommand::DELETE)
        {
            ...
            {
                /// proton: starts.
                predicate = makeASTFunction("is_zero_or_null", predicate);
                /// proton: ends
            }
        ...
        else if (command.type == MutationCommand::READ_COLUMN)
        {
            ...
            throw Exception(ErrorCodes::UNKNOWN_MUTATION_COMMAND, "Unknown mutation command type: {}", command.type);
    }
...
void MutationsInterpreter::prepareMutationStages(std::vector<Stage> & prepared_stages, bool dry_run)
{
    ...
    for (int64_t i = prepared_stages.size() - 1; i >= 0; --i)
    {
        ...
        auto syntax_result = TreeRewriter(context).analyze(
            all_asts, all_columns, source.getStorage(), storage_snapshot, false, true, execute_scalar_subqueries);
