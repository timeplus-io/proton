...
BlockIO InterpreterDeleteQuery::execute()
{
    ...
    else if (table->supportsLightweightDelete())
    {
        if (!getContext()->getSettingsRef().enable_lightweight_delete)
            throw Exception(ErrorCodes::SUPPORT_IS_DISABLED, "Lightweight delete mutate is disabled. Set `enable_lightweight_delete` setting to enable it");

        /// Build "ALTER ... UPDATE _row_exists = 0 WHERE predicate" query
        /// proton: starts(we only support ALTER STREAM instead ALTER TABLE)
        String alter_query =
            "ALTER STREAM " + table->getStorageID().getFullTableName()
