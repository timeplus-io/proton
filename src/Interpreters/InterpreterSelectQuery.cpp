...
void InterpreterSelectQuery::executeFetchColumns(QueryProcessingStage::Enum processing_stage, QueryPlan & query_plan)
{
    ...
    bool optimize_trivial_count = syntax_analyzer_result->optimize_trivial_count && !syntax_analyzer_result->streaming
        && (settings.max_parallel_replicas <= 1) && storage && !row_policy_filter && processing_stage == QueryProcessingStage::FetchColumns
    ...
    if (!query.distinct && !query.limit_with_ties && !query.prewhere() && !query.where() && !query.groupBy() && !query.having()
        && !query.orderBy() && !query.limitBy() && !query.join() && !query_analyzer->hasAggregation() && !query_analyzer->hasWindow()
        && query.limitLength() && limit_length <= std::numeric_limits<UInt64>::max() - limit_offset
    ...
    else if (storage)
    {
        ...
        if (settings.replay_speed > 0)
        {
            ...
            if (Streaming::isAppendStorage(storage->dataStreamSemantic()))
            {
                ...
                if (const auto * storagestream = storage->as<StorageStream>())
                    shards_last_sns = storagestream->getLastSNs();
                else if (const auto * proxy = storage->as<Streaming::ProxyStream>(); proxy)
                {
                    ...
                    if (nested_storage)
                        if (const auto * nested_storage_stream = (*nested_storage)->as<StorageStream>())
                            shards_last_sns = nested_storage_stream->getLastSNs();
                }

                if (!shards_last_sns.size())
                ...
                auto replay_step = std::make_unique<Streaming::ReplayStreamStep>(
                    query_plan.getCurrentDataStream(), settings.replay_speed, std::move(shards_last_sns));
...
void InterpreterSelectQuery::executeRollupOrCube(QueryPlan & query_plan, Modificator modificator)
{
    ...
    auto params = getAggregatorParams(
        query_ptr, *query_analyzer, *context, header_before_transform, keys, query_analyzer->aggregates(), false, settings, 0, 0);
...
void InterpreterSelectQuery::executeMergeSorted(QueryPlan & query_plan, const std::string & description)
{
    ...
    auto merging_sorted
        = std::make_unique<SortingStep>(query_plan.getCurrentDataStream(), std::move(sort_description), max_block_size, limit);
...
void InterpreterSelectQuery::buildStreamingProcessingQueryPlanBeforeJoin(QueryPlan & query_plan)
{
    ...
    if (query_info.hasPartitionByKeys())
    {
        ...
        if (analysis_result.hasJoin())
        {
            ...
            bool only_shuffling_left_stream
                = std::ranges::all_of(query_info.partition_by_keys, [&](const auto & key) { return header.has(key); });
