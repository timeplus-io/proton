...
std::pair<String, Int32> PipelineMetricHandler::executePost(const Poco::JSON::Object::Ptr & payload) const
{
    ...
    /// For query pipeline created without user name (such as materialized view internal query)
    pipeline_metric = query_context->getProcessList().getPipelineMetric(query_id, "");
    if (!pipeline_metric.empty())
        return {pipeline_metric, HTTPResponse::HTTP_OK};

    return {
        jsonErrorResponse("Query Not Found", ErrorCodes::BAD_REQUEST_PARAMETER, query_id),
