...
String getSpecPath(const String & config_path)
{
    ...
    if (config_path_.depth() > 1)
    {
        ...
        spec_path_ = spec_path_.append("proton-spec");
    ...
    else
    {
        ...
        if (Poco::File("/etc/proton-spec/").exists())
        {
            return "/etc/proton-spec/";
        }
    }
    return "";
}
...
std::pair<String, Int32> APISpecHandler::executeGet(const Poco::JSON::Object::Ptr & /*payload*/) const
{
    ...
    if (spec_path.empty())
    {
        return {
            jsonErrorResponse(
                "The proton-spec file : '" + spec_path
                    + "' could not be found, please keep it consistent with the proton-server path",
    ...
    if (!Poco::File(path).exists())
    {
        return {
            jsonErrorResponse(
                "Failed to find the rest-api under the proton-spec , path is : " + path.toString(), ErrorCodes::RESOURCE_NOT_FOUND),
            HTTPResponse::HTTP_NOT_FOUND};
    }
