    ...
    DefaultOrFilter default_or_filter) const
{
    ...
    auto key_types = getOrderedKeyTypes(key_columns_with_types, key_index_map);
...
ColumnUInt8::Ptr
IPAddressDictionary::hasKeys(const ColumnsWithTypeAndName & key_columns_with_types, const std::vector<size_t> & key_index_map) const
{
    validateKeyTypes(getOrderedKeyTypes(key_columns_with_types, key_index_map));

    const auto first_column = key_columns_with_types[key_index_map.front()].column;
...
void registerDictionaryTrie(DictionaryFactory & factory)
{
                             ...
                             bool /*created_from_ddl*/) -> DictionaryPtr
    {
        ...
        bool use_async_executor = timeplus_source && timeplus_source->isLocal() && context->getSettingsRef().dictionary_use_async_executor;
        /// proton: ends
