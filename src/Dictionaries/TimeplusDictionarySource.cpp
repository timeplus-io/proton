...
ConnectionPoolWithFailoverPtr createPool(const TimeplusDictionarySource::Configuration & configuration)
{
...
TimeplusDictionarySource::TimeplusDictionarySource(
...
TimeplusDictionarySource::TimeplusDictionarySource(const TimeplusDictionarySource & other)
...
std::string TimeplusDictionarySource::getUpdateFieldAndDate()
{
...
QueryPipeline TimeplusDictionarySource::loadAllWithSizeHint(std::atomic<size_t> * result_size_hint)
{
    return createStreamForQuery(load_all_query, result_size_hint);
}

QueryPipeline TimeplusDictionarySource::loadAll()
{
    return createStreamForQuery(load_all_query);
}

QueryPipeline TimeplusDictionarySource::loadUpdatedAll()
{
    String load_update_query = getUpdateFieldAndDate();
    return createStreamForQuery(load_update_query);
}

QueryPipeline TimeplusDictionarySource::loadIds(const std::vector<UInt64> & ids)
{
    return createStreamForQuery(query_builder.composeLoadIdsQuery(ids));
}


QueryPipeline TimeplusDictionarySource::loadKeys(const Columns & key_columns, const std::vector<size_t> & requested_rows)
{
...
bool TimeplusDictionarySource::isModified() const
{
...
bool TimeplusDictionarySource::hasUpdateField() const
{
    return !configuration.update_field.empty();
}

std::string TimeplusDictionarySource::toString() const
{
    const std::string & where = configuration.where;
    return "timeplus: " + configuration.db + '.' + configuration.table + (where.empty() ? "" : ", where: " + where);
}

QueryPipeline TimeplusDictionarySource::createStreamForQuery(const String & query, std::atomic<size_t> * result_size_hint)
{
    ...
    /// Always use table query for timeplus source
    context_copy->setSetting("query_mode", String("table"));
    ...
    else
    {
        pipeline = QueryPipeline(std::make_shared<RemoteSource>(
            std::make_shared<RemoteQueryExecutor>(pool, query, empty_sample_block, context_copy),
            false,
            false /* proton: starts */,
            UUIDHelpers::Nil,
            false /* proton: ends */));
...
std::string TimeplusDictionarySource::doInvalidateQuery(const std::string & request) const
{
    LOG_TRACE(logger, "Performing invalidate query");
    ...
    else
    {
        ...
        QueryPipeline pipeline(std::make_shared<RemoteSource>(
            std::make_shared<RemoteQueryExecutor>(pool, request, invalidate_sample_block, context_copy),
            false,
            false /* proton: starts */,
            UUIDHelpers::Nil,
            false /* proton: ends */));
...
void registerDictionarySourceTimeplus(DictionarySourceFactory & factory)
{
    auto create_table_source = [=](const DictionaryStructure & dict_struct,
        ...
        std::string settings_config_prefix = config_prefix + ".timeplus";
        ...
        std::string table = config.getString(settings_config_prefix + ".stream", "");
        ...
        TimeplusDictionarySource::Configuration configuration{
            .host = host,
            .user = user,
            .password = password,
            .quota_key = quota_key,
            ...
            .port = port,
            .is_local = isLocalAddress({host, port}, global_context->getTCPPort().port),
        ...
        if (dictionary_name == configuration.table && dictionary_database == configuration.db)
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "TimeplusDictionarySource stream cannot be dictionary");

        return std::make_unique<TimeplusDictionarySource>(dict_struct, configuration, sample_block, context);
    };

    factory.registerSource("timeplus", create_table_source);
}
