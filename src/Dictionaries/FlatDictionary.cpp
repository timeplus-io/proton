    ...
    DefaultOrFilter default_or_filter) const
{
    ...
    const auto & ids = getColumnVectorData(this, key_columns_with_types[key_index_map.front()].column, backup_storage);
...
ColumnUInt8::Ptr FlatDictionary::hasKeys(const ColumnsWithTypeAndName & key_columns_with_types, const std::vector<size_t> & key_index_map) const
{
    PaddedPODArray<UInt64> backup_storage;
    const auto & keys = getColumnVectorData(this, key_columns_with_types[key_index_map.front()].column, backup_storage);
