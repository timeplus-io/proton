...
namespace DB
{
/** Allows loading dictionaries from local or remote proton instance
  *    @todo use ConnectionPoolWithFailover
  *    @todo invent a way to keep track of source modifications
  */
class TimeplusDictionarySource final : public IDictionarySource
{
public:
    ...
    TimeplusDictionarySource(const TimeplusDictionarySource & other);
    TimeplusDictionarySource & operator=(const TimeplusDictionarySource &) = delete;
    ...
    DictionarySourcePtr clone() const override { return std::make_shared<TimeplusDictionarySource>(*this); }
...
private:
    ...
    LoggerPtr logger = getLogger("TimeplusDictionarySource");
};
