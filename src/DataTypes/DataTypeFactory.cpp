...
{
    return getImpl<false>(full_name, compatible_with_clickhouse);
}

DataTypePtr DataTypeFactory::tryGet(const String & full_name/* proton: starts*/, bool compatible_with_clickhouse/* proton: ends*/) const
{
    return getImpl<true>(full_name, compatible_with_clickhouse);
}

template <bool nullptr_on_error>
DataTypePtr DataTypeFactory::getImpl(const String & full_name/* proton: starts*/, bool compatible_with_clickhouse/* proton: ends*/) const
{
    ...
    return getImpl<nullptr_on_error>(ast, compatible_with_clickhouse);
}

DataTypePtr DataTypeFactory::get(const ASTPtr & ast/* proton: starts */, bool compatible_with_clickhouse/* proton: ends*/) const
{
    return getImpl<false>(ast, compatible_with_clickhouse);
}

DataTypePtr DataTypeFactory::tryGet(const ASTPtr & ast/* proton: starts*/, bool compatible_with_clickhouse/* proton: ends*/) const
{
    return getImpl<true>(ast, compatible_with_clickhouse);
}

template <bool nullptr_on_error>
DataTypePtr DataTypeFactory::getImpl(const ASTPtr & ast/* proton: starts*/, bool compatible_with_clickhouse/* proton: ends*/) const
{
    if (const auto * type = ast->as<ASTDataType>())
    {
        return getImpl<nullptr_on_error>(type->name, type->arguments, compatible_with_clickhouse);
    }

    if (const auto * ident = ast->as<ASTIdentifier>())
    {
        return getImpl<nullptr_on_error>(ident->name(), {}, compatible_with_clickhouse);
    }

    if (const auto * lit = ast->as<ASTLiteral>())
    {
        if (lit->value.isNull())
            return getImpl<nullptr_on_error>("null", {}, compatible_with_clickhouse);
    }
...
{
    return getImpl<false>(family_name_param, parameters, compatible_with_clickhouse);
}

DataTypePtr DataTypeFactory::tryGet(const String & family_name_param, const ASTPtr & parameters/* proton: starts */, bool compatible_with_clickhouse/* proton: ends */) const
{
    return getImpl<true>(family_name_param, parameters, compatible_with_clickhouse);
}

template <bool nullptr_on_error>
DataTypePtr DataTypeFactory::getImpl(const String & family_name_param, const ASTPtr & parameters/* proton: starts */, bool compatible_with_clickhouse/* proton: ends */) const
{
    String family_name;
    ...
    if (endsWith(family_name, "_with_dictionary")/* proton: starts */ || (compatible_with_clickhouse && endsWith(family_name, "WithDictionary"))/* proton: ends */)
    {
        ...
        return getImpl<nullptr_on_error>("low_cardinality", low_cardinality_params, compatible_with_clickhouse);
    }
    ...
    if constexpr (nullptr_on_error)
    {
        ...
        try
        {
            return (*creator)(parameters, compatible_with_clickhouse);
        }
        catch (...)
        {
            return nullptr;
