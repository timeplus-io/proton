...
/// proton: starts.
void SerializationVariant::deserializeBinaryBulkWithMultipleStreamsDiscard(
    size_t limit, DeserializeBinaryBulkSettings & settings, DeserializeBinaryBulkStatePtr & state) const
{
    /// First, deserialize discriminators.
    settings.path.push_back(Substream::VariantDiscriminators);
    ...
    DeserializeBinaryBulkStateVariant * variant_state = nullptr;
    std::vector<size_t> variant_limits;
    ColumnPtr local_discriminators = ColumnVariant::ColumnDiscriminators::create();
    if (auto * discriminators_stream = settings.getter(settings.path))
    {
        variant_state = checkAndGetState<DeserializeBinaryBulkStateVariant>(state);
        auto * discriminators_state = checkAndGetState<DeserializeBinaryBulkStateVariantDiscriminators>(variant_state->discriminators_state);
        ...
        /// Deserialize discriminators according to serialization mode.
        if (discriminators_state->mode.value == DiscriminatorsSerializationMode::BASIC)
            SerializationNumber<ColumnVariant::Discriminator>().deserializeBinaryBulk(*local_discriminators->assumeMutable(), *discriminators_stream, limit, 0);
        else
            variant_limits = deserializeCompactDiscriminators(local_discriminators, limit, discriminators_stream, settings.continuous_reading, *discriminators_state);
    }
    /// It may happen that there is no such stream, in this case just do nothing.
    else
    {
    ...
    {
        variant_limits.resize(variants.size(), 0);
        const auto & discriminators_data = assert_cast<const ColumnVariant::ColumnDiscriminators &>(*local_discriminators).getData();

        if (discriminators_data.size() != limit)
            throw Exception(
                ErrorCodes::INCORRECT_DATA,
                "Expected {} discriminators, but got {}. This can happen if the number of read rows is less than the limit.",
                limit,
    ...
    {
        addVariantElementToPath(settings.path, i);
        variants[i]->deserializeBinaryBulkWithMultipleStreamsDiscard(variant_limits[i], settings, variant_state->variant_states[i]);
        settings.path.pop_back();
    }
    settings.path.pop_back();
}

void SerializationVariant::deserializeBinaryDiscard(ReadBuffer & istr) const
{
    ColumnVariant::Discriminator global_discr;
    readBinaryLittleEndian(global_discr, istr);
    if (global_discr != ColumnVariant::NULL_DISCRIMINATOR)
        variants[global_discr]->deserializeBinaryDiscard(istr);
}
/// proton: ends.
}
