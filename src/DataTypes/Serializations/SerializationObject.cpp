...
SerializationPtr getObjectSerialization(const String & schema_format, [[maybe_unused]] const Names & partial_deserialized_subcolumns)
{
    ...
    if (schema_format == "json")
    {
        ...
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
            "To use data type json with JSON format proton should be built with Simdjson or Rapidjson");
#endif
    }
*/
...
void SerializationObject::deserializeBinaryBulkWithMultipleStreamsDiscard(
    size_t limit, DeserializeBinaryBulkSettings & settings, DeserializeBinaryBulkStatePtr & state) const
{
    ...
    if (structure_state->serialization_version.value == ObjectSerializationVersion::Value::STRING)
    {
        ...
        FormatSettings format_settings = settings.format_settings ? *settings.format_settings : FormatSettings{};
        for (size_t i = 0; i != limit; ++i)
            readStringBinary(discard_data, *data_stream);
    ...
    for (const auto & path : sorted_typed_paths)
    {
        settings.path.push_back(Substream::ObjectTypedPath);
        settings.path.back().object_path_name = path;
        typed_path_serializations.at(path)->deserializeBinaryBulkWithMultipleStreamsDiscard(limit, settings, object_state->typed_path_states[path]);
        settings.path.pop_back();
    }
    ...
    for (const auto & path : structure_state->sorted_dynamic_paths)
    {
        settings.path.push_back(Substream::ObjectDynamicPath);
        settings.path.back().object_path_name = path;
        dynamic_serialization->deserializeBinaryBulkWithMultipleStreamsDiscard(limit, settings, object_state->dynamic_path_states[path]);
        settings.path.pop_back();
    }

    settings.path.push_back(Substream::ObjectSharedData);
    shared_data_serialization->deserializeBinaryBulkWithMultipleStreamsDiscard(limit, settings, object_state->shared_data_state);
    settings.path.pop_back();
    settings.path.pop_back();
}

void SerializationObject::deserializeBinaryDiscard(ReadBuffer & istr) const
{
    size_t number_of_paths;
    readVarUInt(number_of_paths, istr);
    /// Read pairs (path, value).
    for (size_t i = 0; i != number_of_paths; ++i)
    {
        ...
        {
            if (auto it = typed_path_serializations.find(path); it != typed_path_serializations.end())
                it->second->deserializeBinaryDiscard(istr);
            else
                dynamic_serialization->deserializeBinaryDiscard(istr);
        }
        else
        {
            /// Skip value of this path.
