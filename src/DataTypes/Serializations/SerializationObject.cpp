...
void SerializationObject<Parser>::checkSerializationIsSupported(const TSettings & settings) const
{
    if (settings.position_independent_encoding)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
            "DataTypeJson doesn't support serialization with position independent encoding");
}
    ...
    SerializeBinaryBulkStatePtr & state) const
{
    ...
    if (state)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
            "DataTypeJson doesn't support serialization with non-trivial state");
    ...
    DeserializeBinaryBulkStatePtr & state) const
{
    ...
    if (!stream)
        throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA,
            "Cannot read kind of binary serialization of DataTypeJson, because its stream is missing");
    ...
    SubstreamsCache * cache) const
{
    ...
    if (!column->empty())
        throw Exception(ErrorCodes::NOT_IMPLEMENTED,
            "DataTypeJson cannot be deserialized to non-empty column");
    ...
    /// FIXME, partial deserialization. We deserialize fully and filter out unwanted subcolumns
    ...
    DeserializeBinaryBulkStatePtr & state) const
{
    ...
    for (size_t i = 0; i < num_subcolumns; ++i)
    {
        ...
        {
            throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA,
                "Cannot read structure of DataTypeJson, because its stream is missing");
        }
        ...
        {
            throw Exception(ErrorCodes::CANNOT_READ_ALL_DATA,
                "Cannot read subcolumn '{}' of DataTypeJson, because its stream is missing", key.getPath());
        }
    }
