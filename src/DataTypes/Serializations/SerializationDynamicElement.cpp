...
/// proton: starts.
void SerializationDynamicElement::deserializeBinaryBulkWithMultipleStreamsDiscard(
    size_t limit, DeserializeBinaryBulkSettings & settings, DeserializeBinaryBulkStatePtr & state) const
{
    ...
    auto * dynamic_element_state = checkAndGetState<DeserializeBinaryBulkStateDynamicElement>(state);
    settings.path.push_back(Substream::DynamicData);
    dynamic_element_state->variant_serialization->deserializeBinaryBulkWithMultipleStreamsDiscard(
        limit, settings, dynamic_element_state->variant_element_state);
    settings.path.pop_back();
}
/// proton: ends.
