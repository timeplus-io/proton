...
/// proton: starts.
void SerializationDynamic::deserializeBinaryBulkWithMultipleStreamsDiscard(
    size_t limit, DeserializeBinaryBulkSettings & settings, DeserializeBinaryBulkStatePtr & state) const
{
    ...
    settings.path.push_back(Substream::DynamicData);
    dynamic_state->variant_serialization->deserializeBinaryBulkWithMultipleStreamsDiscard(limit, settings, dynamic_state->variant_state);
    settings.path.pop_back();
}

void SerializationDynamic::deserializeBinaryDiscard(ReadBuffer & istr) const
{
    auto field_type = decodeDataType(istr);
    if (isNothing(field_type))
        return;

    field_type->getDefaultSerialization()->deserializeBinaryDiscard(istr);
}
/// proton: ends.
}
