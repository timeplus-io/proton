...
/// proton: starts
void SerializationTuple::deserializeBinaryBulkWithMultipleStreamsDiscard(
    size_t limit,
    DeserializeBinaryBulkSettings & settings,
    DeserializeBinaryBulkStatePtr & state) const
{
    auto * tuple_state = checkAndGetState<DeserializeBinaryBulkStateTuple>(state);
    for (size_t i = 0; i < elems.size(); ++i)
        elems[i]->deserializeBinaryBulkWithMultipleStreamsDiscard(limit, settings, tuple_state->states[i]);
}

void SerializationTuple::deserializeBinaryDiscard(ReadBuffer & istr) const
{
    for (size_t i = 0; i < elems.size(); ++i)
        elems[i]->deserializeBinaryDiscard(istr);
}
/// proton: ends
}
