...
/// proton: starts
void SerializationNullable::deserializeBinaryBulkWithMultipleStreamsDiscard(
    size_t limit,
    DeserializeBinaryBulkSettings & settings,
    DeserializeBinaryBulkStatePtr & state) const
{
    settings.path.push_back(Substream::NullMap);
    if (auto * stream = settings.getter(settings.path))
        SerializationNumber<UInt8>().deserializeBinaryBulkDiscard(*stream, limit);

    settings.path.back() = Substream::NullableElements;
    nested->deserializeBinaryBulkWithMultipleStreamsDiscard(limit, settings, state);
    settings.path.pop_back();
}

void SerializationNullable::deserializeBinaryDiscard(ReadBuffer & istr) const
{
    bool is_null = false;
    readBinary(is_null, istr);
    if (!is_null)
...
void SerializationNullable::serializeBinaryPrefixTree(
    const Field & field, String & encoded, const FormatSettings & settings, bool ascending) const
{
    bool is_null = field.isNull();
    if (ascending)
        /// null value is always smaller than non-null value
        PrefixTreeEncode::encodeVarUIntAscending(!is_null, encoded);
    else
        PrefixTreeEncode::encodeVarUIntDescending(!is_null, encoded);

    if (!is_null)
        nested->serializeBinaryPrefixTree(field, encoded, settings, ascending);
}
    ...
    const IColumn & column, size_t row_num, String & encoded, const FormatSettings & settings, bool ascending) const
{
    ...
    if (!is_null)
        nested->serializeBinaryPrefixTree(col.getNestedColumn(), row_num, encoded, settings, ascending);
}

void SerializationNullable::deserializeBinaryPrefixTree(IColumn & column, std::string_view & data, const FormatSettings & settings, bool ascending) const
{
    ColumnNullable & col = assert_cast<ColumnNullable &>(column);
    bool is_null = false;

    if (ascending)
        is_null = PrefixTreeEncode::decodeVarUIntAscending(data) == 0;
    else
        is_null = PrefixTreeEncode::decodeVarUIntDescending(data) == 0;

    if (is_null)
        col.getNestedColumn().insertDefault();
    else
        nested->deserializeBinaryPrefixTree(col.getNestedColumn(), data, settings, ascending);

    safeAppendToNullMap<void>(col, is_null);
}

void SerializationNullable::deserializeBinaryPrefixTreeDiscard(
    std::string_view & data, const FormatSettings & settings, bool ascending) const
{
    ...
    if (!is_null)
        nested->deserializeBinaryPrefixTreeDiscard(data, settings, ascending);
}
