...
/// proton: starts
void ISerialization::deserializeBinaryBulkWithMultipleStreamsDiscard(
    size_t limit,
    DeserializeBinaryBulkSettings & settings,
    DeserializeBinaryBulkStatePtr & /*state*/) const
{
    ...
    if (ReadBuffer * stream = settings.getter(settings.path))
        deserializeBinaryBulkDiscard(*stream, limit);

    settings.path.pop_back();
}

void ISerialization::deserializeBinaryBulkDiscard(DB::ReadBuffer &, size_t /*limit*/) const
{
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "deserialization discard is not supported for the current column type");
}

void ISerialization::deserializeBinaryDiscard(ReadBuffer & istr) const
{
    deserializeBinaryBulkDiscard(istr, 1);
}

void ISerialization::serializeBinaryPrefixTree(const Field & field, String & /*encoded*/, const FormatSettings &, bool /*ascending*/) const
{
    throw Exception(
        ErrorCodes::NOT_IMPLEMENTED, "Prefix tree serialization is not supported for the current column type '{}'", field.getTypeName());
}

void ISerialization::serializeBinaryPrefixTree(
    const IColumn & column, size_t /*row_num*/, String & /*encoded*/, const FormatSettings &, bool /*ascending*/) const
{
    throw Exception(
        ErrorCodes::NOT_IMPLEMENTED, "Prefix tree serialization is not supported for the current column type '{}'", column.getFamilyName());
}

void ISerialization::deserializeBinaryPrefixTree(
    IColumn & column, std::string_view & /*data*/, const DB::FormatSettings &, bool /*ascending*/) const
{
    throw Exception(
        ErrorCodes::NOT_IMPLEMENTED,
        "Prefix tree deserialization is not supported for the current column type '{}'",
        column.getFamilyName());
}

void ISerialization::deserializeBinaryPrefixTreeDiscard(std::string_view & /*data*/, const DB::FormatSettings &, bool /*ascending*/) const
{
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Prefix tree deserialization discard is not supported for the current column type");
}
/// proton: ends
