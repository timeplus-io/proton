...
public:
    ...
    /// proton: starts
    /// deserialization of a column and then discard what has deserialized. Used for streaming processing
    virtual void deserializeBinaryBulkWithMultipleStreamsDiscard(
        size_t limit,
        DeserializeBinaryBulkSettings & settings,
        DeserializeBinaryBulkStatePtr & state) const;

    /// Common serialization shall override this method
    virtual void deserializeBinaryBulkDiscard(ReadBuffer & /*istr*/, size_t /*limit*/) const;

    /// single value deserialization and discard.
    virtual void deserializeBinaryDiscard(ReadBuffer & /*istr*/) const;

    /// PrefixTree encoding for kv stream
    virtual void serializeBinaryPrefixTree(const Field & field, String & encoded, const FormatSettings &, bool ascending) const;

    /// Serialize one value of a column at specified row number.
    virtual void
    serializeBinaryPrefixTree(const IColumn & column, size_t row_num, String & encoded, const FormatSettings &, bool ascending) const;
    ...
    virtual void deserializeBinaryPrefixTree(IColumn & column, std::string_view & data, const FormatSettings &, bool ascending) const;

    /// Deserialization of a prefix tree encoded column and then discard the parsed value.
    /// It basically uses the parsing side effect to move the input position of \param data to the next encoded value
    /// Overrides can have better implementation to avoid memory allocation.
    virtual void deserializeBinaryPrefixTreeDiscard(std::string_view & data, const DB::FormatSettings &, bool /*ascending*/) const;
    /// proton: ends
