...
static std::pair<DataTypePtr, DataTypeCustomDescPtr> create(const ASTPtr & arguments)
{
    ...
    auto data_type = std::make_shared<DataTypeTuple>(nested_types, nested_names);
    auto custom_name = std::make_unique<DataTypeJsonCustomName>(nested_types, nested_names);
...
{
    auto custom_desc = std::make_unique<DataTypeCustomDesc>(std::make_unique<DataTypeJsonCustomName>(types, names));
