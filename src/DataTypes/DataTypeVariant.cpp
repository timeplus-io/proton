...
DataTypeVariant::DataTypeVariant(const DataTypes & variants_)
{
    ...
    for (const auto & type : variants_)
    {
        ...
        if (isNullableOrLowCardinalityNullable(type))
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "nullable/low_cardinality(nullable) types are not allowed inside variant type");
    ...
    if (variants.empty())
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Type variant should have at least one nested type");

    if (variants.size() > ColumnVariant::MAX_NESTED_COLUMNS)
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Type variant with more than {} nested types is not allowed", ColumnVariant::MAX_NESTED_COLUMNS);
}
