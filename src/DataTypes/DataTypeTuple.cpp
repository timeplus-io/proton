...
size_t DataTypeTuple::getPositionByName(const String & name) const
{
    ...
    for (size_t i = 0; i < size; ++i)
        if (names[i] == name)
            return i;
    throw Exception(ErrorCodes::NOT_FOUND_COLUMN_IN_BLOCK, "the tuple doesn't have element with name '{}'", name);
}
...
{
    if (i == 0 || i > names.size())
        throw Exception(ErrorCodes::ILLEGAL_INDEX, "Index of tuple element ({}) is out range ([1, {}])", i, names.size());
