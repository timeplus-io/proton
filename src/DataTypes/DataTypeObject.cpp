...
static DataTypePtr createJSON(const ASTPtr & arguments, [[maybe_unused]] bool compatible_with_clickhouse = false) /// proton: updated
{
    auto context = CurrentThread::getQueryContext();
    if (!context)
        context = Context::getGlobalContextInstance();

    /// proton: updates. Remove old json type
