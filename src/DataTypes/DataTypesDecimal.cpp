...
static DataTypePtr create(const ASTPtr & arguments, [[maybe_unused]] bool compatible_with_clickhouse = false) /// proton: updated
{
    if (!arguments || arguments->children.size() != 2)
        throw Exception(ErrorCodes::NUMBER_OF_ARGUMENTS_DOESNT_MATCH,
                        "the decimal data type family must have exactly two arguments: precision and scale");
    ...
    if (!precision || precision->value.getType() != Field::Types::UInt64 ||
        ...
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "the decimal data type family must have two numbers as its arguments");
...
static DataTypePtr createExact(const ASTPtr & arguments, [[maybe_unused]] bool compatible_with_clickhouse = false) /// proton: updated
{
    ...
    if (!scale_arg || !(scale_arg->value.getType() == Field::Types::Int64 || scale_arg->value.getType() == Field::Types::UInt64))
        throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "the decimal data type family must have a two numbers as its arguments");
