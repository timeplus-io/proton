...
{
struct CreateDatabaseResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    CreateDatabaseResponseData() = default;

    CreateDatabaseResponseData(cluster::NodeID replica_leader_, int64_t sn_) : replica_leader(replica_leader_), sn(sn_) { }

    CreateDatabaseResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit CreateDatabaseResponseData(Error && err_) : err(std::move(err_)) { }
...
using CreateDatabaseResponseDataPtr = std::shared_ptr<CreateDatabaseResponseData>;
}
