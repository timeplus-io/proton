...
{
void GetAlertRequestData::serialize(DB::WriteBuffer & wb, uint16_t /*version*/) const
{
...
void GetAlertRequestData::doDeserialize(DB::ReadBuffer & rb, uint16_t /*version*/)
{
...
size_t GetAlertRequestData::approximateSerializedSize() const noexcept
{
    return approximateSerializedSizeOf(ns, name) + sizeof(versions_requested) + sizeof(initiator) + sizeof(consistent_read)
        + sizeof(timeout_ms);
}

std::string GetAlertRequestData::doString() const
{
    return fmt::format(
