...
{
struct CreateStoragePolicyResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    CreateStoragePolicyResponseData() = default;

    CreateStoragePolicyResponseData(cluster::NodeID replica_leader_, int64_t sn_) : replica_leader(replica_leader_), sn(sn_) { }

    CreateStoragePolicyResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit CreateStoragePolicyResponseData(Error && err_) : err(std::move(err_)) { }
...
using CreateStoragePolicyResponseDataPtr = std::shared_ptr<CreateStoragePolicyResponseData>;
}
