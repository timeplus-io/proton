...
{
struct ListAlertsResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    ListAlertsResponseData() = default;

    ListAlertsResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit ListAlertsResponseData(Error && err_) : err(std::move(err_)) { }

    explicit ListAlertsResponseData(AlertDescriptorPtrs && descs_, cluster::NodeID replica_leader_, int64_t sn_)
...
using ListAlertsResponseDataPtr = std::shared_ptr<ListAlertsResponseData>;
}
