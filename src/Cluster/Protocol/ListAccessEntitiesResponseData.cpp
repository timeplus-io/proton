...
{
void ListAccessEntitiesResponseData::serialize(DB::WriteBuffer & wb, uint16_t version) const
{
...
void ListAccessEntitiesResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t version)
{
...
size_t ListAccessEntitiesResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string ListAccessEntitiesResponseData::doString() const
{
    if (err.hasError())
        return err.string();
