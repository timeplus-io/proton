...
{
struct CreateFormatSchemaResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    CreateFormatSchemaResponseData() = default;

    CreateFormatSchemaResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit CreateFormatSchemaResponseData(Error && err_) : err(std::move(err_)) { }

    explicit CreateFormatSchemaResponseData(FormatSchemaDescriptor && desc_, cluster::NodeID replica_leader_, int64_t sn_)
        : desc(std::move(desc_)), replica_leader(replica_leader_), sn(sn_)
    {
    }

    explicit CreateFormatSchemaResponseData(const FormatSchemaDescriptor & desc_, cluster::NodeID replica_leader_, int64_t sn_)
...
using CreateFormatSchemaResponseDataPtr = std::shared_ptr<CreateFormatSchemaResponseData>;
}
