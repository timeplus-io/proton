...
{
struct GetAlertResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    GetAlertResponseData() = default;

    GetAlertResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit GetAlertResponseData(Error && err_) : err(std::move(err_)) { }

    explicit GetAlertResponseData(AlertDescriptorPtrs && descs_, cluster::NodeID replica_leader_, int64_t sn_)
        : descs(std::move(descs_)), replica_leader(replica_leader_), sn(sn_)
    {
    }

    protocol::OpCode opCode() const noexcept override { return protocol::OpCode::GetAlert; }
...
public:
    Error err;
    AlertDescriptorPtrs descs;

    cluster::NodeID replica_leader = Nulls::NullNodeID;
    int64_t sn = 0;
};

using GetAlertResponseDataPtr = std::shared_ptr<GetAlertResponseData>;
}
