...
{
struct RenameStreamResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    RenameStreamResponseData() = default;

    RenameStreamResponseData(cluster::NodeID replica_leader_, int64_t sn_) : replica_leader(replica_leader_), sn(sn_) { }

    RenameStreamResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }
    explicit RenameStreamResponseData(Error && err_) : err(std::move(err_)) { }
...
using RenameStreamResponseDataPtr = std::shared_ptr<RenameStreamResponseData>;
}
