...
{
struct CreateAccessEntityResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    CreateAccessEntityResponseData() = default;

    CreateAccessEntityResponseData(cluster::NodeID replica_leader_, int64_t sn_) : replica_leader(replica_leader_), sn(sn_) { }

    explicit CreateAccessEntityResponseData(Error err_) : err(std::move(err_)) { }
    CreateAccessEntityResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }
...
using CreateAccessEntityResponseDataPtr = std::shared_ptr<CreateAccessEntityResponseData>;
}
