...
{
struct UpdateStreamSettingsResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    UpdateStreamSettingsResponseData() = default;

    UpdateStreamSettingsResponseData(cluster::NodeID replica_leader_, int64_t sn_) : replica_leader(replica_leader_), sn(sn_) { }

    UpdateStreamSettingsResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit UpdateStreamSettingsResponseData(Error && err_) : err(std::move(err_)) { }
...
using UpdateStreamSettingsResponseDataPtr = std::shared_ptr<UpdateStreamSettingsResponseData>;
}
