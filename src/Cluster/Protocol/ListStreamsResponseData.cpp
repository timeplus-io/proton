...
{
void ListStreamsResponseData::serialize(DB::WriteBuffer & wb, uint16_t version) const
{
...
void ListStreamsResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t version)
{
...
size_t ListStreamsResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string ListStreamsResponseData::doString() const
{
    if (err.hasError())
        return err.string();
