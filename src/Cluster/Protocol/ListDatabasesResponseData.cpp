...
{
void ListDatabasesResponseData::serialize(DB::WriteBuffer & wb, uint16_t version) const
{
...
void ListDatabasesResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t version)
{
...
size_t ListDatabasesResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string ListDatabasesResponseData::doString() const
{
    if (err.hasError())
        return err.string();
