...
{
void ListAlertsResponseData::serialize(DB::WriteBuffer & wb, uint16_t version) const
{
...
void ListAlertsResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t version)
{
...
size_t ListAlertsResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string ListAlertsResponseData::doString() const
{
    if (err.hasError())
        return err.string();
