...
{
struct DeleteStoragePolicyResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    DeleteStoragePolicyResponseData() = default;

    DeleteStoragePolicyResponseData(cluster::NodeID replica_leader_, int64_t sn_) : replica_leader(replica_leader_), sn(sn_) { }

    DeleteStoragePolicyResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit DeleteStoragePolicyResponseData(Error && err_) : err(std::move(err_)) { }
...
using DeleteStoragePolicyResponseDataPtr = std::shared_ptr<DeleteStoragePolicyResponseData>;
}
