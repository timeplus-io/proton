...
{
void RenameStreamResponseData::serialize(DB::WriteBuffer & wb, uint16_t /*version*/) const
{
...
void RenameStreamResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t /*version*/)
{
...
size_t RenameStreamResponseData::approximateSerializedSize() const noexcept
{
    return err.approximateSerializedSize() + sizeof(replica_leader) + sizeof(sn);
}

std::string RenameStreamResponseData::doString() const
{
