...
{
void ListTasksResponseData::serialize(DB::WriteBuffer & wb, uint16_t version) const
{
...
void ListTasksResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t version)
{
...
size_t ListTasksResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string ListTasksResponseData::doString() const
{
    if (err.hasError())
        return err.string();
