...
{
void GetAlertResponseData::serialize(DB::WriteBuffer & wb, uint16_t version) const
{
...
void GetAlertResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t version)
{
...
size_t GetAlertResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string GetAlertResponseData::doString() const
{
    if (err.hasError())
        return err.string();
