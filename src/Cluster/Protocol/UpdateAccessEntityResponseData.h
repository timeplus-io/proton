...
{
struct UpdateAccessEntityResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    UpdateAccessEntityResponseData() = default;

    UpdateAccessEntityResponseData(cluster::NodeID replica_leader_, int64_t sn_) : replica_leader(replica_leader_), sn(sn_) { }

    explicit UpdateAccessEntityResponseData(Error err_) : err(std::move(err_)) { }
    UpdateAccessEntityResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }
...
using UpdateAccessEntityResponseDataPtr = std::shared_ptr<UpdateAccessEntityResponseData>;
}
