...
{
void DeleteTaskResponseData::serialize(DB::WriteBuffer & wb, uint16_t /*version*/) const
{
...
void DeleteTaskResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t /*version*/)
{
...
size_t DeleteTaskResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string DeleteTaskResponseData::doString() const
{
    if (err.hasError())
        return err.string();
