...
{
void AlterTaskResponseData::serialize(DB::WriteBuffer & wb, uint16_t /*version*/) const
{
...
void AlterTaskResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t /*version*/)
{
...
size_t AlterTaskResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string AlterTaskResponseData::doString() const
{
    if (err.hasError())
        return err.string();
