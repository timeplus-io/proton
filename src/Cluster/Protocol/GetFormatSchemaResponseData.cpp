...
{
void GetFormatSchemaResponseData::serialize(DB::WriteBuffer & wb, uint16_t version) const
{
...
void GetFormatSchemaResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t version)
{
...
size_t GetFormatSchemaResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string GetFormatSchemaResponseData::doString() const
{
    if (err.hasError())
        return err.string();
