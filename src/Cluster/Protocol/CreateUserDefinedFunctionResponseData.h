...
{
struct CreateUserDefinedFunctionResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    CreateUserDefinedFunctionResponseData() = default;

    CreateUserDefinedFunctionResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit CreateUserDefinedFunctionResponseData(Error && err_) : err(std::move(err_)) { }

    explicit CreateUserDefinedFunctionResponseData(UserDefinedFunctionDescriptor && desc_, cluster::NodeID replica_leader_, int64_t sn_)
        : desc(std::move(desc_)), replica_leader(replica_leader_), sn(sn_)
    {
    }

    explicit CreateUserDefinedFunctionResponseData(
        const UserDefinedFunctionDescriptor & desc_, cluster::NodeID replica_leader_, int64_t sn_)
...
using CreateUserDefinedFunctionResponseDataPtr = std::shared_ptr<CreateUserDefinedFunctionResponseData>;
}
