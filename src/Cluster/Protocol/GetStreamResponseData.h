...
{
struct GetStreamResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    GetStreamResponseData() = default;

    GetStreamResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit GetStreamResponseData(Error && err_) : err(std::move(err_)) { }

    explicit GetStreamResponseData(StreamDescriptorPtrs && stream_descs_, cluster::NodeID replica_leader_, int64_t sn_)
...
using GetStreamResponseDataPtr = std::shared_ptr<GetStreamResponseData>;
}
