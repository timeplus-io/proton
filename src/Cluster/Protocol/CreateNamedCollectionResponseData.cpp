...
{
void CreateNamedCollectionResponseData::serialize(DB::WriteBuffer & wb, uint16_t /*version*/) const
{
...
void CreateNamedCollectionResponseData::doDeserialize(DB::ReadBuffer & rb, uint16_t /*version*/)
{
...
size_t CreateNamedCollectionResponseData::approximateSerializedSize() const noexcept
{
    if (err.hasError())
...
std::string CreateNamedCollectionResponseData::doString() const
{
    if (err.hasError())
        return err.string();
