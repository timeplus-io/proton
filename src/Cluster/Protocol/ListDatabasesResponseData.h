...
{
struct ListDatabasesResponseData final : public ProtocolData
{
public:
    /// Used for deserialization
    ListDatabasesResponseData() = default;

    ListDatabasesResponseData(int32_t error_code, std::string && error_message) : err(error_code, std::move(error_message)) { }

    explicit ListDatabasesResponseData(Error && err_) : err(std::move(err_)) { }

    explicit ListDatabasesResponseData(DatabaseDescriptorPtrs && databases_, cluster::NodeID replica_leader_, int64_t sn_)
        : databases(std::move(databases_)), replica_leader(replica_leader_), sn(sn_)
    {
    }
...
public:
    Error err;
    DatabaseDescriptorPtrs databases;

    cluster::NodeID replica_leader = Nulls::NullNodeID;
    int64_t sn = 0;
};

using ListDatabasesResponseDataPtr = std::shared_ptr<ListDatabasesResponseData>;
}
