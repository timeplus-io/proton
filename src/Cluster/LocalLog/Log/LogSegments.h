...
public:
    ...
    /// > sn
    /// \return A list of segments which have base sn greater than the
    /// given sn if exists
    std::vector<LogSegmentPtr> upperSegments(int64_t sn) const;
    ...
    std::vector<LogSegmentPtr> segmentsBetween(int64_t from, int64_t to) const;

    std::vector<LogSegmentPtr> allSegments() const;
