...
#include <Cluster/KafkaLog/KafkaWALStats.h>
...
initRdKafkaHandle(rd_kafka_type_t type, const KConfParams & params, KafkaWALStats * stats, KConfCallback cb_setup)
{
    ...
    if (!kconf)
    {
        ...
        throw DB::Exception(mapErrorCode(rd_kafka_last_error()), "Failed to create kafka conf");
    }
    ...
    for (const auto & param : params)
    {
        ...
        if (ret != RD_KAFKA_CONF_OK)
        {
            ...
            throw DB::Exception(DB::ErrorCodes::INVALID_CONFIG_PARAMETER, "Failed to create kafka conf");
        }
    }
    ...
    /// \rd_kafka_new takes the ownership of \kconf after invoking rd_kafka_new
    /// no matter the invocation is successful or not
...
initRdKafkaTopicHandle(const std::string & topic, const KConfParams & params, rd_kafka_t * rd_kafka, KafkaWALStats * stats)
{
    ...
    if (!tconf)
    {
        ...
        throw DB::Exception(mapErrorCode(rd_kafka_last_error()), "Failed to created underlying streaming store conf");
    }
    ...
    for (const auto & param : params)
    {
        ...
        if (ret != RD_KAFKA_CONF_OK)
        {
            LOG_ERROR(
                ...
                param.second,
                errstr);
            throw DB::Exception(DB::ErrorCodes::INVALID_CONFIG_PARAMETER, "Failed to set underlying streaming store param");
        }
    }
    ...
    if (!topic_handle)
    {
        ...
        throw DB::Exception(mapErrorCode(rd_kafka_last_error()), "Failed to create underlying streaming store handle");
    }

    return topic_handle;
}

SchemaRecordPtr kafkaMsgToRecord(rd_kafka_message_t * msg, const SchemaContext & schema_ctx, bool copy_topic)
{
    assert(msg != nullptr);

    SchemaRecordPtr record = cluster::SchemaRecord::parse(std::string_view(static_cast<const char *>(msg->payload), msg->len), schema_ctx);
...
DescribeResult describeTopic(rd_kafka_topic_t * rkt, struct rd_kafka_s * rk, LoggerPtr log)
{
...
DescribeResult describeTopic(const String & name, struct rd_kafka_s * rk, LoggerPtr log)
{
