...
KafkaWALConsumer::KafkaWALConsumer(std::unique_ptr<KafkaWALSettings> settings_)
    : settings(std::move(settings_)), consumer_handle(nullptr, rd_kafka_destroy), log(getLogger("KafkaWALConsumer"))
{
...
ConsumeResult
KafkaWALConsumer::consume(uint32_t count, int32_t timeout_ms, std::function<SchemaRecordPtr(rd_kafka_message_t *)> deserialize)
{
