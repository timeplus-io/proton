...
getTopicConfig(const String & name, rd_kafka_t * handler, LoggerPtr log)
{
...
AppendResult KafkaWAL::append(SchemaRecord & record, const KafkaWALContext & ctx) const
{
    assert(!record.empty());
...
int32_t KafkaWAL::append(SchemaRecord & record, AppendCallback callback, CallbackData data, const KafkaWALContext & ctx) const
{
    assert(!record.empty());
    assert(ctx.topic_handle);
    ...
    catch (...)
    {
        ...
        if (dr && dr->callback)
            dr->callback(
                AppendResult{
                    .err = DB::getCurrentExceptionCode(), .sn = record.getSN(), .partition = static_cast<int32_t>(record.getShard())},
                dr->data);
        throw;
    }
...
int32_t KafkaWAL::doAppend(SchemaRecord & record, DeliveryReport * dr, const KafkaWALContext & ctx) const
{
    ByteVector data{record.serialize()};
...
#    pragma clang diagnostic ignored "-Wreserved-identifier"
    ...
    const auto & key = record.idempotentKey();
...
AppendResult KafkaWAL::handleError(int err, const SchemaRecord & record, const KafkaWALContext & ctx) const
{
...
int32_t KafkaWAL::commit(int64_t sn, const KafkaWALContext & ctx) const
{
