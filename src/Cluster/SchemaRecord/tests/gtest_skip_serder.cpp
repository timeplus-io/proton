...
void doInsertColumnNumber(DB::Block & block, size_t rows, DB::DataTypePtr data_type)
{
    ...
    for (size_t i = 0; i < rows; ++i)
        col_ptr->insert(IntegerType(static_cast<int32_t>(i) + 100));
...
void insertColumnObject(DB::Block & block, size_t rows)
{
    ...
    auto json_type = std::make_shared<DB::DataTypeObject>(DB::DataTypeObject::SchemaFormat::JSON);
    auto nullable_json_type = std::make_shared<DB::DataTypeObject>(DB::DataTypeObject::SchemaFormat::JSON);
    insert_func(json_type, "json_obj");
    insert_func(nullable_json_type, "nullable_json_obj");
}

void checkColumnObject(DB::ColumnPtr origin, DB::ColumnPtr actual, size_t rows)
{
    ...
    EXPECT_TRUE(col_object->structureEquals(*col_object_got));

    for (size_t row = 0; row < rows; ++row)
    {
        EXPECT_EQ(col_object->compareAt(row, row, *col_object_got, -1), 0);
    }
}

void checkBlock(
    const DB::Block & origin, DB::Block actual, const cluster::SourceColumnsDescription::PhysicalColumnPositions & physical_positions)
{
...
TEST(RecordSerde, Skip)
{
    ...
    cluster::SchemaNativeWriter writer(wb, column_position_all);
    writer.write(block);
    wb.finalize();
    ...
    cluster::SchemaContext schema_ctx(schema_provider);
...
TEST(RecordSerde, SkipPartialJsonSubcolumns)
{
    size_t rows = 3;
    auto block = createBlock(rows);
    ...
    for (size_t pos = 0; pos < block.columns() - 2; ++pos)
    {
        ...
        schema_ctx.column_positions_requested.positions = {static_cast<uint16_t>(pos)};
    ...
    {
        schema_ctx.column_positions_requested.clear();
        for (size_t pos = 0; pos < block.columns() - 2; ++pos)
        {
            if (isObject(block.getByPosition(pos).type))
                schema_ctx.column_positions_requested.subcolumns[schema_ctx.column_positions_requested.positions.size()] = {"data"};

            schema_ctx.column_positions_requested.positions.push_back(pos);
        }
        schema_ctx.schema_version_requested = schema_version;
    ...
    /// Read more partial subcolumns of json and others columns
    {
        ...
        {
            if (isObject(block.getByPosition(pos).type))
                schema_ctx.column_positions_requested.subcolumns[schema_ctx.column_positions_requested.positions.size()]
                    = {"data", "obj.data", "obj.array", "array", "id"};
