...
SourceColumnsDescription::SourceColumnsDescription(
    const DB::NamesAndTypesList & columns_to_read, const DB::Block & schema, const DB::NamesAndTypesList & /*all_extended_columns*/)
{
    ...
    auto add_virtual_col = [this](const auto & column, std::function<Int64(const SchemaRecordPtr & record)> func) {
    ...
    {
        if (column.name == DB::ProtonConsts::RESERVED_APPEND_TIME)
        {
            add_virtual_col(column, [](const SchemaRecordPtr & record) { return record->getAppendTime(); });
        }
        else if (column.name == DB::ProtonConsts::RESERVED_PROCESS_TIME)
        {
            add_virtual_col(column, [](const SchemaRecordPtr &) { return DB::UTCMilliseconds::now(); });
        }
        else if (column.name == DB::ProtonConsts::RESERVED_SHARD)
        {
            add_virtual_col(column, [](const SchemaRecordPtr & record) { return record->getShard(); });
        }
        else if (column.name == DB::ProtonConsts::RESERVED_EVENT_SEQUENCE_ID)
        {
        ...
        else
        {
            ...
            if (column.isSubcolumn())
            {
                ...
                DB::NameAndTypePair subcolumn{name_in_storage, subcolumn_name, column_in_storage.type, column.type};
    ...
    if (physical_column_positions_to_read.positions.empty())
        physical_column_positions_to_read.positions.emplace_back(schema.getPositionByName(DB::ProtonConsts::RESERVED_EVENT_TIME));
}
}
