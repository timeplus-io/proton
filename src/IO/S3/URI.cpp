...
struct URIConverter
{
    static void modifyURI(Poco::URI & uri, NameToNameMap mapper)
    {
        ...
        if (scheme == "s3" && uri.getHost().ends_with("--table-s3"))
            scheme = "s3tables";
        /// proton: ends
        uri = macros.expand(mapper[scheme]).empty() ? uri : Poco::URI(macros.expand(mapper[uri.getScheme()]) + uri.getPathAndQuery());
    }
};
...
URI::URI(const std::string & uri_, bool allow_archive_path_syntax, bool keep_presigned_query_parameters, S3UriStyle uri_style)
{
    ...
    if (context)
    {
        ...
        else
        {
            ...
            mapper["s3tables"] = "https://{bucket}.s3tables.amazonaws.com"; /// proton: added
...
bool URI::tryInitVirtualHostedStyle(bool is_using_aws_private_link_interface, bool use_strict_pattern)
{
    ...
    /// proton: starts add 's3tables'
    static const RE2 virtual_hosted_style_pattern_strict(R"((.+)\.(s3express[\-a-z0-9]+|s3tables|s3|cos|obs|oss-data-acc|oss|eos)([.\-][a-z0-9\-.:]+))");
    /// proton: ends
