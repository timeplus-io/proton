...
template <typename ReturnType>
ReturnType skipJSONFieldImpl(ReadBuffer & buf, std::string_view name_of_field, const FormatSettings::JSON & settings)
{
    static constexpr bool throw_exception = std::is_same_v<ReturnType, void>;
...
void skipToNextWhitespaceOrEOF(ReadBuffer & buf)
{
    while (!buf.eof())
    {
        char * next_pos = find_first_symbols<' '>(buf.position(), buf.buffer().end());
...
{
    s.clear();
    auto parse_func = [&settings](ReadBuffer & in) { skipJSONField(in, "json_field", settings); };
...
{
    s.clear();
    auto parse_func = [&settings](ReadBuffer & in) { return trySkipJSONField(in, "json_field", settings); };
