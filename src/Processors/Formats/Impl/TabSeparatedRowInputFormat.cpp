...
TabSeparatedRowInputFormat::TabSeparatedRowInputFormat(
    ...
    : RowInputFormatWithNamesAndTypes(
        ...
        *in_,
        ...
        ProcessorID::TabSeparatedRowInputFormatID)
    , buf(std::move(in_))
{
}
    ...
    const SerializationPtr & serialization, bool is_last_file_column, const String & /*column_name*/)
{
    ...
    {
        if (as_nullable)
            return SerializationNullable::deserializeNullAsDefaultOrNestedTextRaw(column, *in, format_settings, serialization);
    ...
    if (as_nullable)
        return SerializationNullable::deserializeNullAsDefaultOrNestedTextEscaped(column, *in, format_settings, serialization);
...
void registerFileSegmentationEngineTabSeparated(FormatFactory & factory)
{
    for (bool is_raw : {false, true})
    {
        auto register_func = [&](const String & format_name, bool, bool)
        {
            ...
            factory.registerFileSegmentationEngine(format_name, [is_raw](ReadBuffer & in, DB::Memory<> & memory, size_t min_chunk_size)
            {
