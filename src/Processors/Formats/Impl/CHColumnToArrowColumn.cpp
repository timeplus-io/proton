...
namespace DB
{
    ...
    /// proton: starts
    static String getCKTypeName(const String & type_name)
    {
        static const std::unordered_map<String, String> proton_to_ck_type_map = {
            {"uint8", "UInt8"},
            {"uint16", "UInt16"},
            {"uint32", "UInt32"},
            {"uint64", "UInt64"},
        ...
        auto it = proton_to_ck_type_map.find(type_name);
        if (it != proton_to_ck_type_map.end())
            return it->second;
...
#undef DISPATCH
        ...
        std::unordered_map<String, std::shared_ptr<arrow::Array>> & dictionary_values)
    {
        ...
        String ck_type_name = getCKTypeName(column_type_name);
    ...
    #define DISPATCH(CPP_NUMERIC_TYPE, ARROW_BUILDER_TYPE) \
        else if (#CPP_NUMERIC_TYPE == ck_type_name) \
        { \
        ...
        size_t columns_num)
    {
        ...
        for (size_t column_i = 0; column_i < columns_num; ++column_i)
        {
            ...
            if (!is_arrow_fields_initialized)
            {
                ...
                /// proton: FIXME
                /// field_id is needed for Iceberg, we need to ensure that the field_ids match the ones defined in the iceberg table schema.
                auto kv_metadata = std::make_shared<arrow::KeyValueMetadata>();
                kv_metadata->Append("PARQUET:field_id", fmt::format("{}", column_i + 1)); /// field_id starts from 1

                arrow_fields.emplace_back(std::make_shared<arrow::Field>(header_column.name, arrow_type, is_column_nullable, std::move(kv_metadata)));
            }
