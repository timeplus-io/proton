...
TabSeparatedRowOutputFormat::TabSeparatedRowOutputFormat(
    ...
    ProcessorID pid_)
    : IRowOutputFormat(header_, out_, pid_), with_names(with_names_), with_types(with_types_), is_raw(is_raw_), format_settings(format_settings_)
{
}
...
void TabSeparatedRowOutputFormat::writePrefix()
{
    ...
    {
        /// proton: starts.
        if (format_settings.is_clickhouse_compatible)
            writeLine(header.getClickHouseDataTypeNames());
        else
