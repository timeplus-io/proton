...
ReadFromMergeTree::ReadFromMergeTree(
    ...
    : SourceStepWithFilter(DataStream{
        .header = MergeTreeSelectProcessor::transformHeader(
    ...
    RangesInDataParts && parts_with_ranges, size_t num_streams, AnalysisResult & result, ActionsDAGPtr & result_projection)
{
    ...
    /// proton starts: for changelog_kv or versioned_kv, we always enforce final
    const auto & settings = context->getSettingsRef();
    if (final || (Streaming::isKeyedStorage(data.dataStreamSemantic()) && settings.compact_kv_stream.value))
    /// proton ends
    {
        ...
        if (final && output_each_partition_through_separate_port)
