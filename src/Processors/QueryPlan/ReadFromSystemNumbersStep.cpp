...
public:
    NumbersSource(UInt64 block_size_, UInt64 offset_, UInt64 step_)
        : ISource(createHeader(), true, ProcessorID::NumberSourceID), block_size(block_size_), next(offset_), step(step_)
    {
    }
...
public:
    ...
    NumbersRangedSource(const Ranges & ranges_, RangesStatePtr & ranges_state_, UInt64 base_block_size_)
        : ISource(NumbersSource::createHeader(), true, ProcessorID::NumberRangeSourceID), ranges(ranges_), ranges_state(ranges_state_), base_block_size(base_block_size_)
    {
    }
