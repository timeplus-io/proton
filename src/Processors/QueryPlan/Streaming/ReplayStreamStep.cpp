...
ReplayStreamStep::ReplayStreamStep(
    const DataStream & input_stream_, Float32 replay_speed_, const String & replay_time_col_, std::vector<Int64> shards_last_sns_, std::optional<String> start_time_)
...
void ReplayStreamStep::transformPipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings &)
{
    ...
    pipeline.addSimpleTransform([&](const Block & header) {
        return std::make_shared<ReplayStreamTransform>(header, replay_speed, shards_last_sns[index++], replay_time_col, start_time);
    });
}
