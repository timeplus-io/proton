#include "WindowStep.h"

#include <Processors/Transforms/Streaming/WindowTransform.h>
    ...
    std::vector<WindowFunctionDescription> window_functions)
{
    ...
    /// Transform header:
    /// input header -> input header + result columns of window functions
    for (const auto & f : window_functions)
    {
        ColumnWithTypeAndName column_with_type;
        column_with_type.name = f.column_name;
        column_with_type.type = f.function ? f.function->getResultType() : f.aggregate_function->getReturnType();
...
void WindowStep::transformPipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings &)
{
    ...
    assertBlocksHaveEqualStructure(pipeline.getHeader(), output_stream->header,
        "StreamingWindowStep transform for '" + window_description.window_name + "'");
}
