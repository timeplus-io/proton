...
{
    auto transform_params = std::make_shared<AggregatingTransformParams>(pipeline.getHeader(), std::move(params), final, /*shuffled*/false);
    ...
    if (!memory_efficient_aggregation)
    {
        ...
        { return std::make_shared<MergingAggregatedTransform>(header, transform_params); });
    }
    else
    {
