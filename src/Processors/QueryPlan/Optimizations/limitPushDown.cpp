...
/// proton: starts.
size_t tryPushDownStreamingLimit(QueryPlan::Node * parent_node, QueryPlan::Nodes &)
{
    ...
    auto * limit = typeid_cast<Streaming::LimitStep *>(parent.get());
...
{
    if (auto updated = tryPushDownStreamingLimit(parent_node, nodes); updated != 0)
        return updated;
/// proton: ends
