        ...
        case IntervalKind::Kind::NAME: \
            return [step, scale, &date_lut](Field & field, Int32 jumps_count) { \
                field = Add##NAME##sImpl::execute(static_cast<T>(\
                    field.safeGet<T>()), static_cast<Int32>(step) * jumps_count, date_lut, scale); };
...
#define DECLARE_CASE(NAME) \
                case IntervalKind::Kind::NAME: \
                    ...
                    { \
                        ...
                        auto res = Add##NAME##sImpl::execute(field_decimal.getValue(), converted_step * jumps_count, time_zone, field_decimal.getScale()); \
...
FillingTransform::FillingTransform(
    ...
    : ISimpleTransform(header_, transformHeader(header_, fill_description_), true, ProcessorID::FillingTransformID)
