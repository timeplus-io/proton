...
TotalsHavingTransform::TotalsHavingTransform(
    ...
    bool final_)
    : ISimpleTransform(header, transformHeader(header, expression_  ? &expression_->getActionsDAG() : nullptr, filter_column_, remove_filter_, final_, aggregates_mask_), true, ProcessorID::TotalsHavingTransformID)
