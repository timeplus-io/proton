...
ChunkPair convertToChangelogChunkLegacy(
    AggregatedDataVariants & data, RetractedDataVariants & retracted_data, const AggregatingTransformParams & params)
{
    ...
    auto retracted_chunk
        = mergeBlocksToChunk(params.aggregator.convertToBlocks(retracted_data, params.final, /*clear_states*/ true, /*max_threads*/ 1));
    if (retracted_chunk)
    {
        auto retracted_delta_col = ColumnInt8::create(retracted_chunk.rows(), Int8(-1));
        retracted_chunk.addColumn(std::move(retracted_delta_col));
...
ChunkPair mergeAndConvertToChangelogChunkLegacy(
    ManyAggregatedDataVariants & data, ManyRetractedDataVariants & retracted_data, const AggregatingTransformParams & params)
{
    auto [merged_data, merged_retracted_data] = params.aggregator.mergeRetractedGroupsLegacy(data, retracted_data);
    if (!merged_data)
        return {};

    assert(merged_retracted_data);
    return convertToChangelogChunkLegacy(*merged_data, *merged_retracted_data, params);
}
}
}
}
