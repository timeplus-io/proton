...
public:
    ...
    explicit BlocksListSource(BlocksList && list_)
        : ISource(list_.empty() ? Block() : list_.front().cloneEmpty(), true, ProcessorID::BlockListSourceID)
    ...
    BlocksListSource(BlocksList::iterator & begin_, BlocksList::iterator & end_)
        : ISource(begin_ == end_ ? Block() : begin_->cloneEmpty(), true, ProcessorID::BlockListSourceID)
        , it(begin_), end(end_) {}
