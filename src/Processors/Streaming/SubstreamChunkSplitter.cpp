...
SubstreamChunkSplitter::SubstreamChunkSplitter(std::vector<size_t> key_column_positions_) : key_column_positions(std::move(key_column_positions_))
{
}

std::vector<ChunkWithID> SubstreamChunkSplitter::splitOneRow(Chunk & chunk) const
{
    UInt128 key{};
    SipHash hash;
...
std::vector<ChunkWithID> SubstreamChunkSplitter::split(Chunk & chunk) const
{
