...
void Span::addAttribute(const Exception & e) noexcept
{
    ...
    try
    {
        this->attributes.push_back(Tuple{"proton.exception", getExceptionMessage(e, false)});
    }
    catch (...)
    {
        /// Ignore exceptions
...
SpanHolder::SpanHolder(std::string_view _operation_name)
{
    if (current_thread_trace_context.isTraceEnabled())
    {
        ...
        this->addAttribute("proton.thread_id", getThreadId());
    ...
    const std::weak_ptr<OpenTelemetrySpanLog> & _span_log)
{
    ...
    this->root_span.addAttribute("proton.thread_id", getThreadId());
