...
/// proton: starts. Add `err_mask & 0x20`
#if defined(__arm__)
    const std::string_view access = "<not available on ARM>";
#elif defined(__powerpc__)
    const std::string_view access = "<not available on PowerPC>";
#elif defined(OS_DARWIN)
    const std::string_view access = "<not available on Darwin>";
#elif defined(OS_FREEBSD)
    const std::string_view access = "<not available on FreeBSD>";
#elif !defined(__x86_64__)
    const std::string_view access = "<not available>";
#else
    auto err_mask = context.uc_mcontext.gregs[REG_ERR];
    std::string access = (err_mask & 0x02) ? "write" : "read";
    ...
    switch (info.si_code)
    {
        ...
        case SEGV_BNDERR:
            message = " Bound check failure";
            break;
#endif
#if defined(SEGV_PKUERR)
        case SEGV_PKUERR:
            message = " Memory protection keys (PKU) violation";
            break;
#endif
            /// proton: ends
        default:
    ...
    /// proton: starts
    return fmt::format("Address: {}. Access: {}. {} (si_code={}).", std::move(address), access, message, info.si_code);
    /// proton: ends
}
