...
private:
    ...
    void run()
    {
        auto interval_milliseconds = Context::getGlobalContextInstance()->getServerSettings().bg_reconnect_mysql_dict_interval;
        ...
        if (!reconnectors.empty())
            task_handle->scheduleAfter(Context::getGlobalContextInstance()->getServerSettings().bg_reconnect_mysql_dict_interval);
    }
};
