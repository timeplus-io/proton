...
template <typename WB, typename R>
    requires(std::ranges::range<R>)
void serialize(const R & r, WB && wb, std::optional<VersionType> version = std::nullopt)
{
    writeVarUInt(r.size(), wb);
    for (const auto & elem : r)
        serialize(elem, wb, version);
}

template <typename RB, typename R>
    requires(std::ranges::range<R>)
void deserialize(R & r, RB && rb, std::optional<VersionType> version = std::nullopt)
{
    size_t size;
    readVarUInt(size, rb);
    r.resize(size);
    for (auto & elem : r)
        deserialize(elem, rb, version);
}
