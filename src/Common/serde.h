...
void ALWAYS_INLINE serialize(const S & s, WB && wb)
{
    /// By default use `Revision` for version management, unless it has its own versions
...
void ALWAYS_INLINE deserialize(S & s, RB && rb)
{
    /// By default use `Revision` for version management, unless it has its own versions
    if constexpr (HasGetVersionFromRevision<S>)
