...
proton:
    ...
    DB::XMLDocumentPtr config_xml = processor.processConfig();
    DB::ConfigurationPtr configuration(new Poco::Util::XMLConfiguration(config_xml));


    ASSERT_EQ("1", configuration->getString("proton.field1"));
    ASSERT_EQ("2", configuration->getString("proton.field2"));
    ASSERT_EQ("system", configuration->getString("proton.text_log.database"));
    ASSERT_EQ("text_log", configuration->getString("proton.text_log.table"));
    ASSERT_EQ("ENGINE MergeTree", configuration->getString("proton.text_log.engine"));
    ASSERT_FALSE(configuration->has("proton.text_log.partition_by"));
}
...
proton:
    ...
    ASSERT_EQ("1", configuration->getString("proton.field1"));
    ASSERT_EQ("2", configuration->getString("proton.field2"));
    ASSERT_EQ("3", configuration->getString("proton.field3"));
    ASSERT_EQ("4", configuration->getString("proton.field4"));
    ASSERT_EQ("system", configuration->getString("proton.text_log.database"));
    ASSERT_EQ("text_log", configuration->getString("proton.text_log.table"));
    ASSERT_EQ("ENGINE MergeTree", configuration->getString("proton.text_log.engine"));
    ASSERT_FALSE(configuration->has("proton.text_log.partition_by"));
