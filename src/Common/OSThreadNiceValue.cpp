...
namespace DB
{
    ...
    void OSThreadNiceValue::set([[maybe_unused]] const Int32 value, [[maybe_unused]] const UInt32 thread_id)
    {
        ...
        if (hasLinuxCapability(CAP_SYS_NICE))
        {
            ...
            {
                throwFromErrno("Failed to set the nice value for thread", ErrorCodes::CANNOT_SET_THREAD_PRIORITY);
            }
        }
#endif
    }
    ...
    scope_guard OSThreadNiceValue::scoped([[maybe_unused]] const Int32 value, [[maybe_unused]] const UInt32 thread_id)
    {
        ...
        {
            throwFromErrno("Failed to get the current nice value for thread", ErrorCodes::CANNOT_GET_THREAD_PRIORITY);
        }
