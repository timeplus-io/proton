...
public:
    ...
    void registerClickHouseAlias(const String & alias_name, const String & alias_or_real_name)
    {
        ...
        if (creator_map.count(real_name))
            ...
            throw Exception(ErrorCodes::LOGICAL_ERROR, "{}: can't create ClickHouse alias '{}', the real name '{}' is not registered", factory_name, alias_name, alias_or_real_name);

        if (!clickhouse_names.emplace(alias_name, real_dict_name).second)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "{}: ClickHouse alias name '{}' is not unique", factory_name, alias_name);
    }
    /// proton: ends
