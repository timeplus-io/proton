...
/// proton: starts
#include <Common/SSLConfiguration.h>
...
private:
    EndpointConnectionPool(ConnectionGroup::Ptr group_, String host_, UInt16 port_, bool https_, ProxyConfiguration proxy_configuration_, std::optional<SSLConfiguration> ssl_configuration_ = std::nullopt)    /// proton: add ssl_configuration_
        : host(std::move(host_))
        , port(port_)
        , https(https_)
        , proxy_configuration(std::move(proxy_configuration_))
        , ssl_configuration(std::move(ssl_configuration_))    /// proton: added
        , group(group_)
    {
    }
    ...
    /// proton: starts
    ConnectionPtr createConnectionWithSSLConfiguration()
    {
        if constexpr (std::is_same_v<Session, Poco::Net::HTTPSClientSession>)
        {
            if (ssl_configuration)
            {
                Poco::Net::Context::Ptr context{new Poco::Net::Context(
                    ...
                    /*privateKeyFile=*/ssl_configuration->ssl_key_file,
                    /*certificateFile=*/ssl_configuration->ssl_cert_file,
                    /*caLocation=*/ssl_configuration->ssl_ca_cert_file,
                    /*verificationMode=*/ssl_configuration->verification_mode,
                    /*verificationDepth=*/9,
                    /*loadDefaultCAs=*/true)};

                return PooledConnection::create(this->getWeakFromThis(), group, getMetrics(), host, port, std::move(context));
            }
        }
    ...
    {
        /// proton: starts
        auto connection = createConnectionWithSSLConfiguration();
        /// proton: ends
    ...
    void atConnectionDestroy(PooledConnection & connection)
    {
        ...
        /// proton: starts
        auto connection_to_store = createConnectionWithSSLConfiguration();
        /// proton: ends
    ...
    const std::optional<const SSLConfiguration> ssl_configuration;    /// proton: added
...
struct EndpointPoolKey
{
    ...
    std::optional<SSLConfiguration> ssl_config;    /// proton: added
    ...
    bool operator==(const EndpointPoolKey & rhs) const
    {
        ...
        && ssl_config == rhs.ssl_config;    /// proton: added
    }
};
...
createConnectionPool(ConnectionGroup::Ptr group, std::string host, UInt16 port, bool secure, ProxyConfiguration proxy_configuration, std::optional<SSLConfiguration> ssl_configuration)    /// proton: updates
{
    if (secure)
    {
#if USE_SSL
        return EndpointConnectionPool<Poco::Net::HTTPSClientSession>::create(
            group, std::move(host), port, secure, std::move(proxy_configuration), std::move(ssl_configuration));
#else
        throw Exception(
...
public:
    IHTTPConnectionPoolForEndpoint::Ptr getPool(
        HTTPConnectionGroupType type,
        const Poco::URI & uri,
        const ProxyConfiguration & proxy_configuration,
        std::optional<SSLConfiguration> ssl_configuration) /// proton: updates
    {
        Poco::Timestamp now;
        ...
        return getPoolImpl(type, uri, proxy_configuration, ssl_configuration);    /// proton: updates
    }
...
protected:
    ...
    IExtendedPool::Ptr getPoolImpl(
        ...
        const Poco::URI & uri,
        const ProxyConfiguration & proxy_configuration,
        std::optional<SSLConfiguration> ssl_configuration) /// proton: updates. add ssl_configuration
        TSA_REQUIRES(mutex)
    {
        auto [host, port, secure] = getHostPortSecure(uri, proxy_configuration);
        auto key = EndpointPoolKey{type, host, port, secure, proxy_configuration, ssl_configuration};    /// proton: updates

        auto it = endpoints_pool.find(key);
        if (it != endpoints_pool.end())
            return it->second;

        it = endpoints_pool.emplace(key, createConnectionPool(getGroup(type), std::move(host), port, secure, proxy_configuration, ssl_configuration)).first;
...
IHTTPConnectionPoolForEndpoint::Ptr HTTPConnectionPools::getPool(
    HTTPConnectionGroupType type,
    const Poco::URI & uri,
    const ProxyConfiguration & proxy_configuration,
    std::optional<SSLConfiguration> ssl_configuration) /// proton: add ssl_configuration
{
    return impl->getPool(type, uri, proxy_configuration, ssl_configuration);
}
}
