...
Poco::AutoPtr<Poco::XML::Document> YAMLParser::parseContent(const String& content)
{
    YAML::Node node_yml;
    try
    {
        node_yml = YAML::Load(content);
    }
    catch (const YAML::ParserException& e)
    {
        /// yaml-cpp cannot parse the file because its contents are incorrect
        throw Exception(ErrorCodes::CANNOT_PARSE_YAML, "Unable to parse YAML content: {}, {}", content, e.what());
    }
    ...
    catch (const YAML::TypedBadConversion<std::string>&)
    {
        throw Exception(ErrorCodes::CANNOT_PARSE_YAML,
                        "YAMLParser has encountered node with key "
                        "or value which cannot be represented as string and cannot continue parsing of the content");
    }
    return xml;
}
