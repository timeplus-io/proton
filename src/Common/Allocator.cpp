...
void * allocNoTrack(size_t size, size_t alignment)
{
    ...
    if (alignment <= MALLOC_MIN_ALIGNMENT)
    {
        ...
        if (nullptr == buf)
            throw DB::Exception(DB::ErrorCodes::CANNOT_ALLOCATE_MEMORY, "Allocator: Cannot malloc {}.", ReadableSize(size));
    }
    else
    {
        buf = nullptr;
...
template <bool clear_memory_, bool populate, bool track>
void * Allocator<clear_memory_, populate, track>::alloc(size_t size, size_t alignment)
{
    checkSize(size);
    if constexpr (track)
    {
    ...
    {
        return allocNoTrack<clear_memory_, populate>(size, alignment);
    }
}


template <bool clear_memory_, bool populate, bool track>
void Allocator<clear_memory_, populate, track>::free(void * buf, size_t size)
{
    try
    {
        checkSize(size);
...
template <bool clear_memory_, bool populate, bool track>
void * Allocator<clear_memory_, populate, track>::realloc(void * buf, size_t old_size, size_t new_size, size_t alignment)
{
    checkSize(new_size);
...
// Explicit instantiations for non-tracking allocator variants
