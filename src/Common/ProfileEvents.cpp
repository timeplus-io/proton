...
#define APPLY_FOR_EVENTS(M) \
    M(Query, \
      ...
      "initiated by proton itself. Does not count subqueries.") \
...
\
    ...
    M(MergeTreeDataProjectionWriterSortingBlocksMicroseconds, \
...
\
    ...
    M(WriteBufferFromS3WaitInflightLimitMicroseconds, \
      "Time spent on waiting while some of the current requests are done when its number reached the limit defined by " \
...
\
    ...
    M(FileSegmentPredownloadMicroseconds, \
      "Metric per file segment. Time spent predownloading data to cache (predownloading - finishing file segment download (after someone " \
      "who failed to do that) up to the point current thread was requested to do)") \
...
\
    ...
    M(ThreadPoolReaderPageCacheMissElapsedMicroseconds, \
      "Time spent reading data inside the asynchronous job in ThreadPoolReader - when read was not done from page cache.") \
\
...
\
    M(OverflowBreak, \
      "Number of times, data processing was cancelled by query complexity limitation with setting '*_overflow_mode' = 'break' and the " \
      "result is incomplete.") \
    M(OverflowThrow, \
      "Number of times, data processing was cancelled by query complexity limitation with setting '*_overflow_mode' = 'throw' and " \
      "exception was thrown.") \
    M(OverflowAny, \
      "Number of times approximate GROUP BY was in effect: when aggregation was performed only on top of first 'max_rows_to_group_by' " \
      "unique keys and other keys were ignored due to 'group_by_overflow_mode' = 'any'.") \
\
