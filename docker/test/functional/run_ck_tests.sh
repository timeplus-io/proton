#!/bin/bash
sed -i'' 's/preallocate: true/preallocate: false/g' /etc/proton-server/config.yaml
...
echo "Run test cases"
cd $PROTON_SRC
$PROTON_SRC/tests/ported-clickhouse-test.py $TEST_TYPE --jobs 12 -q $PROTON_SRC/tests/queries_ported 2>&1 \
    | ts '%Y-%m-%d %H:%M:%S' \
