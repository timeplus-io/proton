...
<details>
<summary>Aggregation of 1 billion rows in Timeplus Proton took just <code>39.301 seconds</code> on my machine.</summary>
<pre>
...
## Summary
Timeplus Proton took just `39.301 seconds`[^9] to aggregate 1 billion rows versus `47.053 seconds`[^10] for ClickHouse on my machine [^11].

In other words, Timeplus Proton turned out to be as fast, or in this case, slightly faster than ClickHouse. This was a pleasantly surprising outcome given Timeplus Proton's streaming-first design.
...
[^9]: Tested with Timeplus Proton server `version 1.5.6`.
[^10]: Tested with ClickHouse server `version 24.4.1.1097 (official build)`.
[^11]: Tested on a MacBook Pro (13-inch, 2020, Four Thunderbolt 3 ports), 2 GHz Quad-Core Intel Core i5, 16 GB 3733 MHz LPDDR4X, macOS Monterey 12.6.
